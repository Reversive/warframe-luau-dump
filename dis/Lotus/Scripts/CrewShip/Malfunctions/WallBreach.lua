; This file has been disassembled using luadec 2.0.2 by sztupy (modified by Reversive)

; Name:            
; Defined at line: 0
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       2
; Max Stack Size:  28

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x2d0fad09
  2 [-]: LOADK     R1 K1        ; R1 := "Lotus.Scripts.Eidolon.MiningUtil"
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K0        ; R1 := 0x2d0fad09
  5 [-]: LOADK     R2 K2        ; R2 := "Lotus.Scripts.Libs.TransmissionSet"
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: GETGLOBAL R2 K0        ; R2 := 0x2d0fad09
  8 [-]: LOADK     R3 K3        ; R3 := "Lotus.Scripts.Libs.ObjectiveText"
  9 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 10 [-]: GETGLOBAL R3 K0        ; R3 := 0x2d0fad09
 11 [-]: LOADK     R4 K4        ; R4 := "Lotus.Scripts.Libs.CrewMemberUtilities"
 12 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 13 [-]: LOADNIL   R4 R4        ; R4 := nil
 14 [-]: LOADBOOL  R5 0 0       ; R5 := false
 15 [-]: LOADK     R6 4         ; R6 := 4.000000
 16 [-]: GETGLOBAL R7 K5        ; R7 := 0x0997dbe6
 17 [-]: LOADK     R8 0         ; R8 := 0.000000
 18 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 19 [-]: LOADK     R8 K6        ; R8 := "/Lotus/Types/Game/CrewShip/Malfunctions/WallBreach"
 20 [-]: LOADBOOL  R9 0 0       ; R9 := false
 21 [-]: GETGLOBAL R10 K7       ; R10 := 0x0469f296
 22 [-]: LOADK     R11 K8       ; R11 := "HullRupture"
 23 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 24 [-]: GETGLOBAL R11 K7       ; R11 := 0x0469f296
 25 [-]: LOADK     R12 K9       ; R12 := "HullRuptureRepaired"
 26 [-]: CALL      R11 2 2      ; R11 := R11(R12)
 27 [-]: GETGLOBAL R12 K7       ; R12 := 0x0469f296
 28 [-]: LOADK     R13 K10      ; R13 := "TaskComplete"
 29 [-]: CALL      R12 2 2      ; R12 := R12(R13)
 30 [-]: GETGLOBAL R13 K7       ; R13 := 0x0469f296
 31 [-]: LOADK     R14 K11      ; R14 := "STARTED_FIXING_MALFUNCTION"
 32 [-]: CALL      R13 2 2      ; R13 := R13(R14)
 33 [-]: LOADK     R14 0        ; R14 := -0.750000
 34 [-]: LOADK     R15 K12      ; R15 := 0.200000
 35 [-]: CLOSURE   R16 0        ; R16 := closure(Function #1)
 36 [-]: CLOSURE   R17 1        ; R17 := closure(Function #2)
 37 [-]: MOVE      R0 R1        ; R0 := R1
 38 [-]: CLOSURE   R18 2        ; R18 := closure(Function #3)
 39 [-]: CLOSURE   R19 3        ; R19 := closure(Function #4)
 40 [-]: MOVE      R0 R4        ; R0 := R4
 41 [-]: CLOSURE   R20 4        ; R20 := closure(Function #5)
 42 [-]: MOVE      R0 R14       ; R0 := R14
 43 [-]: MOVE      R0 R15       ; R0 := R15
 44 [-]: CLOSURE   R21 5        ; R21 := closure(Function #6)
 45 [-]: CLOSURE   R22 6        ; R22 := closure(Function #7)
 46 [-]: MOVE      R0 R3        ; R0 := R3
 47 [-]: MOVE      R0 R10       ; R0 := R10
 48 [-]: MOVE      R0 R17       ; R0 := R17
 49 [-]: MOVE      R0 R16       ; R0 := R16
 50 [-]: CLOSURE   R23 7        ; R23 := closure(Function #8)
 51 [-]: MOVE      R0 R3        ; R0 := R3
 52 [-]: MOVE      R0 R12       ; R0 := R12
 53 [-]: MOVE      R0 R13       ; R0 := R13
 54 [-]: MOVE      R0 R17       ; R0 := R17
 55 [-]: MOVE      R0 R16       ; R0 := R16
 56 [-]: MOVE      R0 R11       ; R0 := R11
 57 [-]: CLOSURE   R24 8        ; R24 := closure(Function #9)
 58 [-]: MOVE      R0 R8        ; R0 := R8
 59 [-]: MOVE      R0 R20       ; R0 := R20
 60 [-]: MOVE      R0 R23       ; R0 := R23
 61 [-]: MOVE      R0 R2        ; R0 := R2
 62 [-]: MOVE      R0 R1        ; R0 := R1
 63 [-]: MOVE      R0 R21       ; R0 := R21
 64 [-]: SETGLOBAL R24 K13      ; OnDestroyed := R24
 65 [-]: CLOSURE   R24 9        ; R24 := closure(Function #10)
 66 [-]: CLOSURE   R25 10       ; R25 := closure(Function #11)
 67 [-]: MOVE      R0 R18       ; R0 := R18
 68 [-]: MOVE      R0 R19       ; R0 := R19
 69 [-]: MOVE      R0 R9        ; R0 := R9
 70 [-]: MOVE      R0 R4        ; R0 := R4
 71 [-]: MOVE      R0 R7        ; R0 := R7
 72 [-]: MOVE      R0 R0        ; R0 := R0
 73 [-]: MOVE      R0 R24       ; R0 := R24
 74 [-]: MOVE      R0 R5        ; R0 := R5
 75 [-]: MOVE      R0 R6        ; R0 := R6
 76 [-]: CLOSURE   R26 11       ; R26 := closure(Function #12)
 77 [-]: MOVE      R0 R22       ; R0 := R22
 78 [-]: MOVE      R0 R17       ; R0 := R17
 79 [-]: MOVE      R0 R16       ; R0 := R16
 80 [-]: SETGLOBAL R26 K14      ; RuptureTransmissions := R26
 81 [-]: CLOSURE   R26 12       ; R26 := closure(Function #13)
 82 [-]: MOVE      R0 R1        ; R0 := R1
 83 [-]: SETGLOBAL R26 K15      ; BreachTransmissions := R26
 84 [-]: CLOSURE   R26 13       ; R26 := closure(Function #14)
 85 [-]: MOVE      R0 R4        ; R0 := R4
 86 [-]: MOVE      R0 R5        ; R0 := R5
 87 [-]: CLOSURE   R27 14       ; R27 := closure(Function #15)
 88 [-]: MOVE      R0 R4        ; R0 := R4
 89 [-]: MOVE      R0 R9        ; R0 := R9
 90 [-]: MOVE      R0 R8        ; R0 := R8
 91 [-]: MOVE      R0 R26       ; R0 := R26
 92 [-]: MOVE      R0 R20       ; R0 := R20
 93 [-]: MOVE      R0 R25       ; R0 := R25
 94 [-]: SETGLOBAL R27 K16      ; OnCreated := R27
 95 [-]: RETURN    R0 1         ; return 


; Function #1:
;
; Name:            
; Defined at line: 31
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  11

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x89326c93
  2 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x8b5b1f58]
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: NEWTABLE  R1 0 0       ; R1 := {}
  5 [-]: GETGLOBAL R2 K2        ; R2 := 0xcfc01047
  6 [-]: MOVE      R3 R0        ; R3 := R0
  7 [-]: CALL      R2 2 4       ; R2,R3,R4 := R2(R3)
  8 [-]: JMP       27           ; PC := 27
  9 [-]: SELF      R7 R6 K3     ; R8 := R6; R7 := R6[0xde321e6f]
 10 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 11 [-]: SELF      R7 R7 K4     ; R8 := R7; R7 := R7[0x33c6e9d3]
 12 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 13 [-]: GETGLOBAL R8 K5        ; R8 := 0x7b998233
 14 [-]: MOVE      R9 R7        ; R9 := R7
 15 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 16 [-]: TEST      R8 1         ; if R8 then PC := 27
 17 [-]: JMP       27           ; PC := 27
 18 [-]: SELF      R8 R7 K6     ; R9 := R7; R8 := R7[0xc5334f21]
 19 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 20 [-]: TEST      R8 0         ; if not R8 then PC := 27
 21 [-]: JMP       27           ; PC := 27
 22 [-]: GETGLOBAL R8 K7        ; R8 := 0x33bdd652
 23 [-]: GETTABLE  R8 R8 K8     ; R8 := R8[0x23d5322f]
 24 [-]: MOVE      R9 R1        ; R9 := R1
 25 [-]: MOVE      R10 R6       ; R10 := R6
 26 [-]: CALL      R8 3 1       ; R8(R9,R10)
 27 [-]: TFORLOOP  R2 2         ; R5,R6 :=  R2(R3,R4); if R5 ~= nil then begin PC = 9; R4 := R5 end
 28 [-]: JMP       9            ; PC := 9
 29 [-]: RETURN    R1 2         ; return R1
 30 [-]: RETURN    R0 1         ; return 


; Function #2:
;
; Name:            
; Defined at line: 44
; #Upvalues:       1
; #Parameters:     3
; Is_vararg:       0
; Max Stack Size:  12

  1 [-]: GETGLOBAL R3 K0        ; R3 := 0xcfc01047
  2 [-]: MOVE      R4 R0        ; R4 := R0
  3 [-]: CALL      R3 2 4       ; R3,R4,R5 := R3(R4)
  4 [-]: JMP       11           ; PC := 11
  5 [-]: GETUPVAL  R8 U0        ; R8 := U0
  6 [-]: GETTABLE  R8 R8 K1     ; R8 := R8[0xf22cfc77]
  7 [-]: MOVE      R9 R1        ; R9 := R1
  8 [-]: MOVE      R10 R2       ; R10 := R2
  9 [-]: MOVE      R11 R7       ; R11 := R7
 10 [-]: CALL      R8 4 1       ; R8(R9,R10,R11)
 11 [-]: TFORLOOP  R3 2         ; R6,R7 :=  R3(R4,R5); if R6 ~= nil then begin PC = 5; R5 := R6 end
 12 [-]: JMP       5            ; PC := 5
 13 [-]: RETURN    R0 1         ; return 


; Function #3:
;
; Name:            
; Defined at line: 50
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: NEWTABLE  R0 0 0       ; R0 := {}
  2 [-]: NEWTABLE  R1 0 0       ; R1 := {}
  3 [-]: SETTABLE  R0 K0 R1     ; R0[0xd2715720] := R1
  4 [-]: NEWTABLE  R1 0 0       ; R1 := {}
  5 [-]: SETTABLE  R0 K1 R1     ; R0[0xb40c191a] := R1
  6 [-]: NEWTABLE  R1 0 0       ; R1 := {}
  7 [-]: SETTABLE  R0 K2 R1     ; R0["repairedPoints"] := R1
  8 [-]: GETGLOBAL R1 K4        ; R1 := 0x8ee538c1
  9 [-]: SETTABLE  R0 K3 R1     ; R0["repairedPointDecoType"] := R1
 10 [-]: NEWTABLE  R1 0 0       ; R1 := {}
 11 [-]: SETTABLE  R0 K5 R1     ; R0["repairedPointDecos"] := R1
 12 [-]: SETTABLE  R0 K6 K7     ; R0["projector"] := nil
 13 [-]: GETGLOBAL R1 K9        ; R1 := 0xa421af95
 14 [-]: CALL      R1 1 2       ; R1 := R1()
 15 [-]: SETTABLE  R0 K8 R1     ; R0["boundsMin"] := R1
 16 [-]: GETGLOBAL R1 K9        ; R1 := 0xa421af95
 17 [-]: CALL      R1 1 2       ; R1 := R1()
 18 [-]: SETTABLE  R0 K10 R1    ; R0["boundsMax"] := R1
 19 [-]: SETTABLE  R0 K11 K12   ; R0["emergencyFallback"] := false
 20 [-]: RETURN    R0 2         ; return R0
 21 [-]: RETURN    R0 1         ; return 


; Function #4:
;
; Name:            
; Defined at line: 65
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  8

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
  2 [-]: GETGLOBAL R1 K1        ; R1 := 0x83f4e77c
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: TEST      R0 0         ; if not R0 then PC := 8
  5 [-]: JMP       8            ; PC := 8
  6 [-]: LOADNIL   R0 R0        ; R0 := nil
  7 [-]: RETURN    R0 2         ; return R0
  8 [-]: LOADK     R0 0         ; R0 := 0.000000
  9 [-]: LOADK     R1 K2        ; R1 := 0.050000
 10 [-]: LOADK     R2 1         ; R2 := 1.000000
 11 [-]: LOADNIL   R3 R3        ; R3 := nil
 12 [-]: GETGLOBAL R4 K0        ; R4 := 0x7b998233
 13 [-]: MOVE      R5 R3        ; R5 := R3
 14 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 15 [-]: TEST      R4 0         ; if not R4 then PC := 37
 16 [-]: JMP       37           ; PC := 37
 17 [-]: GETUPVAL  R4 U0        ; R4 := U0
 18 [-]: SELF      R4 R4 K3     ; R5 := R4; R4 := R4[0xc9f6a7d7]
 19 [-]: GETGLOBAL R6 K4        ; R6 := gDynamicDecalProjectorType
 20 [-]: CALL      R4 3 2       ; R4 := R4(R5,R6)
 21 [-]: MOVE      R3 R4        ; R3 := R4
 22 [-]: LT        0 R2 R0      ; if R2 >= R0 then PC := 32
 23 [-]: JMP       32           ; PC := 32
 24 [-]: GETGLOBAL R4 K5        ; R4 := 0x3d106989
 25 [-]: LOADK     R5 K6        ; R5 := "ERROR: Waiting too long for wall breach projector ("
 26 [-]: MOVE      R6 R0        ; R6 := R0
 27 [-]: LOADK     R7 K7        ; R7 := "s)"
 28 [-]: CONCAT    R5 R5 R7     ; R5 := R5 .. R6 .. R7
 29 [-]: CALL      R4 2 1       ; R4(R5)
 30 [-]: LOADNIL   R4 R4        ; R4 := nil
 31 [-]: RETURN    R4 2         ; return R4
 32 [-]: ADD       R0 R0 R1     ; R0 := R0 + R1
 33 [-]: GETGLOBAL R4 K8        ; R4 := 0xcbd666e1
 34 [-]: MOVE      R5 R1        ; R5 := R1
 35 [-]: CALL      R4 2 1       ; R4(R5)
 36 [-]: JMP       12           ; PC := 12
 37 [-]: RETURN    R3 2         ; return R3
 38 [-]: RETURN    R0 1         ; return 


; Function #5:
;
; Name:            
; Defined at line: 89
; #Upvalues:       2
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  10

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x89326c93
  2 [-]: SELF      R1 R1 K1     ; R2 := R1; R1 := R1[0x18d05d30]
  3 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  4 [-]: TEST      R1 0         ; if not R1 then PC := 65
  5 [-]: JMP       65           ; PC := 65
  6 [-]: GETGLOBAL R1 K2        ; R1 := 0xbe190284
  7 [-]: SELF      R1 R1 K3     ; R2 := R1; R1 := R1[0xd7d79b74]
  8 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  9 [-]: GETGLOBAL R2 K4        ; R2 := 0x7b998233
 10 [-]: MOVE      R3 R1        ; R3 := R1
 11 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 12 [-]: TEST      R2 1         ; if R2 then PC := 65
 13 [-]: JMP       65           ; PC := 65
 14 [-]: LOADNIL   R2 R3        ; R2 := R3 := nil
 15 [-]: GETGLOBAL R4 K4        ; R4 := 0x7b998233
 16 [-]: MOVE      R5 R1        ; R5 := R1
 17 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 18 [-]: TEST      R4 1         ; if R4 then PC := 38
 19 [-]: JMP       38           ; PC := 38
 20 [-]: SELF      R4 R1 K5     ; R5 := R1; R4 := R1[0xcd57f819]
 21 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 22 [-]: GETGLOBAL R5 K4        ; R5 := 0x7b998233
 23 [-]: MOVE      R6 R4        ; R6 := R4
 24 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 25 [-]: TEST      R5 1         ; if R5 then PC := 38
 26 [-]: JMP       38           ; PC := 38
 27 [-]: SELF      R5 R4 K6     ; R6 := R4; R5 := R4[0x5163741e]
 28 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 29 [-]: MOVE      R2 R5        ; R2 := R5
 30 [-]: GETGLOBAL R5 K4        ; R5 := 0x7b998233
 31 [-]: MOVE      R6 R2        ; R6 := R2
 32 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 33 [-]: TEST      R5 1         ; if R5 then PC := 38
 34 [-]: JMP       38           ; PC := 38
 35 [-]: SELF      R5 R2 K7     ; R6 := R2; R5 := R2[0xde321e6f]
 36 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 37 [-]: MOVE      R3 R5        ; R3 := R5
 38 [-]: GETGLOBAL R5 K4        ; R5 := 0x7b998233
 39 [-]: MOVE      R6 R3        ; R6 := R3
 40 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 41 [-]: TEST      R5 1         ; if R5 then PC := 65
 42 [-]: JMP       65           ; PC := 65
 43 [-]: TEST      R0 0         ; if not R0 then PC := 51
 44 [-]: JMP       51           ; PC := 51
 45 [-]: SELF      R5 R3 K8     ; R6 := R3; R5 := R3[0x5e6704ff]
 46 [-]: LOADK     R7 64        ; R7 := 64.000000
 47 [-]: LOADK     R8 2         ; R8 := 2.000000
 48 [-]: GETUPVAL  R9 U0        ; R9 := U0
 49 [-]: CALL      R5 5 1       ; R5(R6,R7,R8,R9)
 50 [-]: JMP       65           ; PC := 65
 51 [-]: SELF      R5 R3 K11    ; R6 := R3; R5 := R3[0x12dd9da2]
 52 [-]: LOADK     R7 64        ; R7 := 64.000000
 53 [-]: LOADK     R8 2         ; R8 := 2.000000
 54 [-]: GETUPVAL  R9 U0        ; R9 := U0
 55 [-]: CALL      R5 5 1       ; R5(R6,R7,R8,R9)
 56 [-]: SELF      R5 R2 K12    ; R6 := R2; R5 := R2[0x014db014]
 57 [-]: SELF      R7 R2 K13    ; R8 := R2; R7 := R2[0xd2715720]
 58 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 59 [-]: SELF      R8 R2 K14    ; R9 := R2; R8 := R2[0xb40c191a]
 60 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 61 [-]: GETUPVAL  R9 U1        ; R9 := U1
 62 [-]: MUL       R8 R8 R9     ; R8 := R8 * R9
 63 [-]: ADD       R7 R7 R8     ; R7 := R7 + R8
 64 [-]: CALL      R5 3 1       ; R5(R6,R7)
 65 [-]: RETURN    R0 1         ; return 


; Function #6:
;
; Name:            
; Defined at line: 117
; #Upvalues:       0
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  9

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0xc8802016
  2 [-]: MOVE      R2 R0        ; R2 := R0
  3 [-]: CALL      R1 2 4       ; R1,R2,R3 := R1(R2)
  4 [-]: JMP       20           ; PC := 20
  5 [-]: GETGLOBAL R6 K1        ; R6 := 0x7b998233
  6 [-]: MOVE      R7 R5        ; R7 := R5
  7 [-]: CALL      R6 2 2       ; R6 := R6(R7)
  8 [-]: TEST      R6 1         ; if R6 then PC := 20
  9 [-]: JMP       20           ; PC := 20
 10 [-]: SELF      R6 R5 K2     ; R7 := R5; R6 := R5[0xf2deaf69]
 11 [-]: GETGLOBAL R8 K3        ; R8 := gLotusEffectDecorationType
 12 [-]: CALL      R6 3 2       ; R6 := R6(R7,R8)
 13 [-]: TEST      R6 0         ; if not R6 then PC := 18
 14 [-]: JMP       18           ; PC := 18
 15 [-]: SELF      R6 R5 K4     ; R7 := R5; R6 := R5[0x1db57c6b]
 16 [-]: CALL      R6 2 1       ; R6(R7)
 17 [-]: JMP       20           ; PC := 20
 18 [-]: SELF      R6 R5 K5     ; R7 := R5; R6 := R5[0xa2880940]
 19 [-]: CALL      R6 2 1       ; R6(R7)
 20 [-]: TFORLOOP  R1 2         ; R4,R5 :=  R1(R2,R3); if R4 ~= nil then begin PC = 5; R3 := R4 end
 21 [-]: JMP       5            ; PC := 5
 22 [-]: RETURN    R0 1         ; return 


; Function #7:
;
; Name:            
; Defined at line: 129
; #Upvalues:       4
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: LOADBOOL  R0 0 0       ; R0 := false
  2 [-]: GETGLOBAL R1 K0        ; R1 := 0x0c5e62f9
  3 [-]: LOADK     R2 1         ; R2 := 1.000000
  4 [-]: LOADK     R3 2         ; R3 := 2.000000
  5 [-]: CALL      R1 3 2       ; R1 := R1(R2,R3)
  6 [-]: EQ        0 R1 K1      ; if R1 ~= 2.000000 then PC := 14
  7 [-]: JMP       14           ; PC := 14
  8 [-]: GETUPVAL  R1 U0        ; R1 := U0
  9 [-]: GETTABLE  R1 R1 K2     ; R1 := R1[0xa7d7c25f]
 10 [-]: GETUPVAL  R2 U1        ; R2 := U1
 11 [-]: LOADBOOL  R3 1 0       ; R3 := true
 12 [-]: CALL      R1 3 2       ; R1 := R1(R2,R3)
 13 [-]: MOVE      R0 R1        ; R0 := R1
 14 [-]: EQ        0 R0 K3      ; if R0 ~= false then PC := 22
 15 [-]: JMP       22           ; PC := 22
 16 [-]: GETUPVAL  R1 U2        ; R1 := U2
 17 [-]: GETUPVAL  R2 U3        ; R2 := U3
 18 [-]: CALL      R2 1 2       ; R2 := R2()
 19 [-]: GETGLOBAL R3 K4        ; R3 := 0xe91d7466
 20 [-]: GETUPVAL  R4 U1        ; R4 := U1
 21 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 22 [-]: RETURN    R0 1         ; return 


; Function #8:
;
; Name:            
; Defined at line: 140
; #Upvalues:       6
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: LOADBOOL  R0 0 0       ; R0 := false
  2 [-]: GETGLOBAL R1 K0        ; R1 := 0x0c5e62f9
  3 [-]: LOADK     R2 1         ; R2 := 1.000000
  4 [-]: LOADK     R3 2         ; R3 := 2.000000
  5 [-]: CALL      R1 3 2       ; R1 := R1(R2,R3)
  6 [-]: EQ        0 R1 K1      ; if R1 ~= 2.000000 then PC := 16
  7 [-]: JMP       16           ; PC := 16
  8 [-]: GETUPVAL  R1 U0        ; R1 := U0
  9 [-]: GETTABLE  R1 R1 K2     ; R1 := R1[0x13deb761]
 10 [-]: GETUPVAL  R2 U1        ; R2 := U1
 11 [-]: LOADBOOL  R3 0 0       ; R3 := false
 12 [-]: CLOSURE   R4 0         ; R4 := closure(Function #8.1)
 13 [-]: GETUPVAL  R0 U2        ; R0 := U2
 14 [-]: CALL      R1 4 2       ; R1 := R1(R2,R3,R4)
 15 [-]: MOVE      R0 R1        ; R0 := R1
 16 [-]: EQ        0 R0 K3      ; if R0 ~= false then PC := 24
 17 [-]: JMP       24           ; PC := 24
 18 [-]: GETUPVAL  R1 U3        ; R1 := U3
 19 [-]: GETUPVAL  R2 U4        ; R2 := U4
 20 [-]: CALL      R2 1 2       ; R2 := R2()
 21 [-]: GETGLOBAL R3 K4        ; R3 := 0xe91d7466
 22 [-]: GETUPVAL  R4 U5        ; R4 := U5
 23 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 24 [-]: RETURN    R0 1         ; return 


; Function #8.1:
;
; Name:            
; Defined at line: 143
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  4

  1 [-]: SELF      R1 R0 K0     ; R2 := R0; R1 := R0[0xbd84d75d]
  2 [-]: GETUPVAL  R3 U0        ; R3 := U0
  3 [-]: TAILCALL  R1 3 0       ; R1,... := R1(R2,R3)
  4 [-]: RETURN    R1 0         ; return R1,...
  5 [-]: RETURN    R0 1         ; return 


; Function #9:
;
; Name:            
; Defined at line: 153
; #Upvalues:       6
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  9

  1 [-]: SELF      R1 R0 K0     ; R2 := R0; R1 := R0[0xcde10c4a]
  2 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  3 [-]: SELF      R1 R1 K1     ; R2 := R1; R1 := R1[0xed4e0128]
  4 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  5 [-]: GETUPVAL  R2 U0        ; R2 := U0
  6 [-]: EQ        0 R1 R2      ; if R1 ~= R2 then PC := 38
  7 [-]: JMP       38           ; PC := 38
  8 [-]: GETUPVAL  R1 U1        ; R1 := U1
  9 [-]: LOADBOOL  R2 0 0       ; R2 := false
 10 [-]: CALL      R1 2 1       ; R1(R2)
 11 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
 12 [-]: GETGLOBAL R2 K3        ; R2 := _T
 13 [-]: GETTABLE  R2 R2 K4     ; R2 := R2["Malfunctions"]
 14 [-]: GETTABLE  R2 R2 K5     ; R2 := R2["WallBreachCount"]
 15 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 16 [-]: TEST      R1 1         ; if R1 then PC := 80
 17 [-]: JMP       80           ; PC := 80
 18 [-]: GETGLOBAL R1 K6        ; R1 := 0xbe190284
 19 [-]: SELF      R1 R1 K7     ; R2 := R1; R1 := R1[0xffe25891]
 20 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 21 [-]: TEST      R1 1         ; if R1 then PC := 80
 22 [-]: JMP       80           ; PC := 80
 23 [-]: GETGLOBAL R1 K3        ; R1 := _T
 24 [-]: GETTABLE  R1 R1 K4     ; R1 := R1["Malfunctions"]
 25 [-]: GETGLOBAL R2 K3        ; R2 := _T
 26 [-]: GETTABLE  R2 R2 K4     ; R2 := R2["Malfunctions"]
 27 [-]: GETTABLE  R2 R2 K5     ; R2 := R2["WallBreachCount"]
 28 [-]: SUB       R2 R2 K8     ; R2 := R2 - 1.000000
 29 [-]: SETTABLE  R1 K5 R2     ; R1["WallBreachCount"] := R2
 30 [-]: GETGLOBAL R1 K3        ; R1 := _T
 31 [-]: GETTABLE  R1 R1 K4     ; R1 := R1["Malfunctions"]
 32 [-]: GETTABLE  R1 R1 K5     ; R1 := R1["WallBreachCount"]
 33 [-]: EQ        0 R1 K9      ; if R1 ~= 0.000000 then PC := 80
 34 [-]: JMP       80           ; PC := 80
 35 [-]: GETUPVAL  R1 U2        ; R1 := U2
 36 [-]: CALL      R1 1 1       ; R1()
 37 [-]: JMP       80           ; PC := 80
 38 [-]: GETGLOBAL R1 K6        ; R1 := 0xbe190284
 39 [-]: SELF      R1 R1 K7     ; R2 := R1; R1 := R1[0xffe25891]
 40 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 41 [-]: TEST      R1 1         ; if R1 then PC := 55
 42 [-]: JMP       55           ; PC := 55
 43 [-]: GETUPVAL  R1 U3        ; R1 := U3
 44 [-]: GETTABLE  R1 R1 K10    ; R1 := R1[0x826f2ca6]
 45 [-]: CALL      R1 1 2       ; R1 := R1()
 46 [-]: LT        0 K9 R1      ; if 0.000000 >= R1 then PC := 55
 47 [-]: JMP       55           ; PC := 55
 48 [-]: GETUPVAL  R1 U4        ; R1 := U4
 49 [-]: GETTABLE  R1 R1 K11    ; R1 := R1[0x9742b85b]
 50 [-]: GETGLOBAL R2 K12       ; R2 := 0xe91d7466
 51 [-]: GETGLOBAL R3 K13       ; R3 := 0x0469f296
 52 [-]: LOADK     R4 K14       ; R4 := "BreachEventCleared"
 53 [-]: CALL      R3 2 0       ; R3,... := R3(R4)
 54 [-]: CALL      R1 0 1       ; R1(R2,...)
 55 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
 56 [-]: GETGLOBAL R2 K3        ; R2 := _T
 57 [-]: GETTABLE  R2 R2 K15    ; R2 := R2["RailjackPreDeath"]
 58 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 59 [-]: TEST      R1 1         ; if R1 then PC := 80
 60 [-]: JMP       80           ; PC := 80
 61 [-]: GETGLOBAL R1 K3        ; R1 := _T
 62 [-]: GETTABLE  R1 R1 K15    ; R1 := R1["RailjackPreDeath"]
 63 [-]: SETTABLE  R1 K16 K17   ; R1["NewRepair"] := true
 64 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
 65 [-]: GETGLOBAL R2 K3        ; R2 := _T
 66 [-]: GETTABLE  R2 R2 K15    ; R2 := R2["RailjackPreDeath"]
 67 [-]: GETTABLE  R2 R2 K18    ; R2 := R2["OnRepairedHidePort"]
 68 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 69 [-]: TEST      R1 1         ; if R1 then PC := 80
 70 [-]: JMP       80           ; PC := 80
 71 [-]: GETGLOBAL R1 K3        ; R1 := _T
 72 [-]: GETTABLE  R1 R1 K15    ; R1 := R1["RailjackPreDeath"]
 73 [-]: GETTABLE  R1 R1 K18    ; R1 := R1["OnRepairedHidePort"]
 74 [-]: SELF      R1 R1 K19    ; R2 := R1; R1 := R1[0x8eb2112d]
 75 [-]: LOADK     R3 K20       ; R3 := "TriggerPort"
 76 [-]: CALL      R1 3 1       ; R1(R2,R3)
 77 [-]: GETGLOBAL R1 K3        ; R1 := _T
 78 [-]: GETTABLE  R1 R1 K15    ; R1 := R1["RailjackPreDeath"]
 79 [-]: SETTABLE  R1 K18 K21   ; R1["OnRepairedHidePort"] := nil
 80 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
 81 [-]: GETGLOBAL R2 K3        ; R2 := _T
 82 [-]: GETTABLE  R2 R2 K22    ; R2 := R2["WallBreaches"]
 83 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 84 [-]: TEST      R1 1         ; if R1 then PC := 118
 85 [-]: JMP       118          ; PC := 118
 86 [-]: GETGLOBAL R1 K23       ; R1 := 0xcfc01047
 87 [-]: GETGLOBAL R2 K3        ; R2 := _T
 88 [-]: GETTABLE  R2 R2 K22    ; R2 := R2["WallBreaches"]
 89 [-]: CALL      R1 2 4       ; R1,R2,R3 := R1(R2)
 90 [-]: JMP       116          ; PC := 116
 91 [-]: GETGLOBAL R6 K2        ; R6 := 0x7b998233
 92 [-]: MOVE      R7 R4        ; R7 := R4
 93 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 94 [-]: TEST      R6 1         ; if R6 then PC := 98
 95 [-]: JMP       98           ; PC := 98
 96 [-]: EQ        0 R4 R0      ; if R4 ~= R0 then PC := 116
 97 [-]: JMP       116          ; PC := 116
 98 [-]: GETGLOBAL R6 K2        ; R6 := 0x7b998233
 99 [-]: GETTABLE  R7 R5 K24    ; R7 := R5["projector"]
100 [-]: CALL      R6 2 2       ; R6 := R6(R7)
101 [-]: TEST      R6 1         ; if R6 then PC := 107
102 [-]: JMP       107          ; PC := 107
103 [-]: GETTABLE  R6 R5 K24    ; R6 := R5["projector"]
104 [-]: SELF      R6 R6 K19    ; R7 := R6; R6 := R6[0x8eb2112d]
105 [-]: LOADK     R8 K25       ; R8 := "Hide"
106 [-]: CALL      R6 3 1       ; R6(R7,R8)
107 [-]: GETUPVAL  R6 U5        ; R6 := U5
108 [-]: GETTABLE  R7 R5 K26    ; R7 := R5["pointDecos"]
109 [-]: CALL      R6 2 1       ; R6(R7)
110 [-]: GETUPVAL  R6 U5        ; R6 := U5
111 [-]: GETTABLE  R7 R5 K27    ; R7 := R5["repairedPointDecos"]
112 [-]: CALL      R6 2 1       ; R6(R7)
113 [-]: GETGLOBAL R6 K3        ; R6 := _T
114 [-]: GETTABLE  R6 R6 K22    ; R6 := R6["WallBreaches"]
115 [-]: SETTABLE  R6 R4 K21    ; R6[R4] := nil
116 [-]: TFORLOOP  R1 2         ; R4,R5 :=  R1(R2,R3); if R4 ~= nil then begin PC = 91; R3 := R4 end
117 [-]: JMP       91           ; PC := 91
118 [-]: RETURN    R0 1         ; return 


; Function #10:
;
; Name:            
; Defined at line: 194
; #Upvalues:       0
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: LOADK     R2 0         ; R2 := 0.250000
  2 [-]: GETTABLE  R3 R0 K0     ; R3 := R0["x"]
  3 [-]: SUB       R3 R3 R2     ; R3 := R3 - R2
  4 [-]: SETTABLE  R0 K0 R3     ; R0["x"] := R3
  5 [-]: GETTABLE  R3 R0 K1     ; R3 := R0["y"]
  6 [-]: SUB       R3 R3 R2     ; R3 := R3 - R2
  7 [-]: SETTABLE  R0 K1 R3     ; R0["y"] := R3
  8 [-]: GETTABLE  R3 R0 K2     ; R3 := R0["z"]
  9 [-]: SUB       R3 R3 R2     ; R3 := R3 - R2
 10 [-]: SETTABLE  R0 K2 R3     ; R0["z"] := R3
 11 [-]: GETTABLE  R3 R1 K0     ; R3 := R1["x"]
 12 [-]: ADD       R3 R3 R2     ; R3 := R3 + R2
 13 [-]: SETTABLE  R1 K0 R3     ; R1["x"] := R3
 14 [-]: GETTABLE  R3 R1 K1     ; R3 := R1["y"]
 15 [-]: ADD       R3 R3 R2     ; R3 := R3 + R2
 16 [-]: SETTABLE  R1 K1 R3     ; R1["y"] := R3
 17 [-]: GETTABLE  R3 R1 K2     ; R3 := R1["z"]
 18 [-]: ADD       R3 R3 R2     ; R3 := R3 + R2
 19 [-]: SETTABLE  R1 K2 R3     ; R1["z"] := R3
 20 [-]: MOVE      R3 R0        ; R3 := R0
 21 [-]: MOVE      R4 R1        ; R4 := R1
 22 [-]: RETURN    R3 3         ; return R3,R4
 23 [-]: RETURN    R0 1         ; return 


; Function #11:
;
; Name:            
; Defined at line: 207
; #Upvalues:       9
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  37

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
  2 [-]: GETGLOBAL R1 K1        ; R1 := _T
  3 [-]: GETTABLE  R1 R1 K2     ; R1 := R1["WallBreaches"]
  4 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  5 [-]: TEST      R0 0         ; if not R0 then PC := 10
  6 [-]: JMP       10           ; PC := 10
  7 [-]: GETGLOBAL R0 K1        ; R0 := _T
  8 [-]: NEWTABLE  R1 0 0       ; R1 := {}
  9 [-]: SETTABLE  R0 K2 R1     ; R0["WallBreaches"] := R1
 10 [-]: GETUPVAL  R0 U0        ; R0 := U0
 11 [-]: CALL      R0 1 2       ; R0 := R0()
 12 [-]: GETUPVAL  R1 U1        ; R1 := U1
 13 [-]: CALL      R1 1 2       ; R1 := R1()
 14 [-]: GETUPVAL  R2 U2        ; R2 := U2
 15 [-]: TEST      R2 0         ; if not R2 then PC := 25
 16 [-]: JMP       25           ; PC := 25
 17 [-]: GETGLOBAL R2 K0        ; R2 := 0x7b998233
 18 [-]: MOVE      R3 R1        ; R3 := R1
 19 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 20 [-]: TEST      R2 1         ; if R2 then PC := 25
 21 [-]: JMP       25           ; PC := 25
 22 [-]: SELF      R2 R1 K3     ; R3 := R1; R2 := R1[0x8eb2112d]
 23 [-]: LOADK     R4 K4        ; R4 := "Show"
 24 [-]: CALL      R2 3 1       ; R2(R3,R4)
 25 [-]: SETTABLE  R0 K5 R1     ; R0["projector"] := R1
 26 [-]: GETGLOBAL R2 K0        ; R2 := 0x7b998233
 27 [-]: MOVE      R3 R1        ; R3 := R1
 28 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 29 [-]: GETGLOBAL R3 K6        ; R3 := ZERO_ROTATION
 30 [-]: TEST      R2 1         ; if R2 then PC := 188
 31 [-]: JMP       188          ; PC := 188
 32 [-]: SELF      R4 R1 K7     ; R5 := R1; R4 := R1[0x9546cd1b]
 33 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 34 [-]: SELF      R5 R1 K8     ; R6 := R1; R5 := R1[0xd1586535]
 35 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 36 [-]: SELF      R6 R1 K9     ; R7 := R1; R6 := R1[0xcb3851b8]
 37 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 38 [-]: MOVE      R3 R6        ; R3 := R6
 39 [-]: GETTABLE  R6 R3 K10    ; R6 := R3["bank"]
 40 [-]: GETGLOBAL R7 K11       ; R7 := 0xce35ea75
 41 [-]: ADD       R6 R6 R7     ; R6 := R6 + R7
 42 [-]: SETTABLE  R3 K10 R6    ; R3["bank"] := R6
 43 [-]: GETGLOBAL R6 K12       ; R6 := 0x89326c93
 44 [-]: SELF      R6 R6 K13    ; R7 := R6; R6 := R6[0x05909209]
 45 [-]: GETGLOBAL R8 K14       ; R8 := 0xe22553df
 46 [-]: MOVE      R9 R5        ; R9 := R5
 47 [-]: MOVE      R10 R3       ; R10 := R3
 48 [-]: GETUPVAL  R11 U3       ; R11 := U3
 49 [-]: GETUPVAL  R12 U3       ; R12 := U3
 50 [-]: CALL      R6 7 2       ; R6 := R6(R7,R8,R9,R10,R11,R12)
 51 [-]: NEWTABLE  R7 0 0       ; R7 := {}
 52 [-]: GETGLOBAL R8 K15       ; R8 := 0xffd438ab
 53 [-]: CALL      R8 1 2       ; R8 := R8()
 54 [-]: GETGLOBAL R9 K16       ; R9 := 0x0997dbe6
 55 [-]: GETGLOBAL R10 K17      ; R10 := 0xdd6e4cf8
 56 [-]: LOADK     R11 1        ; R11 := 1.000000
 57 [-]: LOADK     R12 K18      ; R12 := 999999.000000
 58 [-]: CALL      R10 3 0      ; R10,... := R10(R11,R12)
 59 [-]: CALL      R9 0 2       ; R9 := R9(R10,...)
 60 [-]: SETUPVAL  R9 U4        ; U82 := R4
 61 [-]: GETGLOBAL R9 K19       ; R9 := 0x4f6851ff
 62 [-]: GETUPVAL  R10 U4       ; R10 := U4
 63 [-]: CALL      R9 2 1       ; R9(R10)
 64 [-]: NEWTABLE  R9 0 5       ; R9 := {}
 65 [-]: GETGLOBAL R10 K21      ; R10 := 0x674c7ecd
 66 [-]: SETTABLE  R9 K20 R10   ; R9["spacing"] := R10
 67 [-]: SETTABLE  R9 K22 K23   ; R9["horizontalDeviation"] := 0.000000
 68 [-]: SETTABLE  R9 K24 K23   ; R9["verticalDeviation"] := 0.000000
 69 [-]: SETTABLE  R9 K25 K26   ; R9["noise"] := true
 70 [-]: NEWTABLE  R10 0 0      ; R10 := {}
 71 [-]: GETGLOBAL R11 K28      ; R11 := 0x7ed0a956
 72 [-]: LOADK     R12 K29      ; R12 := "/Lotus/Types/Game/CrewShip/Malfunctions/MultiToolHitProxy"
 73 [-]: CALL      R11 2 0      ; R11,... := R11(R12)
 74 [-]: SETLIST   R10 0 1      ; R10[(1-1)*FPF+i] := R(10+i), 1 <= i <= 0
 75 [-]: SETTABLE  R9 K27 R10   ; R9["raycastIgnoreTypes"] := R10
 76 [-]: GETGLOBAL R10 K30      ; R10 := 0xa421af95
 77 [-]: CALL      R10 1 2      ; R10 := R10()
 78 [-]: GETGLOBAL R11 K30      ; R11 := 0xa421af95
 79 [-]: CALL      R11 1 2      ; R11 := R11()
 80 [-]: GETUPVAL  R12 U5       ; R12 := U5
 81 [-]: GETTABLE  R12 R12 K31  ; R12 := R12[0xf61ff9f9]
 82 [-]: MOVE      R13 R9       ; R13 := R9
 83 [-]: MOVE      R14 R5       ; R14 := R5
 84 [-]: MOVE      R15 R3       ; R15 := R3
 85 [-]: MOVE      R16 R4       ; R16 := R4
 86 [-]: LOADBOOL  R17 0 0      ; R17 := false
 87 [-]: MOVE      R18 R6       ; R18 := R6
 88 [-]: MOVE      R19 R7       ; R19 := R7
 89 [-]: MOVE      R20 R10      ; R20 := R10
 90 [-]: MOVE      R21 R11      ; R21 := R11
 91 [-]: CALL      R12 10 3     ; R12,R13 := R12(R13,R14,R15,R16,R17,R18,R19,R20,R21)
 92 [-]: GETGLOBAL R14 K19      ; R14 := 0x4f6851ff
 93 [-]: MOVE      R15 R8       ; R15 := R8
 94 [-]: CALL      R14 2 1      ; R14(R15)
 95 [-]: GETGLOBAL R14 K0       ; R14 := 0x7b998233
 96 [-]: MOVE      R15 R6       ; R15 := R6
 97 [-]: CALL      R14 2 2      ; R14 := R14(R15)
 98 [-]: TEST      R14 1        ; if R14 then PC := 102
 99 [-]: JMP       102          ; PC := 102
100 [-]: TEST      R12 1        ; if R12 then PC := 119
101 [-]: JMP       119          ; PC := 119
102 [-]: GETGLOBAL R14 K32      ; R14 := 0x3d106989
103 [-]: LOADK     R15 K33      ; R15 := "WARNING: WallBreach failed to generate a repair spline for "
104 [-]: GETUPVAL  R16 U3       ; R16 := U3
105 [-]: SELF      R16 R16 K34  ; R17 := R16; R16 := R16[0xed4e0128]
106 [-]: CALL      R16 2 2      ; R16 := R16(R17)
107 [-]: LOADK     R17 K35      ; R17 := " with projector "
108 [-]: SELF      R18 R1 K34   ; R19 := R1; R18 := R1[0xed4e0128]
109 [-]: CALL      R18 2 2      ; R18 := R18(R19)
110 [-]: LOADK     R19 K36      ; R19 := " at "
111 [-]: GETGLOBAL R20 K37      ; R20 := 0x64fb1586
112 [-]: SELF      R21 R1 K8    ; R22 := R1; R21 := R1[0xd1586535]
113 [-]: CALL      R21 2 0      ; R21,... := R21(R22)
114 [-]: CALL      R20 0 2      ; R20 := R20(R21,...)
115 [-]: CONCAT    R15 R15 R20  ; R15 := R15 .. R16 .. R17 .. R18 .. R19 .. R20
116 [-]: CALL      R14 2 1      ; R14(R15)
117 [-]: LOADBOOL  R2 1 0       ; R2 := true
118 [-]: JMP       188          ; PC := 188
119 [-]: GETUPVAL  R14 U6       ; R14 := U6
120 [-]: MOVE      R15 R10      ; R15 := R10
121 [-]: MOVE      R16 R11      ; R16 := R11
122 [-]: CALL      R14 3 3      ; R14,R15 := R14(R15,R16)
123 [-]: MOVE      R11 R15      ; R11 := R15
124 [-]: MOVE      R10 R14      ; R10 := R14
125 [-]: SETTABLE  R0 K38 R10   ; R0["boundsMin"] := R10
126 [-]: SETTABLE  R0 K39 R11   ; R0["boundsMax"] := R11
127 [-]: NEWTABLE  R14 0 0      ; R14 := {}
128 [-]: LOADK     R15 1        ; R15 := 1.000000
129 [-]: LEN       R16 R7       ; R16 := # R7
130 [-]: LOADK     R17 1        ; R17 := 1.000000
131 [-]: FORPREP   R15 137      ; R15 -= R17; PC := 137
132 [-]: GETGLOBAL R19 K40      ; R19 := 0x33bdd652
133 [-]: GETTABLE  R19 R19 K41  ; R19 := R19[0x23d5322f]
134 [-]: MOVE      R20 R14      ; R20 := R14
135 [-]: GETTABLE  R21 R7 R18   ; R21 := R7[R18]
136 [-]: CALL      R19 3 1      ; R19(R20,R21)
137 [-]: FORLOOP   R15 132      ; R15 += R17; if R15 <= R16 then begin PC := 132; R18 := R15 end
138 [-]: GETGLOBAL R19 K40      ; R19 := 0x33bdd652
139 [-]: GETTABLE  R19 R19 K41  ; R19 := R19[0x23d5322f]
140 [-]: MOVE      R20 R14      ; R20 := R14
141 [-]: GETTABLE  R21 R7 K42   ; R21 := R7[1.000000]
142 [-]: CALL      R19 3 1      ; R19(R20,R21)
143 [-]: SELF      R19 R6 K43   ; R20 := R6; R19 := R6[0x986d2ab8]
144 [-]: GETGLOBAL R21 K44      ; R21 := 0x0469f296
145 [-]: LOADK     R22 K45      ; R22 := "AlphaAtten"
146 [-]: CALL      R21 2 2      ; R21 := R21(R22)
147 [-]: LOADK     R22 1        ; R22 := 1.000000
148 [-]: CALL      R19 4 1      ; R19(R20,R21,R22)
149 [-]: SELF      R19 R6 K46   ; R20 := R6; R19 := R6[0x7ceafc23]
150 [-]: MOVE      R21 R14      ; R21 := R14
151 [-]: CALL      R19 3 1      ; R19(R20,R21)
152 [-]: SELF      R19 R6 K47   ; R20 := R6; R19 := R6[0x1a79ea03]
153 [-]: GETGLOBAL R21 K48      ; R21 := 0x5bced4c4
154 [-]: GETTABLE  R21 R21 K49  ; R21 := R21[0x99675e23]
155 [-]: GETGLOBAL R22 K50      ; R22 := 0xe89c04d2
156 [-]: DIV       R22 R22 R13  ; R22 := R22 / R13
157 [-]: CALL      R21 2 0      ; R21,... := R21(R22)
158 [-]: CALL      R19 0 2      ; R19 := R19(R20,...)
159 [-]: LOADK     R20 1        ; R20 := 1.000000
160 [-]: LEN       R21 R19      ; R21 := # R19
161 [-]: LOADK     R22 1        ; R22 := 1.000000
162 [-]: FORPREP   R20 187      ; R20 -= R22; PC := 187
163 [-]: GETGLOBAL R24 K51      ; R24 := 0x492c7f2a
164 [-]: GETTABLE  R25 R19 R23  ; R25 := R19[R23]
165 [-]: MOVE      R26 R3       ; R26 := R3
166 [-]: CALL      R24 3 2      ; R24 := R24(R25,R26)
167 [-]: ADD       R24 R24 R5   ; R24 := R24 + R5
168 [-]: GETGLOBAL R25 K40      ; R25 := 0x33bdd652
169 [-]: GETTABLE  R25 R25 K41  ; R25 := R25[0x23d5322f]
170 [-]: GETTABLE  R26 R0 K52   ; R26 := R0["points"]
171 [-]: MOVE      R27 R24      ; R27 := R24
172 [-]: CALL      R25 3 1      ; R25(R26,R27)
173 [-]: GETUPVAL  R25 U7       ; R25 := U7
174 [-]: TEST      R25 0        ; if not R25 then PC := 187
175 [-]: JMP       187          ; PC := 187
176 [-]: GETGLOBAL R25 K12      ; R25 := 0x89326c93
177 [-]: SELF      R25 R25 K53  ; R26 := R25; R25 := R25[0x9ed3b54e]
178 [-]: MOVE      R27 R24      ; R27 := R24
179 [-]: LOADK     R28 K54      ; R28 := 0.050000
180 [-]: GETGLOBAL R29 K55      ; R29 := 0x60130201
181 [-]: LOADK     R30 255      ; R30 := 255.000000
182 [-]: LOADK     R31 0        ; R31 := 0.000000
183 [-]: LOADK     R32 0        ; R32 := 0.000000
184 [-]: CALL      R29 4 2      ; R29 := R29(R30,R31,R32)
185 [-]: GETUPVAL  R30 U8       ; R30 := U8
186 [-]: CALL      R25 6 1      ; R25(R26,R27,R28,R29,R30)
187 [-]: FORLOOP   R20 163      ; R20 += R22; if R20 <= R21 then begin PC := 163; R23 := R20 end
188 [-]: TEST      R2 0         ; if not R2 then PC := 220
189 [-]: JMP       220          ; PC := 220
190 [-]: GETGLOBAL R25 K32      ; R25 := 0x3d106989
191 [-]: LOADK     R26 K56      ; R26 := "WARNING WallBreach: Falling back to single point repair."
192 [-]: CALL      R25 2 1      ; R25(R26)
193 [-]: GETUPVAL  R25 U3       ; R25 := U3
194 [-]: SELF      R25 R25 K8   ; R26 := R25; R25 := R25[0xd1586535]
195 [-]: CALL      R25 2 2      ; R25 := R25(R26)
196 [-]: GETGLOBAL R26 K30      ; R26 := 0xa421af95
197 [-]: LOADK     R27 1        ; R27 := 1.000000
198 [-]: LOADK     R28 1        ; R28 := 1.000000
199 [-]: LOADK     R29 1        ; R29 := 1.000000
200 [-]: CALL      R26 4 2      ; R26 := R26(R27,R28,R29)
201 [-]: SUB       R25 R25 R26  ; R25 := R25 - R26
202 [-]: SETTABLE  R0 K38 R25   ; R0["boundsMin"] := R25
203 [-]: GETUPVAL  R25 U3       ; R25 := U3
204 [-]: SELF      R25 R25 K8   ; R26 := R25; R25 := R25[0xd1586535]
205 [-]: CALL      R25 2 2      ; R25 := R25(R26)
206 [-]: GETGLOBAL R26 K30      ; R26 := 0xa421af95
207 [-]: LOADK     R27 1        ; R27 := 1.000000
208 [-]: LOADK     R28 1        ; R28 := 1.000000
209 [-]: LOADK     R29 1        ; R29 := 1.000000
210 [-]: CALL      R26 4 2      ; R26 := R26(R27,R28,R29)
211 [-]: ADD       R25 R25 R26  ; R25 := R25 + R26
212 [-]: SETTABLE  R0 K39 R25   ; R0["boundsMax"] := R25
213 [-]: GETGLOBAL R25 K40      ; R25 := 0x33bdd652
214 [-]: GETTABLE  R25 R25 K41  ; R25 := R25[0x23d5322f]
215 [-]: GETTABLE  R26 R0 K52   ; R26 := R0["points"]
216 [-]: GETUPVAL  R27 U3       ; R27 := U3
217 [-]: SELF      R27 R27 K8   ; R28 := R27; R27 := R27[0xd1586535]
218 [-]: CALL      R27 2 0      ; R27,... := R27(R28)
219 [-]: CALL      R25 0 1      ; R25(R26,...)
220 [-]: GETGLOBAL R25 K0       ; R25 := 0x7b998233
221 [-]: GETGLOBAL R26 K57      ; R26 := 0x41501b16
222 [-]: CALL      R25 2 2      ; R25 := R25(R26)
223 [-]: TEST      R25 1        ; if R25 then PC := 244
224 [-]: JMP       244          ; PC := 244
225 [-]: GETGLOBAL R25 K58      ; R25 := 0xc8802016
226 [-]: GETTABLE  R26 R0 K52   ; R26 := R0["points"]
227 [-]: CALL      R25 2 4      ; R25,R26,R27 := R25(R26)
228 [-]: JMP       242          ; PC := 242
229 [-]: GETGLOBAL R30 K12      ; R30 := 0x89326c93
230 [-]: SELF      R30 R30 K13  ; R31 := R30; R30 := R30[0x05909209]
231 [-]: GETGLOBAL R32 K57      ; R32 := 0x41501b16
232 [-]: MOVE      R33 R29      ; R33 := R29
233 [-]: MOVE      R34 R3       ; R34 := R3
234 [-]: GETUPVAL  R35 U3       ; R35 := U3
235 [-]: GETUPVAL  R36 U3       ; R36 := U3
236 [-]: CALL      R30 7 2      ; R30 := R30(R31,R32,R33,R34,R35,R36)
237 [-]: GETGLOBAL R31 K40      ; R31 := 0x33bdd652
238 [-]: GETTABLE  R31 R31 K41  ; R31 := R31[0x23d5322f]
239 [-]: GETTABLE  R32 R0 K59   ; R32 := R0["pointDecos"]
240 [-]: MOVE      R33 R30      ; R33 := R30
241 [-]: CALL      R31 3 1      ; R31(R32,R33)
242 [-]: TFORLOOP  R25 2        ; R28,R29 :=  R25(R26,R27); if R28 ~= nil then begin PC = 229; R27 := R28 end
243 [-]: JMP       229          ; PC := 229
244 [-]: GETGLOBAL R31 K1       ; R31 := _T
245 [-]: GETTABLE  R31 R31 K2   ; R31 := R31["WallBreaches"]
246 [-]: GETUPVAL  R32 U3       ; R32 := U3
247 [-]: SETTABLE  R31 R32 R0   ; R31[R32] := R0
248 [-]: RETURN    R0 1         ; return 


; Function #12:
;
; Name:            
; Defined at line: 295
; #Upvalues:       3
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  7

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
  2 [-]: GETGLOBAL R2 K1        ; R2 := _T
  3 [-]: GETTABLE  R2 R2 K2     ; R2 := R2["Malfunctions"]
  4 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  5 [-]: TEST      R1 0         ; if not R1 then PC := 14
  6 [-]: JMP       14           ; PC := 14
  7 [-]: GETGLOBAL R1 K1        ; R1 := _T
  8 [-]: NEWTABLE  R2 0 0       ; R2 := {}
  9 [-]: SETTABLE  R1 K2 R2     ; R1["Malfunctions"] := R2
 10 [-]: GETGLOBAL R1 K1        ; R1 := _T
 11 [-]: GETTABLE  R1 R1 K2     ; R1 := R1["Malfunctions"]
 12 [-]: NEWTABLE  R2 0 0       ; R2 := {}
 13 [-]: SETTABLE  R1 K3 R2     ; R1["TransmissionTimers"] := R2
 14 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 15 [-]: GETGLOBAL R2 K1        ; R2 := _T
 16 [-]: GETTABLE  R2 R2 K2     ; R2 := R2["Malfunctions"]
 17 [-]: GETTABLE  R2 R2 K3     ; R2 := R2["TransmissionTimers"]
 18 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 19 [-]: TEST      R1 0         ; if not R1 then PC := 25
 20 [-]: JMP       25           ; PC := 25
 21 [-]: GETGLOBAL R1 K1        ; R1 := _T
 22 [-]: GETTABLE  R1 R1 K2     ; R1 := R1["Malfunctions"]
 23 [-]: NEWTABLE  R2 0 0       ; R2 := {}
 24 [-]: SETTABLE  R1 K3 R2     ; R1["TransmissionTimers"] := R2
 25 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 26 [-]: GETGLOBAL R2 K1        ; R2 := _T
 27 [-]: GETTABLE  R2 R2 K2     ; R2 := R2["Malfunctions"]
 28 [-]: GETTABLE  R2 R2 K4     ; R2 := R2["WallBreachCount"]
 29 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 30 [-]: TEST      R1 1         ; if R1 then PC := 37
 31 [-]: JMP       37           ; PC := 37
 32 [-]: GETGLOBAL R1 K1        ; R1 := _T
 33 [-]: GETTABLE  R1 R1 K2     ; R1 := R1["Malfunctions"]
 34 [-]: GETTABLE  R1 R1 K4     ; R1 := R1["WallBreachCount"]
 35 [-]: EQ        0 R1 K5      ; if R1 ~= 1.000000 then PC := 39
 36 [-]: JMP       39           ; PC := 39
 37 [-]: GETUPVAL  R1 U0        ; R1 := U0
 38 [-]: CALL      R1 1 1       ; R1()
 39 [-]: GETGLOBAL R1 K6        ; R1 := 0xa1042f66
 40 [-]: GETGLOBAL R2 K7        ; R2 := 0x8dbe181c
 41 [-]: GETGLOBAL R3 K8        ; R3 := 0x0c62abf7
 42 [-]: CALL      R3 1 2       ; R3 := R3()
 43 [-]: MUL       R2 R2 R3     ; R2 := R2 * R3
 44 [-]: ADD       R1 R1 R2     ; R1 := R1 + R2
 45 [-]: GETGLOBAL R2 K0        ; R2 := 0x7b998233
 46 [-]: GETGLOBAL R3 K1        ; R3 := _T
 47 [-]: GETTABLE  R3 R3 K2     ; R3 := R3["Malfunctions"]
 48 [-]: GETTABLE  R3 R3 K3     ; R3 := R3["TransmissionTimers"]
 49 [-]: GETTABLE  R3 R3 K9     ; R3 := R3["ruptureReminderDueNext"]
 50 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 51 [-]: TEST      R2 0         ; if not R2 then PC := 60
 52 [-]: JMP       60           ; PC := 60
 53 [-]: GETGLOBAL R2 K1        ; R2 := _T
 54 [-]: GETTABLE  R2 R2 K2     ; R2 := R2["Malfunctions"]
 55 [-]: GETTABLE  R2 R2 K3     ; R2 := R2["TransmissionTimers"]
 56 [-]: GETGLOBAL R3 K10       ; R3 := 0x55156ff7
 57 [-]: CALL      R3 1 2       ; R3 := R3()
 58 [-]: ADD       R3 R3 R1     ; R3 := R3 + R1
 59 [-]: SETTABLE  R2 K9 R3     ; R2["ruptureReminderDueNext"] := R3
 60 [-]: GETGLOBAL R2 K0        ; R2 := 0x7b998233
 61 [-]: MOVE      R3 R0        ; R3 := R0
 62 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 63 [-]: TEST      R2 1         ; if R2 then PC := 98
 64 [-]: JMP       98           ; PC := 98
 65 [-]: GETGLOBAL R2 K11       ; R2 := 0xcbd666e1
 66 [-]: MOVE      R3 R1        ; R3 := R1
 67 [-]: CALL      R2 2 1       ; R2(R3)
 68 [-]: GETGLOBAL R2 K10       ; R2 := 0x55156ff7
 69 [-]: CALL      R2 1 2       ; R2 := R2()
 70 [-]: GETGLOBAL R3 K1        ; R3 := _T
 71 [-]: GETTABLE  R3 R3 K2     ; R3 := R3["Malfunctions"]
 72 [-]: GETTABLE  R3 R3 K3     ; R3 := R3["TransmissionTimers"]
 73 [-]: GETTABLE  R3 R3 K9     ; R3 := R3["ruptureReminderDueNext"]
 74 [-]: LE        0 R3 R2      ; if R3 > R2 then PC := 60
 75 [-]: JMP       60           ; PC := 60
 76 [-]: GETUPVAL  R2 U1        ; R2 := U1
 77 [-]: GETUPVAL  R3 U2        ; R3 := U2
 78 [-]: CALL      R3 1 2       ; R3 := R3()
 79 [-]: GETGLOBAL R4 K12       ; R4 := 0xe91d7466
 80 [-]: GETGLOBAL R5 K13       ; R5 := 0x0469f296
 81 [-]: LOADK     R6 K14       ; R6 := "HullRuptureReminder"
 82 [-]: CALL      R5 2 0       ; R5,... := R5(R6)
 83 [-]: CALL      R2 0 1       ; R2(R3,...)
 84 [-]: GETGLOBAL R2 K6        ; R2 := 0xa1042f66
 85 [-]: GETGLOBAL R3 K7        ; R3 := 0x8dbe181c
 86 [-]: GETGLOBAL R4 K8        ; R4 := 0x0c62abf7
 87 [-]: CALL      R4 1 2       ; R4 := R4()
 88 [-]: MUL       R3 R3 R4     ; R3 := R3 * R4
 89 [-]: ADD       R1 R2 R3     ; R1 := R2 + R3
 90 [-]: GETGLOBAL R2 K1        ; R2 := _T
 91 [-]: GETTABLE  R2 R2 K2     ; R2 := R2["Malfunctions"]
 92 [-]: GETTABLE  R2 R2 K3     ; R2 := R2["TransmissionTimers"]
 93 [-]: GETGLOBAL R3 K10       ; R3 := 0x55156ff7
 94 [-]: CALL      R3 1 2       ; R3 := R3()
 95 [-]: ADD       R3 R3 R1     ; R3 := R3 + R1
 96 [-]: SETTABLE  R2 K9 R3     ; R2["ruptureReminderDueNext"] := R3
 97 [-]: JMP       60           ; PC := 60
 98 [-]: RETURN    R0 1         ; return 


; Function #13:
;
; Name:            
; Defined at line: 321
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  6

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETTABLE  R1 R1 K0     ; R1 := R1[0x9742b85b]
  3 [-]: GETGLOBAL R2 K1        ; R2 := 0xe91d7466
  4 [-]: GETGLOBAL R3 K2        ; R3 := 0x0469f296
  5 [-]: LOADK     R4 K3        ; R4 := "BreachEventStarted"
  6 [-]: CALL      R3 2 0       ; R3,... := R3(R4)
  7 [-]: CALL      R1 0 1       ; R1(R2,...)
  8 [-]: LOADK     R1 15        ; R1 := 15.000000
  9 [-]: GETGLOBAL R2 K4        ; R2 := 0x7b998233
 10 [-]: MOVE      R3 R0        ; R3 := R0
 11 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 12 [-]: TEST      R2 1         ; if R2 then PC := 26
 13 [-]: JMP       26           ; PC := 26
 14 [-]: GETGLOBAL R2 K5        ; R2 := 0xcbd666e1
 15 [-]: MOVE      R3 R1        ; R3 := R1
 16 [-]: CALL      R2 2 1       ; R2(R3)
 17 [-]: GETUPVAL  R2 U0        ; R2 := U0
 18 [-]: GETTABLE  R2 R2 K0     ; R2 := R2[0x9742b85b]
 19 [-]: GETGLOBAL R3 K1        ; R3 := 0xe91d7466
 20 [-]: GETGLOBAL R4 K2        ; R4 := 0x0469f296
 21 [-]: LOADK     R5 K6        ; R5 := "BreachEventOngoing"
 22 [-]: CALL      R4 2 0       ; R4,... := R4(R5)
 23 [-]: CALL      R2 0 1       ; R2(R3,...)
 24 [-]: LOADK     R1 15        ; R1 := 15.000000
 25 [-]: JMP       9            ; PC := 9
 26 [-]: RETURN    R0 1         ; return 


; Function #14:
;
; Name:            
; Defined at line: 333
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  13

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0xa421af95
  2 [-]: CALL      R0 1 2       ; R0 := R0()
  3 [-]: NEWTABLE  R1 4 0       ; R1 := {}
  4 [-]: GETGLOBAL R2 K1        ; R2 := gBaseAvatarType
  5 [-]: GETGLOBAL R3 K2        ; R3 := gPickUpType
  6 [-]: GETGLOBAL R4 K3        ; R4 := gRagdollType
  7 [-]: GETGLOBAL R5 K4        ; R5 := gHitProxyType
  8 [-]: SETLIST   R1 4 1       ; R1[(1-1)*FPF+i] := R(1+i), 1 <= i <= 4
  9 [-]: GETGLOBAL R2 K5        ; R2 := 0xf6c6e505
 10 [-]: GETUPVAL  R3 U0        ; R3 := U0
 11 [-]: SELF      R3 R3 K6     ; R4 := R3; R3 := R3[0xcb3851b8]
 12 [-]: CALL      R3 2 0       ; R3,... := R3(R4)
 13 [-]: CALL      R2 0 2       ; R2 := R2(R3,...)
 14 [-]: GETUPVAL  R3 U0        ; R3 := U0
 15 [-]: SELF      R3 R3 K7     ; R4 := R3; R3 := R3[0xd1586535]
 16 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 17 [-]: MUL       R4 R2 K8     ; R4 := R2 * 0.010000
 18 [-]: SUB       R3 R3 R4     ; R3 := R3 - R4
 19 [-]: GETUPVAL  R4 U0        ; R4 := U0
 20 [-]: SELF      R4 R4 K7     ; R5 := R4; R4 := R4[0xd1586535]
 21 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 22 [-]: MUL       R5 R2 K9     ; R5 := R2 * 2.000000
 23 [-]: ADD       R4 R4 R5     ; R4 := R4 + R5
 24 [-]: GETUPVAL  R5 U1        ; R5 := U1
 25 [-]: TEST      R5 0         ; if not R5 then PC := 38
 26 [-]: JMP       38           ; PC := 38
 27 [-]: GETGLOBAL R5 K10       ; R5 := 0x89326c93
 28 [-]: SELF      R5 R5 K11    ; R6 := R5; R5 := R5[0x980336a8]
 29 [-]: MOVE      R7 R3        ; R7 := R3
 30 [-]: MOVE      R8 R4        ; R8 := R4
 31 [-]: GETGLOBAL R9 K12       ; R9 := 0x60130201
 32 [-]: LOADK     R10 0        ; R10 := 0.000000
 33 [-]: LOADK     R11 0        ; R11 := 0.000000
 34 [-]: LOADK     R12 255      ; R12 := 255.000000
 35 [-]: CALL      R9 4 2       ; R9 := R9(R10,R11,R12)
 36 [-]: LOADK     R10 30       ; R10 := 30.000000
 37 [-]: CALL      R5 6 1       ; R5(R6,R7,R8,R9,R10)
 38 [-]: GETGLOBAL R5 K10       ; R5 := 0x89326c93
 39 [-]: SELF      R5 R5 K13    ; R6 := R5; R5 := R5[0x722cd32c]
 40 [-]: MOVE      R7 R3        ; R7 := R3
 41 [-]: MOVE      R8 R4        ; R8 := R4
 42 [-]: MOVE      R9 R1        ; R9 := R1
 43 [-]: LOADNIL   R10 R10      ; R10 := nil
 44 [-]: MOVE      R11 R0       ; R11 := R0
 45 [-]: CALL      R5 7 2       ; R5 := R5(R6,R7,R8,R9,R10,R11)
 46 [-]: TEST      R5 0         ; if not R5 then PC := 53
 47 [-]: JMP       53           ; PC := 53
 48 [-]: GETUPVAL  R6 U0        ; R6 := U0
 49 [-]: SELF      R6 R6 K14    ; R7 := R6; R6 := R6[0x9307aa51]
 50 [-]: MOVE      R8 R0        ; R8 := R0
 51 [-]: CALL      R6 3 1       ; R6(R7,R8)
 52 [-]: JMP       56           ; PC := 56
 53 [-]: GETGLOBAL R6 K15       ; R6 := 0x3d106989
 54 [-]: LOADK     R7 K16       ; R7 := "WARNING: WallBreach failed to place itself on wall"
 55 [-]: CALL      R6 2 1       ; R6(R7)
 56 [-]: RETURN    R0 1         ; return 


; Function #15:
;
; Name:            
; Defined at line: 350
; #Upvalues:       6
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: SETUPVAL  R0 U0        ; U82 := R0
  2 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
  3 [-]: GETUPVAL  R2 U0        ; R2 := U0
  4 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  5 [-]: TEST      R1 1         ; if R1 then PC := 11
  6 [-]: JMP       11           ; PC := 11
  7 [-]: GETGLOBAL R1 K1        ; R1 := 0x11a19c5e
  8 [-]: GETUPVAL  R2 U0        ; R2 := U0
  9 [-]: LOADK     R3 K2        ; R3 := "OnDestroyed"
 10 [-]: CALL      R1 3 1       ; R1(R2,R3)
 11 [-]: GETUPVAL  R1 U0        ; R1 := U0
 12 [-]: SELF      R1 R1 K3     ; R2 := R1; R1 := R1[0xcde10c4a]
 13 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 14 [-]: SELF      R1 R1 K4     ; R2 := R1; R1 := R1[0xed4e0128]
 15 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 16 [-]: GETUPVAL  R2 U2        ; R2 := U2
 17 [-]: EQ        1 R1 R2      ; if R1 == R2 then PC := 20
 18 [-]: JMP       20           ; PC := 20
 19 [-]: LOADBOOL  R1 0 1       ; R1 := false; PC := 20
 20 [-]: LOADBOOL  R1 1 0       ; R1 := true
 21 [-]: SETUPVAL  R1 U1        ; U82 := R1
 22 [-]: GETUPVAL  R1 U1        ; R1 := U1
 23 [-]: TEST      R1 0         ; if not R1 then PC := 51
 24 [-]: JMP       51           ; PC := 51
 25 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 26 [-]: GETGLOBAL R2 K5        ; R2 := _T
 27 [-]: GETTABLE  R2 R2 K6     ; R2 := R2["Malfunctions"]
 28 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 29 [-]: TEST      R1 0         ; if not R1 then PC := 34
 30 [-]: JMP       34           ; PC := 34
 31 [-]: GETGLOBAL R1 K5        ; R1 := _T
 32 [-]: NEWTABLE  R2 0 0       ; R2 := {}
 33 [-]: SETTABLE  R1 K6 R2     ; R1["Malfunctions"] := R2
 34 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 35 [-]: GETGLOBAL R2 K5        ; R2 := _T
 36 [-]: GETTABLE  R2 R2 K6     ; R2 := R2["Malfunctions"]
 37 [-]: GETTABLE  R2 R2 K7     ; R2 := R2["WallBreachCount"]
 38 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 39 [-]: TEST      R1 0         ; if not R1 then PC := 44
 40 [-]: JMP       44           ; PC := 44
 41 [-]: GETGLOBAL R1 K5        ; R1 := _T
 42 [-]: GETTABLE  R1 R1 K6     ; R1 := R1["Malfunctions"]
 43 [-]: SETTABLE  R1 K7 K8     ; R1["WallBreachCount"] := 0.000000
 44 [-]: GETGLOBAL R1 K5        ; R1 := _T
 45 [-]: GETTABLE  R1 R1 K6     ; R1 := R1["Malfunctions"]
 46 [-]: GETGLOBAL R2 K5        ; R2 := _T
 47 [-]: GETTABLE  R2 R2 K6     ; R2 := R2["Malfunctions"]
 48 [-]: GETTABLE  R2 R2 K7     ; R2 := R2["WallBreachCount"]
 49 [-]: ADD       R2 R2 K9     ; R2 := R2 + 1.000000
 50 [-]: SETTABLE  R1 K7 R2     ; R1["WallBreachCount"] := R2
 51 [-]: GETGLOBAL R1 K10       ; R1 := 0x89326c93
 52 [-]: SELF      R1 R1 K11    ; R2 := R1; R1 := R1[0x18d05d30]
 53 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 54 [-]: TEST      R1 0         ; if not R1 then PC := 74
 55 [-]: JMP       74           ; PC := 74
 56 [-]: GETUPVAL  R1 U3        ; R1 := U3
 57 [-]: CALL      R1 1 1       ; R1()
 58 [-]: GETUPVAL  R1 U1        ; R1 := U1
 59 [-]: TEST      R1 0         ; if not R1 then PC := 68
 60 [-]: JMP       68           ; PC := 68
 61 [-]: SELF      R1 R0 K12    ; R2 := R0; R1 := R0[0xd5f7912b]
 62 [-]: GETGLOBAL R3 K13       ; R3 := 0x0469f296
 63 [-]: LOADK     R4 K14       ; R4 := "RuptureTransmissions"
 64 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 65 [-]: LOADBOOL  R4 0 0       ; R4 := false
 66 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 67 [-]: JMP       74           ; PC := 74
 68 [-]: SELF      R1 R0 K12    ; R2 := R0; R1 := R0[0xd5f7912b]
 69 [-]: GETGLOBAL R3 K13       ; R3 := 0x0469f296
 70 [-]: LOADK     R4 K15       ; R4 := "BreachTransmissions"
 71 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 72 [-]: LOADBOOL  R4 0 0       ; R4 := false
 73 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 74 [-]: GETUPVAL  R1 U1        ; R1 := U1
 75 [-]: TEST      R1 0         ; if not R1 then PC := 80
 76 [-]: JMP       80           ; PC := 80
 77 [-]: GETUPVAL  R1 U4        ; R1 := U4
 78 [-]: LOADBOOL  R2 1 0       ; R2 := true
 79 [-]: CALL      R1 2 1       ; R1(R2)
 80 [-]: GETUPVAL  R1 U5        ; R1 := U5
 81 [-]: CALL      R1 1 1       ; R1()
 82 [-]: RETURN    R0 1         ; return 


