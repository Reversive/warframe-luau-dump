; This file has been disassembled using luadec 2.0.2 by sztupy (modified by Reversive)

; Name:            
; Defined at line: 0
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       2
; Max Stack Size:  31

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x2d0fad09
  2 [-]: LOADK     R1 K1        ; R1 := "Lotus.Interface.LotusUtilities"
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K0        ; R1 := 0x2d0fad09
  5 [-]: LOADK     R2 K2        ; R2 := "Lotus.Scripts.Libs.LandscapeLib"
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: GETGLOBAL R2 K0        ; R2 := 0x2d0fad09
  8 [-]: LOADK     R3 K3        ; R3 := "Lotus.Scripts.Libs.TransmissionSet"
  9 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 10 [-]: GETGLOBAL R3 K0        ; R3 := 0x2d0fad09
 11 [-]: LOADK     R4 K4        ; R4 := "Lotus.Interface.Libs.TimerMgr"
 12 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 13 [-]: GETGLOBAL R4 K5        ; R4 := 0x88efc25e
 14 [-]: LOADK     R5 K6        ; R5 := "/Lotus/Types/Game/MarkerInfos/EidolonObjectiveMarker"
 15 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 16 [-]: GETGLOBAL R5 K7        ; R5 := 0xb009bbc6
 17 [-]: LOADK     R6 K8        ; R6 := "/Lotus/Sounds/UI/NewUI/ObjectiveMarkers/PoEDynamicMissionMarker"
 18 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 19 [-]: GETGLOBAL R6 K9        ; R6 := 0x7ed0a956
 20 [-]: LOADK     R7 K10       ; R7 := "/Lotus/Powersuits/Wisp/WispBaseSuit"
 21 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 22 [-]: GETGLOBAL R7 K11       ; R7 := 0x0469f296
 23 [-]: LOADK     R8 K12       ; R8 := "MODE_STATE"
 24 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 25 [-]: LOADNIL   R8 R11       ; R8 := R9 := R10 := R11 := nil
 26 [-]: LOADK     R12 0        ; R12 := 0.000000
 27 [-]: LOADK     R13 0        ; R13 := 0.000000
 28 [-]: LOADNIL   R14 R15      ; R14 := R15 := nil
 29 [-]: LOADK     R16 0        ; R16 := 0.000000
 30 [-]: LOADK     R17 1        ; R17 := 1.000000
 31 [-]: LOADK     R18 2        ; R18 := 2.000000
 32 [-]: MOVE      R19 R16      ; R19 := R16
 33 [-]: LOADNIL   R20 R20      ; R20 := nil
 34 [-]: LOADK     R21 K13      ; R21 := "/Lotus/Language/InfestedPlainsEvent/EventEncounterFindCanister"
 35 [-]: LOADNIL   R22 R25      ; R22 := R23 := R24 := R25 := nil
 36 [-]: NEWTABLE  R26 0 0      ; R26 := {}
 37 [-]: CLOSURE   R27 0        ; R27 := closure(Function #1)
 38 [-]: MOVE      R0 R2        ; R0 := R2
 39 [-]: MOVE      R0 R15       ; R0 := R15
 40 [-]: CLOSURE   R28 1        ; R28 := closure(Function #2)
 41 [-]: MOVE      R0 R19       ; R0 := R19
 42 [-]: MOVE      R0 R17       ; R0 := R17
 43 [-]: MOVE      R0 R23       ; R0 := R23
 44 [-]: MOVE      R0 R27       ; R0 := R27
 45 [-]: MOVE      R0 R18       ; R0 := R18
 46 [-]: MOVE      R0 R1        ; R0 := R1
 47 [-]: MOVE      R0 R21       ; R0 := R21
 48 [-]: CLOSURE   R20 2        ; R20 := closure(Function #3)
 49 [-]: MOVE      R0 R19       ; R0 := R19
 50 [-]: MOVE      R0 R8        ; R0 := R8
 51 [-]: MOVE      R0 R7        ; R0 := R7
 52 [-]: MOVE      R0 R28       ; R0 := R28
 53 [-]: CLOSURE   R29 3        ; R29 := closure(Function #4)
 54 [-]: MOVE      R0 R8        ; R0 := R8
 55 [-]: MOVE      R0 R14       ; R0 := R14
 56 [-]: MOVE      R0 R9        ; R0 := R9
 57 [-]: MOVE      R0 R10       ; R0 := R10
 58 [-]: MOVE      R0 R11       ; R0 := R11
 59 [-]: MOVE      R0 R12       ; R0 := R12
 60 [-]: MOVE      R0 R13       ; R0 := R13
 61 [-]: MOVE      R0 R15       ; R0 := R15
 62 [-]: MOVE      R0 R26       ; R0 := R26
 63 [-]: MOVE      R0 R23       ; R0 := R23
 64 [-]: MOVE      R0 R3        ; R0 := R3
 65 [-]: MOVE      R0 R1        ; R0 := R1
 66 [-]: MOVE      R0 R0        ; R0 := R0
 67 [-]: MOVE      R0 R24       ; R0 := R24
 68 [-]: MOVE      R0 R22       ; R0 := R22
 69 [-]: MOVE      R0 R4        ; R0 := R4
 70 [-]: MOVE      R0 R25       ; R0 := R25
 71 [-]: MOVE      R0 R20       ; R0 := R20
 72 [-]: MOVE      R0 R17       ; R0 := R17
 73 [-]: MOVE      R0 R7        ; R0 := R7
 74 [-]: CLOSURE   R30 4        ; R30 := closure(Function #5)
 75 [-]: MOVE      R0 R29       ; R0 := R29
 76 [-]: MOVE      R0 R19       ; R0 := R19
 77 [-]: MOVE      R0 R17       ; R0 := R17
 78 [-]: MOVE      R0 R14       ; R0 := R14
 79 [-]: MOVE      R0 R20       ; R0 := R20
 80 [-]: MOVE      R0 R18       ; R0 := R18
 81 [-]: MOVE      R0 R23       ; R0 := R23
 82 [-]: MOVE      R0 R2        ; R0 := R2
 83 [-]: MOVE      R0 R15       ; R0 := R15
 84 [-]: MOVE      R0 R22       ; R0 := R22
 85 [-]: MOVE      R0 R1        ; R0 := R1
 86 [-]: MOVE      R0 R9        ; R0 := R9
 87 [-]: SETGLOBAL R30 K14      ; EventStart := R30
 88 [-]: CLOSURE   R30 5        ; R30 := closure(Function #6)
 89 [-]: MOVE      R0 R26       ; R0 := R26
 90 [-]: SETGLOBAL R30 K15      ; OnPlayersChanged := R30
 91 [-]: CLOSURE   R30 6        ; R30 := closure(Function #7)
 92 [-]: MOVE      R0 R10       ; R0 := R10
 93 [-]: SETGLOBAL R30 K16      ; OnActivated := R30
 94 [-]: CLOSURE   R30 7        ; R30 := closure(Function #8)
 95 [-]: MOVE      R0 R6        ; R0 := R6
 96 [-]: SETGLOBAL R30 K17      ; CanisterCompleteScript := R30
 97 [-]: CLOSURE   R30 8        ; R30 := closure(Function #9)
 98 [-]: MOVE      R0 R22       ; R0 := R22
 99 [-]: SETGLOBAL R30 K18      ; Activated := R30
100 [-]: CLOSURE   R30 9        ; R30 := closure(Function #10)
101 [-]: MOVE      R0 R26       ; R0 := R26
102 [-]: MOVE      R0 R5        ; R0 := R5
103 [-]: MOVE      R0 R2        ; R0 := R2
104 [-]: MOVE      R0 R0        ; R0 := R0
105 [-]: SETGLOBAL R30 K19      ; OnPrimed := R30
106 [-]: RETURN    R0 1         ; return 


; Function #1:
;
; Name:            
; Defined at line: 45
; #Upvalues:       2
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETTABLE  R1 R1 K0     ; R82 := R1[0x9742b85b]
  3 [-]: GETUPVAL  R2 U1        ; R2 := U1
  4 [-]: MOVE      R3 R0        ; R3 := R0
  5 [-]: CALL      R1 3 1       ; R1(R2,R3)
  6 [-]: RETURN    R0 1         ; return 


; Function #2:
;
; Name:            
; Defined at line: 49
; #Upvalues:       7
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  7

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: GETUPVAL  R1 U1        ; R1 := U1
  3 [-]: EQ        0 R0 R1      ; if R0 ~= R1 then PC := 15
  4 [-]: JMP       15           ; PC := 15
  5 [-]: GETUPVAL  R0 U2        ; R0 := U2
  6 [-]: SELF      R0 R0 K0     ; R1 := R0; R0 := R0[0xbd2e96ea]
  7 [-]: LOADK     R2 10        ; R2 := 10.000000
  8 [-]: GETUPVAL  R3 U3        ; R3 := U3
  9 [-]: LOADBOOL  R4 0 0       ; R4 := false
 10 [-]: GETGLOBAL R5 K1        ; R5 := 0x0469f296
 11 [-]: LOADK     R6 K2        ; R6 := "OnTheWay"
 12 [-]: CALL      R5 2 0       ; R5,... := R5(R6)
 13 [-]: CALL      R0 0 1       ; R0(R1,...)
 14 [-]: JMP       25           ; PC := 25
 15 [-]: GETUPVAL  R0 U0        ; R0 := U0
 16 [-]: GETUPVAL  R1 U4        ; R1 := U4
 17 [-]: EQ        0 R0 R1      ; if R0 ~= R1 then PC := 25
 18 [-]: JMP       25           ; PC := 25
 19 [-]: GETUPVAL  R0 U5        ; R0 := U5
 20 [-]: GETTABLE  R0 R0 K3     ; R82 := R0[0xe2871589]
 21 [-]: LOADK     R1 K4        ; R1 := "<MISSION_MARKER_GENERIC>"
 22 [-]: LOADK     R2 35        ; R2 := 35.000000
 23 [-]: GETUPVAL  R3 U6        ; R3 := U6
 24 [-]: CALL      R0 4 1       ; R0(R1,R2,R3)
 25 [-]: RETURN    R0 1         ; return 


; Function #3:
;
; Name:            
; Defined at line: 59
; #Upvalues:       4
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: EQ        1 R1 R0      ; if R1 == R0 then PC := 18
  3 [-]: JMP       18           ; PC := 18
  4 [-]: SETUPVAL  R0 U0        ; U82 := 
  5 [-]: GETUPVAL  R1 U1        ; R1 := U1
  6 [-]: SELF      R1 R1 K0     ; R2 := R1; R1 := R1[0x751f061d]
  7 [-]: GETUPVAL  R3 U2        ; R3 := U2
  8 [-]: MOVE      R4 R0        ; R4 := R0
  9 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 10 [-]: GETUPVAL  R1 U3        ; R1 := U3
 11 [-]: CALL      R1 1 1       ; R1()
 12 [-]: GETGLOBAL R1 K1        ; R1 := 0x3d106989
 13 [-]: LOADK     R2 K2        ; R2 := "EventInfestedPoison.lua::SetModeState - New State: "
 14 [-]: GETUPVAL  R3 U0        ; R3 := U0
 15 [-]: CONCAT    R2 R2 R3     ; R2 := R2 .. R3
 16 [-]: CALL      R1 2 1       ; R1(R2)
 17 [-]: JMP       21           ; PC := 21
 18 [-]: GETGLOBAL R1 K1        ; R1 := 0x3d106989
 19 [-]: LOADK     R2 K3        ; R2 := "EventInfestedPoison.lua::SetModeState - trying to set mode to state we're already in"
 20 [-]: CALL      R1 2 1       ; R1(R2)
 21 [-]: RETURN    R0 1         ; return 


; Function #4:
;
; Name:            
; Defined at line: 70
; #Upvalues:       20
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  7

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0xbe190284
  2 [-]: SETUPVAL  R1 U0        ; U82 := 
  3 [-]: SELF      R1 R0 K1     ; R2 := R0; R1 := R0[0x891629fa]
  4 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  5 [-]: SETUPVAL  R1 U1        ; U82 := 
  6 [-]: GETGLOBAL R1 K2        ; R1 := 0x89326c93
  7 [-]: SELF      R1 R1 K3     ; R2 := R1; R1 := R1[0x29ef273d]
  8 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  9 [-]: SELF      R1 R1 K4     ; R2 := R1; R1 := R1[0x66905cb0]
 10 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 11 [-]: SETUPVAL  R1 U2        ; U82 := 
 12 [-]: SETUPVAL  R0 U3        ; U82 := 
 13 [-]: SELF      R1 R0 K5     ; R2 := R0; R1 := R0[0xd1586535]
 14 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 15 [-]: SETUPVAL  R1 U4        ; U82 := 
 16 [-]: SELF      R1 R0 K6     ; R2 := R0; R1 := R0[0xc5b92518]
 17 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 18 [-]: SETUPVAL  R1 U5        ; U82 := 
 19 [-]: SELF      R1 R0 K7     ; R2 := R0; R1 := R0[0x7c97b143]
 20 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 21 [-]: SETUPVAL  R1 U6        ; U82 := 
 22 [-]: SELF      R1 R0 K8     ; R2 := R0; R1 := R0[0x4c976eda]
 23 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 24 [-]: SELF      R2 R1 K9     ; R3 := R1; R2 := R1[0xe4c355e2]
 25 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 26 [-]: SETUPVAL  R2 U7        ; U82 := 
 27 [-]: GETUPVAL  R2 U2        ; R2 := U2
 28 [-]: SELF      R2 R2 K10    ; R3 := R2; R2 := R2[0x2d2bdbb8]
 29 [-]: LOADBOOL  R4 0 0       ; R4 := false
 30 [-]: CALL      R2 3 1       ; R2(R3,R4)
 31 [-]: GETGLOBAL R2 K2        ; R2 := 0x89326c93
 32 [-]: SELF      R2 R2 K11    ; R3 := R2; R2 := R2[0xb7d33840]
 33 [-]: LOADK     R4 K12       ; R4 := "OnPlayersChanged"
 34 [-]: CALL      R2 3 1       ; R2(R3,R4)
 35 [-]: GETGLOBAL R2 K2        ; R2 := 0x89326c93
 36 [-]: SELF      R2 R2 K13    ; R3 := R2; R2 := R2[0x7d108ddb]
 37 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 38 [-]: SETUPVAL  R2 U8        ; U82 := 
 39 [-]: GETUPVAL  R2 U10       ; R2 := U10
 40 [-]: GETTABLE  R2 R2 K14    ; R82 := R2[0xde474187]
 41 [-]: CALL      R2 1 2       ; R2 := R2()
 42 [-]: SETUPVAL  R2 U9        ; U82 := 
 43 [-]: GETUPVAL  R2 U11       ; R2 := U11
 44 [-]: GETTABLE  R2 R2 K15    ; R82 := R2[0x242f0aac]
 45 [-]: GETUPVAL  R3 U12       ; R3 := U12
 46 [-]: GETTABLE  R3 R3 K16    ; R3 := R3["HT_LABEL"]
 47 [-]: CALL      R2 2 1       ; R2(R3)
 48 [-]: GETGLOBAL R2 K2        ; R2 := 0x89326c93
 49 [-]: SELF      R2 R2 K17    ; R3 := R2; R2 := R2[0xc7b81e8d]
 50 [-]: GETGLOBAL R4 K18       ; R4 := 0x0469f296
 51 [-]: LOADK     R5 K19       ; R5 := "CanisterAction"
 52 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 53 [-]: GETUPVAL  R5 U4        ; R5 := U4
 54 [-]: CALL      R2 4 2       ; R2 := R2(R3,R4,R5)
 55 [-]: SETUPVAL  R2 U13       ; U82 := 
 56 [-]: GETUPVAL  R2 U13       ; R2 := U13
 57 [-]: SELF      R2 R2 K20    ; R3 := R2; R2 := R2[0x383d2e7d]
 58 [-]: CALL      R2 2 1       ; R2(R3)
 59 [-]: GETGLOBAL R2 K21       ; R2 := 0x11a19c5e
 60 [-]: GETUPVAL  R3 U13       ; R3 := U13
 61 [-]: LOADK     R4 K22       ; R4 := "OnActivated"
 62 [-]: CALL      R2 3 1       ; R2(R3,R4)
 63 [-]: GETGLOBAL R2 K2        ; R2 := 0x89326c93
 64 [-]: SELF      R2 R2 K23    ; R3 := R2; R2 := R2[0x05909209]
 65 [-]: GETUPVAL  R4 U15       ; R4 := U15
 66 [-]: GETUPVAL  R5 U13       ; R5 := U13
 67 [-]: SELF      R5 R5 K5     ; R6 := R5; R5 := R5[0xd1586535]
 68 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 69 [-]: GETGLOBAL R6 K24       ; R6 := ZERO_ROTATION
 70 [-]: CALL      R2 5 2       ; R2 := R2(R3,R4,R5,R6)
 71 [-]: SETUPVAL  R2 U14       ; U82 := 
 72 [-]: GETUPVAL  R2 U14       ; R2 := U14
 73 [-]: SELF      R2 R2 K25    ; R3 := R2; R2 := R2[0x5004be24]
 74 [-]: LOADK     R4 30        ; R4 := 30.000000
 75 [-]: CALL      R2 3 1       ; R2(R3,R4)
 76 [-]: GETUPVAL  R2 U14       ; R2 := U14
 77 [-]: SELF      R2 R2 K26    ; R3 := R2; R2 := R2[0x53bc0559]
 78 [-]: GETGLOBAL R4 K27       ; R4 := 0xb7cbd06b
 79 [-]: LOADK     R5 30        ; R5 := 30.000000
 80 [-]: LOADK     R6 5000      ; R6 := 5000.000000
 81 [-]: CALL      R4 3 0       ; R4,... := R4(R5,R6)
 82 [-]: CALL      R2 0 1       ; R2(R3,...)
 83 [-]: GETGLOBAL R2 K2        ; R2 := 0x89326c93
 84 [-]: SELF      R2 R2 K17    ; R3 := R2; R2 := R2[0xc7b81e8d]
 85 [-]: GETGLOBAL R4 K18       ; R4 := 0x0469f296
 86 [-]: LOADK     R5 K28       ; R5 := "CanisterLookTrigger"
 87 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 88 [-]: GETUPVAL  R5 U13       ; R5 := U13
 89 [-]: SELF      R5 R5 K5     ; R6 := R5; R5 := R5[0xd1586535]
 90 [-]: CALL      R5 2 0       ; R5,... := R5(R6)
 91 [-]: CALL      R2 0 2       ; R2 := R2(R3,...)
 92 [-]: SETUPVAL  R2 U16       ; U82 := 
 93 [-]: GETUPVAL  R2 U16       ; R2 := U16
 94 [-]: SELF      R2 R2 K29    ; R3 := R2; R2 := R2[0x8eb2112d]
 95 [-]: LOADK     R4 K30       ; R4 := "Enable"
 96 [-]: CALL      R2 3 1       ; R2(R3,R4)
 97 [-]: GETGLOBAL R2 K21       ; R2 := 0x11a19c5e
 98 [-]: GETUPVAL  R3 U16       ; R3 := U16
 99 [-]: LOADK     R4 K31       ; R4 := "Activated"
100 [-]: CALL      R2 3 1       ; R2(R3,R4)
101 [-]: GETUPVAL  R2 U3        ; R2 := U3
102 [-]: SELF      R2 R2 K32    ; R3 := R2; R2 := R2[0xabe61691]
103 [-]: CALL      R2 2 2       ; R2 := R2(R3)
104 [-]: EQ        0 R2 K33     ; if R2 ~= 0.000000 then PC := 114
105 [-]: JMP       114          ; PC := 114
106 [-]: GETUPVAL  R2 U3        ; R2 := U3
107 [-]: SELF      R2 R2 K34    ; R3 := R2; R2 := R2[0x5b18bb5d]
108 [-]: LOADK     R4 1         ; R4 := 1.000000
109 [-]: CALL      R2 3 1       ; R2(R3,R4)
110 [-]: GETUPVAL  R2 U17       ; R2 := U17
111 [-]: GETUPVAL  R3 U18       ; R3 := U18
112 [-]: CALL      R2 2 1       ; R2(R3)
113 [-]: JMP       121          ; PC := 121
114 [-]: GETUPVAL  R2 U17       ; R2 := U17
115 [-]: GETUPVAL  R3 U0        ; R3 := U0
116 [-]: SELF      R3 R3 K35    ; R4 := R3; R3 := R3[0x0eb34c69]
117 [-]: GETUPVAL  R5 U19       ; R5 := U19
118 [-]: GETUPVAL  R6 U18       ; R6 := U18
119 [-]: CALL      R3 4 0       ; R3,... := R3(R4,R5,R6)
120 [-]: CALL      R2 0 1       ; R2(R3,...)
121 [-]: SELF      R2 R0 K36    ; R3 := R0; R2 := R0[0xefe6cad1]
122 [-]: CALL      R2 2 2       ; R2 := R2(R3)
123 [-]: EQ        0 R2 K38     ; if R2 ~= 1.000000 then PC := 128
124 [-]: JMP       128          ; PC := 128
125 [-]: SELF      R2 R0 K39    ; R3 := R0; R2 := R0[0xfe9dc265]
126 [-]: LOADK     R4 2         ; R4 := 2.000000
127 [-]: CALL      R2 3 1       ; R2(R3,R4)
128 [-]: RETURN    R0 1         ; return 


; Function #5:
;
; Name:            
; Defined at line: 115
; #Upvalues:       12
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  6

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: MOVE      R2 R0        ; R2 := R0
  3 [-]: CALL      R1 2 1       ; R1(R2)
  4 [-]: LOADK     R1 0         ; R1 := 0.000000
  5 [-]: SELF      R2 R0 K0     ; R3 := R0; R2 := R0[0xefe6cad1]
  6 [-]: CALL      R2 2 2       ; R2 := R2(R3)
  7 [-]: LT        0 R2 K2      ; if R2 >= 4.000000 then PC := 42
  8 [-]: JMP       42           ; PC := 42
  9 [-]: GETGLOBAL R2 K3        ; R2 := 0x67652851
 10 [-]: CALL      R2 1 2       ; R2 := R2()
 11 [-]: MOVE      R1 R2        ; R1 := R2
 12 [-]: GETUPVAL  R2 U1        ; R2 := U1
 13 [-]: GETUPVAL  R3 U2        ; R3 := U2
 14 [-]: EQ        0 R2 R3      ; if R2 ~= R3 then PC := 30
 15 [-]: JMP       30           ; PC := 30
 16 [-]: GETUPVAL  R2 U3        ; R2 := U3
 17 [-]: SELF      R2 R2 K0     ; R3 := R2; R2 := R2[0xefe6cad1]
 18 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 19 [-]: LE        1 K4 R2      ; if 2.000000 <= R2 then PC := 26
 20 [-]: JMP       26           ; PC := 26
 21 [-]: GETUPVAL  R2 U3        ; R2 := U3
 22 [-]: SELF      R2 R2 K5     ; R3 := R2; R2 := R2[0xd8140b94]
 23 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 24 [-]: TEST      R2 1         ; if R2 then PC := 34
 25 [-]: JMP       34           ; PC := 34
 26 [-]: GETUPVAL  R2 U4        ; R2 := U4
 27 [-]: GETUPVAL  R3 U5        ; R3 := U5
 28 [-]: CALL      R2 2 1       ; R2(R3)
 29 [-]: JMP       34           ; PC := 34
 30 [-]: GETUPVAL  R2 U1        ; R2 := U1
 31 [-]: GETUPVAL  R3 U5        ; R3 := U5
 32 [-]: EQ        0 R2 R3      ; if R2 ~= R3 then PC := 34
 33 [-]: JMP       34           ; PC := 34
 34 [-]: GETUPVAL  R2 U6        ; R2 := U6
 35 [-]: SELF      R2 R2 K6     ; R3 := R2; R2 := R2[0xfaa69527]
 36 [-]: MOVE      R4 R1        ; R4 := R1
 37 [-]: CALL      R2 3 1       ; R2(R3,R4)
 38 [-]: GETGLOBAL R2 K7        ; R2 := 0xcbd666e1
 39 [-]: LOADK     R3 0         ; R3 := 0.000000
 40 [-]: CALL      R2 2 1       ; R2(R3)
 41 [-]: JMP       5            ; PC := 5
 42 [-]: GETUPVAL  R2 U7        ; R2 := U7
 43 [-]: GETTABLE  R2 R2 K8     ; R82 := R2[0x9742b85b]
 44 [-]: GETUPVAL  R3 U8        ; R3 := U8
 45 [-]: GETGLOBAL R4 K9        ; R4 := 0x0469f296
 46 [-]: LOADK     R5 K10       ; R5 := "FoundPoison"
 47 [-]: CALL      R4 2 0       ; R4,... := R4(R5)
 48 [-]: CALL      R2 0 1       ; R2(R3,...)
 49 [-]: GETGLOBAL R2 K11       ; R2 := 0x7b998233
 50 [-]: GETUPVAL  R3 U9        ; R3 := U9
 51 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 52 [-]: TEST      R2 1         ; if R2 then PC := 57
 53 [-]: JMP       57           ; PC := 57
 54 [-]: GETUPVAL  R2 U9        ; R2 := U9
 55 [-]: SELF      R2 R2 K12    ; R3 := R2; R2 := R2[0xa2880940]
 56 [-]: CALL      R2 2 1       ; R2(R3)
 57 [-]: GETUPVAL  R2 U10       ; R2 := U10
 58 [-]: GETTABLE  R2 R2 K13    ; R82 := R2[0xe69049eb]
 59 [-]: GETUPVAL  R3 U3        ; R3 := U3
 60 [-]: CALL      R2 2 1       ; R2(R3)
 61 [-]: GETUPVAL  R2 U11       ; R2 := U11
 62 [-]: SELF      R2 R2 K14    ; R3 := R2; R2 := R2[0x2d2bdbb8]
 63 [-]: LOADBOOL  R4 1 0       ; R4 := true
 64 [-]: CALL      R2 3 1       ; R2(R3,R4)
 65 [-]: RETURN    R0 1         ; return 


; Function #6:
;
; Name:            
; Defined at line: 146
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x89326c93
  2 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x7d108ddb]
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: SETUPVAL  R0 U0        ; U82 := 
  5 [-]: RETURN    R0 1         ; return 


; Function #7:
;
; Name:            
; Defined at line: 150
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  4

  1 [-]: SELF      R1 R0 K0     ; R2 := R0; R1 := R0[0xf4e253b6]
  2 [-]: CALL      R1 2 1       ; R1(R2)
  3 [-]: GETUPVAL  R1 U0        ; R1 := U0
  4 [-]: SELF      R1 R1 K1     ; R2 := R1; R1 := R1[0xfe9dc265]
  5 [-]: LOADK     R3 4         ; R3 := 4.000000
  6 [-]: CALL      R1 3 1       ; R1(R2,R3)
  7 [-]: RETURN    R0 1         ; return 


; Function #8:
;
; Name:            
; Defined at line: 155
; #Upvalues:       1
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  12

  1 [-]: GETGLOBAL R2 K0        ; R2 := 0xebbfcc19
  2 [-]: SELF      R3 R1 K1     ; R4 := R1; R3 := R1[0xf2deaf69]
  3 [-]: GETGLOBAL R5 K2        ; R5 := gLotusVehicleAvatarType
  4 [-]: CALL      R3 3 2       ; R3 := R3(R4,R5)
  5 [-]: TEST      R3 0         ; if not R3 then PC := 10
  6 [-]: JMP       10           ; PC := 10
  7 [-]: SELF      R3 R1 K3     ; R4 := R1; R3 := R1[0xff005826]
  8 [-]: CALL      R3 2 2       ; R3 := R3(R4)
  9 [-]: MOVE      R1 R3        ; R1 := R3
 10 [-]: GETGLOBAL R3 K4        ; R3 := 0x7b998233
 11 [-]: MOVE      R4 R1        ; R4 := R1
 12 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 13 [-]: TEST      R3 0         ; if not R3 then PC := 16
 14 [-]: JMP       16           ; PC := 16
 15 [-]: RETURN    R0 1         ; return 
 16 [-]: SELF      R3 R1 K5     ; R4 := R1; R3 := R1[0xde321e6f]
 17 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 18 [-]: SELF      R4 R3 K6     ; R5 := R3; R4 := R3[0xf7d48ee0]
 19 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 20 [-]: GETGLOBAL R5 K4        ; R5 := 0x7b998233
 21 [-]: MOVE      R6 R4        ; R6 := R4
 22 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 23 [-]: TEST      R5 1         ; if R5 then PC := 31
 24 [-]: JMP       31           ; PC := 31
 25 [-]: SELF      R5 R4 K1     ; R6 := R4; R5 := R4[0xf2deaf69]
 26 [-]: GETUPVAL  R7 U0        ; R7 := U0
 27 [-]: CALL      R5 3 2       ; R5 := R5(R6,R7)
 28 [-]: TEST      R5 0         ; if not R5 then PC := 31
 29 [-]: JMP       31           ; PC := 31
 30 [-]: GETGLOBAL R2 K7        ; R2 := 0x9a0ee26a
 31 [-]: SELF      R5 R0 K8     ; R6 := R0; R5 := R0[0xf4e253b6]
 32 [-]: CALL      R5 2 1       ; R5(R6)
 33 [-]: SELF      R5 R1 K9     ; R6 := R1; R5 := R1[0x020d4331]
 34 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 35 [-]: SELF      R5 R5 K10    ; R6 := R5; R5 := R5[0x553549e8]
 36 [-]: SELF      R7 R0 K11    ; R8 := R0; R7 := R0[0xcb3851b8]
 37 [-]: CALL      R7 2 0       ; R7,... := R7(R8)
 38 [-]: CALL      R5 0 1       ; R5(R6,...)
 39 [-]: SELF      R5 R1 K12    ; R6 := R1; R5 := R1[0x7027c544]
 40 [-]: MOVE      R7 R2        ; R7 := R2
 41 [-]: LOADBOOL  R8 0 0       ; R8 := false
 42 [-]: LOADK     R9 3         ; R9 := 3.000000
 43 [-]: LOADK     R10 1        ; R10 := 1.000000
 44 [-]: LOADBOOL  R11 1 0      ; R11 := true
 45 [-]: CALL      R5 7 1       ; R5(R6,R7,R8,R9,R10,R11)
 46 [-]: SELF      R5 R1 K14    ; R6 := R1; R5 := R1[0x21b4c60e]
 47 [-]: LOADK     R7 K15       ; R7 := "GrabCanister"
 48 [-]: LOADK     R8 1         ; R8 := 1.000000
 49 [-]: CALL      R5 4 1       ; R5(R6,R7,R8)
 50 [-]: GETGLOBAL R5 K4        ; R5 := 0x7b998233
 51 [-]: GETGLOBAL R6 K16       ; R6 := 0x5e0dade6
 52 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 53 [-]: TEST      R5 1         ; if R5 then PC := 58
 54 [-]: JMP       58           ; PC := 58
 55 [-]: GETGLOBAL R5 K16       ; R5 := 0x5e0dade6
 56 [-]: SELF      R5 R5 K17    ; R6 := R5; R5 := R5[0xa2880940]
 57 [-]: CALL      R5 2 1       ; R5(R6)
 58 [-]: GETGLOBAL R5 K18       ; R5 := 0x89326c93
 59 [-]: SELF      R5 R5 K19    ; R6 := R5; R5 := R5[0x18d05d30]
 60 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 61 [-]: TEST      R5 0         ; if not R5 then PC := 75
 62 [-]: JMP       75           ; PC := 75
 63 [-]: SELF      R5 R3 K20    ; R6 := R3; R5 := R3[0x02a0d8e1]
 64 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 65 [-]: SELF      R6 R3 K21    ; R7 := R3; R6 := R3[0x0b5ec5b5]
 66 [-]: LOADBOOL  R8 1 0       ; R8 := true
 67 [-]: CALL      R6 3 1       ; R6(R7,R8)
 68 [-]: SELF      R6 R1 K22    ; R7 := R1; R6 := R1[0x511d26b8]
 69 [-]: GETGLOBAL R8 K23       ; R8 := 0x3d658f08
 70 [-]: LOADBOOL  R9 1 0       ; R9 := true
 71 [-]: CALL      R6 4 1       ; R6(R7,R8,R9)
 72 [-]: SELF      R6 R3 K21    ; R7 := R3; R6 := R3[0x0b5ec5b5]
 73 [-]: MOVE      R8 R5        ; R8 := R5
 74 [-]: CALL      R6 3 1       ; R6(R7,R8)
 75 [-]: GETGLOBAL R6 K4        ; R6 := 0x7b998233
 76 [-]: MOVE      R7 R1        ; R7 := R1
 77 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 78 [-]: TEST      R6 1         ; if R6 then PC := 89
 79 [-]: JMP       89           ; PC := 89
 80 [-]: SELF      R6 R1 K24    ; R7 := R1; R6 := R1[0x16e0b3da]
 81 [-]: MOVE      R8 R2        ; R8 := R2
 82 [-]: CALL      R6 3 2       ; R6 := R6(R7,R8)
 83 [-]: TEST      R6 0         ; if not R6 then PC := 89
 84 [-]: JMP       89           ; PC := 89
 85 [-]: GETGLOBAL R6 K25       ; R6 := 0xcbd666e1
 86 [-]: LOADK     R7 0         ; R7 := 0.000000
 87 [-]: CALL      R6 2 1       ; R6(R7)
 88 [-]: JMP       75           ; PC := 75
 89 [-]: RETURN    R0 1         ; return 


; Function #9:
;
; Name:            
; Defined at line: 194
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  3

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: SELF      R1 R1 K0     ; R2 := R1; R1 := R1[0xf4e253b6]
  3 [-]: CALL      R1 2 1       ; R1(R2)
  4 [-]: RETURN    R0 1         ; return 


; Function #10:
;
; Name:            
; Defined at line: 198
; #Upvalues:       4
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  25

  1 [-]: GETGLOBAL R2 K0        ; R2 := 0x7b998233
  2 [-]: MOVE      R3 R0        ; R3 := R0
  3 [-]: CALL      R2 2 2       ; R2 := R2(R3)
  4 [-]: TEST      R2 0         ; if not R2 then PC := 7
  5 [-]: JMP       7            ; PC := 7
  6 [-]: RETURN    R0 1         ; return 
  7 [-]: TEST      R1 0         ; if not R1 then PC := 12
  8 [-]: JMP       12           ; PC := 12
  9 [-]: GETGLOBAL R2 K1        ; R2 := _T
 10 [-]: SETTABLE  R2 K2 K3     ; R2["MigratedMissionPrimed"] := true
 11 [-]: JMP       14           ; PC := 14
 12 [-]: GETGLOBAL R2 K1        ; R2 := _T
 13 [-]: SETTABLE  R2 K4 K3     ; R2["MissionPrimed"] := true
 14 [-]: SELF      R2 R0 K5     ; R3 := R0; R2 := R0[0xe86a236e]
 15 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 16 [-]: SELF      R3 R2 K6     ; R4 := R2; R3 := R2[0xe223e2b1]
 17 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 18 [-]: GETGLOBAL R4 K1        ; R4 := _T
 19 [-]: GETTABLE  R4 R4 K7     ; R4 := R4["TransmissionSet"]
 20 [-]: GETGLOBAL R5 K0        ; R5 := 0x7b998233
 21 [-]: MOVE      R6 R4        ; R6 := R4
 22 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 23 [-]: TEST      R5 0         ; if not R5 then PC := 28
 24 [-]: JMP       28           ; PC := 28
 25 [-]: SELF      R5 R2 K8     ; R6 := R2; R5 := R2[0xe4c355e2]
 26 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 27 [-]: MOVE      R4 R5        ; R4 := R5
 28 [-]: GETGLOBAL R5 K9        ; R5 := 0x89326c93
 29 [-]: SELF      R5 R5 K10    ; R6 := R5; R5 := R5[0xb7d33840]
 30 [-]: LOADK     R7 K11       ; R7 := "OnPlayersChanged"
 31 [-]: CALL      R5 3 1       ; R5(R6,R7)
 32 [-]: GETGLOBAL R5 K9        ; R5 := 0x89326c93
 33 [-]: SELF      R5 R5 K12    ; R6 := R5; R5 := R5[0x7d108ddb]
 34 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 35 [-]: SETUPVAL  R5 U0        ; U82 := 
 36 [-]: SELF      R5 R0 K13    ; R6 := R0; R5 := R0[0x659d451f]
 37 [-]: GETUPVAL  R7 U1        ; R7 := U1
 38 [-]: LOADBOOL  R8 0 0       ; R8 := false
 39 [-]: LOADK     R9 0         ; R9 := 0.000000
 40 [-]: LOADBOOL  R10 1 0      ; R10 := true
 41 [-]: CALL      R5 6 1       ; R5(R6,R7,R8,R9,R10)
 42 [-]: GETUPVAL  R5 U2        ; R5 := U2
 43 [-]: GETTABLE  R5 R5 K14    ; R82 := R5[0x9742b85b]
 44 [-]: MOVE      R6 R4        ; R6 := R4
 45 [-]: GETGLOBAL R7 K15       ; R7 := 0x0469f296
 46 [-]: MOVE      R8 R3        ; R8 := R3
 47 [-]: LOADK     R9 K16       ; R9 := "_Primed"
 48 [-]: CONCAT    R8 R8 R9     ; R8 := R8 .. R9
 49 [-]: CALL      R7 2 0       ; R7,... := R7(R8)
 50 [-]: CALL      R5 0 1       ; R5(R6,...)
 51 [-]: SELF      R5 R2 K17    ; R6 := R2; R5 := R2[0xaa1950d4]
 52 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 53 [-]: GETGLOBAL R6 K9        ; R6 := 0x89326c93
 54 [-]: SELF      R6 R6 K18    ; R7 := R6; R6 := R6[0x05909209]
 55 [-]: MOVE      R8 R5        ; R8 := R5
 56 [-]: SELF      R9 R0 K19    ; R10 := R0; R9 := R0[0xd1586535]
 57 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 58 [-]: GETGLOBAL R10 K20      ; R10 := 0x0639523f
 59 [-]: ADD       R9 R9 R10    ; R9 := R9 + R10
 60 [-]: GETGLOBAL R10 K21      ; R10 := ZERO_ROTATION
 61 [-]: CALL      R6 5 2       ; R6 := R6(R7,R8,R9,R10)
 62 [-]: SELF      R7 R6 K22    ; R8 := R6; R7 := R6[0x5004be24]
 63 [-]: SELF      R9 R0 K23    ; R10 := R0; R9 := R0[0xc5b92518]
 64 [-]: CALL      R9 2 0       ; R9,... := R9(R10)
 65 [-]: CALL      R7 0 1       ; R7(R8,...)
 66 [-]: LOADNIL   R7 R7        ; R7 := nil
 67 [-]: GETGLOBAL R8 K1        ; R8 := _T
 68 [-]: GETTABLE  R8 R8 K24    ; R82 := R8[0x8ee923fe]
 69 [-]: LOADK     R9 K25       ; R9 := "PrimedObjectiveTracker"
 70 [-]: GETUPVAL  R10 U3       ; R10 := U3
 71 [-]: GETTABLE  R10 R10 K26  ; R10 := R10["HT_LABEL"]
 72 [-]: LOADK     R11 K27      ; R11 := 0.150000
 73 [-]: LOADK     R12 4        ; R12 := 4.000000
 74 [-]: CALL      R8 5 2       ; R8 := R8(R9,R10,R11,R12)
 75 [-]: LOADK     R9 K28       ; R9 := "<p><font color=\""
 76 [-]: GETTABLE  R10 R8 K29   ; R82 := R10[0xe2c898b9]
 77 [-]: LOADK     R11 35       ; R11 := 35.000000
 78 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 79 [-]: LOADK     R11 K31      ; R11 := "\">"
 80 [-]: GETTABLE  R12 R8 K32   ; R82 := R12[0x603636ad]
 81 [-]: LOADK     R13 K33      ; R13 := "<MISSION_MARKER_GENERIC>"
 82 [-]: CALL      R12 2 2      ; R12 := R12(R13)
 83 [-]: LOADK     R13 K34      ; R13 := "</font>"
 84 [-]: CONCAT    R9 R9 R13    ; R9 := R9 .. R10 .. R11 .. R12 .. R13
 85 [-]: MOVE      R10 R9       ; R10 := R9
 86 [-]: LOADK     R11 K35      ; R11 := "<font color=\""
 87 [-]: GETTABLE  R12 R8 K29   ; R82 := R12[0xe2c898b9]
 88 [-]: LOADK     R13 37       ; R13 := 37.000000
 89 [-]: CALL      R12 2 2      ; R12 := R12(R13)
 90 [-]: LOADK     R13 K31      ; R13 := "\">"
 91 [-]: GETTABLE  R14 R8 K32   ; R82 := R14[0x603636ad]
 92 [-]: SELF      R15 R2 K36   ; R16 := R2; R15 := R2[0x05b875d3]
 93 [-]: CALL      R15 2 2      ; R15 := R15(R16)
 94 [-]: SELF      R15 R15 K37  ; R16 := R15; R15 := R15[0x6d604ba7]
 95 [-]: CALL      R15 2 0      ; R15,... := R15(R16)
 96 [-]: CALL      R14 0 2      ; R14 := R14(R15,...)
 97 [-]: LOADK     R15 K38      ; R15 := "</font></p>"
 98 [-]: CONCAT    R9 R10 R15   ; R9 := R10 .. R11 .. R12 .. R13 .. R14 .. R15
 99 [-]: GETTABLE  R10 R8 K39   ; R82 := R10[0x3f8a850c]
100 [-]: MOVE      R11 R9       ; R11 := R9
101 [-]: CALL      R10 2 1      ; R10(R11)
102 [-]: SELF      R10 R0 K40   ; R11 := R0; R10 := R0[0xe79e7ef4]
103 [-]: CALL      R10 2 2      ; R10 := R10(R11)
104 [-]: SELF      R10 R10 K41  ; R11 := R10; R10 := R10[0x9435eb6d]
105 [-]: CALL      R10 2 2      ; R10 := R10(R11)
106 [-]: LOADBOOL  R11 0 0      ; R11 := false
107 [-]: LOADBOOL  R12 0 0      ; R12 := false
108 [-]: GETGLOBAL R13 K0       ; R13 := 0x7b998233
109 [-]: MOVE      R14 R0       ; R14 := R0
110 [-]: CALL      R13 2 2      ; R13 := R13(R14)
111 [-]: TEST      R13 1        ; if R13 then PC := 179
112 [-]: JMP       179          ; PC := 179
113 [-]: GETGLOBAL R13 K0       ; R13 := 0x7b998233
114 [-]: MOVE      R14 R7       ; R14 := R7
115 [-]: CALL      R13 2 2      ; R13 := R13(R14)
116 [-]: TEST      R13 0        ; if not R13 then PC := 123
117 [-]: JMP       123          ; PC := 123
118 [-]: GETGLOBAL R13 K1       ; R13 := _T
119 [-]: GETTABLE  R13 R13 K42  ; R82 := R13[0xca312f51]
120 [-]: LOADK     R14 K43      ; R14 := "MissionTimer"
121 [-]: CALL      R13 2 2      ; R13 := R13(R14)
122 [-]: MOVE      R7 R13       ; R7 := R13
123 [-]: TEST      R11 1        ; if R11 then PC := 158
124 [-]: JMP       158          ; PC := 158
125 [-]: GETGLOBAL R13 K44      ; R13 := 0xc8802016
126 [-]: GETUPVAL  R14 U0       ; R14 := U0
127 [-]: CALL      R13 2 4      ; R13,R14,R15 := R13(R14)
128 [-]: JMP       156          ; PC := 156
129 [-]: SELF      R18 R17 K45  ; R19 := R17; R18 := R17[0xbb610e5b]
130 [-]: CALL      R18 2 2      ; R18 := R18(R19)
131 [-]: GETGLOBAL R19 K0       ; R19 := 0x7b998233
132 [-]: MOVE      R20 R18      ; R20 := R18
133 [-]: CALL      R19 2 2      ; R19 := R19(R20)
134 [-]: TEST      R19 1        ; if R19 then PC := 156
135 [-]: JMP       156          ; PC := 156
136 [-]: SELF      R19 R18 K40  ; R20 := R18; R19 := R18[0xe79e7ef4]
137 [-]: CALL      R19 2 2      ; R19 := R19(R20)
138 [-]: GETGLOBAL R20 K0       ; R20 := 0x7b998233
139 [-]: MOVE      R21 R19      ; R21 := R19
140 [-]: CALL      R20 2 2      ; R20 := R20(R21)
141 [-]: TEST      R20 1        ; if R20 then PC := 156
142 [-]: JMP       156          ; PC := 156
143 [-]: SELF      R20 R19 K41  ; R21 := R19; R20 := R19[0x9435eb6d]
144 [-]: CALL      R20 2 2      ; R20 := R20(R21)
145 [-]: EQ        0 R20 R10    ; if R20 ~= R10 then PC := 156
146 [-]: JMP       156          ; PC := 156
147 [-]: GETUPVAL  R20 U2       ; R20 := U2
148 [-]: GETTABLE  R20 R20 K14  ; R82 := R20[0x9742b85b]
149 [-]: MOVE      R21 R4       ; R21 := R4
150 [-]: GETGLOBAL R22 K15      ; R22 := 0x0469f296
151 [-]: LOADK     R23 K46      ; R23 := "Arrived"
152 [-]: CALL      R22 2 0      ; R22,... := R22(R23)
153 [-]: CALL      R20 0 1      ; R20(R21,...)
154 [-]: LOADBOOL  R11 1 0      ; R11 := true
155 [-]: JMP       158          ; PC := 158
156 [-]: TFORLOOP  R13 2        ; R16,R17 :=  R13(R14,R15); if R16 ~= nil then begin PC = 129; R15 := R16 end
157 [-]: JMP       129          ; PC := 129
158 [-]: SELF      R20 R0 K47   ; R21 := R0; R20 := R0[0x96a11bd4]
159 [-]: CALL      R20 2 2      ; R20 := R20(R21)
160 [-]: LT        0 K48 R20    ; if 0.000000 >= R20 then PC := 164
161 [-]: JMP       164          ; PC := 164
162 [-]: JMP       179          ; PC := 179
163 [-]: JMP       175          ; PC := 175
164 [-]: GETGLOBAL R20 K0       ; R20 := 0x7b998233
165 [-]: MOVE      R21 R7       ; R21 := R7
166 [-]: CALL      R20 2 2      ; R20 := R20(R21)
167 [-]: TEST      R20 1        ; if R20 then PC := 175
168 [-]: JMP       175          ; PC := 175
169 [-]: GETTABLE  R20 R7 K49   ; R20 := R7["Data"]
170 [-]: GETTABLE  R20 R20 K50  ; R20 := R20["Time"]
171 [-]: LE        0 R20 K48    ; if R20 > 0.000000 then PC := 175
172 [-]: JMP       175          ; PC := 175
173 [-]: LOADBOOL  R12 1 0      ; R12 := true
174 [-]: JMP       179          ; PC := 179
175 [-]: GETGLOBAL R20 K51      ; R20 := 0xcbd666e1
176 [-]: LOADK     R21 0        ; R21 := 0.000000
177 [-]: CALL      R20 2 1      ; R20(R21)
178 [-]: JMP       108          ; PC := 108
179 [-]: GETGLOBAL R20 K0       ; R20 := 0x7b998233
180 [-]: MOVE      R21 R6       ; R21 := R6
181 [-]: CALL      R20 2 2      ; R20 := R20(R21)
182 [-]: TEST      R20 1        ; if R20 then PC := 186
183 [-]: JMP       186          ; PC := 186
184 [-]: SELF      R20 R6 K52   ; R21 := R6; R20 := R6[0xa2880940]
185 [-]: CALL      R20 2 1      ; R20(R21)
186 [-]: GETGLOBAL R20 K1       ; R20 := _T
187 [-]: GETTABLE  R20 R20 K53  ; R82 := R20[0x1a41a3c1]
188 [-]: MOVE      R21 R8       ; R21 := R8
189 [-]: CALL      R20 2 1      ; R20(R21)
190 [-]: TEST      R12 0        ; if not R12 then PC := 204
191 [-]: JMP       204          ; PC := 204
192 [-]: GETUPVAL  R20 U2       ; R20 := U2
193 [-]: GETTABLE  R20 R20 K14  ; R82 := R20[0x9742b85b]
194 [-]: MOVE      R21 R4       ; R21 := R4
195 [-]: GETGLOBAL R22 K15      ; R22 := 0x0469f296
196 [-]: MOVE      R23 R3       ; R23 := R3
197 [-]: LOADK     R24 K54      ; R24 := "_Ignored"
198 [-]: CONCAT    R23 R23 R24  ; R23 := R23 .. R24
199 [-]: CALL      R22 2 0      ; R22,... := R22(R23)
200 [-]: CALL      R20 0 1      ; R20(R21,...)
201 [-]: GETGLOBAL R20 K1       ; R20 := _T
202 [-]: SETTABLE  R20 K4 K55   ; R20["MissionPrimed"] := false
203 [-]: JMP       219          ; PC := 219
204 [-]: GETGLOBAL R20 K0       ; R20 := 0x7b998233
205 [-]: MOVE      R21 R0       ; R21 := R0
206 [-]: CALL      R20 2 2      ; R20 := R20(R21)
207 [-]: TEST      R20 1        ; if R20 then PC := 217
208 [-]: JMP       217          ; PC := 217
209 [-]: SELF      R20 R0 K47   ; R21 := R0; R20 := R0[0x96a11bd4]
210 [-]: CALL      R20 2 2      ; R20 := R20(R21)
211 [-]: LT        0 K48 R20    ; if 0.000000 >= R20 then PC := 217
212 [-]: JMP       217          ; PC := 217
213 [-]: GETGLOBAL R20 K51      ; R20 := 0xcbd666e1
214 [-]: LOADK     R21 0        ; R21 := 0.000000
215 [-]: CALL      R20 2 1      ; R20(R21)
216 [-]: JMP       204          ; PC := 204
217 [-]: GETGLOBAL R20 K1       ; R20 := _T
218 [-]: SETTABLE  R20 K4 K55   ; R20["MissionPrimed"] := false
219 [-]: RETURN    R0 1         ; return 


