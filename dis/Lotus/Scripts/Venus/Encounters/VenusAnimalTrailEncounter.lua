; This file has been disassembled using luadec 2.0.2 by sztupy (modified by Reversive)

; Name:            
; Defined at line: 0
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       2
; Max Stack Size:  35

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x0469f296
  2 [-]: LOADK     R1 K1        ; R1 := "AnimalWeenie"
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K0        ; R1 := 0x0469f296
  5 [-]: LOADK     R2 K2        ; R2 := "HighlightAtten"
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: GETGLOBAL R2 K0        ; R2 := 0x0469f296
  8 [-]: LOADK     R3 K3        ; R3 := "TrailEncounterMarker"
  9 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 10 [-]: LOADK     R3 0         ; R3 := 0.000000
 11 [-]: LOADK     R4 1         ; R4 := 1.000000
 12 [-]: GETGLOBAL R5 K4        ; R5 := 0x2d0fad09
 13 [-]: LOADK     R6 K5        ; R6 := "Lotus.Scripts.Libs.TransmissionSet"
 14 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 15 [-]: GETGLOBAL R6 K4        ; R6 := 0x2d0fad09
 16 [-]: LOADK     R7 K6        ; R7 := "Lotus.Scripts.Libs.CommonGamemodeFunctions"
 17 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 18 [-]: GETGLOBAL R7 K4        ; R7 := 0x2d0fad09
 19 [-]: LOADK     R8 K7        ; R8 := "Lotus.Scripts.Libs.SpawnObjectsOnTerrain"
 20 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 21 [-]: GETGLOBAL R8 K4        ; R8 := 0x2d0fad09
 22 [-]: LOADK     R9 K8        ; R9 := "Lotus.Scripts.Libs.ConservationLib"
 23 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 24 [-]: LOADNIL   R9 R18       ; R9 := R10 := R11 := R12 := R13 := R14 := R15 := R16 := R17 := R18 := nil
 25 [-]: LOADBOOL  R19 0 0      ; R19 := false
 26 [-]: LOADNIL   R20 R22      ; R20 := R21 := R22 := nil
 27 [-]: CLOSURE   R23 0        ; R23 := closure(Function #1)
 28 [-]: CLOSURE   R24 1        ; R24 := closure(Function #2)
 29 [-]: MOVE      R0 R20       ; R0 := R20
 30 [-]: MOVE      R0 R12       ; R0 := R12
 31 [-]: CLOSURE   R25 2        ; R25 := closure(Function #3)
 32 [-]: MOVE      R0 R14       ; R0 := R14
 33 [-]: MOVE      R0 R13       ; R0 := R13
 34 [-]: MOVE      R0 R17       ; R0 := R17
 35 [-]: MOVE      R0 R9        ; R0 := R9
 36 [-]: CLOSURE   R26 3        ; R26 := closure(Function #4)
 37 [-]: MOVE      R0 R12       ; R0 := R12
 38 [-]: MOVE      R0 R3        ; R0 := R3
 39 [-]: MOVE      R0 R4        ; R0 := R4
 40 [-]: MOVE      R0 R5        ; R0 := R5
 41 [-]: MOVE      R0 R14       ; R0 := R14
 42 [-]: MOVE      R0 R9        ; R0 := R9
 43 [-]: MOVE      R0 R18       ; R0 := R18
 44 [-]: MOVE      R0 R11       ; R0 := R11
 45 [-]: MOVE      R0 R2        ; R0 := R2
 46 [-]: SETGLOBAL R26 K9       ; OnTouched := R26
 47 [-]: CLOSURE   R26 4        ; R26 := closure(Function #5)
 48 [-]: MOVE      R0 R7        ; R0 := R7
 49 [-]: MOVE      R0 R12       ; R0 := R12
 50 [-]: MOVE      R0 R21       ; R0 := R21
 51 [-]: CLOSURE   R27 5        ; R27 := closure(Function #6)
 52 [-]: MOVE      R0 R9        ; R0 := R9
 53 [-]: MOVE      R0 R18       ; R0 := R18
 54 [-]: CLOSURE   R28 6        ; R28 := closure(Function #7)
 55 [-]: MOVE      R0 R11       ; R0 := R11
 56 [-]: MOVE      R0 R22       ; R0 := R22
 57 [-]: MOVE      R0 R18       ; R0 := R18
 58 [-]: MOVE      R0 R9        ; R0 := R9
 59 [-]: MOVE      R0 R23       ; R0 := R23
 60 [-]: MOVE      R0 R7        ; R0 := R7
 61 [-]: MOVE      R0 R0        ; R0 := R0
 62 [-]: MOVE      R0 R12       ; R0 := R12
 63 [-]: MOVE      R0 R26       ; R0 := R26
 64 [-]: CLOSURE   R29 7        ; R29 := closure(Function #8)
 65 [-]: MOVE      R0 R13       ; R0 := R13
 66 [-]: MOVE      R0 R28       ; R0 := R28
 67 [-]: MOVE      R0 R12       ; R0 := R12
 68 [-]: MOVE      R0 R20       ; R0 := R20
 69 [-]: MOVE      R0 R17       ; R0 := R17
 70 [-]: MOVE      R0 R27       ; R0 := R27
 71 [-]: MOVE      R0 R19       ; R0 := R19
 72 [-]: MOVE      R0 R10       ; R0 := R10
 73 [-]: MOVE      R0 R9        ; R0 := R9
 74 [-]: CLOSURE   R30 8        ; R30 := closure(Function #9)
 75 [-]: MOVE      R0 R19       ; R0 := R19
 76 [-]: MOVE      R0 R9        ; R0 := R9
 77 [-]: MOVE      R0 R29       ; R0 := R29
 78 [-]: MOVE      R0 R1        ; R0 := R1
 79 [-]: CLOSURE   R31 9        ; R31 := closure(Function #10)
 80 [-]: MOVE      R0 R13       ; R0 := R13
 81 [-]: CLOSURE   R32 10       ; R32 := closure(Function #11)
 82 [-]: MOVE      R0 R8        ; R0 := R8
 83 [-]: MOVE      R0 R14       ; R0 := R14
 84 [-]: CLOSURE   R33 11       ; R33 := closure(Function #12)
 85 [-]: MOVE      R0 R12       ; R0 := R12
 86 [-]: MOVE      R0 R9        ; R0 := R9
 87 [-]: MOVE      R0 R32       ; R0 := R32
 88 [-]: MOVE      R0 R30       ; R0 := R30
 89 [-]: MOVE      R0 R31       ; R0 := R31
 90 [-]: CLOSURE   R34 12       ; R34 := closure(Function #13)
 91 [-]: MOVE      R0 R12       ; R0 := R12
 92 [-]: MOVE      R0 R15       ; R0 := R15
 93 [-]: MOVE      R0 R16       ; R0 := R16
 94 [-]: MOVE      R0 R21       ; R0 := R21
 95 [-]: MOVE      R0 R8        ; R0 := R8
 96 [-]: MOVE      R0 R14       ; R0 := R14
 97 [-]: MOVE      R0 R2        ; R0 := R2
 98 [-]: MOVE      R0 R9        ; R0 := R9
 99 [-]: MOVE      R0 R11       ; R0 := R11
100 [-]: MOVE      R0 R20       ; R0 := R20
101 [-]: MOVE      R0 R6        ; R0 := R6
102 [-]: MOVE      R0 R24       ; R0 := R24
103 [-]: MOVE      R0 R33       ; R0 := R33
104 [-]: MOVE      R0 R25       ; R0 := R25
105 [-]: SETGLOBAL R34 K10      ; AnimalTrailEvent := R34
106 [-]: RETURN    R0 1         ; return 


; Function #1:
;
; Name:            
; Defined at line: 67
; #Upvalues:       0
; #Parameters:     5
; Is_vararg:       0
; Max Stack Size:  7

  1 [-]: SUB       R5 R4 R0     ; R5 := R4 - R0
  2 [-]: SUB       R6 R3 R2     ; R6 := R3 - R2
  3 [-]: MUL       R5 R5 R6     ; R5 := R5 * R6
  4 [-]: SUB       R6 R1 R0     ; R6 := R1 - R0
  5 [-]: DIV       R5 R5 R6     ; R5 := R5 / R6
  6 [-]: ADD       R5 R2 R5     ; R5 := R2 + R5
  7 [-]: RETURN    R5 2         ; return R5
  8 [-]: RETURN    R0 1         ; return 


; Function #2:
;
; Name:            
; Defined at line: 71
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: EQ        0 R0 K0      ; if R0 ~= nil then PC := 6
  3 [-]: JMP       6            ; PC := 6
  4 [-]: LOADBOOL  R0 1 0       ; R0 := true
  5 [-]: RETURN    R0 2         ; return R0
  6 [-]: GETUPVAL  R0 U1        ; R0 := U1
  7 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x86d3529e]
  8 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  9 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
 10 [-]: MOVE      R2 R0        ; R2 := R0
 11 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 12 [-]: TEST      R1 1         ; if R1 then PC := 26
 13 [-]: JMP       26           ; PC := 26
 14 [-]: SELF      R1 R0 K3     ; R2 := R0; R1 := R0[0xefe6cad1]
 15 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 16 [-]: LT        0 K5 R1      ; if 2.000000 >= R1 then PC := 28
 17 [-]: JMP       28           ; PC := 28
 18 [-]: GETGLOBAL R2 K6        ; R2 := 0x3d106989
 19 [-]: LOADK     R3 K7        ; R3 := "TRAIL EVENT TERMINATED BY ROOT PARENT STATUS "
 20 [-]: MOVE      R4 R1        ; R4 := R1
 21 [-]: CONCAT    R3 R3 R4     ; R3 := R3 .. R4
 22 [-]: CALL      R2 2 1       ; R2(R3)
 23 [-]: LOADBOOL  R2 1 0       ; R2 := true
 24 [-]: RETURN    R2 2         ; return R2
 25 [-]: JMP       28           ; PC := 28
 26 [-]: LOADBOOL  R2 1 0       ; R2 := true
 27 [-]: RETURN    R2 2         ; return R2
 28 [-]: LOADBOOL  R2 0 0       ; R2 := false
 29 [-]: RETURN    R2 2         ; return R2
 30 [-]: RETURN    R0 1         ; return 


; Function #3:
;
; Name:            
; Defined at line: 91
; #Upvalues:       4
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  11

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
  2 [-]: GETUPVAL  R2 U0        ; R2 := U0
  3 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  4 [-]: TEST      R1 1         ; if R1 then PC := 9
  5 [-]: JMP       9            ; PC := 9
  6 [-]: GETUPVAL  R1 U0        ; R1 := U0
  7 [-]: SELF      R1 R1 K1     ; R2 := R1; R1 := R1[0xa2880940]
  8 [-]: CALL      R1 2 1       ; R1(R2)
  9 [-]: LOADK     R1 1         ; R1 := 1.000000
 10 [-]: GETUPVAL  R2 U1        ; R2 := U1
 11 [-]: LEN       R2 R2        ; R2 := # R2
 12 [-]: LOADK     R3 1         ; R3 := 1.000000
 13 [-]: FORPREP   R1 24        ; R1 -= R3; PC := 24
 14 [-]: GETGLOBAL R5 K0        ; R5 := 0x7b998233
 15 [-]: GETUPVAL  R6 U1        ; R6 := U1
 16 [-]: GETTABLE  R6 R6 R4     ; R6 := R6[R4]
 17 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 18 [-]: TEST      R5 1         ; if R5 then PC := 24
 19 [-]: JMP       24           ; PC := 24
 20 [-]: GETUPVAL  R5 U1        ; R5 := U1
 21 [-]: GETTABLE  R5 R5 R4     ; R5 := R5[R4]
 22 [-]: SELF      R5 R5 K1     ; R6 := R5; R5 := R5[0xa2880940]
 23 [-]: CALL      R5 2 1       ; R5(R6)
 24 [-]: FORLOOP   R1 14        ; R1 += R3; if R1 <= R2 then begin PC := 14; R4 := R1 end
 25 [-]: LOADK     R5 1         ; R5 := 1.000000
 26 [-]: GETUPVAL  R6 U2        ; R6 := U2
 27 [-]: LEN       R6 R6        ; R6 := # R6
 28 [-]: LOADK     R7 1         ; R7 := 1.000000
 29 [-]: FORPREP   R5 34        ; R5 -= R7; PC := 34
 30 [-]: GETUPVAL  R9 U2        ; R9 := U2
 31 [-]: GETTABLE  R9 R9 R8     ; R9 := R9[R8]
 32 [-]: SELF      R9 R9 K1     ; R10 := R9; R9 := R9[0xa2880940]
 33 [-]: CALL      R9 2 1       ; R9(R10)
 34 [-]: FORLOOP   R5 30        ; R5 += R7; if R5 <= R6 then begin PC := 30; R8 := R5 end
 35 [-]: GETGLOBAL R9 K0        ; R9 := 0x7b998233
 36 [-]: GETUPVAL  R10 U3       ; R10 := U3
 37 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 38 [-]: TEST      R9 1         ; if R9 then PC := 43
 39 [-]: JMP       43           ; PC := 43
 40 [-]: GETUPVAL  R9 U3        ; R9 := U3
 41 [-]: SELF      R9 R9 K1     ; R10 := R9; R9 := R9[0xa2880940]
 42 [-]: CALL      R9 2 1       ; R9(R10)
 43 [-]: RETURN    R0 1         ; return 


; Function #4:
;
; Name:            
; Defined at line: 111
; #Upvalues:       9
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  15

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
  2 [-]: GETUPVAL  R2 U0        ; R2 := U0
  3 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  4 [-]: TEST      R1 1         ; if R1 then PC := 89
  5 [-]: JMP       89           ; PC := 89
  6 [-]: GETUPVAL  R1 U0        ; R1 := U0
  7 [-]: SELF      R1 R1 K1     ; R2 := R1; R1 := R1[0x86d3529e]
  8 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  9 [-]: SELF      R2 R1 K2     ; R3 := R1; R2 := R1[0xabe61691]
 10 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 11 [-]: GETUPVAL  R3 U1        ; R3 := U1
 12 [-]: EQ        0 R2 R3      ; if R2 ~= R3 then PC := 89
 13 [-]: JMP       89           ; PC := 89
 14 [-]: SELF      R3 R0 K3     ; R4 := R0; R3 := R0[0x0d09d3c0]
 15 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 16 [-]: LOADK     R4 1         ; R4 := 1.000000
 17 [-]: LEN       R5 R3        ; R5 := # R3
 18 [-]: LOADK     R6 1         ; R6 := 1.000000
 19 [-]: FORPREP   R4 88        ; R4 -= R6; PC := 88
 20 [-]: GETTABLE  R8 R3 R7     ; R8 := R3[R7]
 21 [-]: SELF      R8 R8 K4     ; R9 := R8; R8 := R8[0xf2deaf69]
 22 [-]: GETGLOBAL R10 K5       ; R10 := gTennoAvatarType
 23 [-]: CALL      R8 3 2       ; R8 := R8(R9,R10)
 24 [-]: TEST      R8 0         ; if not R8 then PC := 88
 25 [-]: JMP       88           ; PC := 88
 26 [-]: GETGLOBAL R8 K6        ; R8 := 0x3d106989
 27 [-]: LOADK     R9 K7        ; R9 := "Trail: Player arrived, completing"
 28 [-]: CALL      R8 2 1       ; R8(R9)
 29 [-]: SELF      R8 R1 K8     ; R9 := R1; R8 := R1[0x5b18bb5d]
 30 [-]: GETUPVAL  R10 U2       ; R10 := U2
 31 [-]: CALL      R8 3 1       ; R8(R9,R10)
 32 [-]: GETUPVAL  R8 U0        ; R8 := U0
 33 [-]: SELF      R8 R8 K9     ; R9 := R8; R8 := R8[0xfe9dc265]
 34 [-]: LOADK     R10 3        ; R10 := 3.000000
 35 [-]: CALL      R8 3 1       ; R8(R9,R10)
 36 [-]: GETUPVAL  R8 U3        ; R8 := U3
 37 [-]: GETTABLE  R8 R8 K11    ; R82 := R8[0x9742b85b]
 38 [-]: GETGLOBAL R9 K12       ; R9 := 0x54a69a60
 39 [-]: GETGLOBAL R10 K13      ; R10 := 0x0469f296
 40 [-]: LOADK     R11 K14      ; R11 := "EquipLure"
 41 [-]: CALL      R10 2 0      ; R10,... := R10(R11)
 42 [-]: CALL      R8 0 1       ; R8(R9,...)
 43 [-]: GETGLOBAL R8 K0        ; R8 := 0x7b998233
 44 [-]: GETUPVAL  R9 U4        ; R9 := U4
 45 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 46 [-]: TEST      R8 1         ; if R8 then PC := 51
 47 [-]: JMP       51           ; PC := 51
 48 [-]: GETUPVAL  R8 U4        ; R8 := U4
 49 [-]: SELF      R8 R8 K15    ; R9 := R8; R8 := R8[0xa2880940]
 50 [-]: CALL      R8 2 1       ; R8(R9)
 51 [-]: GETGLOBAL R8 K0        ; R8 := 0x7b998233
 52 [-]: GETUPVAL  R9 U5        ; R9 := U5
 53 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 54 [-]: TEST      R8 1         ; if R8 then PC := 88
 55 [-]: JMP       88           ; PC := 88
 56 [-]: GETUPVAL  R8 U6        ; R8 := U6
 57 [-]: LT        0 K16 R8     ; if 0.000000 >= R8 then PC := 88
 58 [-]: JMP       88           ; PC := 88
 59 [-]: GETGLOBAL R8 K17       ; R8 := 0x89326c93
 60 [-]: SELF      R8 R8 K18    ; R9 := R8; R8 := R8[0x29ef273d]
 61 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 62 [-]: SELF      R8 R8 K19    ; R9 := R8; R8 := R8[0x66905cb0]
 63 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 64 [-]: SELF      R9 R8 K20    ; R10 := R8; R9 := R8[0x0e8c38e5]
 65 [-]: GETUPVAL  R11 U7       ; R11 := U7
 66 [-]: GETUPVAL  R12 U6       ; R12 := U6
 67 [-]: GETTABLE  R11 R11 R12  ; R11 := R11[R12]
 68 [-]: CALL      R9 3 2       ; R9 := R9(R10,R11)
 69 [-]: GETGLOBAL R10 K17      ; R10 := 0x89326c93
 70 [-]: SELF      R10 R10 K21  ; R11 := R10; R10 := R10[0x05909209]
 71 [-]: GETGLOBAL R12 K22      ; R12 := 0x56683b9e
 72 [-]: MOVE      R13 R9       ; R13 := R9
 73 [-]: GETGLOBAL R14 K23      ; R14 := ZERO_ROTATION
 74 [-]: CALL      R10 5 2      ; R10 := R10(R11,R12,R13,R14)
 75 [-]: SETUPVAL  R10 U4       ; U82 := 

 76 [-]: GETUPVAL  R10 U4       ; R10 := U4
 77 [-]: SELF      R10 R10 K24  ; R11 := R10; R10 := R10[0x53bc0559]
 78 [-]: GETGLOBAL R12 K25      ; R12 := 0xb7cbd06b
 79 [-]: LOADK     R13 1        ; R13 := 1.000000
 80 [-]: LOADK     R14 5000     ; R14 := 5000.000000
 81 [-]: CALL      R12 3 0      ; R12,... := R12(R13,R14)
 82 [-]: CALL      R10 0 1      ; R10(R11,...)
 83 [-]: GETUPVAL  R10 U4       ; R10 := U4
 84 [-]: SELF      R10 R10 K26  ; R11 := R10; R10 := R10[0x3273ba96]
 85 [-]: GETUPVAL  R12 U8       ; R12 := U8
 86 [-]: CALL      R10 3 1      ; R10(R11,R12)
 87 [-]: RETURN    R0 1         ; return 
 88 [-]: FORLOOP   R4 20        ; R4 += R6; if R4 <= R5 then begin PC := 20; R7 := R4 end
 89 [-]: RETURN    R0 1         ; return 


; Function #5:
;
; Name:            
; Defined at line: 145
; #Upvalues:       3
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  16

  1 [-]: LOADK     R2 1         ; R2 := 1.000000
  2 [-]: GETGLOBAL R3 K0        ; R3 := 0x8f240e42
  3 [-]: LOADK     R4 1         ; R4 := 1.000000
  4 [-]: FORPREP   R2 83        ; R2 -= R4; PC := 83
  5 [-]: GETGLOBAL R6 K1        ; R6 := 0x1d12a0c2
  6 [-]: GETGLOBAL R7 K2        ; R7 := 0x55730e1a
  7 [-]: LOADK     R8 1         ; R8 := 1.000000
  8 [-]: GETGLOBAL R9 K1        ; R9 := 0x1d12a0c2
  9 [-]: LEN       R9 R9        ; R9 := # R9
 10 [-]: CALL      R7 3 2       ; R7 := R7(R8,R9)
 11 [-]: GETTABLE  R6 R6 R7     ; R6 := R6[R7]
 12 [-]: GETUPVAL  R7 U0        ; R7 := U0
 13 [-]: GETTABLE  R7 R7 K3     ; R82 := R7[0x29595ba9]
 14 [-]: MOVE      R8 R0        ; R8 := R0
 15 [-]: LOADK     R9 0         ; R9 := 0.500000
 16 [-]: LOADBOOL  R10 0 0      ; R10 := false
 17 [-]: LOADK     R11 K4       ; R11 := 0.100000
 18 [-]: LOADK     R12 85       ; R12 := 85.000000
 19 [-]: LOADK     R13 3        ; R13 := 3.000000
 20 [-]: CALL      R7 7 3       ; R7,R8 := R7(R8,R9,R10,R11,R12,R13)
 21 [-]: LOADNIL   R9 R9        ; R9 := nil
 22 [-]: TEST      R7 0         ; if not R7 then PC := 38
 23 [-]: JMP       38           ; PC := 38
 24 [-]: TEST      R8 0         ; if not R8 then PC := 38
 25 [-]: JMP       38           ; PC := 38
 26 [-]: GETGLOBAL R10 K5       ; R10 := 0x89326c93
 27 [-]: SELF      R10 R10 K6   ; R11 := R10; R10 := R10[0x05909209]
 28 [-]: MOVE      R12 R6       ; R12 := R6
 29 [-]: MOVE      R13 R7       ; R13 := R7
 30 [-]: MOVE      R14 R8       ; R14 := R8
 31 [-]: GETUPVAL  R15 U1       ; R15 := U1
 32 [-]: CALL      R10 6 2      ; R10 := R10(R11,R12,R13,R14,R15)
 33 [-]: MOVE      R9 R10       ; R9 := R10
 34 [-]: GETGLOBAL R10 K7       ; R10 := 0xcbd666e1
 35 [-]: LOADK     R11 0        ; R11 := 0.000000
 36 [-]: CALL      R10 2 1      ; R10(R11)
 37 [-]: JMP       49           ; PC := 49
 38 [-]: GETGLOBAL R10 K8       ; R10 := 0x3d106989
 39 [-]: LOADK     R11 K9       ; R11 := "Couldn't find good valid spawnpoint for placing call point weenie, placing anyway."
 40 [-]: CALL      R10 2 1      ; R10(R11)
 41 [-]: GETGLOBAL R10 K5       ; R10 := 0x89326c93
 42 [-]: SELF      R10 R10 K6   ; R11 := R10; R10 := R10[0x05909209]
 43 [-]: MOVE      R12 R6       ; R12 := R6
 44 [-]: MOVE      R13 R0       ; R13 := R0
 45 [-]: GETGLOBAL R14 K10      ; R14 := ZERO_ROTATION
 46 [-]: GETUPVAL  R15 U1       ; R15 := U1
 47 [-]: CALL      R10 6 2      ; R10 := R10(R11,R12,R13,R14,R15)
 48 [-]: MOVE      R9 R10       ; R9 := R10
 49 [-]: GETGLOBAL R10 K11      ; R10 := 0x7b998233
 50 [-]: MOVE      R11 R9       ; R11 := R9
 51 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 52 [-]: TEST      R10 1        ; if R10 then PC := 83
 53 [-]: JMP       83           ; PC := 83
 54 [-]: GETUPVAL  R10 U1       ; R10 := U1
 55 [-]: SELF      R10 R10 K12  ; R11 := R10; R10 := R10[0x86d3529e]
 56 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 57 [-]: SELF      R10 R10 K13  ; R11 := R10; R10 := R10[0x21dfc654]
 58 [-]: MOVE      R12 R9       ; R12 := R9
 59 [-]: CALL      R10 3 1      ; R10(R11,R12)
 60 [-]: SELF      R10 R9 K14   ; R11 := R9; R10 := R9[0xc9f6a7d7]
 61 [-]: GETGLOBAL R12 K15      ; R12 := gScriptTriggerType
 62 [-]: CALL      R10 3 2      ; R10 := R10(R11,R12)
 63 [-]: GETGLOBAL R11 K11      ; R11 := 0x7b998233
 64 [-]: MOVE      R12 R10      ; R12 := R10
 65 [-]: CALL      R11 2 2      ; R11 := R11(R12)
 66 [-]: TEST      R11 1        ; if R11 then PC := 72
 67 [-]: JMP       72           ; PC := 72
 68 [-]: GETGLOBAL R11 K16      ; R11 := 0x11a19c5e
 69 [-]: MOVE      R12 R10      ; R12 := R10
 70 [-]: LOADK     R13 K17      ; R13 := "OnTouched"
 71 [-]: CALL      R11 3 1      ; R11(R12,R13)
 72 [-]: GETGLOBAL R11 K18      ; R11 := 0x33bdd652
 73 [-]: GETTABLE  R11 R11 K19  ; R82 := R11[0x23d5322f]
 74 [-]: MOVE      R12 R1       ; R12 := R1
 75 [-]: MOVE      R13 R9       ; R13 := R9
 76 [-]: CALL      R11 3 1      ; R11(R12,R13)
 77 [-]: EQ        0 R5 K20     ; if R5 ~= 1.000000 then PC := 83
 78 [-]: JMP       83           ; PC := 83
 79 [-]: GETUPVAL  R11 U2       ; R11 := U2
 80 [-]: SELF      R11 R11 K21  ; R12 := R11; R11 := R11[0xadfb12fc]
 81 [-]: MOVE      R13 R9       ; R13 := R9
 82 [-]: CALL      R11 3 1      ; R11(R12,R13)
 83 [-]: FORLOOP   R2 5         ; R2 += R4; if R2 <= R3 then begin PC := 5; R5 := R2 end
 84 [-]: RETURN    R0 1         ; return 


; Function #6:
;
; Name:            
; Defined at line: 174
; #Upvalues:       2
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  19

  1 [-]: NEWTABLE  R2 0 0       ; R2 := {}
  2 [-]: LOADNIL   R3 R4        ; R3 := R4 := nil
  3 [-]: GETUPVAL  R5 U0        ; R5 := U0
  4 [-]: SELF      R5 R5 K0     ; R6 := R5; R5 := R5[0x77384d14]
  5 [-]: CALL      R5 2 2       ; R5 := R5(R6)
  6 [-]: SELF      R6 R5 K1     ; R7 := R5; R6 := R5[0xf04f37dd]
  7 [-]: CALL      R6 2 2       ; R6 := R6(R7)
  8 [-]: LEN       R7 R6        ; R7 := # R6
  9 [-]: GETGLOBAL R8 K2        ; R8 := 0x00046924
 10 [-]: CALL      R8 1 2       ; R8 := R8()
 11 [-]: GETGLOBAL R9 K3        ; R9 := 0x7b998233
 12 [-]: GETUPVAL  R10 U0       ; R10 := U0
 13 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 14 [-]: TEST      R9 1         ; if R9 then PC := 19
 15 [-]: JMP       19           ; PC := 19
 16 [-]: SELF      R9 R5 K4     ; R10 := R5; R9 := R5[0xcb3851b8]
 17 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 18 [-]: MOVE      R8 R9        ; R8 := R9
 19 [-]: EQ        0 R7 K5      ; if R7 ~= 0.000000 then PC := 24
 20 [-]: JMP       24           ; PC := 24
 21 [-]: GETGLOBAL R9 K6        ; R9 := 0x3d106989
 22 [-]: LOADK     R10 K7       ; R10 := "Trying to create footprint meshes - Animal trail quad draw has no points?!"
 23 [-]: CALL      R9 2 1       ; R9(R10)
 24 [-]: LOADK     R9 1         ; R9 := 1.000000
 25 [-]: GETUPVAL  R10 U1       ; R10 := U1
 26 [-]: LOADK     R11 1        ; R11 := 1.000000
 27 [-]: FORPREP   R9 76        ; R9 -= R11; PC := 76
 28 [-]: GETGLOBAL R13 K3       ; R13 := 0x7b998233
 29 [-]: GETUPVAL  R14 U0       ; R14 := U0
 30 [-]: CALL      R13 2 2      ; R13 := R13(R14)
 31 [-]: TEST      R13 1        ; if R13 then PC := 55
 32 [-]: JMP       55           ; PC := 55
 33 [-]: GETTABLE  R3 R6 R12    ; R3 := R6[R12]
 34 [-]: LT        0 R12 R7     ; if R12 >= R7 then PC := 43
 35 [-]: JMP       43           ; PC := 43
 36 [-]: GETGLOBAL R13 K8       ; R13 := 0x20b7f774
 37 [-]: MOVE      R14 R3       ; R14 := R3
 38 [-]: ADD       R15 R12 K9   ; R15 := R12 + 1.000000
 39 [-]: GETTABLE  R15 R6 R15   ; R15 := R6[R15]
 40 [-]: CALL      R13 3 2      ; R13 := R13(R14,R15)
 41 [-]: MOVE      R4 R13       ; R4 := R13
 42 [-]: JMP       55           ; PC := 55
 43 [-]: LT        0 K9 R12     ; if 1.000000 >= R12 then PC := 52
 44 [-]: JMP       52           ; PC := 52
 45 [-]: GETGLOBAL R13 K8       ; R13 := 0x20b7f774
 46 [-]: SUB       R14 R12 K9   ; R14 := R12 - 1.000000
 47 [-]: GETTABLE  R14 R6 R14   ; R14 := R6[R14]
 48 [-]: MOVE      R15 R3       ; R15 := R3
 49 [-]: CALL      R13 3 2      ; R13 := R13(R14,R15)
 50 [-]: MOVE      R4 R13       ; R4 := R13
 51 [-]: JMP       55           ; PC := 55
 52 [-]: GETGLOBAL R13 K2       ; R13 := 0x00046924
 53 [-]: CALL      R13 1 2      ; R13 := R13()
 54 [-]: MOVE      R4 R13       ; R4 := R13
 55 [-]: TEST      R3 0         ; if not R3 then PC := 76
 56 [-]: JMP       76           ; PC := 76
 57 [-]: TEST      R4 0         ; if not R4 then PC := 76
 58 [-]: JMP       76           ; PC := 76
 59 [-]: GETGLOBAL R13 K10      ; R13 := 0x89326c93
 60 [-]: SELF      R13 R13 K11  ; R14 := R13; R13 := R13[0x05909209]
 61 [-]: GETGLOBAL R15 K12      ; R15 := 0x52c99df1
 62 [-]: MOVE      R16 R3       ; R16 := R3
 63 [-]: MOVE      R17 R4       ; R17 := R4
 64 [-]: MOVE      R18 R0       ; R18 := R0
 65 [-]: CALL      R13 6 2      ; R13 := R13(R14,R15,R16,R17,R18)
 66 [-]: GETGLOBAL R14 K3       ; R14 := 0x7b998233
 67 [-]: MOVE      R15 R13      ; R15 := R13
 68 [-]: CALL      R14 2 2      ; R14 := R14(R15)
 69 [-]: TEST      R14 1        ; if R14 then PC := 76
 70 [-]: JMP       76           ; PC := 76
 71 [-]: GETGLOBAL R14 K13      ; R14 := 0x33bdd652
 72 [-]: GETTABLE  R14 R14 K14  ; R82 := R14[0x23d5322f]
 73 [-]: MOVE      R15 R2       ; R15 := R2
 74 [-]: MOVE      R16 R13      ; R16 := R13
 75 [-]: CALL      R14 3 1      ; R14(R15,R16)
 76 [-]: FORLOOP   R9 28        ; R9 += R11; if R9 <= R10 then begin PC := 28; R12 := R9 end
 77 [-]: RETURN    R2 2         ; return R2
 78 [-]: RETURN    R0 1         ; return 


; Function #7:
;
; Name:            
; Defined at line: 215
; #Upvalues:       9
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  22

  1 [-]: GETGLOBAL R2 K0        ; R2 := 0x89326c93
  2 [-]: SELF      R2 R2 K1     ; R3 := R2; R2 := R2[0x29ef273d]
  3 [-]: CALL      R2 2 2       ; R2 := R2(R3)
  4 [-]: SELF      R3 R2 K2     ; R4 := R2; R3 := R2[0x66905cb0]
  5 [-]: CALL      R3 2 2       ; R3 := R3(R4)
  6 [-]: NEWTABLE  R4 0 0       ; R4 := {}
  7 [-]: LOADNIL   R5 R6        ; R5 := R6 := nil
  8 [-]: LOADK     R7 0         ; R7 := 0.000000
  9 [-]: GETGLOBAL R8 K3        ; R8 := 0x41562363
 10 [-]: LT        0 K4 R8      ; if 0.000000 >= R8 then PC := 18
 11 [-]: JMP       18           ; PC := 18
 12 [-]: GETGLOBAL R8 K5        ; R8 := 0x5bced4c4
 13 [-]: GETTABLE  R8 R8 K6     ; R82 := R8[0x55f27c30]
 14 [-]: GETGLOBAL R9 K3        ; R9 := 0x41562363
 15 [-]: DIV       R9 R1 R9     ; R9 := R1 / R9
 16 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 17 [-]: MOVE      R7 R8        ; R7 := R8
 18 [-]: GETUPVAL  R8 U0        ; R8 := U0
 19 [-]: LEN       R8 R8        ; R8 := # R8
 20 [-]: EQ        0 R8 K4      ; if R8 ~= 0.000000 then PC := 31
 21 [-]: JMP       31           ; PC := 31
 22 [-]: GETUPVAL  R9 U1        ; R9 := U1
 23 [-]: TEST      R9 1         ; if R9 then PC := 30
 24 [-]: JMP       30           ; PC := 30
 25 [-]: GETGLOBAL R9 K7        ; R9 := 0x3d106989
 26 [-]: LOADK     R10 K8       ; R10 := "CONSERVATION ERROR - Animal trail encounter has no viable path points?!"
 27 [-]: CALL      R9 2 1       ; R9(R10)
 28 [-]: LOADBOOL  R9 1 0       ; R9 := true
 29 [-]: SETUPVAL  R9 U1        ; U82 := 	
 30 [-]: RETURN    R0 1         ; return 
 31 [-]: GETGLOBAL R9 K5        ; R9 := 0x5bced4c4
 32 [-]: GETTABLE  R9 R9 K9     ; R82 := R9[0x99675e23]
 33 [-]: GETGLOBAL R10 K10      ; R10 := 0xb9dc8e30
 34 [-]: MUL       R10 R8 R10   ; R10 := R8 * R10
 35 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 36 [-]: SETUPVAL  R9 U2        ; U82 := 	
 37 [-]: LOADK     R9 1         ; R9 := 1.000000
 38 [-]: SUB       R10 R7 K11   ; R10 := R7 - 1.000000
 39 [-]: LOADK     R11 1        ; R11 := 1.000000
 40 [-]: FORPREP   R9 108       ; R9 -= R11; PC := 108
 41 [-]: GETGLOBAL R13 K12      ; R13 := 0x7b998233
 42 [-]: GETUPVAL  R14 U3       ; R14 := U3
 43 [-]: CALL      R13 2 2      ; R13 := R13(R14)
 44 [-]: TEST      R13 1        ; if R13 then PC := 71
 45 [-]: JMP       71           ; PC := 71
 46 [-]: LT        0 K4 R8      ; if 0.000000 >= R8 then PC := 71
 47 [-]: JMP       71           ; PC := 71
 48 [-]: GETGLOBAL R13 K5       ; R13 := 0x5bced4c4
 49 [-]: GETTABLE  R13 R13 K6   ; R82 := R13[0x55f27c30]
 50 [-]: GETUPVAL  R14 U4       ; R14 := U4
 51 [-]: LOADK     R15 1        ; R15 := 1.000000
 52 [-]: MOVE      R16 R7       ; R16 := R7
 53 [-]: LOADK     R17 1        ; R17 := 1.000000
 54 [-]: MOVE      R18 R8       ; R18 := R8
 55 [-]: MOVE      R19 R12      ; R19 := R12
 56 [-]: CALL      R14 6 0      ; R14,... := R14(R15,R16,R17,R18,R19)
 57 [-]: CALL      R13 0 2      ; R13 := R13(R14,...)
 58 [-]: SELF      R14 R3 K13   ; R15 := R3; R14 := R3[0x0e8c38e5]
 59 [-]: GETUPVAL  R16 U0       ; R16 := U0
 60 [-]: GETTABLE  R16 R16 R13  ; R16 := R16[R13]
 61 [-]: CALL      R14 3 2      ; R14 := R14(R15,R16)
 62 [-]: GETUPVAL  R15 U5       ; R15 := U5
 63 [-]: GETTABLE  R15 R15 K14  ; R82 := R15[0x29595ba9]
 64 [-]: MOVE      R16 R14      ; R16 := R14
 65 [-]: LOADK     R17 1        ; R17 := 1.000000
 66 [-]: LOADBOOL  R18 0 0      ; R18 := false
 67 [-]: LOADK     R19 K15      ; R19 := 0.100000
 68 [-]: CALL      R15 5 3      ; R15,R16 := R15(R16,R17,R18,R19)
 69 [-]: MOVE      R6 R16       ; R6 := R16
 70 [-]: MOVE      R5 R15       ; R5 := R15
 71 [-]: TEST      R5 0         ; if not R5 then PC := 108
 72 [-]: JMP       108          ; PC := 108
 73 [-]: TEST      R6 0         ; if not R6 then PC := 108
 74 [-]: JMP       108          ; PC := 108
 75 [-]: GETGLOBAL R15 K16      ; R15 := 0xb707937c
 76 [-]: GETGLOBAL R16 K17      ; R16 := 0x55730e1a
 77 [-]: LOADK     R17 1        ; R17 := 1.000000
 78 [-]: GETGLOBAL R18 K16      ; R18 := 0xb707937c
 79 [-]: LEN       R18 R18      ; R18 := # R18
 80 [-]: CALL      R16 3 2      ; R16 := R16(R17,R18)
 81 [-]: GETTABLE  R15 R15 R16  ; R15 := R15[R16]
 82 [-]: GETGLOBAL R16 K0       ; R16 := 0x89326c93
 83 [-]: SELF      R16 R16 K18  ; R17 := R16; R16 := R16[0x05909209]
 84 [-]: MOVE      R18 R15      ; R18 := R15
 85 [-]: MOVE      R19 R5       ; R19 := R5
 86 [-]: MOVE      R20 R6       ; R20 := R6
 87 [-]: MOVE      R21 R0       ; R21 := R0
 88 [-]: CALL      R16 6 2      ; R16 := R16(R17,R18,R19,R20,R21)
 89 [-]: GETGLOBAL R17 K12      ; R17 := 0x7b998233
 90 [-]: MOVE      R18 R16      ; R18 := R16
 91 [-]: CALL      R17 2 2      ; R17 := R17(R18)
 92 [-]: TEST      R17 1        ; if R17 then PC := 108
 93 [-]: JMP       108          ; PC := 108
 94 [-]: SELF      R17 R16 K19  ; R18 := R16; R17 := R16[0x3273ba96]
 95 [-]: GETUPVAL  R19 U6       ; R19 := U6
 96 [-]: CALL      R17 3 1      ; R17(R18,R19)
 97 [-]: GETUPVAL  R17 U7       ; R17 := U7
 98 [-]: SELF      R17 R17 K20  ; R18 := R17; R17 := R17[0x86d3529e]
 99 [-]: CALL      R17 2 2      ; R17 := R17(R18)
100 [-]: SELF      R17 R17 K21  ; R18 := R17; R17 := R17[0x21dfc654]
101 [-]: MOVE      R19 R16      ; R19 := R16
102 [-]: CALL      R17 3 1      ; R17(R18,R19)
103 [-]: GETGLOBAL R17 K22      ; R17 := 0x33bdd652
104 [-]: GETTABLE  R17 R17 K23  ; R82 := R17[0x23d5322f]
105 [-]: MOVE      R18 R4       ; R18 := R4
106 [-]: MOVE      R19 R16      ; R19 := R16
107 [-]: CALL      R17 3 1      ; R17(R18,R19)
108 [-]: FORLOOP   R9 41        ; R9 += R11; if R9 <= R10 then begin PC := 41; R12 := R9 end
109 [-]: GETGLOBAL R17 K24      ; R17 := 0x1d12a0c2
110 [-]: LEN       R17 R17      ; R17 := # R17
111 [-]: LT        0 K4 R17     ; if 0.000000 >= R17 then PC := 122
112 [-]: JMP       122          ; PC := 122
113 [-]: GETUPVAL  R17 U2       ; R17 := U2
114 [-]: LT        0 K4 R17     ; if 0.000000 >= R17 then PC := 122
115 [-]: JMP       122          ; PC := 122
116 [-]: GETUPVAL  R17 U8       ; R17 := U8
117 [-]: GETUPVAL  R18 U0       ; R18 := U0
118 [-]: GETUPVAL  R19 U2       ; R19 := U2
119 [-]: GETTABLE  R18 R18 R19  ; R18 := R18[R19]
120 [-]: MOVE      R19 R4       ; R19 := R4
121 [-]: CALL      R17 3 1      ; R17(R18,R19)
122 [-]: RETURN    R4 2         ; return R4
123 [-]: RETURN    R0 1         ; return 


; Function #8:
;
; Name:            
; Defined at line: 268
; #Upvalues:       9
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  3

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: EQ        0 R0 K0      ; if R0 ~= nil then PC := 9
  3 [-]: JMP       9            ; PC := 9
  4 [-]: GETUPVAL  R0 U1        ; R0 := U1
  5 [-]: GETUPVAL  R1 U2        ; R1 := U2
  6 [-]: GETUPVAL  R2 U3        ; R2 := U3
  7 [-]: CALL      R0 3 2       ; R0 := R0(R1,R2)
  8 [-]: SETUPVAL  R0 U0        ; U82 := 
  9 [-]: GETUPVAL  R0 U4        ; R0 := U4
 10 [-]: EQ        0 R0 K0      ; if R0 ~= nil then PC := 22
 11 [-]: JMP       22           ; PC := 22
 12 [-]: GETGLOBAL R0 K1        ; R0 := 0x7b998233
 13 [-]: GETGLOBAL R1 K2        ; R1 := 0x52c99df1
 14 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 15 [-]: TEST      R0 1         ; if R0 then PC := 22
 16 [-]: JMP       22           ; PC := 22
 17 [-]: GETUPVAL  R0 U5        ; R0 := U5
 18 [-]: GETUPVAL  R1 U2        ; R1 := U2
 19 [-]: GETUPVAL  R2 U3        ; R2 := U3
 20 [-]: CALL      R0 3 2       ; R0 := R0(R1,R2)
 21 [-]: SETUPVAL  R0 U4        ; U82 := 
 22 [-]: LOADBOOL  R0 0 0       ; R0 := false
 23 [-]: SETUPVAL  R0 U6        ; U82 := 
 24 [-]: GETUPVAL  R0 U7        ; R0 := U7
 25 [-]: EQ        0 R0 K0      ; if R0 ~= nil then PC := 45
 26 [-]: JMP       45           ; PC := 45
 27 [-]: GETGLOBAL R0 K1        ; R0 := 0x7b998233
 28 [-]: GETUPVAL  R1 U8        ; R1 := U8
 29 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 30 [-]: TEST      R0 1         ; if R0 then PC := 45
 31 [-]: JMP       45           ; PC := 45
 32 [-]: GETUPVAL  R0 U8        ; R0 := U8
 33 [-]: SELF      R0 R0 K3     ; R1 := R0; R0 := R0[0x77384d14]
 34 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 35 [-]: GETGLOBAL R1 K1        ; R1 := 0x7b998233
 36 [-]: MOVE      R2 R0        ; R2 := R0
 37 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 38 [-]: TEST      R1 1         ; if R1 then PC := 45
 39 [-]: JMP       45           ; PC := 45
 40 [-]: SELF      R1 R0 K4     ; R2 := R0; R1 := R0[0xf04f37dd]
 41 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 42 [-]: SETUPVAL  R1 U7        ; U82 := 
 43 [-]: LOADBOOL  R1 1 0       ; R1 := true
 44 [-]: SETUPVAL  R1 U6        ; U82 := 
 45 [-]: RETURN    R0 1         ; return 


; Function #9:
;
; Name:            
; Defined at line: 290
; #Upvalues:       4
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  10

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x89326c93
  2 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x78298275]
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
  5 [-]: MOVE      R2 R0        ; R2 := R0
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: TEST      R1 1         ; if R1 then PC := 75
  8 [-]: JMP       75           ; PC := 75
  9 [-]: SELF      R1 R0 K3     ; R2 := R0; R1 := R0[0xde321e6f]
 10 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 11 [-]: SELF      R2 R1 K4     ; R3 := R1; R2 := R1[0x881b6b90]
 12 [-]: LOADK     R4 0         ; R4 := 0.000000
 13 [-]: CALL      R2 3 2       ; R2 := R2(R3,R4)
 14 [-]: GETUPVAL  R3 U0        ; R3 := U0
 15 [-]: TEST      R3 1         ; if R3 then PC := 25
 16 [-]: JMP       25           ; PC := 25
 17 [-]: GETGLOBAL R3 K2        ; R3 := 0x7b998233
 18 [-]: GETUPVAL  R4 U1        ; R4 := U1
 19 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 20 [-]: TEST      R3 0         ; if not R3 then PC := 23
 21 [-]: JMP       23           ; PC := 23
 22 [-]: RETURN    R0 1         ; return 
 23 [-]: GETUPVAL  R3 U2        ; R3 := U2
 24 [-]: CALL      R3 1 1       ; R3()
 25 [-]: LOADBOOL  R3 0 0       ; R3 := false
 26 [-]: GETGLOBAL R4 K2        ; R4 := 0x7b998233
 27 [-]: MOVE      R5 R2        ; R5 := R2
 28 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 29 [-]: TEST      R4 1         ; if R4 then PC := 40
 30 [-]: JMP       40           ; PC := 40
 31 [-]: SELF      R4 R2 K6     ; R5 := R2; R4 := R2[0xf2deaf69]
 32 [-]: GETGLOBAL R6 K7        ; R6 := 0xda671e35
 33 [-]: CALL      R4 3 2       ; R4 := R4(R5,R6)
 34 [-]: TESTSET   R3 R4 1      ; if R4 then PC := 40 else R3 := R4
 35 [-]: JMP       40           ; PC := 40
 36 [-]: SELF      R4 R2 K6     ; R5 := R2; R4 := R2[0xf2deaf69]
 37 [-]: GETGLOBAL R6 K8        ; R6 := 0x37528f9d
 38 [-]: CALL      R4 3 2       ; R4 := R4(R5,R6)
 39 [-]: MOVE      R3 R4        ; R3 := R4
 40 [-]: GETGLOBAL R4 K2        ; R4 := 0x7b998233
 41 [-]: GETUPVAL  R5 U1        ; R5 := U1
 42 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 43 [-]: TEST      R4 1         ; if R4 then PC := 75
 44 [-]: JMP       75           ; PC := 75
 45 [-]: GETUPVAL  R4 U1        ; R4 := U1
 46 [-]: SELF      R4 R4 K9     ; R5 := R4; R4 := R4[0x77384d14]
 47 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 48 [-]: GETGLOBAL R5 K2        ; R5 := 0x7b998233
 49 [-]: MOVE      R6 R4        ; R6 := R4
 50 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 51 [-]: TEST      R5 1         ; if R5 then PC := 75
 52 [-]: JMP       75           ; PC := 75
 53 [-]: SELF      R5 R4 K10    ; R6 := R4; R5 := R4[0x6af8445c]
 54 [-]: GETUPVAL  R7 U3        ; R7 := U3
 55 [-]: LOADK     R8 1         ; R8 := 1.000000
 56 [-]: CALL      R5 4 2       ; R5 := R5(R6,R7,R8)
 57 [-]: TEST      R3 0         ; if not R3 then PC := 67
 58 [-]: JMP       67           ; PC := 67
 59 [-]: GETGLOBAL R6 K11       ; R6 := 0xb1225cbf
 60 [-]: EQ        1 R5 R6      ; if R5 == R6 then PC := 67
 61 [-]: JMP       67           ; PC := 67
 62 [-]: SELF      R6 R4 K12    ; R7 := R4; R6 := R4[0x986d2ab8]
 63 [-]: GETUPVAL  R8 U3        ; R8 := U3
 64 [-]: GETGLOBAL R9 K11       ; R9 := 0xb1225cbf
 65 [-]: CALL      R6 4 1       ; R6(R7,R8,R9)
 66 [-]: JMP       75           ; PC := 75
 67 [-]: EQ        0 R3 K13     ; if R3 ~= false then PC := 75
 68 [-]: JMP       75           ; PC := 75
 69 [-]: EQ        1 R5 K14     ; if R5 == 0.000000 then PC := 75
 70 [-]: JMP       75           ; PC := 75
 71 [-]: SELF      R6 R4 K12    ; R7 := R4; R6 := R4[0x986d2ab8]
 72 [-]: GETUPVAL  R8 U3        ; R8 := U3
 73 [-]: LOADK     R9 0         ; R9 := 0.000000
 74 [-]: CALL      R6 4 1       ; R6(R7,R8,R9)
 75 [-]: RETURN    R0 1         ; return 


; Function #10:
;
; Name:            
; Defined at line: 326
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  16

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x89326c93
  2 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x78298275]
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
  5 [-]: MOVE      R2 R0        ; R2 := R0
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: TEST      R1 1         ; if R1 then PC := 80
  8 [-]: JMP       80           ; PC := 80
  9 [-]: GETUPVAL  R1 U0        ; R1 := U0
 10 [-]: LEN       R1 R1        ; R1 := # R1
 11 [-]: LT        0 K3 R1      ; if 0.000000 >= R1 then PC := 80
 12 [-]: JMP       80           ; PC := 80
 13 [-]: SELF      R1 R0 K4     ; R2 := R0; R1 := R0[0xde321e6f]
 14 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 15 [-]: SELF      R2 R1 K5     ; R3 := R1; R2 := R1[0x881b6b90]
 16 [-]: LOADK     R4 0         ; R4 := 0.000000
 17 [-]: CALL      R2 3 2       ; R2 := R2(R3,R4)
 18 [-]: LOADBOOL  R3 0 0       ; R3 := false
 19 [-]: GETGLOBAL R4 K2        ; R4 := 0x7b998233
 20 [-]: MOVE      R5 R2        ; R5 := R2
 21 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 22 [-]: TEST      R4 1         ; if R4 then PC := 33
 23 [-]: JMP       33           ; PC := 33
 24 [-]: SELF      R4 R2 K7     ; R5 := R2; R4 := R2[0xf2deaf69]
 25 [-]: GETGLOBAL R6 K8        ; R6 := 0xda671e35
 26 [-]: CALL      R4 3 2       ; R4 := R4(R5,R6)
 27 [-]: TESTSET   R3 R4 1      ; if R4 then PC := 33 else R3 := R4
 28 [-]: JMP       33           ; PC := 33
 29 [-]: SELF      R4 R2 K7     ; R5 := R2; R4 := R2[0xf2deaf69]
 30 [-]: GETGLOBAL R6 K9        ; R6 := 0x37528f9d
 31 [-]: CALL      R4 3 2       ; R4 := R4(R5,R6)
 32 [-]: MOVE      R3 R4        ; R3 := R4
 33 [-]: LOADK     R4 1         ; R4 := 1.000000
 34 [-]: GETUPVAL  R5 U0        ; R5 := U0
 35 [-]: LEN       R5 R5        ; R5 := # R5
 36 [-]: LOADK     R6 1         ; R6 := 1.000000
 37 [-]: FORPREP   R4 79        ; R4 -= R6; PC := 79
 38 [-]: GETUPVAL  R8 U0        ; R8 := U0
 39 [-]: GETTABLE  R8 R8 R7     ; R8 := R8[R7]
 40 [-]: GETGLOBAL R9 K2        ; R9 := 0x7b998233
 41 [-]: MOVE      R10 R8       ; R10 := R8
 42 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 43 [-]: TEST      R9 1         ; if R9 then PC := 79
 44 [-]: JMP       79           ; PC := 79
 45 [-]: SELF      R9 R0 K10    ; R10 := R0; R9 := R0[0xbebad19f]
 46 [-]: MOVE      R11 R8       ; R11 := R8
 47 [-]: CALL      R9 3 2       ; R9 := R9(R10,R11)
 48 [-]: TESTSET   R10 R3 0     ; if not R3 then PC := 55 else R10 := R3
 49 [-]: JMP       55           ; PC := 55
 50 [-]: GETGLOBAL R10 K11      ; R10 := 0xd385fcf5
 51 [-]: LT        1 R9 R10     ; if R9 < R10 then PC := 54
 52 [-]: JMP       54           ; PC := 54
 53 [-]: LOADBOOL  R10 0 1      ; R10 := false; PC := 54
 54 [-]: LOADBOOL  R10 1 0      ; R10 := true
 55 [-]: SELF      R11 R8 K12   ; R12 := R8; R11 := R8[0xc9f6a7d7]
 56 [-]: GETGLOBAL R13 K13      ; R13 := 0x64bf8fdb
 57 [-]: CALL      R11 3 2      ; R11 := R11(R12,R13)
 58 [-]: GETGLOBAL R12 K2       ; R12 := 0x7b998233
 59 [-]: MOVE      R13 R11      ; R13 := R11
 60 [-]: CALL      R12 2 2      ; R12 := R12(R13)
 61 [-]: TEST      R12 0        ; if not R12 then PC := 70
 62 [-]: JMP       70           ; PC := 70
 63 [-]: TEST      R10 0        ; if not R10 then PC := 70
 64 [-]: JMP       70           ; PC := 70
 65 [-]: SELF      R12 R8 K14   ; R13 := R8; R12 := R8[0x47901f07]
 66 [-]: GETGLOBAL R14 K13      ; R14 := 0x64bf8fdb
 67 [-]: GETGLOBAL R15 K15      ; R15 := EMPTY_SYMBOL
 68 [-]: CALL      R12 4 1      ; R12(R13,R14,R15)
 69 [-]: JMP       79           ; PC := 79
 70 [-]: GETGLOBAL R12 K2       ; R12 := 0x7b998233
 71 [-]: MOVE      R13 R11      ; R13 := R11
 72 [-]: CALL      R12 2 2      ; R12 := R12(R13)
 73 [-]: TEST      R12 1        ; if R12 then PC := 79
 74 [-]: JMP       79           ; PC := 79
 75 [-]: TEST      R10 1        ; if R10 then PC := 79
 76 [-]: JMP       79           ; PC := 79
 77 [-]: SELF      R12 R11 K16  ; R13 := R11; R12 := R11[0xf5b3034c]
 78 [-]: CALL      R12 2 1      ; R12(R13)
 79 [-]: FORLOOP   R4 38        ; R4 += R6; if R4 <= R5 then begin PC := 38; R7 := R4 end
 80 [-]: RETURN    R0 1         ; return 


; Function #11:
;
; Name:            
; Defined at line: 357
; #Upvalues:       2
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETTABLE  R1 R1 K0     ; R82 := R1[0xd0825b2c]
  3 [-]: CALL      R1 1 2       ; R1 := R1()
  4 [-]: GETGLOBAL R2 K1        ; R2 := 0x7b998233
  5 [-]: GETUPVAL  R3 U1        ; R3 := U1
  6 [-]: CALL      R2 2 2       ; R2 := R2(R3)
  7 [-]: TEST      R2 1         ; if R2 then PC := 17
  8 [-]: JMP       17           ; PC := 17
  9 [-]: GETUPVAL  R2 U0        ; R2 := U0
 10 [-]: GETTABLE  R2 R2 K2     ; R2 := R2["states"]
 11 [-]: GETTABLE  R2 R2 K3     ; R2 := R2["PITCH_GAME_CALLS_COMPLETE"]
 12 [-]: LE        0 R2 R1      ; if R2 > R1 then PC := 17
 13 [-]: JMP       17           ; PC := 17
 14 [-]: GETUPVAL  R2 U1        ; R2 := U1
 15 [-]: SELF      R2 R2 K4     ; R3 := R2; R2 := R2[0xa2880940]
 16 [-]: CALL      R2 2 1       ; R2(R3)
 17 [-]: RETURN    R0 1         ; return 


; Function #12:
;
; Name:            
; Defined at line: 367
; #Upvalues:       5
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  3

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
  2 [-]: GETUPVAL  R2 U0        ; R2 := U0
  3 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  4 [-]: TEST      R1 0         ; if not R1 then PC := 7
  5 [-]: JMP       7            ; PC := 7
  6 [-]: RETURN    R0 1         ; return 
  7 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
  8 [-]: GETUPVAL  R2 U1        ; R2 := U1
  9 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 10 [-]: TEST      R1 1         ; if R1 then PC := 19
 11 [-]: JMP       19           ; PC := 19
 12 [-]: GETUPVAL  R1 U2        ; R1 := U2
 13 [-]: MOVE      R2 R0        ; R2 := R0
 14 [-]: CALL      R1 2 1       ; R1(R2)
 15 [-]: GETUPVAL  R1 U3        ; R1 := U3
 16 [-]: CALL      R1 1 1       ; R1()
 17 [-]: GETUPVAL  R1 U4        ; R1 := U4
 18 [-]: CALL      R1 1 1       ; R1()
 19 [-]: RETURN    R0 1         ; return 


; Function #13:
;
; Name:            
; Defined at line: 379
; #Upvalues:       14
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  18

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x3d106989
  2 [-]: LOADK     R2 K1        ; R2 := "Trail Encounter Started."
  3 [-]: CALL      R1 2 1       ; R1(R2)
  4 [-]: SETUPVAL  R0 U0        ; U82 := 
  5 [-]: GETUPVAL  R1 U0        ; R1 := U0
  6 [-]: SELF      R1 R1 K2     ; R2 := R1; R1 := R1[0xd1586535]
  7 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  8 [-]: SETUPVAL  R1 U1        ; U82 := 
  9 [-]: SELF      R1 R0 K3     ; R2 := R0; R1 := R0[0x86d3529e]
 10 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 11 [-]: LOADNIL   R2 R2        ; R2 := nil
 12 [-]: GETGLOBAL R3 K4        ; R3 := 0x7b998233
 13 [-]: MOVE      R4 R1        ; R4 := R1
 14 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 15 [-]: TEST      R3 1         ; if R3 then PC := 62
 16 [-]: JMP       62           ; PC := 62
 17 [-]: SELF      R3 R1 K5     ; R4 := R1; R3 := R1[0x4c976eda]
 18 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 19 [-]: MOVE      R2 R3        ; R2 := R3
 20 [-]: GETGLOBAL R3 K6        ; R3 := 0x7ed0a956
 21 [-]: LOADK     R4 K7        ; R4 := "/Lotus/Types/Gameplay/Conservation/AnimalStartGameplayObject"
 22 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 23 [-]: SELF      R4 R1 K8     ; R5 := R1; R4 := R1[0x5ce127ee]
 24 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 25 [-]: LOADK     R5 1         ; R5 := 1.000000
 26 [-]: LEN       R6 R4        ; R6 := # R4
 27 [-]: LOADK     R7 1         ; R7 := 1.000000
 28 [-]: FORPREP   R5 56        ; R5 -= R7; PC := 56
 29 [-]: GETTABLE  R9 R4 R8     ; R9 := R4[R8]
 30 [-]: GETGLOBAL R10 K4       ; R10 := 0x7b998233
 31 [-]: MOVE      R11 R9       ; R11 := R9
 32 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 33 [-]: TEST      R10 1        ; if R10 then PC := 45
 34 [-]: JMP       45           ; PC := 45
 35 [-]: SELF      R10 R9 K9    ; R11 := R9; R10 := R9[0xf2deaf69]
 36 [-]: MOVE      R12 R3       ; R12 := R3
 37 [-]: CALL      R10 3 2      ; R10 := R10(R11,R12)
 38 [-]: TEST      R10 0        ; if not R10 then PC := 45
 39 [-]: JMP       45           ; PC := 45
 40 [-]: SELF      R10 R9 K2    ; R11 := R9; R10 := R9[0xd1586535]
 41 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 42 [-]: SETUPVAL  R10 U2       ; U82 := 

 43 [-]: JMP       57           ; PC := 57
 44 [-]: JMP       56           ; PC := 56
 45 [-]: GETGLOBAL R10 K4       ; R10 := 0x7b998233
 46 [-]: MOVE      R11 R9       ; R11 := R9
 47 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 48 [-]: TEST      R10 0        ; if not R10 then PC := 56
 49 [-]: JMP       56           ; PC := 56
 50 [-]: GETGLOBAL R10 K10      ; R10 := 0x33bdd652
 51 [-]: GETTABLE  R10 R10 K11  ; R82 := R10[0x9c1f3b5a]
 52 [-]: MOVE      R11 R4       ; R11 := R4
 53 [-]: MOVE      R12 R8       ; R12 := R8
 54 [-]: CALL      R10 3 1      ; R10(R11,R12)
 55 [-]: SUB       R8 R8 K12    ; R8 := R8 - 1.000000
 56 [-]: FORLOOP   R5 29        ; R5 += R7; if R5 <= R6 then begin PC := 29; R8 := R5 end
 57 [-]: GETUPVAL  R10 U4       ; R10 := U4
 58 [-]: GETTABLE  R10 R10 K13  ; R82 := R10[0x4ed58f32]
 59 [-]: MOVE      R11 R1       ; R11 := R1
 60 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 61 [-]: SETUPVAL  R10 U3       ; U82 := 

 62 [-]: GETUPVAL  R10 U2       ; R10 := U2
 63 [-]: EQ        0 R10 K14    ; if R10 ~= nil then PC := 72
 64 [-]: JMP       72           ; PC := 72
 65 [-]: GETGLOBAL R10 K0       ; R10 := 0x3d106989
 66 [-]: LOADK     R11 K15      ; R11 := "AnimalTrail: No start point found"
 67 [-]: CALL      R10 2 1      ; R10(R11)
 68 [-]: SELF      R10 R0 K16   ; R11 := R0; R10 := R0[0xfe9dc265]
 69 [-]: LOADK     R12 6        ; R12 := 6.000000
 70 [-]: CALL      R10 3 1      ; R10(R11,R12)
 71 [-]: RETURN    R0 1         ; return 
 72 [-]: GETGLOBAL R10 K4       ; R10 := 0x7b998233
 73 [-]: MOVE      R11 R1       ; R11 := R1
 74 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 75 [-]: TEST      R10 1        ; if R10 then PC := 249
 76 [-]: JMP       249          ; PC := 249
 77 [-]: GETGLOBAL R10 K4       ; R10 := 0x7b998233
 78 [-]: GETUPVAL  R11 U5       ; R11 := U5
 79 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 80 [-]: TEST      R10 0        ; if not R10 then PC := 92
 81 [-]: JMP       92           ; PC := 92
 82 [-]: GETGLOBAL R10 K18      ; R10 := 0x89326c93
 83 [-]: SELF      R10 R10 K19  ; R11 := R10; R10 := R10[0x462c251c]
 84 [-]: GETUPVAL  R12 U6       ; R12 := U6
 85 [-]: SELF      R13 R0 K2    ; R14 := R0; R13 := R0[0xd1586535]
 86 [-]: CALL      R13 2 2      ; R13 := R13(R14)
 87 [-]: LOADK     R14 0        ; R14 := 0.000000
 88 [-]: SELF      R15 R0 K20   ; R16 := R0; R15 := R0[0xf6cf204f]
 89 [-]: CALL      R15 2 0      ; R15,... := R15(R16)
 90 [-]: CALL      R10 0 2      ; R10 := R10(R11,...)
 91 [-]: SETUPVAL  R10 U5       ; U82 := 

 92 [-]: GETGLOBAL R10 K21      ; R10 := 0x5cb1ffd5
 93 [-]: TEST      R10 0        ; if not R10 then PC := 107
 94 [-]: JMP       107          ; PC := 107
 95 [-]: GETGLOBAL R10 K18      ; R10 := 0x89326c93
 96 [-]: SELF      R10 R10 K22  ; R11 := R10; R10 := R10[0x9ed3b54e]
 97 [-]: SELF      R12 R0 K2    ; R13 := R0; R12 := R0[0xd1586535]
 98 [-]: CALL      R12 2 2      ; R12 := R12(R13)
 99 [-]: LOADK     R13 4        ; R13 := 4.000000
100 [-]: GETGLOBAL R14 K23      ; R14 := 0x60130201
101 [-]: LOADK     R15 0        ; R15 := 0.000000
102 [-]: LOADK     R16 0        ; R16 := 0.000000
103 [-]: LOADK     R17 192      ; R17 := 192.000000
104 [-]: CALL      R14 4 2      ; R14 := R14(R15,R16,R17)
105 [-]: LOADK     R15 K24      ; R15 := 9999.000000
106 [-]: CALL      R10 6 1      ; R10(R11,R12,R13,R14,R15)
107 [-]: GETGLOBAL R10 K4       ; R10 := 0x7b998233
108 [-]: GETUPVAL  R11 U7       ; R11 := U7
109 [-]: CALL      R10 2 2      ; R10 := R10(R11)
110 [-]: TEST      R10 0        ; if not R10 then PC := 232
111 [-]: JMP       232          ; PC := 232
112 [-]: GETGLOBAL R10 K18      ; R10 := 0x89326c93
113 [-]: SELF      R10 R10 K25  ; R11 := R10; R10 := R10[0x05909209]
114 [-]: GETGLOBAL R12 K26      ; R12 := 0x595510b0
115 [-]: GETUPVAL  R13 U2       ; R13 := U2
116 [-]: GETGLOBAL R14 K27      ; R14 := ZERO_ROTATION
117 [-]: GETUPVAL  R15 U0       ; R15 := U0
118 [-]: CALL      R10 6 2      ; R10 := R10(R11,R12,R13,R14,R15)
119 [-]: SETUPVAL  R10 U7       ; U82 := 

120 [-]: GETGLOBAL R10 K4       ; R10 := 0x7b998233
121 [-]: GETUPVAL  R11 U7       ; R11 := U7
122 [-]: CALL      R10 2 2      ; R10 := R10(R11)
123 [-]: TEST      R10 0        ; if not R10 then PC := 129
124 [-]: JMP       129          ; PC := 129
125 [-]: GETGLOBAL R10 K0       ; R10 := 0x3d106989
126 [-]: LOADK     R11 K28      ; R11 := "No trail effect created"
127 [-]: CALL      R10 2 1      ; R10(R11)
128 [-]: JMP       232          ; PC := 232
129 [-]: GETUPVAL  R10 U7       ; R10 := U7
130 [-]: SELF      R10 R10 K29  ; R11 := R10; R10 := R10[0x9e9c67cb]
131 [-]: GETUPVAL  R12 U0       ; R12 := U0
132 [-]: SELF      R12 R12 K2   ; R13 := R12; R12 := R12[0xd1586535]
133 [-]: CALL      R12 2 0      ; R12,... := R12(R13)
134 [-]: CALL      R10 0 1      ; R10(R11,...)
135 [-]: GETUPVAL  R10 U7       ; R10 := U7
136 [-]: SELF      R10 R10 K30  ; R11 := R10; R10 := R10[0x896e7684]
137 [-]: GETGLOBAL R12 K31      ; R12 := 0x4133ccbc
138 [-]: CALL      R10 3 1      ; R10(R11,R12)
139 [-]: GETUPVAL  R10 U7       ; R10 := U7
140 [-]: SELF      R10 R10 K32  ; R11 := R10; R10 := R10[0xa45718b3]
141 [-]: CALL      R10 2 2      ; R10 := R10(R11)
142 [-]: SETUPVAL  R10 U8       ; U82 := 

143 [-]: GETUPVAL  R10 U8       ; R10 := U8
144 [-]: LEN       R10 R10      ; R10 := # R10
145 [-]: LT        0 K33 R10    ; if 0.000000 >= R10 then PC := 229
146 [-]: JMP       229          ; PC := 229
147 [-]: GETUPVAL  R10 U7       ; R10 := U7
148 [-]: SELF      R10 R10 K34  ; R11 := R10; R10 := R10[0x296b6ab0]
149 [-]: CALL      R10 2 2      ; R10 := R10(R11)
150 [-]: SETUPVAL  R10 U9       ; U82 := 

151 [-]: GETUPVAL  R10 U8       ; R10 := U8
152 [-]: GETTABLE  R10 R10 K12  ; R10 := R10[1.000000]
153 [-]: SETUPVAL  R10 U2       ; U82 := 

154 [-]: GETUPVAL  R10 U7       ; R10 := U7
155 [-]: SELF      R10 R10 K35  ; R11 := R10; R10 := R10[0x00831739]
156 [-]: GETGLOBAL R12 K36      ; R12 := 0x5bced4c4
157 [-]: GETTABLE  R12 R12 K37  ; R82 := R12[0x55f27c30]
158 [-]: GETUPVAL  R13 U9       ; R13 := U9
159 [-]: GETGLOBAL R14 K38      ; R14 := 0xeee98c27
160 [-]: DIV       R13 R13 R14  ; R13 := R13 / R14
161 [-]: CALL      R12 2 0      ; R12,... := R12(R13)
162 [-]: CALL      R10 0 1      ; R10(R11,...)
163 [-]: GETUPVAL  R10 U7       ; R10 := U7
164 [-]: SELF      R10 R10 K39  ; R11 := R10; R10 := R10[0x88051e24]
165 [-]: CALL      R10 2 1      ; R10(R11)
166 [-]: GETUPVAL  R10 U0       ; R10 := U0
167 [-]: SELF      R10 R10 K3   ; R11 := R10; R10 := R10[0x86d3529e]
168 [-]: CALL      R10 2 2      ; R10 := R10(R11)
169 [-]: SELF      R10 R10 K40  ; R11 := R10; R10 := R10[0x21dfc654]
170 [-]: GETUPVAL  R12 U7       ; R12 := U7
171 [-]: CALL      R10 3 1      ; R10(R11,R12)
172 [-]: GETGLOBAL R10 K4       ; R10 := 0x7b998233
173 [-]: GETUPVAL  R11 U5       ; R11 := U5
174 [-]: CALL      R10 2 2      ; R10 := R10(R11)
175 [-]: TEST      R10 0        ; if not R10 then PC := 232
176 [-]: JMP       232          ; PC := 232
177 [-]: GETGLOBAL R10 K4       ; R10 := 0x7b998233
178 [-]: GETGLOBAL R11 K41      ; R11 := 0x0c845ec6
179 [-]: CALL      R10 2 2      ; R10 := R10(R11)
180 [-]: TEST      R10 1        ; if R10 then PC := 232
181 [-]: JMP       232          ; PC := 232
182 [-]: GETGLOBAL R10 K4       ; R10 := 0x7b998233
183 [-]: GETGLOBAL R11 K42      ; R11 := 0x56683b9e
184 [-]: CALL      R10 2 2      ; R10 := R10(R11)
185 [-]: TEST      R10 1        ; if R10 then PC := 232
186 [-]: JMP       232          ; PC := 232
187 [-]: GETGLOBAL R10 K0       ; R10 := 0x3d106989
188 [-]: LOADK     R11 K43      ; R11 := "CREATING MARKER"
189 [-]: CALL      R10 2 1      ; R10(R11)
190 [-]: GETUPVAL  R10 U2       ; R10 := U2
191 [-]: GETUPVAL  R11 U0       ; R11 := U0
192 [-]: SELF      R11 R11 K2   ; R12 := R11; R11 := R11[0xd1586535]
193 [-]: CALL      R11 2 2      ; R11 := R11(R12)
194 [-]: SUB       R10 R10 R11  ; R10 := R10 - R11
195 [-]: DIV       R10 R10 K44  ; R10 := R10 / 2.000000
196 [-]: GETUPVAL  R11 U0       ; R11 := U0
197 [-]: SELF      R11 R11 K2   ; R12 := R11; R11 := R11[0xd1586535]
198 [-]: CALL      R11 2 2      ; R11 := R11(R12)
199 [-]: ADD       R10 R10 R11  ; R10 := R10 + R11
200 [-]: GETUPVAL  R11 U10      ; R11 := U10
201 [-]: GETTABLE  R11 R11 K45  ; R82 := R11[0xcdcbd25d]
202 [-]: GETGLOBAL R12 K41      ; R12 := 0x0c845ec6
203 [-]: MOVE      R13 R10      ; R13 := R10
204 [-]: SELF      R14 R0 K20   ; R15 := R0; R14 := R0[0xf6cf204f]
205 [-]: CALL      R14 2 2      ; R14 := R14(R15)
206 [-]: GETUPVAL  R15 U6       ; R15 := U6
207 [-]: CALL      R11 5 2      ; R11 := R11(R12,R13,R14,R15)
208 [-]: SETUPVAL  R11 U5       ; U82 := 
209 [-]: GETUPVAL  R11 U0       ; R11 := U0
210 [-]: SELF      R11 R11 K46  ; R12 := R11; R11 := R11[0x1f420a3a]
211 [-]: GETUPVAL  R13 U2       ; R13 := U2
212 [-]: CALL      R11 3 2      ; R11 := R11(R12,R13)
213 [-]: GETUPVAL  R12 U5       ; R12 := U5
214 [-]: SELF      R12 R12 K47  ; R13 := R12; R12 := R12[0x5004be24]
215 [-]: MOVE      R14 R11      ; R14 := R11
216 [-]: CALL      R12 3 1      ; R12(R13,R14)
217 [-]: GETUPVAL  R12 U5       ; R12 := U5
218 [-]: SELF      R12 R12 K48  ; R13 := R12; R12 := R12[0x53bc0559]
219 [-]: GETGLOBAL R14 K49      ; R14 := 0xb7cbd06b
220 [-]: LOADK     R15 5000     ; R15 := 5000.000000
221 [-]: LOADK     R16 5000     ; R16 := 5000.000000
222 [-]: CALL      R14 3 0      ; R14,... := R14(R15,R16)
223 [-]: CALL      R12 0 1      ; R12(R13,...)
224 [-]: GETUPVAL  R12 U5       ; R12 := U5
225 [-]: SELF      R12 R12 K50  ; R13 := R12; R12 := R12[0x3273ba96]
226 [-]: GETUPVAL  R14 U6       ; R14 := U6
227 [-]: CALL      R12 3 1      ; R12(R13,R14)
228 [-]: JMP       232          ; PC := 232
229 [-]: GETGLOBAL R12 K0       ; R12 := 0x3d106989
230 [-]: LOADK     R13 K51      ; R13 := "AnimalTrail: Failed - zero length path"
231 [-]: CALL      R12 2 1      ; R12(R13)
232 [-]: SELF      R12 R0 K16   ; R13 := R0; R12 := R0[0xfe9dc265]
233 [-]: LOADK     R14 2        ; R14 := 2.000000
234 [-]: CALL      R12 3 1      ; R12(R13,R14)
235 [-]: GETUPVAL  R12 U11      ; R12 := U11
236 [-]: CALL      R12 1 2      ; R12 := R12()
237 [-]: TEST      R12 1        ; if R12 then PC := 252
238 [-]: JMP       252          ; PC := 252
239 [-]: GETGLOBAL R12 K52      ; R12 := 0x67652851
240 [-]: CALL      R12 1 2      ; R12 := R12()
241 [-]: GETUPVAL  R13 U12      ; R13 := U12
242 [-]: MOVE      R14 R12      ; R14 := R12
243 [-]: CALL      R13 2 1      ; R13(R14)
244 [-]: GETGLOBAL R13 K53      ; R13 := 0xcbd666e1
245 [-]: MOVE      R14 R12      ; R14 := R12
246 [-]: CALL      R13 2 1      ; R13(R14)
247 [-]: JMP       235          ; PC := 235
248 [-]: JMP       252          ; PC := 252
249 [-]: GETGLOBAL R13 K0       ; R13 := 0x3d106989
250 [-]: LOADK     R14 K54      ; R14 := "AnimalTrail: Failed to find path for new chaining encounter"
251 [-]: CALL      R13 2 1      ; R13(R14)
252 [-]: GETGLOBAL R13 K0       ; R13 := 0x3d106989
253 [-]: LOADK     R14 K55      ; R14 := "AnimalTrail: Shutting down"
254 [-]: CALL      R13 2 1      ; R13(R14)
255 [-]: GETUPVAL  R13 U13      ; R13 := U13
256 [-]: CALL      R13 1 1      ; R13()
257 [-]: RETURN    R0 1         ; return 


