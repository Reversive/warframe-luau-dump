; Name:            
; Defined at line: 1
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       1
; Max Stack Size:  1

       1 [-]: DUPCLOSURE R0 K0; 
       2 [-]: SETGLOBAL R0 K1; "createEntity" = var0
       3 [-]: DUPCLOSURE R0 K2; 
       4 [-]: SETGLOBAL R0 K3; "createMultipleEntities" = var0
       5 [-]: DUPCLOSURE R0 K4; 
       6 [-]: SETGLOBAL R0 K5; "getRandomWeightedEntity" = var0
       7 [-]: DUPCLOSURE R0 K6; 
       8 [-]: SETGLOBAL R0 K7; "createRandomlySelectedWeightedEntity" = var0
       9 [-]: RETURN R0 0  ; 


; Name:            
; Defined at line: 16
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  7

       0 [-]: GETIMPORT R0 1; var0 = 0xBD0E9845
       1 [-]: NAMECALL R0 R0 K2; var1 = var0; var0 = var0[0xD1586535]
       2 [-]: CALL R0 2 2  ; var0 = var0(var1)
       3 [-]: GETIMPORT R1 1; var1 = 0xBD0E9845
       4 [-]: NAMECALL R1 R1 K3; var2 = var1; var1 = var1[0xCB3851B8]
       5 [-]: CALL R1 2 2  ; var1 = var1(var2)
       6 [-]: GETIMPORT R3 1; var3 = 0xBD0E9845
       7 [-]: FASTCALL1 64 R3 L0; 
       8 [-]: GETIMPORT R2 5; var2 = 0x7B998233
       9 [-]: CALL R2 2 2  ; var2 = var2(var3)
L 0:  10 [-]: JUMPIF R2 L2 ; goto L2 if var2
      11 [-]: GETIMPORT R3 7; var3 = 0xC411A0C0
      12 [-]: FASTCALL1 64 R3 L1; 
      13 [-]: GETIMPORT R2 5; var2 = 0x7B998233
      14 [-]: CALL R2 2 2  ; var2 = var2(var3)
L 1:  15 [-]: JUMPIF R2 L2 ; goto L2 if var2
      16 [-]: GETIMPORT R2 9; var2 = 0xCBD666E1
      17 [-]: GETIMPORT R3 11; var3 = 0x99547DBB
      18 [-]: CALL R2 2 1  ; var2(var3)
      19 [-]: GETIMPORT R2 13; var2 = 0x89326C93
      20 [-]: GETIMPORT R4 7; var4 = 0xC411A0C0
      21 [-]: MOVE R5 R0   ; var5 = var0
      22 [-]: MOVE R6 R1   ; var6 = var1
      23 [-]: NAMECALL R2 R2 K14; var3 = var2; var2 = var2[0x05909209]
      24 [-]: CALL R2 5 1  ; var2(var3, var4, var5, var6)
L 2:  25 [-]: RETURN R0 0  ; 


; Name:            
; Defined at line: 26
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  7

       0 [-]: GETIMPORT R0 1; var0 = 0xBD0E9845
       1 [-]: NAMECALL R0 R0 K2; var1 = var0; var0 = var0[0xD1586535]
       2 [-]: CALL R0 2 2  ; var0 = var0(var1)
       3 [-]: GETIMPORT R1 1; var1 = 0xBD0E9845
       4 [-]: NAMECALL R1 R1 K3; var2 = var1; var1 = var1[0xCB3851B8]
       5 [-]: CALL R1 2 2  ; var1 = var1(var2)
       6 [-]: GETIMPORT R3 1; var3 = 0xBD0E9845
       7 [-]: FASTCALL1 64 R3 L0; 
       8 [-]: GETIMPORT R2 5; var2 = 0x7B998233
       9 [-]: CALL R2 2 2  ; var2 = var2(var3)
L 0:  10 [-]: JUMPIF R2 L5 ; goto L5 if var2
      11 [-]: GETIMPORT R3 7; var3 = 0xC411A0C0
      12 [-]: FASTCALL1 64 R3 L1; 
      13 [-]: GETIMPORT R2 5; var2 = 0x7B998233
      14 [-]: CALL R2 2 2  ; var2 = var2(var3)
L 1:  15 [-]: JUMPIF R2 L5 ; goto L5 if var2
      16 [-]: GETIMPORT R2 9; var2 = 0xCBD666E1
      17 [-]: GETIMPORT R3 11; var3 = 0x99547DBB
      18 [-]: CALL R2 2 1  ; var2(var3)
      19 [-]: GETIMPORT R2 13; var2 = 0x89326C93
      20 [-]: GETIMPORT R4 7; var4 = 0xC411A0C0
      21 [-]: MOVE R5 R0   ; var5 = var0
      22 [-]: MOVE R6 R1   ; var6 = var1
      23 [-]: NAMECALL R2 R2 K14; var3 = var2; var2 = var2[0x05909209]
      24 [-]: CALL R2 5 1  ; var2(var3, var4, var5, var6)
      25 [-]: GETIMPORT R2 9; var2 = 0xCBD666E1
      26 [-]: GETIMPORT R3 16; var3 = 0x4119CD27
      27 [-]: CALL R2 2 1  ; var2(var3)
      28 [-]: GETIMPORT R3 18; var3 = 0x9B82BA54
      29 [-]: FASTCALL1 64 R3 L2; 
      30 [-]: GETIMPORT R2 5; var2 = 0x7B998233
      31 [-]: CALL R2 2 2  ; var2 = var2(var3)
L 2:  32 [-]: JUMPIF R2 L3 ; goto L3 if var2
      33 [-]: GETIMPORT R2 13; var2 = 0x89326C93
      34 [-]: GETIMPORT R4 18; var4 = 0x9B82BA54
      35 [-]: MOVE R5 R0   ; var5 = var0
      36 [-]: MOVE R6 R1   ; var6 = var1
      37 [-]: NAMECALL R2 R2 K14; var3 = var2; var2 = var2[0x05909209]
      38 [-]: CALL R2 5 1  ; var2(var3, var4, var5, var6)
L 3:  39 [-]: GETIMPORT R2 9; var2 = 0xCBD666E1
      40 [-]: GETIMPORT R3 20; var3 = 0xA48A1CFA
      41 [-]: CALL R2 2 1  ; var2(var3)
      42 [-]: GETIMPORT R3 22; var3 = 0x0E1298F1
      43 [-]: FASTCALL1 64 R3 L4; 
      44 [-]: GETIMPORT R2 5; var2 = 0x7B998233
      45 [-]: CALL R2 2 2  ; var2 = var2(var3)
L 4:  46 [-]: JUMPIF R2 L5 ; goto L5 if var2
      47 [-]: GETIMPORT R2 13; var2 = 0x89326C93
      48 [-]: GETIMPORT R4 22; var4 = 0x0E1298F1
      49 [-]: MOVE R5 R0   ; var5 = var0
      50 [-]: MOVE R6 R1   ; var6 = var1
      51 [-]: NAMECALL R2 R2 K14; var3 = var2; var2 = var2[0x05909209]
      52 [-]: CALL R2 5 1  ; var2(var3, var4, var5, var6)
L 5:  53 [-]: RETURN R0 0  ; 


; Name:            
; Defined at line: 45
; #Upvalues:       0
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  10

       0 [-]: LOADN R2 0   ; var2 = 0
       1 [-]: LOADNIL R3   ; var3 = nil
       2 [-]: GETIMPORT R4 1; var4 = 0xC8802016
       3 [-]: MOVE R5 R1   ; var5 = var1
       4 [-]: CALL R4 2 4  ; var4, var5, var6 = var4(var5)
       5 [-]: FORGPREP_INEXT R4 L1; 
L 0:   6 [-]: ADD R2 R2 R8 ; var2 = var2 + var8
L 1:   7 [-]: FORGLOOP R4 L0 2 [inext]; 
       8 [-]: GETIMPORT R4 4; var4 = 0x5BCED4C4[0x3630E649]
       9 [-]: MOVE R5 R2   ; var5 = var2
      10 [-]: CALL R4 2 2  ; var4 = var4(var5)
      11 [-]: MOVE R3 R4   ; var3 = var4
      12 [-]: GETIMPORT R4 1; var4 = 0xC8802016
      13 [-]: MOVE R5 R1   ; var5 = var1
      14 [-]: CALL R4 2 4  ; var4, var5, var6 = var4(var5)
      15 [-]: FORGPREP_INEXT R4 L3; 
L 2:  16 [-]: SUB R3 R3 R8 ; var3 = var3 - var8
      17 [-]: LOADN R9 0   ; var9 = 0
      18 [-]: JUMPIFNOTLE R3 R9 L3; goto L3 if var3 > var117442845
      19 [-]: GETTABLE R9 R0 R7; var9 = var0[var7]
      20 [-]: RETURN R9 1  ; 
L 3:  21 [-]: FORGLOOP R4 L2 2 [inext]; 
      22 [-]: RETURN R0 0  ; 


; Name:            
; Defined at line: 63
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  8

       0 [-]: GETIMPORT R0 1; var0 = 0xBD0E9845
       1 [-]: NAMECALL R0 R0 K2; var1 = var0; var0 = var0[0xD1586535]
       2 [-]: CALL R0 2 2  ; var0 = var0(var1)
       3 [-]: GETIMPORT R1 1; var1 = 0xBD0E9845
       4 [-]: NAMECALL R1 R1 K3; var2 = var1; var1 = var1[0xCB3851B8]
       5 [-]: CALL R1 2 2  ; var1 = var1(var2)
       6 [-]: GETGLOBAL R2 K4; var2 = "getRandomWeightedEntity"
       7 [-]: GETIMPORT R3 6; var3 = 0x5BC0C693
       8 [-]: GETIMPORT R4 8; var4 = 0x3651C8A6
       9 [-]: CALL R2 3 2  ; var2 = var2(var3, var4)
      10 [-]: GETIMPORT R4 1; var4 = 0xBD0E9845
      11 [-]: FASTCALL1 64 R4 L0; 
      12 [-]: GETIMPORT R3 10; var3 = 0x7B998233
      13 [-]: CALL R3 2 2  ; var3 = var3(var4)
L 0:  14 [-]: JUMPIF R3 L2 ; goto L2 if var3
      15 [-]: FASTCALL1 64 R2 L1; 
      16 [-]: MOVE R4 R2   ; var4 = var2
      17 [-]: GETIMPORT R3 10; var3 = 0x7B998233
      18 [-]: CALL R3 2 2  ; var3 = var3(var4)
L 1:  19 [-]: JUMPIF R3 L2 ; goto L2 if var3
      20 [-]: GETIMPORT R3 12; var3 = 0xCBD666E1
      21 [-]: GETIMPORT R4 14; var4 = 0x99547DBB
      22 [-]: CALL R3 2 1  ; var3(var4)
      23 [-]: GETIMPORT R3 16; var3 = 0x89326C93
      24 [-]: MOVE R5 R2   ; var5 = var2
      25 [-]: MOVE R6 R0   ; var6 = var0
      26 [-]: MOVE R7 R1   ; var7 = var1
      27 [-]: NAMECALL R3 R3 K17; var4 = var3; var3 = var3[0x05909209]
      28 [-]: CALL R3 5 1  ; var3(var4, var5, var6, var7)
L 2:  29 [-]: RETURN R0 0  ; 



