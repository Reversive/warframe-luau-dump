; This file has been disassembled using luadec 2.0.2 by sztupy (modified by Reversive)

; Name:            
; Defined at line: 0
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       2
; Max Stack Size:  20

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x2d0fad09
  2 [-]: LOADK     R1 K1        ; R1 := "Lotus.Scripts.Libs.AbilitiesLib"
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K0        ; R1 := 0x2d0fad09
  5 [-]: LOADK     R2 K2        ; R2 := "Lotus.Powersuits.PowersuitAbilities.PVPLib"
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: GETGLOBAL R2 K3        ; R2 := 0x0469f296
  8 [-]: LOADK     R3 K4        ; R3 := "EnergyVampireII"
  9 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 10 [-]: CONST     R3 0         ; R3 := 0.250000
 11 [-]: CONST     R4 100       ; R4 := 100.000000
 12 [-]: CONST     R5 5         ; R5 := 5.000000
 13 [-]: CONST     R6 15        ; R6 := 15.000000
 14 [-]: CONST     R7 10        ; R7 := 10.000000
 15 [-]: CONST     R8 75        ; R8 := 75.000000
 16 [-]: CLOSURE   R9 0         ; R9 := closure(Function #1)
 17 [-]: MOVE      R0 R1        ; R0 := R1
 18 [-]: MOVE      R0 R4        ; R0 := R4
 19 [-]: MOVE      R0 R5        ; R0 := R5
 20 [-]: MOVE      R0 R6        ; R0 := R6
 21 [-]: MOVE      R0 R7        ; R0 := R7
 22 [-]: CLOSURE   R10 1        ; R10 := closure(Function #2)
 23 [-]: MOVE      R0 R4        ; R0 := R4
 24 [-]: MOVE      R0 R5        ; R0 := R5
 25 [-]: MOVE      R0 R6        ; R0 := R6
 26 [-]: MOVE      R0 R7        ; R0 := R7
 27 [-]: MOVE      R0 R3        ; R0 := R3
 28 [-]: CLOSURE   R11 2        ; R11 := closure(Function #3)
 29 [-]: MOVE      R0 R8        ; R0 := R8
 30 [-]: CLOSURE   R12 3        ; R12 := closure(Function #4)
 31 [-]: MOVE      R0 R11       ; R0 := R11
 32 [-]: MOVE      R0 R8        ; R0 := R8
 33 [-]: CLOSURE   R13 4        ; R13 := closure(Function #5)
 34 [-]: MOVE      R0 R9        ; R0 := R9
 35 [-]: MOVE      R0 R4        ; R0 := R4
 36 [-]: MOVE      R0 R5        ; R0 := R5
 37 [-]: MOVE      R0 R6        ; R0 := R6
 38 [-]: MOVE      R0 R7        ; R0 := R7
 39 [-]: MOVE      R0 R3        ; R0 := R3
 40 [-]: MOVE      R0 R10       ; R0 := R10
 41 [-]: MOVE      R0 R12       ; R0 := R12
 42 [-]: SETGLOBAL R13 K5       ; GetAbilityUpgradeLevelInfo := R13
 43 [-]: CLOSURE   R13 5        ; R13 := closure(Function #6)
 44 [-]: MOVE      R0 R11       ; R0 := R11
 45 [-]: MOVE      R0 R8        ; R0 := R8
 46 [-]: SETGLOBAL R13 K6       ; GetAugmentDescriptionInfo := R13
 47 [-]: CLOSURE   R13 6        ; R13 := closure(Function #7)
 48 [-]: MOVE      R0 R2        ; R0 := R2
 49 [-]: CLOSURE   R14 7        ; R14 := closure(Function #8)
 50 [-]: MOVE      R0 R2        ; R0 := R2
 51 [-]: CLOSURE   R15 8        ; R15 := closure(Function #9)
 52 [-]: MOVE      R0 R1        ; R0 := R1
 53 [-]: MOVE      R0 R4        ; R0 := R4
 54 [-]: CLOSURE   R16 9        ; R16 := closure(Function #10)
 55 [-]: MOVE      R0 R9        ; R0 := R9
 56 [-]: MOVE      R0 R4        ; R0 := R4
 57 [-]: MOVE      R0 R10       ; R0 := R10
 58 [-]: MOVE      R0 R1        ; R0 := R1
 59 [-]: MOVE      R0 R15       ; R0 := R15
 60 [-]: CLOSURE   R17 10       ; R17 := closure(Function #11)
 61 [-]: MOVE      R0 R16       ; R0 := R16
 62 [-]: MOVE      R0 R1        ; R0 := R1
 63 [-]: SETGLOBAL R17 K7       ; EvaluateAbility := R17
 64 [-]: CLOSURE   R17 11       ; R17 := closure(Function #12)
 65 [-]: MOVE      R0 R16       ; R0 := R16
 66 [-]: SETGLOBAL R17 K8       ; NpcEvaluateAbility := R17
 67 [-]: CLOSURE   R17 12       ; R17 := closure(Function #13)
 68 [-]: MOVE      R0 R1        ; R0 := R1
 69 [-]: SETGLOBAL R17 K9       ; InitializeAbility := R17
 70 [-]: CLOSURE   R17 13       ; R17 := closure(Function #14)
 71 [-]: MOVE      R0 R13       ; R0 := R13
 72 [-]: MOVE      R0 R5        ; R0 := R5
 73 [-]: MOVE      R0 R3        ; R0 := R3
 74 [-]: MOVE      R0 R11       ; R0 := R11
 75 [-]: MOVE      R0 R8        ; R0 := R8
 76 [-]: MOVE      R0 R0        ; R0 := R0
 77 [-]: MOVE      R0 R6        ; R0 := R6
 78 [-]: MOVE      R0 R7        ; R0 := R7
 79 [-]: CLOSURE   R18 14       ; R18 := closure(Function #15)
 80 [-]: MOVE      R0 R9        ; R0 := R9
 81 [-]: MOVE      R0 R4        ; R0 := R4
 82 [-]: MOVE      R0 R5        ; R0 := R5
 83 [-]: MOVE      R0 R6        ; R0 := R6
 84 [-]: MOVE      R0 R7        ; R0 := R7
 85 [-]: MOVE      R0 R3        ; R0 := R3
 86 [-]: MOVE      R0 R10       ; R0 := R10
 87 [-]: MOVE      R0 R0        ; R0 := R0
 88 [-]: MOVE      R0 R17       ; R0 := R17
 89 [-]: SETGLOBAL R18 K10      ; ActivateAbility := R18
 90 [-]: CLOSURE   R18 15       ; R18 := closure(Function #16)
 91 [-]: MOVE      R0 R14       ; R0 := R14
 92 [-]: CLOSURE   R19 16       ; R19 := closure(Function #17)
 93 [-]: MOVE      R0 R18       ; R0 := R18
 94 [-]: SETGLOBAL R19 K11      ; DeactivateAbility := R19
 95 [-]: CLOSURE   R19 17       ; R19 := closure(Function #18)
 96 [-]: MOVE      R0 R9        ; R0 := R9
 97 [-]: MOVE      R0 R4        ; R0 := R4
 98 [-]: MOVE      R0 R5        ; R0 := R5
 99 [-]: MOVE      R0 R6        ; R0 := R6
100 [-]: MOVE      R0 R10       ; R0 := R10
101 [-]: SETGLOBAL R19 K12      ; CrewShipInfo := R19
102 [-]: CLOSURE   R19 18       ; R19 := closure(Function #19)
103 [-]: MOVE      R0 R15       ; R0 := R15
104 [-]: SETGLOBAL R19 K13      ; CrewShipEval := R19
105 [-]: CLOSURE   R19 19       ; R19 := closure(Function #20)
106 [-]: MOVE      R0 R0        ; R0 := R0
107 [-]: MOVE      R0 R9        ; R0 := R9
108 [-]: MOVE      R0 R4        ; R0 := R4
109 [-]: MOVE      R0 R5        ; R0 := R5
110 [-]: MOVE      R0 R6        ; R0 := R6
111 [-]: MOVE      R0 R7        ; R0 := R7
112 [-]: MOVE      R0 R3        ; R0 := R3
113 [-]: MOVE      R0 R10       ; R0 := R10
114 [-]: MOVE      R0 R17       ; R0 := R17
115 [-]: MOVE      R0 R18       ; R0 := R18
116 [-]: SETGLOBAL R19 K14      ; CrewShipActivate := R19
117 [-]: RETURN    R0 1         ; return 


; Function #1:
;
; Name:            
; Defined at line: 27
; #Upvalues:       5
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETTABLE  R1 R1 K0     ; R1 := R1[0x32316a21]
  3 [-]: CALL      R1 1 2       ; R1 := R1()
  4 [-]: TEST      R1 1         ; if R1 then PC := 48
  5 [-]: JMP       48           ; PC := 48
  6 [-]: EQ        0 R0 K1      ; if R0 ~= 1.000000 then PC := 17
  7 [-]: JMP       17           ; PC := 17
  8 [-]: CONST     R1 100       ; R1 := 100.000000
  9 [-]: SETUPVAL  R1 U1        ; U82 := R1
 10 [-]: CONST     R1 5         ; R1 := 5.000000
 11 [-]: SETUPVAL  R1 U2        ; U82 := R2
 12 [-]: CONST     R1 15        ; R1 := 15.000000
 13 [-]: SETUPVAL  R1 U3        ; U82 := R3
 14 [-]: CONST     R1 10        ; R1 := 10.000000
 15 [-]: SETUPVAL  R1 U4        ; U82 := R4
 16 [-]: JMP       89           ; PC := 89
 17 [-]: EQ        0 R0 K2      ; if R0 ~= 2.000000 then PC := 28
 18 [-]: JMP       28           ; PC := 28
 19 [-]: CONST     R1 100       ; R1 := 100.000000
 20 [-]: SETUPVAL  R1 U1        ; U82 := R1
 21 [-]: CONST     R1 6         ; R1 := 6.000000
 22 [-]: SETUPVAL  R1 U2        ; U82 := R2
 23 [-]: CONST     R1 17        ; R1 := 17.000000
 24 [-]: SETUPVAL  R1 U3        ; U82 := R3
 25 [-]: CONST     R1 15        ; R1 := 15.000000
 26 [-]: SETUPVAL  R1 U4        ; U82 := R4
 27 [-]: JMP       89           ; PC := 89
 28 [-]: EQ        0 R0 K3      ; if R0 ~= 3.000000 then PC := 39
 29 [-]: JMP       39           ; PC := 39
 30 [-]: CONST     R1 100       ; R1 := 100.000000
 31 [-]: SETUPVAL  R1 U1        ; U82 := R1
 32 [-]: CONST     R1 7         ; R1 := 7.000000
 33 [-]: SETUPVAL  R1 U2        ; U82 := R2
 34 [-]: CONST     R1 20        ; R1 := 20.000000
 35 [-]: SETUPVAL  R1 U3        ; U82 := R3
 36 [-]: CONST     R1 20        ; R1 := 20.000000
 37 [-]: SETUPVAL  R1 U4        ; U82 := R4
 38 [-]: JMP       89           ; PC := 89
 39 [-]: CONST     R1 100       ; R1 := 100.000000
 40 [-]: SETUPVAL  R1 U1        ; U82 := R1
 41 [-]: CONST     R1 9         ; R1 := 9.000000
 42 [-]: SETUPVAL  R1 U2        ; U82 := R2
 43 [-]: CONST     R1 25        ; R1 := 25.000000
 44 [-]: SETUPVAL  R1 U3        ; U82 := R3
 45 [-]: CONST     R1 25        ; R1 := 25.000000
 46 [-]: SETUPVAL  R1 U4        ; U82 := R4
 47 [-]: JMP       89           ; PC := 89
 48 [-]: EQ        0 R0 K1      ; if R0 ~= 1.000000 then PC := 59
 49 [-]: JMP       59           ; PC := 59
 50 [-]: CONST     R1 13        ; R1 := 13.000000
 51 [-]: SETUPVAL  R1 U1        ; U82 := R1
 52 [-]: CONST     R1 4         ; R1 := 4.000000
 53 [-]: SETUPVAL  R1 U2        ; U82 := R2
 54 [-]: CONST     R1 13        ; R1 := 13.000000
 55 [-]: SETUPVAL  R1 U3        ; U82 := R3
 56 [-]: CONST     R1 7         ; R1 := 7.000000
 57 [-]: SETUPVAL  R1 U4        ; U82 := R4
 58 [-]: JMP       89           ; PC := 89
 59 [-]: EQ        0 R0 K2      ; if R0 ~= 2.000000 then PC := 70
 60 [-]: JMP       70           ; PC := 70
 61 [-]: CONST     R1 14        ; R1 := 14.000000
 62 [-]: SETUPVAL  R1 U1        ; U82 := R1
 63 [-]: CONST     R1 6         ; R1 := 6.000000
 64 [-]: SETUPVAL  R1 U2        ; U82 := R2
 65 [-]: CONST     R1 14        ; R1 := 14.000000
 66 [-]: SETUPVAL  R1 U3        ; U82 := R3
 67 [-]: CONST     R1 8         ; R1 := 8.000000
 68 [-]: SETUPVAL  R1 U4        ; U82 := R4
 69 [-]: JMP       89           ; PC := 89
 70 [-]: EQ        0 R0 K3      ; if R0 ~= 3.000000 then PC := 81
 71 [-]: JMP       81           ; PC := 81
 72 [-]: CONST     R1 15        ; R1 := 15.000000
 73 [-]: SETUPVAL  R1 U1        ; U82 := R1
 74 [-]: CONST     R1 8         ; R1 := 8.000000
 75 [-]: SETUPVAL  R1 U2        ; U82 := R2
 76 [-]: CONST     R1 15        ; R1 := 15.000000
 77 [-]: SETUPVAL  R1 U3        ; U82 := R3
 78 [-]: CONST     R1 9         ; R1 := 9.000000
 79 [-]: SETUPVAL  R1 U4        ; U82 := R4
 80 [-]: JMP       89           ; PC := 89
 81 [-]: CONST     R1 16        ; R1 := 16.000000
 82 [-]: SETUPVAL  R1 U1        ; U82 := R1
 83 [-]: CONST     R1 10        ; R1 := 10.000000
 84 [-]: SETUPVAL  R1 U2        ; U82 := R2
 85 [-]: CONST     R1 16        ; R1 := 16.000000
 86 [-]: SETUPVAL  R1 U3        ; U82 := R3
 87 [-]: CONST     R1 10        ; R1 := 10.000000
 88 [-]: SETUPVAL  R1 U4        ; U82 := R4
 89 [-]: RETURN    R0 1         ; return 


; Function #2:
;
; Name:            
; Defined at line: 83
; #Upvalues:       5
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  15

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETUPVAL  R2 U1        ; R2 := U1
  3 [-]: GETUPVAL  R3 U2        ; R3 := U2
  4 [-]: GETUPVAL  R4 U3        ; R4 := U3
  5 [-]: GETGLOBAL R5 K0        ; R5 := 0x34291f5c
  6 [-]: GETTABLE  R5 R5 K1     ; R5 := R5[0x7258f36f]
  7 [-]: GETUPVAL  R6 U4        ; R6 := U4
  8 [-]: CALL      R5 2 2       ; R5 := R5(R6)
  9 [-]: GETGLOBAL R6 K2        ; R6 := 0x7b998233
 10 [-]: MOVE      R7 R0        ; R7 := R0
 11 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 12 [-]: TEST      R6 1         ; if R6 then PC := 59
 13 [-]: JMP       59           ; PC := 59
 14 [-]: SELF      R6 R0 K3     ; R7 := R0; R6 := R0[0xde321e6f]
 15 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 16 [-]: SELF      R7 R6 K4     ; R8 := R6; R7 := R6[0xf7d48ee0]
 17 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 18 [-]: GETGLOBAL R8 K2        ; R8 := 0x7b998233
 19 [-]: MOVE      R9 R7        ; R9 := R7
 20 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 21 [-]: TEST      R8 1         ; if R8 then PC := 59
 22 [-]: JMP       59           ; PC := 59
 23 [-]: SELF      R8 R7 K5     ; R9 := R7; R8 := R7[0xcde10c4a]
 24 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 25 [-]: SELF      R9 R6 K6     ; R10 := R6; R9 := R6[0xe9f54086]
 26 [-]: GETUPVAL  R11 U0       ; R11 := U0
 27 [-]: CONST     R12 9        ; R12 := 9.000000
 28 [-]: MOVE      R13 R8       ; R13 := R8
 29 [-]: MOVE      R14 R7       ; R14 := R7
 30 [-]: CALL      R9 6 2       ; R9 := R9(R10,R11,R12,R13,R14)
 31 [-]: MOVE      R1 R9        ; R1 := R9
 32 [-]: SELF      R9 R6 K6     ; R10 := R6; R9 := R6[0xe9f54086]
 33 [-]: GETUPVAL  R11 U1       ; R11 := U1
 34 [-]: CONST     R12 3        ; R12 := 3.000000
 35 [-]: MOVE      R13 R8       ; R13 := R8
 36 [-]: MOVE      R14 R7       ; R14 := R7
 37 [-]: CALL      R9 6 2       ; R9 := R9(R10,R11,R12,R13,R14)
 38 [-]: MOVE      R2 R9        ; R2 := R9
 39 [-]: SELF      R9 R6 K6     ; R10 := R6; R9 := R6[0xe9f54086]
 40 [-]: GETUPVAL  R11 U2       ; R11 := U2
 41 [-]: CONST     R12 9        ; R12 := 9.000000
 42 [-]: MOVE      R13 R8       ; R13 := R8
 43 [-]: MOVE      R14 R7       ; R14 := R7
 44 [-]: CALL      R9 6 2       ; R9 := R9(R10,R11,R12,R13,R14)
 45 [-]: MOVE      R3 R9        ; R3 := R9
 46 [-]: SELF      R9 R6 K6     ; R10 := R6; R9 := R6[0xe9f54086]
 47 [-]: GETUPVAL  R11 U3       ; R11 := U3
 48 [-]: CONST     R12 10       ; R12 := 10.000000
 49 [-]: MOVE      R13 R8       ; R13 := R8
 50 [-]: MOVE      R14 R7       ; R14 := R7
 51 [-]: CALL      R9 6 2       ; R9 := R9(R10,R11,R12,R13,R14)
 52 [-]: MOVE      R4 R9        ; R4 := R9
 53 [-]: SELF      R9 R6 K8     ; R10 := R6; R9 := R6[0x54ba011d]
 54 [-]: MOVE      R11 R5       ; R11 := R5
 55 [-]: CONST     R12 10       ; R12 := 10.000000
 56 [-]: MOVE      R13 R8       ; R13 := R8
 57 [-]: MOVE      R14 R7       ; R14 := R7
 58 [-]: CALL      R9 6 1       ; R9(R10,R11,R12,R13,R14)
 59 [-]: MOVE      R9 R1        ; R9 := R1
 60 [-]: MOVE      R10 R2       ; R10 := R2
 61 [-]: MOVE      R11 R3       ; R11 := R3
 62 [-]: MOVE      R12 R4       ; R12 := R4
 63 [-]: MOVE      R13 R5       ; R13 := R5
 64 [-]: RETURN    R9 6         ; return R9,R10,R11,R12,R13
 65 [-]: RETURN    R0 1         ; return 


; Function #3:
;
; Name:            
; Defined at line: 109
; #Upvalues:       1
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  3

  1 [-]: EQ        0 R1 K1      ; if R1 ~= 1.000000 then PC := 20
  2 [-]: JMP       20           ; PC := 20
  3 [-]: EQ        0 R0 K1      ; if R0 ~= 1.000000 then PC := 8
  4 [-]: JMP       8            ; PC := 8
  5 [-]: LOADK     R2 K2        ; R2 := 0.850000
  6 [-]: SETUPVAL  R2 U0        ; U82 := R0
  7 [-]: JMP       20           ; PC := 20
  8 [-]: EQ        0 R0 K3      ; if R0 ~= 2.000000 then PC := 13
  9 [-]: JMP       13           ; PC := 13
 10 [-]: CONST     R2 1         ; R2 := 1.000000
 11 [-]: SETUPVAL  R2 U0        ; U82 := R0
 12 [-]: JMP       20           ; PC := 20
 13 [-]: EQ        0 R0 K4      ; if R0 ~= 3.000000 then PC := 18
 14 [-]: JMP       18           ; PC := 18
 15 [-]: LOADK     R2 K5        ; R2 := 1.200000
 16 [-]: SETUPVAL  R2 U0        ; U82 := R0
 17 [-]: JMP       20           ; PC := 20
 18 [-]: CONST     R2 1         ; R2 := 1.500000
 19 [-]: SETUPVAL  R2 U0        ; U82 := R0
 20 [-]: RETURN    R0 1         ; return 


; Function #4:
;
; Name:            
; Defined at line: 123
; #Upvalues:       2
; #Parameters:     3
; Is_vararg:       0
; Max Stack Size:  13

  1 [-]: GETGLOBAL R3 K0        ; R3 := 0x7b998233
  2 [-]: MOVE      R4 R1        ; R4 := R1
  3 [-]: CALL      R3 2 2       ; R3 := R3(R4)
  4 [-]: TEST      R3 0         ; if not R3 then PC := 7
  5 [-]: JMP       7            ; PC := 7
  6 [-]: RETURN    R0 1         ; return 
  7 [-]: SELF      R3 R1 K1     ; R4 := R1; R3 := R1[0xde321e6f]
  8 [-]: CALL      R3 2 2       ; R3 := R3(R4)
  9 [-]: SELF      R4 R3 K2     ; R5 := R3; R4 := R3[0xf7d48ee0]
 10 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 11 [-]: GETGLOBAL R5 K0        ; R5 := 0x7b998233
 12 [-]: MOVE      R6 R4        ; R6 := R4
 13 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 14 [-]: TEST      R5 0         ; if not R5 then PC := 17
 15 [-]: JMP       17           ; PC := 17
 16 [-]: RETURN    R0 1         ; return 
 17 [-]: SELF      R5 R4 K3     ; R6 := R4; R5 := R4[0xa2356091]
 18 [-]: MOVE      R7 R2        ; R7 := R2
 19 [-]: CALL      R5 3 2       ; R5 := R5(R6,R7)
 20 [-]: SELF      R6 R4 K4     ; R7 := R4; R6 := R4[0xd836367c]
 21 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 22 [-]: LE        0 R6 R5      ; if R6 > R5 then PC := 25
 23 [-]: JMP       25           ; PC := 25
 24 [-]: RETURN    R0 1         ; return 
 25 [-]: SELF      R6 R4 K5     ; R7 := R4; R6 := R4[0x5063edc3]
 26 [-]: MOVE      R8 R5        ; R8 := R5
 27 [-]: CALL      R6 3 2       ; R6 := R6(R7,R8)
 28 [-]: LE        0 R6 K6      ; if R6 > 0.000000 then PC := 31
 29 [-]: JMP       31           ; PC := 31
 30 [-]: RETURN    R0 1         ; return 
 31 [-]: SELF      R7 R4 K7     ; R8 := R4; R7 := R4[0x75ecaf0b]
 32 [-]: MOVE      R9 R5        ; R9 := R5
 33 [-]: CALL      R7 3 2       ; R7 := R7(R8,R9)
 34 [-]: GETUPVAL  R8 U0        ; R8 := U0
 35 [-]: MOVE      R9 R6        ; R9 := R6
 36 [-]: MOVE      R10 R7       ; R10 := R7
 37 [-]: CALL      R8 3 1       ; R8(R9,R10)
 38 [-]: EQ        0 R7 K9      ; if R7 ~= 1.000000 then PC := 60
 39 [-]: JMP       60           ; PC := 60
 40 [-]: GETGLOBAL R8 K10       ; R8 := 0x33bdd652
 41 [-]: GETTABLE  R8 R8 K11    ; R8 := R8[0x23d5322f]
 42 [-]: MOVE      R9 R0        ; R9 := R0
 43 [-]: NEWTABLE  R10 0 2      ; R10 := {}
 44 [-]: SETTABLE  R10 K12 K13  ; R10["Label"] := "/Lotus/Language/Suits/EnergyVampireAbilityAugment1Name"
 45 [-]: SETTABLE  R10 K14 K15  ; R10["Title"] := true
 46 [-]: CALL      R8 3 1       ; R8(R9,R10)
 47 [-]: GETGLOBAL R8 K10       ; R8 := 0x33bdd652
 48 [-]: GETTABLE  R8 R8 K11    ; R8 := R8[0x23d5322f]
 49 [-]: MOVE      R9 R0        ; R9 := R0
 50 [-]: NEWTABLE  R10 0 3      ; R10 := {}
 51 [-]: SETTABLE  R10 K12 K16  ; R10["Label"] := "/Lotus/Language/Game/ABILITY_ENERGY_CONVERSION_NO_UNIT"
 52 [-]: GETGLOBAL R11 K18      ; R11 := 0x5bced4c4
 53 [-]: GETTABLE  R11 R11 K19  ; R11 := R11[0x55f27c30]
 54 [-]: GETUPVAL  R12 U1       ; R12 := U1
 55 [-]: MUL       R12 R12 K20  ; R12 := R12 * 100.000000
 56 [-]: CALL      R11 2 2      ; R11 := R11(R12)
 57 [-]: SETTABLE  R10 K17 R11  ; R10["Value"] := R11
 58 [-]: SETTABLE  R10 K21 K22  ; R10["ValueUnit"] := "/Lotus/Language/Game/UNIT_PERCENT"
 59 [-]: CALL      R8 3 1       ; R8(R9,R10)
 60 [-]: RETURN    R0 1         ; return 


; Function #5:
;
; Name:            
; Defined at line: 153
; #Upvalues:       8
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  6

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: GETGLOBAL R1 K0        ; R1 := _T
  3 [-]: GETTABLE  R1 R1 K1     ; R1 := R1["AbilityLevelQueryParms"]
  4 [-]: GETTABLE  R1 R1 K2     ; R1 := R1["Level"]
  5 [-]: CALL      R0 2 1       ; R0(R1)
  6 [-]: GETGLOBAL R0 K0        ; R0 := _T
  7 [-]: GETTABLE  R0 R0 K1     ; R0 := R0["AbilityLevelQueryParms"]
  8 [-]: GETTABLE  R0 R0 K3     ; R0 := R0["Modded"]
  9 [-]: EQ        0 R0 K4      ; if R0 ~= true then PC := 25
 10 [-]: JMP       25           ; PC := 25
 11 [-]: GETUPVAL  R0 U6        ; R0 := U6
 12 [-]: GETGLOBAL R1 K0        ; R1 := _T
 13 [-]: GETTABLE  R1 R1 K1     ; R1 := R1["AbilityLevelQueryParms"]
 14 [-]: GETTABLE  R1 R1 K5     ; R1 := R1["Avatar"]
 15 [-]: CALL      R0 2 6       ; R0,R1,R2,R3,R4 := R0(R1)
 16 [-]: SETUPVAL  R4 U5        ; U82 := R5
 17 [-]: SETUPVAL  R3 U4        ; U82 := R4
 18 [-]: SETUPVAL  R2 U3        ; U82 := R3
 19 [-]: SETUPVAL  R1 U2        ; U82 := R2
 20 [-]: SETUPVAL  R0 U1        ; U82 := R1
 21 [-]: GETUPVAL  R0 U5        ; R0 := U5
 22 [-]: SELF      R0 R0 K6     ; R1 := R0; R0 := R0[0x838305de]
 23 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 24 [-]: SETUPVAL  R0 U5        ; U82 := R5
 25 [-]: NEWTABLE  R0 0 0       ; R0 := {}
 26 [-]: GETGLOBAL R1 K7        ; R1 := 0x33bdd652
 27 [-]: GETTABLE  R1 R1 K8     ; R1 := R1[0x23d5322f]
 28 [-]: MOVE      R2 R0        ; R2 := R0
 29 [-]: NEWTABLE  R3 0 3       ; R3 := {}
 30 [-]: SETTABLE  R3 K9 K10    ; R3["Label"] := "/Lotus/Language/Game/ENERGY_PER_PULSE"
 31 [-]: GETUPVAL  R4 U4        ; R4 := U4
 32 [-]: SETTABLE  R3 K11 R4    ; R3["Value"] := R4
 33 [-]: SETTABLE  R3 K12 K13   ; R3["ValueIcon"] := "<ENERGY>"
 34 [-]: CALL      R1 3 1       ; R1(R2,R3)
 35 [-]: GETGLOBAL R1 K7        ; R1 := 0x33bdd652
 36 [-]: GETTABLE  R1 R1 K8     ; R1 := R1[0x23d5322f]
 37 [-]: MOVE      R2 R0        ; R2 := R0
 38 [-]: NEWTABLE  R3 0 3       ; R3 := {}
 39 [-]: SETTABLE  R3 K9 K14    ; R3["Label"] := "/Lotus/Language/Labels/WEAPON_RANGE"
 40 [-]: GETUPVAL  R4 U1        ; R4 := U1
 41 [-]: SETTABLE  R3 K11 R4    ; R3["Value"] := R4
 42 [-]: SETTABLE  R3 K15 K16   ; R3["ValueUnit"] := "/Lotus/Language/Game/UNIT_METER"
 43 [-]: CALL      R1 3 1       ; R1(R2,R3)
 44 [-]: GETGLOBAL R1 K7        ; R1 := 0x33bdd652
 45 [-]: GETTABLE  R1 R1 K8     ; R1 := R1[0x23d5322f]
 46 [-]: MOVE      R2 R0        ; R2 := R0
 47 [-]: NEWTABLE  R3 0 3       ; R3 := {}
 48 [-]: SETTABLE  R3 K9 K17    ; R3["Label"] := "/Lotus/Language/Game/ABILITY_RADIUS"
 49 [-]: GETUPVAL  R4 U3        ; R4 := U3
 50 [-]: SETTABLE  R3 K11 R4    ; R3["Value"] := R4
 51 [-]: SETTABLE  R3 K15 K16   ; R3["ValueUnit"] := "/Lotus/Language/Game/UNIT_METER"
 52 [-]: CALL      R1 3 1       ; R1(R2,R3)
 53 [-]: GETGLOBAL R1 K7        ; R1 := 0x33bdd652
 54 [-]: GETTABLE  R1 R1 K8     ; R1 := R1[0x23d5322f]
 55 [-]: MOVE      R2 R0        ; R2 := R0
 56 [-]: NEWTABLE  R3 0 3       ; R3 := {}
 57 [-]: SETTABLE  R3 K9 K18    ; R3["Label"] := "/Lotus/Language/Menu/DURATION"
 58 [-]: GETUPVAL  R4 U2        ; R4 := U2
 59 [-]: SETTABLE  R3 K11 R4    ; R3["Value"] := R4
 60 [-]: SETTABLE  R3 K15 K19   ; R3["ValueUnit"] := "/Lotus/Language/Game/UNIT_SECOND"
 61 [-]: CALL      R1 3 1       ; R1(R2,R3)
 62 [-]: GETGLOBAL R1 K7        ; R1 := 0x33bdd652
 63 [-]: GETTABLE  R1 R1 K8     ; R1 := R1[0x23d5322f]
 64 [-]: MOVE      R2 R0        ; R2 := R0
 65 [-]: NEWTABLE  R3 0 4       ; R3 := {}
 66 [-]: SETTABLE  R3 K9 K20    ; R3["Label"] := "/Lotus/Language/Game/DAMAGE"
 67 [-]: GETGLOBAL R4 K21       ; R4 := 0x5bced4c4
 68 [-]: GETTABLE  R4 R4 K22    ; R4 := R4[0x55f27c30]
 69 [-]: GETUPVAL  R5 U5        ; R5 := U5
 70 [-]: MUL       R5 R5 K23    ; R5 := R5 * 100.000000
 71 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 72 [-]: SETTABLE  R3 K11 R4    ; R3["Value"] := R4
 73 [-]: SETTABLE  R3 K12 K24   ; R3["ValueIcon"] := "<DT_FINISHER>"
 74 [-]: SETTABLE  R3 K15 K25   ; R3["ValueUnit"] := "/Lotus/Language/Game/UNIT_PERCENT"
 75 [-]: CALL      R1 3 1       ; R1(R2,R3)
 76 [-]: GETUPVAL  R1 U7        ; R1 := U7
 77 [-]: MOVE      R2 R0        ; R2 := R0
 78 [-]: GETGLOBAL R3 K0        ; R3 := _T
 79 [-]: GETTABLE  R3 R3 K1     ; R3 := R3["AbilityLevelQueryParms"]
 80 [-]: GETTABLE  R3 R3 K5     ; R3 := R3["Avatar"]
 81 [-]: GETGLOBAL R4 K0        ; R4 := _T
 82 [-]: GETTABLE  R4 R4 K1     ; R4 := R4["AbilityLevelQueryParms"]
 83 [-]: GETTABLE  R4 R4 K26    ; R4 := R4["Ability"]
 84 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 85 [-]: GETGLOBAL R1 K0        ; R1 := _T
 86 [-]: GETTABLE  R1 R1 K1     ; R1 := R1["AbilityLevelQueryParms"]
 87 [-]: GETTABLE  R1 R1 K3     ; R1 := R1["Modded"]
 88 [-]: SETTABLE  R0 K3 R1     ; R0[0x5bced4c4] := R1
 89 [-]: GETGLOBAL R1 K0        ; R1 := _T
 90 [-]: SETTABLE  R1 K27 R0    ; R1["AbilityUpgradeLevelInfo"] := R0
 91 [-]: RETURN    R0 1         ; return 


; Function #6:
;
; Name:            
; Defined at line: 174
; #Upvalues:       2
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  6

  1 [-]: NEWTABLE  R2 0 0       ; R2 := {}
  2 [-]: GETUPVAL  R3 U0        ; R3 := U0
  3 [-]: MOVE      R4 R0        ; R4 := R0
  4 [-]: MOVE      R5 R1        ; R5 := R1
  5 [-]: CALL      R3 3 1       ; R3(R4,R5)
  6 [-]: EQ        0 R1 K1      ; if R1 ~= 1.000000 then PC := 16
  7 [-]: JMP       16           ; PC := 16
  8 [-]: NEWTABLE  R3 0 1       ; R3 := {}
  9 [-]: GETGLOBAL R4 K3        ; R4 := 0x5bced4c4
 10 [-]: GETTABLE  R4 R4 K4     ; R4 := R4[0x55f27c30]
 11 [-]: GETUPVAL  R5 U1        ; R5 := U1
 12 [-]: MUL       R5 R5 K5     ; R5 := R5 * 100.000000
 13 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 14 [-]: SETTABLE  R3 K2 R4     ; R3["SHIELD_PCT"] := R4
 15 [-]: MOVE      R2 R3        ; R2 := R3
 16 [-]: GETGLOBAL R3 K6        ; R3 := cjson
 17 [-]: GETTABLE  R3 R3 K7     ; R3 := R3[0xb139d7bc]
 18 [-]: MOVE      R4 R2        ; R4 := R2
 19 [-]: TAILCALL  R3 2 0       ; R3,... := R3(R4)
 20 [-]: RETURN    R3 0         ; return R3,...
 21 [-]: RETURN    R0 1         ; return 


; Function #7:
;
; Name:            
; Defined at line: 187
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  6

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
  2 [-]: MOVE      R2 R0        ; R2 := R0
  3 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  4 [-]: TEST      R1 1         ; if R1 then PC := 24
  5 [-]: JMP       24           ; PC := 24
  6 [-]: SELF      R1 R0 K1     ; R2 := R0; R1 := R0[0x1ac1655c]
  7 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  8 [-]: SELF      R2 R1 K2     ; R3 := R1; R2 := R1[0xaa0faa2c]
  9 [-]: CONST     R4 5         ; R4 := 5.000000
 10 [-]: GETUPVAL  R5 U0        ; R5 := U0
 11 [-]: CALL      R2 4 1       ; R2(R3,R4,R5)
 12 [-]: SELF      R2 R1 K2     ; R3 := R1; R2 := R1[0xaa0faa2c]
 13 [-]: CONST     R4 6         ; R4 := 6.000000
 14 [-]: GETUPVAL  R5 U0        ; R5 := U0
 15 [-]: CALL      R2 4 1       ; R2(R3,R4,R5)
 16 [-]: SELF      R2 R1 K2     ; R3 := R1; R2 := R1[0xaa0faa2c]
 17 [-]: CONST     R4 3         ; R4 := 3.000000
 18 [-]: GETUPVAL  R5 U0        ; R5 := U0
 19 [-]: CALL      R2 4 1       ; R2(R3,R4,R5)
 20 [-]: SELF      R2 R1 K2     ; R3 := R1; R2 := R1[0xaa0faa2c]
 21 [-]: CONST     R4 9         ; R4 := 9.000000
 22 [-]: GETUPVAL  R5 U0        ; R5 := U0
 23 [-]: CALL      R2 4 1       ; R2(R3,R4,R5)
 24 [-]: RETURN    R0 1         ; return 


; Function #8:
;
; Name:            
; Defined at line: 197
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  6

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
  2 [-]: MOVE      R2 R0        ; R2 := R0
  3 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  4 [-]: TEST      R1 1         ; if R1 then PC := 24
  5 [-]: JMP       24           ; PC := 24
  6 [-]: SELF      R1 R0 K1     ; R2 := R0; R1 := R0[0x1ac1655c]
  7 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  8 [-]: SELF      R2 R1 K2     ; R3 := R1; R2 := R1[0x0f68c2b7]
  9 [-]: CONST     R4 5         ; R4 := 5.000000
 10 [-]: GETUPVAL  R5 U0        ; R5 := U0
 11 [-]: CALL      R2 4 1       ; R2(R3,R4,R5)
 12 [-]: SELF      R2 R1 K2     ; R3 := R1; R2 := R1[0x0f68c2b7]
 13 [-]: CONST     R4 6         ; R4 := 6.000000
 14 [-]: GETUPVAL  R5 U0        ; R5 := U0
 15 [-]: CALL      R2 4 1       ; R2(R3,R4,R5)
 16 [-]: SELF      R2 R1 K2     ; R3 := R1; R2 := R1[0x0f68c2b7]
 17 [-]: CONST     R4 3         ; R4 := 3.000000
 18 [-]: GETUPVAL  R5 U0        ; R5 := U0
 19 [-]: CALL      R2 4 1       ; R2(R3,R4,R5)
 20 [-]: SELF      R2 R1 K2     ; R3 := R1; R2 := R1[0x0f68c2b7]
 21 [-]: CONST     R4 9         ; R4 := 9.000000
 22 [-]: GETUPVAL  R5 U0        ; R5 := U0
 23 [-]: CALL      R2 4 1       ; R2(R3,R4,R5)
 24 [-]: RETURN    R0 1         ; return 


; Function #9:
;
; Name:            
; Defined at line: 207
; #Upvalues:       2
; #Parameters:     3
; Is_vararg:       0
; Max Stack Size:  13

  1 [-]: CLOSURE   R3 0         ; R3 := closure(Function #9.1)
  2 [-]: GETUPVAL  R0 U0        ; R0 := U0
  3 [-]: MOVE      R0 R0        ; R0 := R0
  4 [-]: LOADNIL   R4 R4        ; R4 := nil
  5 [-]: GETGLOBAL R5 K0        ; R5 := 0xc8802016
  6 [-]: MOVE      R6 R1        ; R6 := R1
  7 [-]: CALL      R5 2 4       ; R5,R6,R7 := R5(R6)
  8 [-]: JMP       16           ; PC := 16
  9 [-]: MOVE      R10 R3       ; R10 := R3
 10 [-]: MOVE      R11 R9       ; R11 := R9
 11 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 12 [-]: TEST      R10 0        ; if not R10 then PC := 16
 13 [-]: JMP       16           ; PC := 16
 14 [-]: MOVE      R4 R9        ; R4 := R9
 15 [-]: JMP       18           ; PC := 18
 16 [-]: TFORLOOP  R5 2         ; R8,R9 :=  R5(R6,R7); if R8 ~= nil then begin PC = 9; R7 := R8 end
 17 [-]: JMP       9            ; PC := 9
 18 [-]: GETGLOBAL R10 K1       ; R10 := 0x7b998233
 19 [-]: MOVE      R11 R4       ; R11 := R4
 20 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 21 [-]: TEST      R10 0        ; if not R10 then PC := 29
 22 [-]: JMP       29           ; PC := 29
 23 [-]: LOADKB    R10 0 0      ; R10 := false
 24 [-]: GETGLOBAL R11 K2       ; R11 := 0x0469f296
 25 [-]: LOADK     R12 K3       ; R12 := "/Lotus/Language/Game/AbilityErrorInvalidTarget"
 26 [-]: CALL      R11 2 0      ; R11,... := R11(R12)
 27 [-]: RETURN    R10 0        ; return R10,...
 28 [-]: JMP       40           ; PC := 40
 29 [-]: SELF      R10 R4 K4    ; R11 := R4; R10 := R4[0x1f420a3a]
 30 [-]: MOVE      R12 R2       ; R12 := R2
 31 [-]: CALL      R10 3 2      ; R10 := R10(R11,R12)
 32 [-]: GETUPVAL  R11 U1       ; R11 := U1
 33 [-]: LT        0 R11 R10    ; if R11 >= R10 then PC := 40
 34 [-]: JMP       40           ; PC := 40
 35 [-]: LOADKB    R10 0 0      ; R10 := false
 36 [-]: GETGLOBAL R11 K2       ; R11 := 0x0469f296
 37 [-]: LOADK     R12 K5       ; R12 := "/Lotus/Language/Game/AbilityErrorOutOfRange"
 38 [-]: CALL      R11 2 0      ; R11,... := R11(R12)
 39 [-]: RETURN    R10 0        ; return R10,...
 40 [-]: LOADKB    R10 1 0      ; R10 := true
 41 [-]: MOVE      R11 R4       ; R11 := R4
 42 [-]: RETURN    R10 3        ; return R10,R11
 43 [-]: RETURN    R0 1         ; return 


; Function #9.1:
;
; Name:            
; Defined at line: 208
; #Upvalues:       2
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
  2 [-]: MOVE      R2 R0        ; R2 := R0
  3 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  4 [-]: TEST      R1 1         ; if R1 then PC := 20
  5 [-]: JMP       20           ; PC := 20
  6 [-]: SELF      R1 R0 K1     ; R2 := R0; R1 := R0[0xf2deaf69]
  7 [-]: GETGLOBAL R3 K2        ; R3 := gBaseAvatarType
  8 [-]: CALL      R1 3 2       ; R1 := R1(R2,R3)
  9 [-]: TEST      R1 0         ; if not R1 then PC := 20
 10 [-]: JMP       20           ; PC := 20
 11 [-]: SELF      R1 R0 K3     ; R2 := R0; R1 := R0[0x2047cfe7]
 12 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 13 [-]: TEST      R1 1         ; if R1 then PC := 20
 14 [-]: JMP       20           ; PC := 20
 15 [-]: SELF      R1 R0 K4     ; R2 := R0; R1 := R0[0xc4dff581]
 16 [-]: CONST     R3 0         ; R3 := 0.000000
 17 [-]: CALL      R1 3 2       ; R1 := R1(R2,R3)
 18 [-]: TEST      R1 0         ; if not R1 then PC := 22
 19 [-]: JMP       22           ; PC := 22
 20 [-]: LOADKB    R1 0 0       ; R1 := false
 21 [-]: RETURN    R1 2         ; return R1
 22 [-]: GETUPVAL  R1 U0        ; R1 := U0
 23 [-]: GETTABLE  R1 R1 K6     ; R1 := R1[0x32316a21]
 24 [-]: CALL      R1 1 2       ; R1 := R1()
 25 [-]: TEST      R1 0         ; if not R1 then PC := 36
 26 [-]: JMP       36           ; PC := 36
 27 [-]: GETUPVAL  R1 U0        ; R1 := U0
 28 [-]: GETTABLE  R1 R1 K7     ; R1 := R1[0xfabc505b]
 29 [-]: GETUPVAL  R2 U1        ; R2 := U1
 30 [-]: MOVE      R3 R0        ; R3 := R0
 31 [-]: CALL      R1 3 2       ; R1 := R1(R2,R3)
 32 [-]: TEST      R1 0         ; if not R1 then PC := 36
 33 [-]: JMP       36           ; PC := 36
 34 [-]: LOADKB    R1 1 0       ; R1 := true
 35 [-]: RETURN    R1 2         ; return R1
 36 [-]: GETUPVAL  R1 U1        ; R1 := U1
 37 [-]: SELF      R1 R1 K8     ; R2 := R1; R1 := R1[0x35844cf2]
 38 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 39 [-]: TEST      R1 1         ; if R1 then PC := 48
 40 [-]: JMP       48           ; PC := 48
 41 [-]: GETUPVAL  R1 U0        ; R1 := U0
 42 [-]: GETTABLE  R1 R1 K6     ; R1 := R1[0x32316a21]
 43 [-]: CALL      R1 1 2       ; R1 := R1()
 44 [-]: TEST      R1 1         ; if R1 then PC := 48
 45 [-]: JMP       48           ; PC := 48
 46 [-]: LOADKB    R1 1 0       ; R1 := true
 47 [-]: RETURN    R1 2         ; return R1
 48 [-]: SELF      R1 R0 K9     ; R2 := R0; R1 := R0[0xee0bc178]
 49 [-]: GETUPVAL  R3 U1        ; R3 := U1
 50 [-]: CALL      R1 3 2       ; R1 := R1(R2,R3)
 51 [-]: TEST      R1 0         ; if not R1 then PC := 55
 52 [-]: JMP       55           ; PC := 55
 53 [-]: LOADKB    R1 0 0       ; R1 := false
 54 [-]: RETURN    R1 2         ; return R1
 55 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 56 [-]: SELF      R2 R0 K10    ; R3 := R0; R2 := R0[0x5e651723]
 57 [-]: CALL      R2 2 0       ; R2,... := R2(R3)
 58 [-]: CALL      R1 0 2       ; R1 := R1(R2,...)
 59 [-]: TEST      R1 1         ; if R1 then PC := 63
 60 [-]: JMP       63           ; PC := 63
 61 [-]: LOADKB    R1 0 0       ; R1 := false
 62 [-]: RETURN    R1 2         ; return R1
 63 [-]: SELF      R1 R0 K11    ; R2 := R0; R1 := R0[0x1ac1655c]
 64 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 65 [-]: SELF      R2 R0 K12    ; R3 := R0; R2 := R0[0x0542d42b]
 66 [-]: GETGLOBAL R4 K13       ; R4 := 0x2bfeb5c7
 67 [-]: CALL      R2 3 2       ; R2 := R2(R3,R4)
 68 [-]: TEST      R2 1         ; if R2 then PC := 80
 69 [-]: JMP       80           ; PC := 80
 70 [-]: SELF      R2 R1 K14    ; R3 := R1; R2 := R1[0x2079c1dd]
 71 [-]: GETUPVAL  R4 U1        ; R4 := U1
 72 [-]: CALL      R2 3 2       ; R2 := R2(R3,R4)
 73 [-]: LT        1 R2 K15     ; if R2 < 0.000100 then PC := 80
 74 [-]: JMP       80           ; PC := 80
 75 [-]: SELF      R2 R1 K16    ; R3 := R1; R2 := R1[0x8109443c]
 76 [-]: CONST     R4 17        ; R4 := 17.000000
 77 [-]: CALL      R2 3 2       ; R2 := R2(R3,R4)
 78 [-]: TEST      R2 1         ; if R2 then PC := 82
 79 [-]: JMP       82           ; PC := 82
 80 [-]: LOADKB    R2 0 0       ; R2 := false
 81 [-]: RETURN    R2 2         ; return R2
 82 [-]: SELF      R2 R1 K18    ; R3 := R1; R2 := R1[0x68d1b91d]
 83 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 84 [-]: TEST      R2 0         ; if not R2 then PC := 92
 85 [-]: JMP       92           ; PC := 92
 86 [-]: SELF      R2 R1 K19    ; R3 := R1; R2 := R1[0xf92ccf3d]
 87 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 88 [-]: TEST      R2 1         ; if R2 then PC := 92
 89 [-]: JMP       92           ; PC := 92
 90 [-]: LOADKB    R2 0 0       ; R2 := false
 91 [-]: RETURN    R2 2         ; return R2
 92 [-]: LOADKB    R2 1 0       ; R2 := true
 93 [-]: RETURN    R2 2         ; return R2
 94 [-]: RETURN    R0 1         ; return 


; Function #10:
;
; Name:            
; Defined at line: 268
; #Upvalues:       5
; #Parameters:     3
; Is_vararg:       0
; Max Stack Size:  12

  1 [-]: GETUPVAL  R3 U0        ; R3 := U0
  2 [-]: MOVE      R4 R1        ; R4 := R1
  3 [-]: CALL      R3 2 1       ; R3(R4)
  4 [-]: GETUPVAL  R3 U2        ; R3 := U2
  5 [-]: MOVE      R4 R0        ; R4 := R0
  6 [-]: CALL      R3 2 2       ; R3 := R3(R4)
  7 [-]: SETUPVAL  R3 U1        ; U82 := R1
  8 [-]: LOADNIL   R3 R3        ; R3 := nil
  9 [-]: GETUPVAL  R4 U3        ; R4 := U3
 10 [-]: GETTABLE  R4 R4 K0     ; R4 := R4[0x32316a21]
 11 [-]: CALL      R4 1 2       ; R4 := R4()
 12 [-]: TEST      R4 0         ; if not R4 then PC := 28
 13 [-]: JMP       28           ; PC := 28
 14 [-]: SELF      R4 R0 K1     ; R5 := R0; R4 := R0[0x80846b00]
 15 [-]: CONST     R6 1         ; R6 := 1.000000
 16 [-]: GETUPVAL  R7 U1        ; R7 := U1
 17 [-]: GETUPVAL  R8 U3        ; R8 := U3
 18 [-]: GETTABLE  R8 R8 K2     ; R8 := R8[0xfbdcfe72]
 19 [-]: GETGLOBAL R9 K3        ; R9 := 0x19849b93
 20 [-]: MOVE      R10 R0       ; R10 := R0
 21 [-]: MOVE      R11 R2       ; R11 := R2
 22 [-]: CALL      R8 4 2       ; R8 := R8(R9,R10,R11)
 23 [-]: LOADKB    R9 0 0       ; R9 := false
 24 [-]: LOADKB    R10 1 0      ; R10 := true
 25 [-]: CALL      R4 7 2       ; R4 := R4(R5,R6,R7,R8,R9,R10)
 26 [-]: MOVE      R3 R4        ; R3 := R4
 27 [-]: JMP       36           ; PC := 36
 28 [-]: SELF      R4 R0 K1     ; R5 := R0; R4 := R0[0x80846b00]
 29 [-]: CONST     R6 1         ; R6 := 1.000000
 30 [-]: GETUPVAL  R7 U1        ; R7 := U1
 31 [-]: CONST     R8 1         ; R8 := 1.000000
 32 [-]: LOADKB    R9 0 0       ; R9 := false
 33 [-]: LOADKB    R10 1 0      ; R10 := true
 34 [-]: CALL      R4 7 2       ; R4 := R4(R5,R6,R7,R8,R9,R10)
 35 [-]: MOVE      R3 R4        ; R3 := R4
 36 [-]: GETGLOBAL R4 K4        ; R4 := 0x7b998233
 37 [-]: MOVE      R5 R3        ; R5 := R3
 38 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 39 [-]: TEST      R4 0         ; if not R4 then PC := 43
 40 [-]: JMP       43           ; PC := 43
 41 [-]: NEWTABLE  R4 0 0       ; R4 := {}
 42 [-]: MOVE      R3 R4        ; R3 := R4
 43 [-]: SELF      R4 R0 K5     ; R5 := R0; R4 := R0[0x35844cf2]
 44 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 45 [-]: TEST      R4 0         ; if not R4 then PC := 57
 46 [-]: JMP       57           ; PC := 57
 47 [-]: GETGLOBAL R4 K6        ; R4 := 0x33bdd652
 48 [-]: GETTABLE  R4 R4 K7     ; R4 := R4[0x23d5322f]
 49 [-]: MOVE      R5 R3        ; R5 := R3
 50 [-]: CONST     R6 1         ; R6 := 1.000000
 51 [-]: SELF      R7 R0 K8     ; R8 := R0; R7 := R0[0xde321e6f]
 52 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 53 [-]: SELF      R7 R7 K9     ; R8 := R7; R7 := R7[0x7c09e541]
 54 [-]: CALL      R7 2 0       ; R7,... := R7(R8)
 55 [-]: CALL      R4 0 1       ; R4(R5,...)
 56 [-]: JMP       78           ; PC := 78
 57 [-]: SELF      R4 R0 K10    ; R5 := R0; R4 := R0[0xfa9e477f]
 58 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 59 [-]: GETGLOBAL R5 K4        ; R5 := 0x7b998233
 60 [-]: MOVE      R6 R4        ; R6 := R4
 61 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 62 [-]: TEST      R5 1         ; if R5 then PC := 78
 63 [-]: JMP       78           ; PC := 78
 64 [-]: GETGLOBAL R5 K4        ; R5 := 0x7b998233
 65 [-]: SELF      R6 R4 K11    ; R7 := R4; R6 := R4[0xa39bb54b]
 66 [-]: CALL      R6 2 0       ; R6,... := R6(R7)
 67 [-]: CALL      R5 0 2       ; R5 := R5(R6,...)
 68 [-]: TEST      R5 1         ; if R5 then PC := 78
 69 [-]: JMP       78           ; PC := 78
 70 [-]: GETGLOBAL R5 K6        ; R5 := 0x33bdd652
 71 [-]: GETTABLE  R5 R5 K7     ; R5 := R5[0x23d5322f]
 72 [-]: MOVE      R6 R3        ; R6 := R3
 73 [-]: CONST     R7 1         ; R7 := 1.000000
 74 [-]: SELF      R8 R4 K11    ; R9 := R4; R8 := R4[0xa39bb54b]
 75 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 76 [-]: GETTABLE  R8 R8 K12    ; R8 := R8["avatar"]
 77 [-]: CALL      R5 4 1       ; R5(R6,R7,R8)
 78 [-]: GETUPVAL  R5 U4        ; R5 := U4
 79 [-]: MOVE      R6 R0        ; R6 := R0
 80 [-]: MOVE      R7 R3        ; R7 := R3
 81 [-]: SELF      R8 R0 K13    ; R9 := R0; R8 := R0[0xf6ebd926]
 82 [-]: CALL      R8 2 0       ; R8,... := R8(R9)
 83 [-]: CALL      R5 0 3       ; R5,R6 := R5(R6,...)
 84 [-]: TEST      R5 1         ; if R5 then PC := 89
 85 [-]: JMP       89           ; PC := 89
 86 [-]: LOADNIL   R7 R7        ; R7 := nil
 87 [-]: MOVE      R8 R6        ; R8 := R6
 88 [-]: RETURN    R7 3         ; return R7,R8
 89 [-]: RETURN    R6 2         ; return R6
 90 [-]: RETURN    R0 1         ; return 


; Function #11:
;
; Name:            
; Defined at line: 299
; #Upvalues:       2
; #Parameters:     3
; Is_vararg:       0
; Max Stack Size:  11

  1 [-]: GETUPVAL  R3 U0        ; R3 := U0
  2 [-]: MOVE      R4 R1        ; R4 := R1
  3 [-]: MOVE      R5 R2        ; R5 := R2
  4 [-]: MOVE      R6 R0        ; R6 := R0
  5 [-]: CALL      R3 4 3       ; R3,R4 := R3(R4,R5,R6)
  6 [-]: EQ        0 R3 K0      ; if R3 ~= nil then PC := 28
  7 [-]: JMP       28           ; PC := 28
  8 [-]: GETUPVAL  R5 U1        ; R5 := U1
  9 [-]: GETTABLE  R5 R5 K1     ; R5 := R5[0x32316a21]
 10 [-]: CALL      R5 1 2       ; R5 := R5()
 11 [-]: TEST      R5 0         ; if not R5 then PC := 16
 12 [-]: JMP       16           ; PC := 16
 13 [-]: LOADKB    R5 1 0       ; R5 := true
 14 [-]: RETURN    R5 2         ; return R5
 15 [-]: JMP       33           ; PC := 33
 16 [-]: SELF      R5 R1 K2     ; R6 := R1; R5 := R1[0x659d451f]
 17 [-]: GETGLOBAL R7 K3        ; R7 := 0xe144cbfd
 18 [-]: LOADKB    R8 0 0       ; R8 := false
 19 [-]: CONST     R9 0         ; R9 := 0.000000
 20 [-]: LOADKB    R10 0 0      ; R10 := false
 21 [-]: CALL      R5 6 1       ; R5(R6,R7,R8,R9,R10)
 22 [-]: SELF      R5 R1 K4     ; R6 := R1; R5 := R1[0xd7091d77]
 23 [-]: MOVE      R7 R4        ; R7 := R4
 24 [-]: CALL      R5 3 1       ; R5(R6,R7)
 25 [-]: LOADKB    R5 0 0       ; R5 := false
 26 [-]: RETURN    R5 2         ; return R5
 27 [-]: JMP       33           ; PC := 33
 28 [-]: SELF      R5 R0 K5     ; R6 := R0; R5 := R0[0x48d05257]
 29 [-]: MOVE      R7 R3        ; R7 := R3
 30 [-]: CALL      R5 3 1       ; R5(R6,R7)
 31 [-]: LOADKB    R5 1 0       ; R5 := true
 32 [-]: RETURN    R5 2         ; return R5
 33 [-]: RETURN    R0 1         ; return 


; Function #12:
;
; Name:            
; Defined at line: 317
; #Upvalues:       1
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  8

  1 [-]: CONST     R2 0         ; R2 := 0.000000
  2 [-]: GETUPVAL  R3 U0        ; R3 := U0
  3 [-]: MOVE      R4 R1        ; R4 := R1
  4 [-]: CONST     R5 1         ; R5 := 1.000000
  5 [-]: MOVE      R6 R0        ; R6 := R0
  6 [-]: CALL      R3 4 3       ; R3,R4 := R3(R4,R5,R6)
  7 [-]: EQ        1 R3 K0      ; if R3 == nil then PC := 13
  8 [-]: JMP       13           ; PC := 13
  9 [-]: SELF      R5 R0 K1     ; R6 := R0; R5 := R0[0x48d05257]
 10 [-]: MOVE      R7 R3        ; R7 := R3
 11 [-]: CALL      R5 3 1       ; R5(R6,R7)
 12 [-]: LOADK     R2 K2        ; R2 := 0.200000
 13 [-]: RETURN    R2 2         ; return R2
 14 [-]: RETURN    R0 1         ; return 


; Function #13:
;
; Name:            
; Defined at line: 330
; #Upvalues:       1
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  6

  1 [-]: GETUPVAL  R2 U0        ; R2 := U0
  2 [-]: GETTABLE  R2 R2 K0     ; R2 := R2[0xe4ae0e66]
  3 [-]: CALL      R2 1 2       ; R2 := R2()
  4 [-]: TEST      R2 0         ; if not R2 then PC := 13
  5 [-]: JMP       13           ; PC := 13
  6 [-]: GETGLOBAL R2 K1        ; R2 := 0x6687f6e0
  7 [-]: SELF      R2 R2 K2     ; R3 := R2; R2 := R2[0x3a147087]
  8 [-]: GETGLOBAL R4 K3        ; R4 := 0xbe190284
  9 [-]: SELF      R4 R4 K4     ; R5 := R4; R4 := R4[0xc911409e]
 10 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 11 [-]: ADD       R4 R4 K5     ; R4 := R4 + 100.000000
 12 [-]: CALL      R2 3 1       ; R2(R3,R4)
 13 [-]: RETURN    R0 1         ; return 


; Function #14:
;
; Name:            
; Defined at line: 336
; #Upvalues:       8
; #Parameters:     5
; Is_vararg:       0
; Max Stack Size:  54

  1 [-]: GETGLOBAL R5 K0        ; R5 := 0x7b998233
  2 [-]: MOVE      R6 R4        ; R6 := R4
  3 [-]: CALL      R5 2 2       ; R5 := R5(R6)
  4 [-]: TEST      R5 0         ; if not R5 then PC := 8
  5 [-]: JMP       8            ; PC := 8
  6 [-]: LOADKB    R5 1 0       ; R5 := true
  7 [-]: RETURN    R5 2         ; return R5
  8 [-]: GETUPVAL  R5 U0        ; R5 := U0
  9 [-]: MOVE      R6 R4        ; R6 := R4
 10 [-]: CALL      R5 2 1       ; R5(R6)
 11 [-]: SELF      R5 R4 K1     ; R6 := R4; R5 := R4[0x47901f07]
 12 [-]: SELF      R7 R2 K2     ; R8 := R2; R7 := R2[0xbc4ebb44]
 13 [-]: GETGLOBAL R9 K3        ; R9 := 0x0469f296
 14 [-]: LOADK     R10 K4       ; R10 := "EVAttach"
 15 [-]: CALL      R9 2 0       ; R9,... := R9(R10)
 16 [-]: CALL      R7 0 2       ; R7 := R7(R8,...)
 17 [-]: GETGLOBAL R8 K5        ; R8 := EMPTY_SYMBOL
 18 [-]: SELF      R9 R4 K6     ; R10 := R4; R9 := R4[0xd1586535]
 19 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 20 [-]: SELF      R10 R4 K7    ; R11 := R4; R10 := R4[0xcb3851b8]
 21 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 22 [-]: MOVE      R11 R0       ; R11 := R0
 23 [-]: CALL      R5 7 1       ; R5(R6,R7,R8,R9,R10,R11)
 24 [-]: SELF      R5 R0 K8     ; R6 := R0; R5 := R0[0x6df09e59]
 25 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 26 [-]: TEST      R5 0         ; if not R5 then PC := 41
 27 [-]: JMP       41           ; PC := 41
 28 [-]: SELF      R5 R4 K1     ; R6 := R4; R5 := R4[0x47901f07]
 29 [-]: SELF      R7 R2 K2     ; R8 := R2; R7 := R2[0xbc4ebb44]
 30 [-]: GETGLOBAL R9 K3        ; R9 := 0x0469f296
 31 [-]: LOADK     R10 K9       ; R10 := "primeAttachEffect"
 32 [-]: CALL      R9 2 0       ; R9,... := R9(R10)
 33 [-]: CALL      R7 0 2       ; R7 := R7(R8,...)
 34 [-]: GETGLOBAL R8 K3        ; R8 := 0x0469f296
 35 [-]: LOADK     R9 K10       ; R9 := "GAME_C1_HIP1"
 36 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 37 [-]: GETGLOBAL R9 K11       ; R9 := ZERO_VECTOR
 38 [-]: GETGLOBAL R10 K12      ; R10 := ZERO_ROTATION
 39 [-]: MOVE      R11 R0       ; R11 := R0
 40 [-]: CALL      R5 7 1       ; R5(R6,R7,R8,R9,R10,R11)
 41 [-]: SELF      R5 R4 K13    ; R6 := R4; R5 := R4[0xc4dff581]
 42 [-]: CONST     R7 8         ; R7 := 8.000000
 43 [-]: CALL      R5 3 2       ; R5 := R5(R6,R7)
 44 [-]: TEST      R5 1         ; if R5 then PC := 65
 45 [-]: JMP       65           ; PC := 65
 46 [-]: SELF      R5 R4 K15    ; R6 := R4; R5 := R4[0xf2deaf69]
 47 [-]: GETGLOBAL R7 K16       ; R7 := gLotusNpcAvatarType
 48 [-]: CALL      R5 3 2       ; R5 := R5(R6,R7)
 49 [-]: TEST      R5 0         ; if not R5 then PC := 65
 50 [-]: JMP       65           ; PC := 65
 51 [-]: SELF      R5 R4 K17    ; R6 := R4; R5 := R4[0x444ae2c8]
 52 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 53 [-]: TEST      R5 1         ; if R5 then PC := 65
 54 [-]: JMP       65           ; PC := 65
 55 [-]: SELF      R5 R4 K18    ; R6 := R4; R5 := R4[0x0f89a4d4]
 56 [-]: GETGLOBAL R7 K3        ; R7 := 0x0469f296
 57 [-]: LOADK     R8 K19       ; R8 := "TRINITY_MIND_CONTROL"
 58 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 59 [-]: LOADKB    R8 0 0       ; R8 := false
 60 [-]: CONST     R9 3         ; R9 := 3.000000
 61 [-]: CONST     R10 2        ; R10 := 2.000000
 62 [-]: LOADKB    R11 1 0      ; R11 := true
 63 [-]: CONST     R12 0        ; R12 := 0.000000
 64 [-]: CALL      R5 8 1       ; R5(R6,R7,R8,R9,R10,R11,R12)
 65 [-]: GETGLOBAL R5 K21       ; R5 := 0x89326c93
 66 [-]: SELF      R5 R5 K22    ; R6 := R5; R5 := R5[0x7d108ddb]
 67 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 68 [-]: GETUPVAL  R6 U1        ; R6 := U1
 69 [-]: DIV       R6 R6 K23    ; R6 := R6 / 4.000000
 70 [-]: SELF      R7 R4 K24    ; R8 := R4; R7 := R4[0x1ac1655c]
 71 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 72 [-]: SELF      R7 R7 K25    ; R8 := R7; R7 := R7[0x2079c1dd]
 73 [-]: MOVE      R9 R1        ; R9 := R1
 74 [-]: CALL      R7 3 2       ; R7 := R7(R8,R9)
 75 [-]: LT        0 K26 R7     ; if 0.000000 >= R7 then PC := 88
 76 [-]: JMP       88           ; PC := 88
 77 [-]: GETGLOBAL R8 K20       ; R8 := 0x34291f5c
 78 [-]: GETTABLE  R8 R8 K27    ; R8 := R8[0x7258f36f]
 79 [-]: GETUPVAL  R9 U2        ; R9 := U2
 80 [-]: SELF      R9 R9 K28    ; R10 := R9; R9 := R9[0x111f713c]
 81 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 82 [-]: DIV       R9 R9 R7     ; R9 := R9 / R7
 83 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 84 [-]: SELF      R9 R8 K29    ; R10 := R8; R9 := R8[0xe4c4dc01]
 85 [-]: GETUPVAL  R11 U2       ; R11 := U2
 86 [-]: CALL      R9 3 1       ; R9(R10,R11)
 87 [-]: SETUPVAL  R8 U2        ; U82 := R2
 88 [-]: GETGLOBAL R9 K30       ; R9 := 0x6687f6e0
 89 [-]: SELF      R9 R9 K31    ; R10 := R9; R9 := R9[0xcde10c4a]
 90 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 91 [-]: MOVE      R10 R6       ; R10 := R6
 92 [-]: SELF      R11 R0 K32   ; R12 := R0; R11 := R0[0x5063edc3]
 93 [-]: CALL      R11 2 2      ; R11 := R11(R12)
 94 [-]: SELF      R12 R0 K33   ; R13 := R0; R12 := R0[0x75ecaf0b]
 95 [-]: CALL      R12 2 2      ; R12 := R12(R13)
 96 [-]: CONST     R13 0        ; R13 := 0.000000
 97 [-]: LT        0 K26 R11    ; if 0.000000 >= R11 then PC := 106
 98 [-]: JMP       106          ; PC := 106
 99 [-]: EQ        0 R12 K34    ; if R12 ~= 1.000000 then PC := 106
100 [-]: JMP       106          ; PC := 106
101 [-]: GETUPVAL  R14 U3       ; R14 := U3
102 [-]: MOVE      R15 R11      ; R15 := R11
103 [-]: MOVE      R16 R12      ; R16 := R12
104 [-]: CALL      R14 3 1      ; R14(R15,R16)
105 [-]: GETUPVAL  R13 U4       ; R13 := U4
106 [-]: SELF      R14 R0 K35   ; R15 := R0; R14 := R0[0x79f6af86]
107 [-]: LOADKB    R16 1 0      ; R16 := true
108 [-]: CALL      R14 3 1      ; R14(R15,R16)
109 [-]: SELF      R14 R0 K2    ; R15 := R0; R14 := R0[0xbc4ebb44]
110 [-]: GETGLOBAL R16 K3       ; R16 := 0x0469f296
111 [-]: LOADK     R17 K36      ; R17 := "EVPulse"
112 [-]: CALL      R16 2 0      ; R16,... := R16(R17)
113 [-]: CALL      R14 0 2      ; R14 := R14(R15,...)
114 [-]: EQ        0 R1 R3      ; if R1 ~= R3 then PC := 117
115 [-]: JMP       117          ; PC := 117
116 [-]: LOADKB    R15 0 1      ; R15 := false; PC := 117
117 [-]: LOADKB    R15 1 0      ; R15 := true
118 [-]: TEST      R15 1        ; if R15 then PC := 131
119 [-]: JMP       131          ; PC := 131
120 [-]: GETGLOBAL R16 K37      ; R16 := _T
121 [-]: GETTABLE  R16 R16 K38  ; R16 := R16["AddAbilityTimer"]
122 [-]: TEST      R16 0        ; if not R16 then PC := 131
123 [-]: JMP       131          ; PC := 131
124 [-]: GETGLOBAL R16 K37      ; R16 := _T
125 [-]: GETTABLE  R16 R16 K39  ; R16 := R16[0xcc4ac7a6]
126 [-]: MOVE      R17 R9       ; R17 := R9
127 [-]: MOVE      R18 R1       ; R18 := R1
128 [-]: GETUPVAL  R19 U1       ; R19 := U1
129 [-]: CONST     R20 0        ; R20 := 0.000000
130 [-]: CALL      R16 5 1      ; R16(R17,R18,R19,R20)
131 [-]: GETGLOBAL R16 K0       ; R16 := 0x7b998233
132 [-]: MOVE      R17 R4       ; R17 := R4
133 [-]: CALL      R16 2 2      ; R16 := R16(R17)
134 [-]: TEST      R16 1        ; if R16 then PC := 333
135 [-]: JMP       333          ; PC := 333
136 [-]: SELF      R16 R4 K40   ; R17 := R4; R16 := R4[0x2047cfe7]
137 [-]: CALL      R16 2 2      ; R16 := R16(R17)
138 [-]: TEST      R16 1        ; if R16 then PC := 333
139 [-]: JMP       333          ; PC := 333
140 [-]: GETUPVAL  R16 U1       ; R16 := U1
141 [-]: LT        0 K26 R16    ; if 0.000000 >= R16 then PC := 333
142 [-]: JMP       333          ; PC := 333
143 [-]: SELF      R16 R4 K13   ; R17 := R4; R16 := R4[0xc4dff581]
144 [-]: CONST     R18 0        ; R18 := 0.000000
145 [-]: CALL      R16 3 2      ; R16 := R16(R17,R18)
146 [-]: TEST      R16 1        ; if R16 then PC := 333
147 [-]: JMP       333          ; PC := 333
148 [-]: LE        0 R6 R10     ; if R6 > R10 then PC := 321
149 [-]: JMP       321          ; PC := 321
150 [-]: GETGLOBAL R16 K21      ; R16 := 0x89326c93
151 [-]: SELF      R16 R16 K41  ; R17 := R16; R16 := R16[0x05909209]
152 [-]: MOVE      R18 R14      ; R18 := R14
153 [-]: SELF      R19 R4 K42   ; R20 := R4; R19 := R4[0xf6ebd926]
154 [-]: CALL      R19 2 2      ; R19 := R19(R20)
155 [-]: SELF      R20 R4 K7    ; R21 := R4; R20 := R4[0xcb3851b8]
156 [-]: CALL      R20 2 2      ; R20 := R20(R21)
157 [-]: MOVE      R21 R1       ; R21 := R1
158 [-]: CALL      R16 6 1      ; R16(R17,R18,R19,R20,R21)
159 [-]: GETGLOBAL R16 K21      ; R16 := 0x89326c93
160 [-]: SELF      R16 R16 K43  ; R17 := R16; R16 := R16[0x18d05d30]
161 [-]: CALL      R16 2 2      ; R16 := R16(R17)
162 [-]: TEST      R16 0        ; if not R16 then PC := 320
163 [-]: JMP       320          ; PC := 320
164 [-]: SELF      R16 R4 K44   ; R17 := R4; R16 := R4[0xd2715720]
165 [-]: CALL      R16 2 2      ; R16 := R16(R17)
166 [-]: SELF      R17 R4 K45   ; R18 := R4; R17 := R4[0xb40c191a]
167 [-]: CALL      R17 2 2      ; R17 := R17(R18)
168 [-]: MUL       R18 R17 K46  ; R18 := R17 * 0.250000
169 [-]: GETGLOBAL R19 K47      ; R19 := 0x5bced4c4
170 [-]: GETTABLE  R19 R19 K48  ; R19 := R19[0xb62ecfe0]
171 [-]: MOVE      R20 R18      ; R20 := R18
172 [-]: MOVE      R21 R16      ; R21 := R16
173 [-]: CALL      R19 3 2      ; R19 := R19(R20,R21)
174 [-]: DIV       R19 R19 K23  ; R19 := R19 / 4.000000
175 [-]: GETUPVAL  R20 U2       ; R20 := U2
176 [-]: SELF      R20 R20 K28  ; R21 := R20; R20 := R20[0x111f713c]
177 [-]: CALL      R20 2 2      ; R20 := R20(R21)
178 [-]: MUL       R19 R19 R20  ; R19 := R19 * R20
179 [-]: GETGLOBAL R20 K47      ; R20 := 0x5bced4c4
180 [-]: GETTABLE  R20 R20 K48  ; R20 := R20[0xb62ecfe0]
181 [-]: MOVE      R21 R19      ; R21 := R19
182 [-]: CONST     R22 5        ; R22 := 5.000000
183 [-]: CALL      R20 3 2      ; R20 := R20(R21,R22)
184 [-]: MOVE      R19 R20      ; R19 := R20
185 [-]: GETGLOBAL R20 K20      ; R20 := 0x34291f5c
186 [-]: GETTABLE  R20 R20 K27  ; R20 := R20[0x7258f36f]
187 [-]: MOVE      R21 R19      ; R21 := R19
188 [-]: CALL      R20 2 2      ; R20 := R20(R21)
189 [-]: SELF      R21 R20 K29  ; R22 := R20; R21 := R20[0xe4c4dc01]
190 [-]: GETUPVAL  R23 U2       ; R23 := U2
191 [-]: CALL      R21 3 1      ; R21(R22,R23)
192 [-]: GETGLOBAL R21 K20      ; R21 := 0x34291f5c
193 [-]: GETTABLE  R21 R21 K49  ; R21 := R21[0x35c16153]
194 [-]: CALL      R21 1 2      ; R21 := R21()
195 [-]: SELF      R22 R21 K50  ; R23 := R21; R22 := R21[0xf326045f]
196 [-]: MOVE      R24 R20      ; R24 := R20
197 [-]: CALL      R22 3 1      ; R22(R23,R24)
198 [-]: SELF      R22 R21 K51  ; R23 := R21; R22 := R21[0x1586e35e]
199 [-]: CONST     R24 17       ; R24 := 17.000000
200 [-]: CONST     R25 1        ; R25 := 1.000000
201 [-]: CALL      R22 4 1      ; R22(R23,R24,R25)
202 [-]: SELF      R22 R21 K52  ; R23 := R21; R22 := R21[0x86cd00cb]
203 [-]: MOVE      R24 R1       ; R24 := R1
204 [-]: CALL      R22 3 1      ; R22(R23,R24)
205 [-]: SELF      R22 R21 K53  ; R23 := R21; R22 := R21[0xf4dc3420]
206 [-]: MOVE      R24 R0       ; R24 := R0
207 [-]: CALL      R22 3 1      ; R22(R23,R24)
208 [-]: SELF      R22 R4 K54   ; R23 := R4; R22 := R4[0x479483bb]
209 [-]: MOVE      R24 R21      ; R24 := R21
210 [-]: CALL      R22 3 1      ; R22(R23,R24)
211 [-]: CONST     R22 1        ; R22 := 1.000000
212 [-]: LEN       R23 R5       ; R23 := # R5
213 [-]: CONST     R24 1        ; R24 := 1.000000
214 [-]: FORPREP   R22 319      ; R22 -= R24; PC := 319
215 [-]: GETTABLE  R26 R5 R25   ; R26 := R5[R25]
216 [-]: GETGLOBAL R27 K0       ; R27 := 0x7b998233
217 [-]: MOVE      R28 R26      ; R28 := R26
218 [-]: CALL      R27 2 2      ; R27 := R27(R28)
219 [-]: TEST      R27 1        ; if R27 then PC := 319
220 [-]: JMP       319          ; PC := 319
221 [-]: SELF      R27 R26 K55  ; R28 := R26; R27 := R26[0xa534c3ac]
222 [-]: CALL      R27 2 2      ; R27 := R27(R28)
223 [-]: GETGLOBAL R28 K0       ; R28 := 0x7b998233
224 [-]: MOVE      R29 R27      ; R29 := R27
225 [-]: CALL      R28 2 2      ; R28 := R28(R29)
226 [-]: TEST      R28 0        ; if not R28 then PC := 231
227 [-]: JMP       231          ; PC := 231
228 [-]: SELF      R28 R26 K56  ; R29 := R26; R28 := R26[0xbb610e5b]
229 [-]: CALL      R28 2 2      ; R28 := R28(R29)
230 [-]: MOVE      R27 R28      ; R27 := R28
231 [-]: GETGLOBAL R28 K0       ; R28 := 0x7b998233
232 [-]: MOVE      R29 R27      ; R29 := R27
233 [-]: CALL      R28 2 2      ; R28 := R28(R29)
234 [-]: TEST      R28 1        ; if R28 then PC := 319
235 [-]: JMP       319          ; PC := 319
236 [-]: SELF      R28 R27 K15  ; R29 := R27; R28 := R27[0xf2deaf69]
237 [-]: GETGLOBAL R30 K57      ; R30 := gTennoAvatarType
238 [-]: CALL      R28 3 2      ; R28 := R28(R29,R30)
239 [-]: TEST      R28 0        ; if not R28 then PC := 319
240 [-]: JMP       319          ; PC := 319
241 [-]: SELF      R28 R27 K58  ; R29 := R27; R28 := R27[0xee0bc178]
242 [-]: MOVE      R30 R1       ; R30 := R1
243 [-]: CALL      R28 3 2      ; R28 := R28(R29,R30)
244 [-]: TEST      R28 0        ; if not R28 then PC := 319
245 [-]: JMP       319          ; PC := 319
246 [-]: SELF      R28 R27 K59  ; R29 := R27; R28 := R27[0x753a7ea6]
247 [-]: MOVE      R30 R1       ; R30 := R1
248 [-]: CALL      R28 3 2      ; R28 := R28(R29,R30)
249 [-]: TEST      R28 0        ; if not R28 then PC := 319
250 [-]: JMP       319          ; PC := 319
251 [-]: GETUPVAL  R28 U5       ; R28 := U5
252 [-]: GETTABLE  R28 R28 K60  ; R28 := R28[0xe00cc5f0]
253 [-]: MOVE      R29 R27      ; R29 := R27
254 [-]: CALL      R28 2 2      ; R28 := R28(R29)
255 [-]: TEST      R28 1        ; if R28 then PC := 319
256 [-]: JMP       319          ; PC := 319
257 [-]: SELF      R28 R27 K61  ; R29 := R27; R28 := R27[0xbebad19f]
258 [-]: MOVE      R30 R4       ; R30 := R4
259 [-]: CALL      R28 3 2      ; R28 := R28(R29,R30)
260 [-]: GETUPVAL  R29 U6       ; R29 := U6
261 [-]: LE        0 R28 R29    ; if R28 > R29 then PC := 319
262 [-]: JMP       319          ; PC := 319
263 [-]: SELF      R29 R27 K62  ; R30 := R27; R29 := R27[0xde321e6f]
264 [-]: CALL      R29 2 2      ; R29 := R29(R30)
265 [-]: SELF      R30 R29 K63  ; R31 := R29; R30 := R29[0xf7d48ee0]
266 [-]: CALL      R30 2 2      ; R30 := R30(R31)
267 [-]: GETGLOBAL R31 K0       ; R31 := 0x7b998233
268 [-]: MOVE      R32 R30      ; R32 := R30
269 [-]: CALL      R31 2 2      ; R31 := R31(R32)
270 [-]: TEST      R31 1        ; if R31 then PC := 319
271 [-]: JMP       319          ; PC := 319
272 [-]: SELF      R31 R30 K64  ; R32 := R30; R31 := R30[0xded54c60]
273 [-]: CALL      R31 2 2      ; R31 := R31(R32)
274 [-]: SELF      R32 R30 K65  ; R33 := R30; R32 := R30[0x58a4d5ac]
275 [-]: CALL      R32 2 2      ; R32 := R32(R33)
276 [-]: GETGLOBAL R33 K47      ; R33 := 0x5bced4c4
277 [-]: GETTABLE  R33 R33 K66  ; R33 := R33[0xac1b386a]
278 [-]: SUB       R34 R31 R32  ; R34 := R31 - R32
279 [-]: GETUPVAL  R35 U7       ; R35 := U7
280 [-]: CALL      R33 3 2      ; R33 := R33(R34,R35)
281 [-]: LT        0 K26 R33    ; if 0.000000 >= R33 then PC := 306
282 [-]: JMP       306          ; PC := 306
283 [-]: SELF      R34 R1 K67   ; R35 := R1; R34 := R1[0x35844cf2]
284 [-]: CALL      R34 2 2      ; R34 := R34(R35)
285 [-]: TEST      R34 0        ; if not R34 then PC := 299
286 [-]: JMP       299          ; PC := 299
287 [-]: SELF      R34 R1 K68   ; R35 := R1; R34 := R1[0x5e651723]
288 [-]: CALL      R34 2 2      ; R34 := R34(R35)
289 [-]: SELF      R34 R34 K69  ; R35 := R34; R34 := R34[0x61c34fa9]
290 [-]: CALL      R34 2 2      ; R34 := R34(R35)
291 [-]: GETGLOBAL R35 K0       ; R35 := 0x7b998233
292 [-]: MOVE      R36 R34      ; R36 := R34
293 [-]: CALL      R35 2 2      ; R35 := R35(R36)
294 [-]: TEST      R35 1        ; if R35 then PC := 299
295 [-]: JMP       299          ; PC := 299
296 [-]: SELF      R35 R34 K70  ; R36 := R34; R35 := R34[0x3849c9b8]
297 [-]: MOVE      R37 R33      ; R37 := R33
298 [-]: CALL      R35 3 1      ; R35(R36,R37)
299 [-]: SELF      R35 R30 K71  ; R36 := R30; R35 := R30[0xfc80301e]
300 [-]: MOVE      R37 R33      ; R37 := R33
301 [-]: CALL      R35 3 1      ; R35(R36,R37)
302 [-]: SELF      R35 R29 K72  ; R36 := R29; R35 := R29[0x7bc127aa]
303 [-]: GETGLOBAL R37 K73      ; R37 := 0x4a851630
304 [-]: MOVE      R38 R33      ; R38 := R33
305 [-]: CALL      R35 4 1      ; R35(R36,R37,R38)
306 [-]: GETUPVAL  R35 U7       ; R35 := U7
307 [-]: LT        0 R33 R35    ; if R33 >= R35 then PC := 319
308 [-]: JMP       319          ; PC := 319
309 [-]: LT        0 K26 R13    ; if 0.000000 >= R13 then PC := 319
310 [-]: JMP       319          ; PC := 319
311 [-]: SELF      R35 R27 K24  ; R36 := R27; R35 := R27[0x1ac1655c]
312 [-]: CALL      R35 2 2      ; R35 := R35(R36)
313 [-]: SELF      R35 R35 K74  ; R36 := R35; R35 := R35[0x60bf5f59]
314 [-]: GETUPVAL  R37 U7       ; R37 := U7
315 [-]: SUB       R37 R37 R33  ; R37 := R37 - R33
316 [-]: MUL       R37 R37 R13  ; R37 := R37 * R13
317 [-]: LOADKB    R38 1 0      ; R38 := true
318 [-]: CALL      R35 4 1      ; R35(R36,R37,R38)
319 [-]: FORLOOP   R22 215      ; R22 += R24; if R22 <= R23 then begin PC := 215; R25 := R22 end
320 [-]: CONST     R10 0        ; R10 := 0.000000
321 [-]: GETGLOBAL R35 K75      ; R35 := 0x67652851
322 [-]: CALL      R35 1 2      ; R35 := R35()
323 [-]: ADD       R10 R10 R35  ; R10 := R10 + R35
324 [-]: GETUPVAL  R35 U1       ; R35 := U1
325 [-]: GETGLOBAL R36 K75      ; R36 := 0x67652851
326 [-]: CALL      R36 1 2      ; R36 := R36()
327 [-]: SUB       R35 R35 R36  ; R35 := R35 - R36
328 [-]: SETUPVAL  R35 U1       ; U82 := R1
329 [-]: GETGLOBAL R35 K76      ; R35 := 0xcbd666e1
330 [-]: CONST     R36 0        ; R36 := 0.000000
331 [-]: CALL      R35 2 1      ; R35(R36)
332 [-]: JMP       131          ; PC := 131
333 [-]: GETGLOBAL R35 K0       ; R35 := 0x7b998233
334 [-]: MOVE      R36 R4       ; R36 := R4
335 [-]: CALL      R35 2 2      ; R35 := R35(R36)
336 [-]: TEST      R35 1        ; if R35 then PC := 475
337 [-]: JMP       475          ; PC := 475
338 [-]: SELF      R35 R4 K40   ; R36 := R4; R35 := R4[0x2047cfe7]
339 [-]: CALL      R35 2 2      ; R35 := R35(R36)
340 [-]: TEST      R35 0        ; if not R35 then PC := 475
341 [-]: JMP       475          ; PC := 475
342 [-]: GETUPVAL  R35 U1       ; R35 := U1
343 [-]: LT        0 K26 R35    ; if 0.000000 >= R35 then PC := 475
344 [-]: JMP       475          ; PC := 475
345 [-]: SELF      R35 R4 K13   ; R36 := R4; R35 := R4[0xc4dff581]
346 [-]: CONST     R37 0        ; R37 := 0.000000
347 [-]: CALL      R35 3 2      ; R35 := R35(R36,R37)
348 [-]: TEST      R35 1        ; if R35 then PC := 475
349 [-]: JMP       475          ; PC := 475
350 [-]: GETGLOBAL R35 K21      ; R35 := 0x89326c93
351 [-]: SELF      R35 R35 K41  ; R36 := R35; R35 := R35[0x05909209]
352 [-]: MOVE      R37 R14      ; R37 := R14
353 [-]: SELF      R38 R4 K42   ; R39 := R4; R38 := R4[0xf6ebd926]
354 [-]: CALL      R38 2 2      ; R38 := R38(R39)
355 [-]: SELF      R39 R4 K7    ; R40 := R4; R39 := R4[0xcb3851b8]
356 [-]: CALL      R39 2 2      ; R39 := R39(R40)
357 [-]: MOVE      R40 R1       ; R40 := R1
358 [-]: CALL      R35 6 1      ; R35(R36,R37,R38,R39,R40)
359 [-]: GETGLOBAL R35 K21      ; R35 := 0x89326c93
360 [-]: SELF      R35 R35 K43  ; R36 := R35; R35 := R35[0x18d05d30]
361 [-]: CALL      R35 2 2      ; R35 := R35(R36)
362 [-]: TEST      R35 0        ; if not R35 then PC := 475
363 [-]: JMP       475          ; PC := 475
364 [-]: GETUPVAL  R35 U1       ; R35 := U1
365 [-]: DIV       R35 R35 R6   ; R35 := R35 / R6
366 [-]: CONST     R36 1        ; R36 := 1.000000
367 [-]: LEN       R37 R5       ; R37 := # R5
368 [-]: CONST     R38 1        ; R38 := 1.000000
369 [-]: FORPREP   R36 474      ; R36 -= R38; PC := 474
370 [-]: GETTABLE  R40 R5 R39   ; R40 := R5[R39]
371 [-]: GETGLOBAL R41 K0       ; R41 := 0x7b998233
372 [-]: MOVE      R42 R40      ; R42 := R40
373 [-]: CALL      R41 2 2      ; R41 := R41(R42)
374 [-]: TEST      R41 1        ; if R41 then PC := 474
375 [-]: JMP       474          ; PC := 474
376 [-]: SELF      R41 R40 K55  ; R42 := R40; R41 := R40[0xa534c3ac]
377 [-]: CALL      R41 2 2      ; R41 := R41(R42)
378 [-]: GETGLOBAL R42 K0       ; R42 := 0x7b998233
379 [-]: MOVE      R43 R41      ; R43 := R41
380 [-]: CALL      R42 2 2      ; R42 := R42(R43)
381 [-]: TEST      R42 0        ; if not R42 then PC := 386
382 [-]: JMP       386          ; PC := 386
383 [-]: SELF      R42 R40 K56  ; R43 := R40; R42 := R40[0xbb610e5b]
384 [-]: CALL      R42 2 2      ; R42 := R42(R43)
385 [-]: MOVE      R41 R42      ; R41 := R42
386 [-]: GETGLOBAL R42 K0       ; R42 := 0x7b998233
387 [-]: MOVE      R43 R41      ; R43 := R41
388 [-]: CALL      R42 2 2      ; R42 := R42(R43)
389 [-]: TEST      R42 1        ; if R42 then PC := 474
390 [-]: JMP       474          ; PC := 474
391 [-]: SELF      R42 R41 K15  ; R43 := R41; R42 := R41[0xf2deaf69]
392 [-]: GETGLOBAL R44 K57      ; R44 := gTennoAvatarType
393 [-]: CALL      R42 3 2      ; R42 := R42(R43,R44)
394 [-]: TEST      R42 0        ; if not R42 then PC := 474
395 [-]: JMP       474          ; PC := 474
396 [-]: SELF      R42 R41 K58  ; R43 := R41; R42 := R41[0xee0bc178]
397 [-]: MOVE      R44 R1       ; R44 := R1
398 [-]: CALL      R42 3 2      ; R42 := R42(R43,R44)
399 [-]: TEST      R42 0        ; if not R42 then PC := 474
400 [-]: JMP       474          ; PC := 474
401 [-]: SELF      R42 R41 K59  ; R43 := R41; R42 := R41[0x753a7ea6]
402 [-]: MOVE      R44 R1       ; R44 := R1
403 [-]: CALL      R42 3 2      ; R42 := R42(R43,R44)
404 [-]: TEST      R42 0        ; if not R42 then PC := 474
405 [-]: JMP       474          ; PC := 474
406 [-]: GETUPVAL  R42 U5       ; R42 := U5
407 [-]: GETTABLE  R42 R42 K60  ; R42 := R42[0xe00cc5f0]
408 [-]: MOVE      R43 R41      ; R43 := R41
409 [-]: CALL      R42 2 2      ; R42 := R42(R43)
410 [-]: TEST      R42 1        ; if R42 then PC := 474
411 [-]: JMP       474          ; PC := 474
412 [-]: SELF      R42 R41 K61  ; R43 := R41; R42 := R41[0xbebad19f]
413 [-]: MOVE      R44 R4       ; R44 := R4
414 [-]: CALL      R42 3 2      ; R42 := R42(R43,R44)
415 [-]: GETUPVAL  R43 U6       ; R43 := U6
416 [-]: LE        0 R42 R43    ; if R42 > R43 then PC := 474
417 [-]: JMP       474          ; PC := 474
418 [-]: SELF      R43 R41 K62  ; R44 := R41; R43 := R41[0xde321e6f]
419 [-]: CALL      R43 2 2      ; R43 := R43(R44)
420 [-]: SELF      R44 R43 K63  ; R45 := R43; R44 := R43[0xf7d48ee0]
421 [-]: CALL      R44 2 2      ; R44 := R44(R45)
422 [-]: GETGLOBAL R45 K0       ; R45 := 0x7b998233
423 [-]: MOVE      R46 R44      ; R46 := R44
424 [-]: CALL      R45 2 2      ; R45 := R45(R46)
425 [-]: TEST      R45 1        ; if R45 then PC := 474
426 [-]: JMP       474          ; PC := 474
427 [-]: SELF      R45 R44 K64  ; R46 := R44; R45 := R44[0xded54c60]
428 [-]: CALL      R45 2 2      ; R45 := R45(R46)
429 [-]: SELF      R46 R44 K65  ; R47 := R44; R46 := R44[0x58a4d5ac]
430 [-]: CALL      R46 2 2      ; R46 := R46(R47)
431 [-]: GETUPVAL  R47 U7       ; R47 := U7
432 [-]: MUL       R47 R47 R35  ; R47 := R47 * R35
433 [-]: GETGLOBAL R48 K47      ; R48 := 0x5bced4c4
434 [-]: GETTABLE  R48 R48 K66  ; R48 := R48[0xac1b386a]
435 [-]: SUB       R49 R45 R46  ; R49 := R45 - R46
436 [-]: MOVE      R50 R47      ; R50 := R47
437 [-]: CALL      R48 3 2      ; R48 := R48(R49,R50)
438 [-]: LT        0 K26 R48    ; if 0.000000 >= R48 then PC := 463
439 [-]: JMP       463          ; PC := 463
440 [-]: SELF      R49 R1 K67   ; R50 := R1; R49 := R1[0x35844cf2]
441 [-]: CALL      R49 2 2      ; R49 := R49(R50)
442 [-]: TEST      R49 0        ; if not R49 then PC := 463
443 [-]: JMP       463          ; PC := 463
444 [-]: SELF      R49 R1 K68   ; R50 := R1; R49 := R1[0x5e651723]
445 [-]: CALL      R49 2 2      ; R49 := R49(R50)
446 [-]: SELF      R49 R49 K69  ; R50 := R49; R49 := R49[0x61c34fa9]
447 [-]: CALL      R49 2 2      ; R49 := R49(R50)
448 [-]: GETGLOBAL R50 K0       ; R50 := 0x7b998233
449 [-]: MOVE      R51 R49      ; R51 := R49
450 [-]: CALL      R50 2 2      ; R50 := R50(R51)
451 [-]: TEST      R50 1        ; if R50 then PC := 456
452 [-]: JMP       456          ; PC := 456
453 [-]: SELF      R50 R49 K70  ; R51 := R49; R50 := R49[0x3849c9b8]
454 [-]: MOVE      R52 R48      ; R52 := R48
455 [-]: CALL      R50 3 1      ; R50(R51,R52)
456 [-]: SELF      R50 R44 K71  ; R51 := R44; R50 := R44[0xfc80301e]
457 [-]: MOVE      R52 R48      ; R52 := R48
458 [-]: CALL      R50 3 1      ; R50(R51,R52)
459 [-]: SELF      R50 R43 K72  ; R51 := R43; R50 := R43[0x7bc127aa]
460 [-]: GETGLOBAL R52 K73      ; R52 := 0x4a851630
461 [-]: MOVE      R53 R48      ; R53 := R48
462 [-]: CALL      R50 4 1      ; R50(R51,R52,R53)
463 [-]: LT        0 R48 R47    ; if R48 >= R47 then PC := 474
464 [-]: JMP       474          ; PC := 474
465 [-]: LT        0 K26 R13    ; if 0.000000 >= R13 then PC := 474
466 [-]: JMP       474          ; PC := 474
467 [-]: SELF      R50 R41 K24  ; R51 := R41; R50 := R41[0x1ac1655c]
468 [-]: CALL      R50 2 2      ; R50 := R50(R51)
469 [-]: SELF      R50 R50 K74  ; R51 := R50; R50 := R50[0x60bf5f59]
470 [-]: SUB       R52 R47 R48  ; R52 := R47 - R48
471 [-]: MUL       R52 R52 R13  ; R52 := R52 * R13
472 [-]: LOADKB    R53 1 0      ; R53 := true
473 [-]: CALL      R50 4 1      ; R50(R51,R52,R53)
474 [-]: FORLOOP   R36 370      ; R36 += R38; if R36 <= R37 then begin PC := 370; R39 := R36 end
475 [-]: RETURN    R0 1         ; return 


; Function #15:
;
; Name:            
; Defined at line: 507
; #Upvalues:       9
; #Parameters:     4
; Is_vararg:       0
; Max Stack Size:  13

  1 [-]: GETUPVAL  R4 U0        ; R4 := U0
  2 [-]: MOVE      R5 R3        ; R5 := R3
  3 [-]: CALL      R4 2 1       ; R4(R5)
  4 [-]: GETUPVAL  R4 U6        ; R4 := U6
  5 [-]: MOVE      R5 R1        ; R5 := R1
  6 [-]: CALL      R4 2 6       ; R4,R5,R6,R7,R8 := R4(R5)
  7 [-]: SETUPVAL  R8 U5        ; U82 := R5
  8 [-]: SETUPVAL  R7 U4        ; U82 := R4
  9 [-]: SETUPVAL  R6 U3        ; U82 := R3
 10 [-]: SETUPVAL  R5 U2        ; U82 := R2
 11 [-]: SETUPVAL  R4 U1        ; U82 := R1
 12 [-]: SELF      R4 R1 K0     ; R5 := R1; R4 := R1[0xeea7f8c4]
 13 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 14 [-]: SELF      R5 R1 K1     ; R6 := R1; R5 := R1[0x020d4331]
 15 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 16 [-]: SELF      R5 R5 K2     ; R6 := R5; R5 := R5[0x553549e8]
 17 [-]: MOVE      R7 R4        ; R7 := R4
 18 [-]: CALL      R5 3 1       ; R5(R6,R7)
 19 [-]: SELF      R5 R1 K3     ; R6 := R1; R5 := R1[0x47901f07]
 20 [-]: SELF      R7 R0 K4     ; R8 := R0; R7 := R0[0xbc4ebb44]
 21 [-]: GETGLOBAL R9 K5        ; R9 := 0x0469f296
 22 [-]: LOADK     R10 K6       ; R10 := "EVCast"
 23 [-]: CALL      R9 2 0       ; R9,... := R9(R10)
 24 [-]: CALL      R7 0 2       ; R7 := R7(R8,...)
 25 [-]: GETGLOBAL R8 K5        ; R8 := 0x0469f296
 26 [-]: LOADK     R9 K7        ; R9 := "GAME_L1_WEAPON1"
 27 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 28 [-]: GETGLOBAL R9 K8        ; R9 := ZERO_VECTOR
 29 [-]: GETGLOBAL R10 K9       ; R10 := ZERO_ROTATION
 30 [-]: MOVE      R11 R0       ; R11 := R0
 31 [-]: CALL      R5 7 1       ; R5(R6,R7,R8,R9,R10,R11)
 32 [-]: GETUPVAL  R5 U7        ; R5 := U7
 33 [-]: GETTABLE  R5 R5 K10    ; R5 := R5[0x8d11e79e]
 34 [-]: MOVE      R6 R0        ; R6 := R0
 35 [-]: GETGLOBAL R7 K11       ; R7 := 0x0ed8b456
 36 [-]: LOADK     R8 K12       ; R8 := "ActivateMindControl"
 37 [-]: LOADKB    R9 0 0       ; R9 := false
 38 [-]: CONST     R10 2        ; R10 := 2.000000
 39 [-]: CONST     R11 1        ; R11 := 1.000000
 40 [-]: LOADKB    R12 1 0      ; R12 := true
 41 [-]: CALL      R5 8 1       ; R5(R6,R7,R8,R9,R10,R11,R12)
 42 [-]: SELF      R5 R1 K14    ; R6 := R1; R5 := R1[0x659d451f]
 43 [-]: GETGLOBAL R7 K15       ; R7 := 0x520e413d
 44 [-]: LOADKB    R8 0 0       ; R8 := false
 45 [-]: CONST     R9 0         ; R9 := 0.000000
 46 [-]: LOADKB    R10 1 0      ; R10 := true
 47 [-]: CALL      R5 6 1       ; R5(R6,R7,R8,R9,R10)
 48 [-]: SELF      R5 R0 K16    ; R6 := R0; R5 := R0[0x0d0482e0]
 49 [-]: CALL      R5 2 1       ; R5(R6)
 50 [-]: GETUPVAL  R5 U8        ; R5 := U8
 51 [-]: MOVE      R6 R0        ; R6 := R0
 52 [-]: MOVE      R7 R1        ; R7 := R1
 53 [-]: MOVE      R8 R0        ; R8 := R0
 54 [-]: MOVE      R9 R1        ; R9 := R1
 55 [-]: MOVE      R10 R2       ; R10 := R2
 56 [-]: CALL      R5 6 1       ; R5(R6,R7,R8,R9,R10)
 57 [-]: RETURN    R0 1         ; return 


; Function #16:
;
; Name:            
; Defined at line: 525
; #Upvalues:       1
; #Parameters:     4
; Is_vararg:       0
; Max Stack Size:  12

  1 [-]: GETGLOBAL R4 K0        ; R4 := 0x7b998233
  2 [-]: MOVE      R5 R2        ; R5 := R2
  3 [-]: CALL      R4 2 2       ; R4 := R4(R5)
  4 [-]: TEST      R4 1         ; if R4 then PC := 22
  5 [-]: JMP       22           ; PC := 22
  6 [-]: SELF      R4 R2 K1     ; R5 := R2; R4 := R2[0xf2deaf69]
  7 [-]: GETGLOBAL R6 K2        ; R6 := gLotusNpcAvatarType
  8 [-]: CALL      R4 3 2       ; R4 := R4(R5,R6)
  9 [-]: TEST      R4 0         ; if not R4 then PC := 22
 10 [-]: JMP       22           ; PC := 22
 11 [-]: SELF      R4 R2 K3     ; R5 := R2; R4 := R2[0x444ae2c8]
 12 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 13 [-]: TEST      R4 0         ; if not R4 then PC := 22
 14 [-]: JMP       22           ; PC := 22
 15 [-]: SELF      R4 R2 K4     ; R5 := R2; R4 := R2[0x7027c544]
 16 [-]: LOADNIL   R6 R6        ; R6 := nil
 17 [-]: LOADKB    R7 0 0       ; R7 := false
 18 [-]: CONST     R8 2         ; R8 := 2.000000
 19 [-]: CONST     R9 1         ; R9 := 1.000000
 20 [-]: LOADKB    R10 0 0      ; R10 := false
 21 [-]: CALL      R4 7 1       ; R4(R5,R6,R7,R8,R9,R10)
 22 [-]: GETGLOBAL R4 K0        ; R4 := 0x7b998233
 23 [-]: MOVE      R5 R2        ; R5 := R2
 24 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 25 [-]: TEST      R4 1         ; if R4 then PC := 67
 26 [-]: JMP       67           ; PC := 67
 27 [-]: GETUPVAL  R4 U0        ; R4 := U0
 28 [-]: MOVE      R5 R2        ; R5 := R2
 29 [-]: CALL      R4 2 1       ; R4(R5)
 30 [-]: SELF      R4 R2 K6     ; R5 := R2; R4 := R2[0xc9f6a7d7]
 31 [-]: SELF      R6 R0 K7     ; R7 := R0; R6 := R0[0xbc4ebb44]
 32 [-]: GETGLOBAL R8 K8        ; R8 := 0x0469f296
 33 [-]: LOADK     R9 K9        ; R9 := "EVAttach"
 34 [-]: CALL      R8 2 0       ; R8,... := R8(R9)
 35 [-]: CALL      R6 0 0       ; R6,... := R6(R7,...)
 36 [-]: CALL      R4 0 2       ; R4 := R4(R5,...)
 37 [-]: GETGLOBAL R5 K0        ; R5 := 0x7b998233
 38 [-]: MOVE      R6 R4        ; R6 := R4
 39 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 40 [-]: TEST      R5 1         ; if R5 then PC := 44
 41 [-]: JMP       44           ; PC := 44
 42 [-]: SELF      R5 R4 K10    ; R6 := R4; R5 := R4[0xa2880940]
 43 [-]: CALL      R5 2 1       ; R5(R6)
 44 [-]: SELF      R5 R2 K6     ; R6 := R2; R5 := R2[0xc9f6a7d7]
 45 [-]: SELF      R7 R0 K7     ; R8 := R0; R7 := R0[0xbc4ebb44]
 46 [-]: GETGLOBAL R9 K8        ; R9 := 0x0469f296
 47 [-]: LOADK     R10 K11      ; R10 := "primeAttachEffect"
 48 [-]: CALL      R9 2 0       ; R9,... := R9(R10)
 49 [-]: CALL      R7 0 0       ; R7,... := R7(R8,...)
 50 [-]: CALL      R5 0 2       ; R5 := R5(R6,...)
 51 [-]: GETGLOBAL R6 K0        ; R6 := 0x7b998233
 52 [-]: MOVE      R7 R5        ; R7 := R5
 53 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 54 [-]: TEST      R6 1         ; if R6 then PC := 58
 55 [-]: JMP       58           ; PC := 58
 56 [-]: SELF      R6 R5 K10    ; R7 := R5; R6 := R5[0xa2880940]
 57 [-]: CALL      R6 2 1       ; R6(R7)
 58 [-]: GETGLOBAL R6 K12       ; R6 := 0x89326c93
 59 [-]: SELF      R6 R6 K13    ; R7 := R6; R6 := R6[0x05909209]
 60 [-]: MOVE      R8 R3        ; R8 := R3
 61 [-]: SELF      R9 R2 K14    ; R10 := R2; R9 := R2[0xf6ebd926]
 62 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 63 [-]: SELF      R10 R2 K15   ; R11 := R2; R10 := R2[0xcb3851b8]
 64 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 65 [-]: MOVE      R11 R1       ; R11 := R1
 66 [-]: CALL      R6 6 1       ; R6(R7,R8,R9,R10,R11)
 67 [-]: RETURN    R0 1         ; return 


; Function #17:
;
; Name:            
; Defined at line: 545
; #Upvalues:       1
; #Parameters:     3
; Is_vararg:       0
; Max Stack Size:  9

  1 [-]: GETGLOBAL R3 K0        ; R3 := _T
  2 [-]: GETTABLE  R3 R3 K1     ; R3 := R3["AddAbilityTimer"]
  3 [-]: TEST      R3 0         ; if not R3 then PC := 14
  4 [-]: JMP       14           ; PC := 14
  5 [-]: GETGLOBAL R3 K0        ; R3 := _T
  6 [-]: GETTABLE  R3 R3 K2     ; R3 := R3[0xcc4ac7a6]
  7 [-]: GETGLOBAL R4 K3        ; R4 := 0x6687f6e0
  8 [-]: SELF      R4 R4 K4     ; R5 := R4; R4 := R4[0xcde10c4a]
  9 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 10 [-]: MOVE      R5 R1        ; R5 := R1
 11 [-]: CONST     R6 0         ; R6 := 0.000000
 12 [-]: CONST     R7 0         ; R7 := 0.000000
 13 [-]: CALL      R3 5 1       ; R3(R4,R5,R6,R7)
 14 [-]: SELF      R3 R0 K5     ; R4 := R0; R3 := R0[0xbc4ebb44]
 15 [-]: GETGLOBAL R5 K6        ; R5 := 0x0469f296
 16 [-]: LOADK     R6 K7        ; R6 := "EVEnd"
 17 [-]: CALL      R5 2 0       ; R5,... := R5(R6)
 18 [-]: CALL      R3 0 2       ; R3 := R3(R4,...)
 19 [-]: GETUPVAL  R4 U0        ; R4 := U0
 20 [-]: MOVE      R5 R0        ; R5 := R0
 21 [-]: MOVE      R6 R1        ; R6 := R1
 22 [-]: MOVE      R7 R2        ; R7 := R2
 23 [-]: MOVE      R8 R3        ; R8 := R3
 24 [-]: CALL      R4 5 1       ; R4(R5,R6,R7,R8)
 25 [-]: RETURN    R0 1         ; return 


; Function #18:
;
; Name:            
; Defined at line: 554
; #Upvalues:       5
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  9

  1 [-]: GETGLOBAL R0 K0        ; R0 := _T
  2 [-]: GETTABLE  R0 R0 K1     ; R0 := R0["CrewShipAbilityInfo"]
  3 [-]: GETTABLE  R0 R0 K2     ; R0 := R0["mAbility"]
  4 [-]: SELF      R1 R0 K3     ; R2 := R0; R1 := R0[0x3f703537]
  5 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  6 [-]: GETUPVAL  R2 U0        ; R2 := U0
  7 [-]: SELF      R3 R1 K4     ; R4 := R1; R3 := R1[0xa776e126]
  8 [-]: SELF      R5 R1 K5     ; R6 := R1; R5 := R1[0xa2356091]
  9 [-]: SELF      R7 R0 K6     ; R8 := R0; R7 := R0[0xcde10c4a]
 10 [-]: CALL      R7 2 0       ; R7,... := R7(R8)
 11 [-]: CALL      R5 0 0       ; R5,... := R5(R6,...)
 12 [-]: CALL      R3 0 0       ; R3,... := R3(R4,...)
 13 [-]: CALL      R2 0 1       ; R2(R3,...)
 14 [-]: GETUPVAL  R2 U4        ; R2 := U4
 15 [-]: SELF      R3 R1 K7     ; R4 := R1; R3 := R1[0x5163741e]
 16 [-]: CALL      R3 2 0       ; R3,... := R3(R4)
 17 [-]: CALL      R2 0 4       ; R2,R3,R4 := R2(R3,...)
 18 [-]: SETUPVAL  R4 U3        ; U82 := R3
 19 [-]: SETUPVAL  R3 U2        ; U82 := R2
 20 [-]: SETUPVAL  R2 U1        ; U82 := R1
 21 [-]: GETGLOBAL R2 K0        ; R2 := _T
 22 [-]: GETTABLE  R2 R2 K1     ; R2 := R2["CrewShipAbilityInfo"]
 23 [-]: NEWTABLE  R3 0 2       ; R3 := {}
 24 [-]: GETUPVAL  R4 U3        ; R4 := U3
 25 [-]: SETTABLE  R3 K9 R4     ; R3["Radius"] := R4
 26 [-]: SELF      R4 R0 K11    ; R5 := R0; R4 := R0[0x7e627183]
 27 [-]: LOADKB    R6 1 0       ; R6 := true
 28 [-]: CALL      R4 3 2       ; R4 := R4(R5,R6)
 29 [-]: SETTABLE  R3 K10 R4    ; R3["EnergyCost"] := R4
 30 [-]: SETTABLE  R2 K8 R3     ; R2["mAbilityInfo"] := R3
 31 [-]: RETURN    R0 1         ; return 


; Function #19:
;
; Name:            
; Defined at line: 564
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  8

  1 [-]: GETGLOBAL R1 K0        ; R1 := _T
  2 [-]: GETTABLE  R1 R1 K1     ; R1 := R1["CrewShipAbilityEval"]
  3 [-]: GETTABLE  R1 R1 K2     ; R1 := R1["pos"]
  4 [-]: GETGLOBAL R2 K3        ; R2 := 0x89326c93
  5 [-]: SELF      R2 R2 K4     ; R3 := R2; R2 := R2[0xfb669000]
  6 [-]: GETGLOBAL R4 K5        ; R4 := gLotusNpcAvatarType
  7 [-]: MOVE      R5 R1        ; R5 := R1
  8 [-]: CONST     R6 0         ; R6 := 0.000000
  9 [-]: GETGLOBAL R7 K0        ; R7 := _T
 10 [-]: GETTABLE  R7 R7 K1     ; R7 := R7["CrewShipAbilityEval"]
 11 [-]: GETTABLE  R7 R7 K6     ; R7 := R7["radius"]
 12 [-]: CALL      R2 6 2       ; R2 := R2(R3,R4,R5,R6,R7)
 13 [-]: TEST      R2 1         ; if R2 then PC := 16
 14 [-]: JMP       16           ; PC := 16
 15 [-]: NEWTABLE  R2 0 0       ; R2 := {}
 16 [-]: GETGLOBAL R3 K7        ; R3 := 0x33bdd652
 17 [-]: GETTABLE  R3 R3 K8     ; R3 := R3[0xf21b1d8e]
 18 [-]: MOVE      R4 R2        ; R4 := R2
 19 [-]: CLOSURE   R5 0         ; R5 := closure(Function #19.1)
 20 [-]: MOVE      R0 R1        ; R0 := R1
 21 [-]: CALL      R3 3 1       ; R3(R4,R5)
 22 [-]: GETUPVAL  R3 U0        ; R3 := U0
 23 [-]: MOVE      R4 R0        ; R4 := R0
 24 [-]: MOVE      R5 R2        ; R5 := R2
 25 [-]: MOVE      R6 R1        ; R6 := R1
 26 [-]: CALL      R3 4 3       ; R3,R4 := R3(R4,R5,R6)
 27 [-]: GETGLOBAL R5 K0        ; R5 := _T
 28 [-]: NEWTABLE  R6 0 2       ; R6 := {}
 29 [-]: SETTABLE  R6 K9 R3     ; R6["success"] := R3
 30 [-]: TEST      R3 0         ; if not R3 then PC := 34
 31 [-]: JMP       34           ; PC := 34
 32 [-]: TESTSET   R7 R4 1      ; if R4 then PC := 35 else R7 := R4
 33 [-]: JMP       35           ; PC := 35
 34 [-]: LOADNIL   R7 R7        ; R7 := nil
 35 [-]: SETTABLE  R6 K10 R7    ; R6["target"] := R7
 36 [-]: SETTABLE  R5 K1 R6     ; R5["CrewShipAbilityEval"] := R6
 37 [-]: RETURN    R0 1         ; return 


; Function #19.1:
;
; Name:            
; Defined at line: 567
; #Upvalues:       1
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  6

  1 [-]: SELF      R2 R0 K0     ; R3 := R0; R2 := R0[0x1f420a3a]
  2 [-]: GETUPVAL  R4 U0        ; R4 := U0
  3 [-]: CALL      R2 3 2       ; R2 := R2(R3,R4)
  4 [-]: SELF      R3 R1 K0     ; R4 := R1; R3 := R1[0x1f420a3a]
  5 [-]: GETUPVAL  R5 U0        ; R5 := U0
  6 [-]: CALL      R3 3 2       ; R3 := R3(R4,R5)
  7 [-]: LT        1 R2 R3      ; if R2 < R3 then PC := 10
  8 [-]: JMP       10           ; PC := 10
  9 [-]: LOADKB    R2 0 1       ; R2 := false; PC := 10
 10 [-]: LOADKB    R2 1 0       ; R2 := true
 11 [-]: RETURN    R2 2         ; return R2
 12 [-]: RETURN    R0 1         ; return 


; Function #20:
;
; Name:            
; Defined at line: 573
; #Upvalues:       10
; #Parameters:     8
; Is_vararg:       0
; Max Stack Size:  16

  1 [-]: GETGLOBAL R8 K0        ; R8 := 0x6687f6e0
  2 [-]: SELF      R8 R8 K1     ; R9 := R8; R8 := R8[0xcde10c4a]
  3 [-]: CALL      R8 2 2       ; R8 := R8(R9)
  4 [-]: SELF      R8 R8 K2     ; R9 := R8; R8 := R8[0xe223e2b1]
  5 [-]: CALL      R8 2 2       ; R8 := R8(R9)
  6 [-]: GETUPVAL  R9 U0        ; R9 := U0
  7 [-]: GETTABLE  R9 R9 K3     ; R9 := R9[0x5ef687a2]
  8 [-]: MOVE      R10 R8       ; R10 := R8
  9 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 10 [-]: TEST      R9 0         ; if not R9 then PC := 14
 11 [-]: JMP       14           ; PC := 14
 12 [-]: LOADKB    R9 1 0       ; R9 := true
 13 [-]: RETURN    R9 2         ; return R9
 14 [-]: GETUPVAL  R9 U1        ; R9 := U1
 15 [-]: MOVE      R10 R4       ; R10 := R4
 16 [-]: CALL      R9 2 1       ; R9(R10)
 17 [-]: GETUPVAL  R9 U7        ; R9 := U7
 18 [-]: MOVE      R10 R3       ; R10 := R3
 19 [-]: CALL      R9 2 6       ; R9,R10,R11,R12,R13 := R9(R10)
 20 [-]: SETUPVAL  R13 U6       ; U82 := R6
 21 [-]: SETUPVAL  R12 U5       ; U82 := R5
 22 [-]: SETUPVAL  R11 U4       ; U82 := R4
 23 [-]: SETUPVAL  R10 U3       ; U82 := R3
 24 [-]: SETUPVAL  R9 U2        ; U82 := R2
 25 [-]: SELF      R9 R2 K4     ; R10 := R2; R9 := R2[0xbc4ebb44]
 26 [-]: GETGLOBAL R11 K5       ; R11 := 0x0469f296
 27 [-]: LOADK     R12 K6       ; R12 := "EVEnd"
 28 [-]: CALL      R11 2 0      ; R11,... := R11(R12)
 29 [-]: CALL      R9 0 2       ; R9 := R9(R10,...)
 30 [-]: GETUPVAL  R10 U8       ; R10 := U8
 31 [-]: MOVE      R11 R1       ; R11 := R1
 32 [-]: MOVE      R12 R0       ; R12 := R0
 33 [-]: MOVE      R13 R2       ; R13 := R2
 34 [-]: MOVE      R14 R3       ; R14 := R3
 35 [-]: MOVE      R15 R7       ; R15 := R7
 36 [-]: CALL      R10 6 1      ; R10(R11,R12,R13,R14,R15)
 37 [-]: GETUPVAL  R10 U9       ; R10 := U9
 38 [-]: MOVE      R11 R1       ; R11 := R1
 39 [-]: MOVE      R12 R0       ; R12 := R0
 40 [-]: MOVE      R13 R7       ; R13 := R7
 41 [-]: MOVE      R14 R9       ; R14 := R9
 42 [-]: CALL      R10 5 1      ; R10(R11,R12,R13,R14)
 43 [-]: GETUPVAL  R10 U0       ; R10 := U0
 44 [-]: GETTABLE  R10 R10 K7   ; R10 := R10[0x6b3430b5]
 45 [-]: MOVE      R11 R8       ; R11 := R8
 46 [-]: CALL      R10 2 1      ; R10(R11)
 47 [-]: RETURN    R0 1         ; return 


