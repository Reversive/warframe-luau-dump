; This file has been disassembled using luadec 2.0.2 by sztupy (modified by Reversive)

; Name:            
; Defined at line: 0
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       2
; Max Stack Size:  33

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x2d0fad09
  2 [-]: LOADK     R1 K1        ; R1 := "Lotus.Interface.UIUtilities"
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K0        ; R1 := 0x2d0fad09
  5 [-]: LOADK     R2 K2        ; R2 := "EE.Interface.Utilities"
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: LOADBOOL  R2 0 0       ; R2 := false
  8 [-]: NEWTABLE  R3 0 0       ; R3 := {}
  9 [-]: LOADBOOL  R4 0 0       ; R4 := false
 10 [-]: LOADK     R5 1         ; R5 := 1.500000
 11 [-]: LOADK     R6 0         ; R6 := 0.750000
 12 [-]: LOADK     R7 K3        ; R7 := 0.900000
 13 [-]: LOADK     R8 0         ; R8 := 0.000000
 14 [-]: LOADK     R9 1         ; R9 := 1.000000
 15 [-]: LOADK     R10 K3       ; R10 := 0.900000
 16 [-]: LOADNIL   R11 R11      ; R11 := nil
 17 [-]: LOADK     R12 0        ; R12 := 0.500000
 18 [-]: LOADK     R13 0        ; R13 := 0.000000
 19 [-]: LOADK     R14 4        ; R14 := 4.000000
 20 [-]: LOADNIL   R15 R15      ; R15 := nil
 21 [-]: LOADBOOL  R16 1 0      ; R16 := true
 22 [-]: LOADBOOL  R17 1 0      ; R17 := true
 23 [-]: LOADBOOL  R18 0 0      ; R18 := false
 24 [-]: LOADNIL   R19 R21      ; R19 := R20 := R21 := nil
 25 [-]: LOADK     R22 -1       ; R22 := -1.000000
 26 [-]: LOADK     R23 K4       ; R23 := 7.308500
 27 [-]: LOADK     R24 K5       ; R24 := -223.300003
 28 [-]: LOADK     R25 K6       ; R25 := 0.800000
 29 [-]: LOADK     R26 61       ; R26 := 61.000000
 30 [-]: LOADK     R27 -226     ; R27 := -226.000000
 31 [-]: LOADK     R28 225      ; R28 := 225.000000
 32 [-]: CLOSURE   R29 0        ; R29 := closure(Function #1)
 33 [-]: MOVE      R0 R21       ; R0 := R21
 34 [-]: SETGLOBAL R29 K7       ; Shutdown := R29
 35 [-]: CLOSURE   R29 1        ; R29 := closure(Function #2)
 36 [-]: MOVE      R0 R15       ; R0 := R15
 37 [-]: MOVE      R0 R17       ; R0 := R17
 38 [-]: MOVE      R0 R13       ; R0 := R13
 39 [-]: MOVE      R0 R14       ; R0 := R14
 40 [-]: SETGLOBAL R29 K8       ; RecallProjectile := R29
 41 [-]: CLOSURE   R29 2        ; R29 := closure(Function #3)
 42 [-]: MOVE      R0 R2        ; R0 := R2
 43 [-]: MOVE      R0 R3        ; R0 := R3
 44 [-]: CLOSURE   R30 3        ; R30 := closure(Function #4)
 45 [-]: MOVE      R0 R5        ; R0 := R5
 46 [-]: MOVE      R0 R9        ; R0 := R9
 47 [-]: MOVE      R0 R7        ; R0 := R7
 48 [-]: MOVE      R0 R1        ; R0 := R1
 49 [-]: MOVE      R0 R26       ; R0 := R26
 50 [-]: MOVE      R0 R6        ; R0 := R6
 51 [-]: MOVE      R0 R24       ; R0 := R24
 52 [-]: MOVE      R0 R23       ; R0 := R23
 53 [-]: MOVE      R0 R25       ; R0 := R25
 54 [-]: CLOSURE   R31 4        ; R31 := closure(Function #5)
 55 [-]: MOVE      R0 R19       ; R0 := R19
 56 [-]: MOVE      R0 R21       ; R0 := R21
 57 [-]: SETGLOBAL R31 K9       ; Initialize := R31
 58 [-]: CLOSURE   R31 5        ; R31 := closure(Function #6)
 59 [-]: MOVE      R0 R9        ; R0 := R9
 60 [-]: MOVE      R0 R8        ; R0 := R8
 61 [-]: MOVE      R0 R5        ; R0 := R5
 62 [-]: MOVE      R0 R13       ; R0 := R13
 63 [-]: MOVE      R0 R14       ; R0 := R14
 64 [-]: MOVE      R0 R18       ; R0 := R18
 65 [-]: MOVE      R0 R15       ; R0 := R15
 66 [-]: MOVE      R0 R30       ; R0 := R30
 67 [-]: MOVE      R0 R7        ; R0 := R7
 68 [-]: CLOSURE   R32 6        ; R32 := closure(Function #7)
 69 [-]: MOVE      R0 R2        ; R0 := R2
 70 [-]: MOVE      R0 R3        ; R0 := R3
 71 [-]: MOVE      R0 R20       ; R0 := R20
 72 [-]: MOVE      R0 R22       ; R0 := R22
 73 [-]: MOVE      R0 R9        ; R0 := R9
 74 [-]: MOVE      R0 R14       ; R0 := R14
 75 [-]: MOVE      R0 R30       ; R0 := R30
 76 [-]: MOVE      R0 R8        ; R0 := R8
 77 [-]: MOVE      R0 R13       ; R0 := R13
 78 [-]: MOVE      R0 R15       ; R0 := R15
 79 [-]: MOVE      R0 R4        ; R0 := R4
 80 [-]: MOVE      R0 R6        ; R0 := R6
 81 [-]: MOVE      R0 R7        ; R0 := R7
 82 [-]: MOVE      R0 R5        ; R0 := R5
 83 [-]: MOVE      R0 R10       ; R0 := R10
 84 [-]: MOVE      R0 R19       ; R0 := R19
 85 [-]: MOVE      R0 R1        ; R0 := R1
 86 [-]: MOVE      R0 R11       ; R0 := R11
 87 [-]: MOVE      R0 R31       ; R0 := R31
 88 [-]: MOVE      R0 R12       ; R0 := R12
 89 [-]: MOVE      R0 R16       ; R0 := R16
 90 [-]: MOVE      R0 R27       ; R0 := R27
 91 [-]: MOVE      R0 R28       ; R0 := R28
 92 [-]: SETGLOBAL R32 K10      ; Update := R32
 93 [-]: CLOSURE   R32 7        ; R32 := closure(Function #8)
 94 [-]: MOVE      R0 R0        ; R0 := R0
 95 [-]: MOVE      R0 R21       ; R0 := R21
 96 [-]: SETGLOBAL R32 K11      ; HandleHudScale := R32
 97 [-]: RETURN    R0 1         ; return 


; Function #1:
;
; Name:            
; Defined at line: 43
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
  2 [-]: GETGLOBAL R1 K1        ; R1 := _T
  3 [-]: GETTABLE  R1 R1 K2     ; R1 := R1["ShowWeaponPanel"]
  4 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  5 [-]: TEST      R0 1         ; if R0 then PC := 10
  6 [-]: JMP       10           ; PC := 10
  7 [-]: GETGLOBAL R0 K1        ; R0 := _T
  8 [-]: GETTABLE  R0 R0 K3     ; R0 := R0[0xdf2147fb]
  9 [-]: CALL      R0 1 1       ; R0()
 10 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
 11 [-]: GETGLOBAL R1 K1        ; R1 := _T
 12 [-]: GETTABLE  R1 R1 K4     ; R1 := R1["ShowAbilityPanel"]
 13 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 14 [-]: TEST      R0 1         ; if R0 then PC := 19
 15 [-]: JMP       19           ; PC := 19
 16 [-]: GETGLOBAL R0 K1        ; R0 := _T
 17 [-]: GETTABLE  R0 R0 K5     ; R0 := R0[0xc206a867]
 18 [-]: CALL      R0 1 1       ; R0()
 19 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
 20 [-]: GETGLOBAL R1 K1        ; R1 := _T
 21 [-]: GETTABLE  R1 R1 K6     ; R1 := R1["HUD_GetAnchorMgr"]
 22 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 23 [-]: TEST      R0 1         ; if R0 then PC := 32
 24 [-]: JMP       32           ; PC := 32
 25 [-]: GETGLOBAL R0 K1        ; R0 := _T
 26 [-]: GETTABLE  R0 R0 K7     ; R0 := R0[0xf6b77431]
 27 [-]: CALL      R0 1 2       ; R0 := R0()
 28 [-]: SELF      R1 R0 K8     ; R2 := R0; R1 := R0[0x7f19c438]
 29 [-]: GETGLOBAL R3 K9        ; R3 := 0xae91e43b
 30 [-]: LOADK     R4 K10       ; R4 := "Gauge"
 31 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 32 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 33 [-]: GETUPVAL  R2 U0        ; R2 := U0
 34 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 35 [-]: TEST      R1 1         ; if R1 then PC := 40
 36 [-]: JMP       40           ; PC := 40
 37 [-]: GETUPVAL  R1 U0        ; R1 := U0
 38 [-]: SELF      R1 R1 K11    ; R2 := R1; R1 := R1[0x32302b4a]
 39 [-]: CALL      R1 2 1       ; R1(R2)
 40 [-]: RETURN    R0 1         ; return 


; Function #2:
;
; Name:            
; Defined at line: 62
; #Upvalues:       4
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: EQ        0 R0 K0      ; if R0 ~= nil then PC := 11
  3 [-]: JMP       11           ; PC := 11
  4 [-]: LOADBOOL  R0 1 0       ; R0 := true
  5 [-]: SETUPVAL  R0 U0        ; U82 := R0
  6 [-]: GETUPVAL  R0 U1        ; R0 := U1
  7 [-]: TEST      R0 0         ; if not R0 then PC := 11
  8 [-]: JMP       11           ; PC := 11
  9 [-]: GETUPVAL  R0 U3        ; R0 := U3
 10 [-]: SETUPVAL  R0 U2        ; U82 := R2
 11 [-]: RETURN    R0 1         ; return 


; Function #3:
;
; Name:            
; Defined at line: 72
; #Upvalues:       2
; #Parameters:     1
; Is_vararg:       3
; Max Stack Size:  7

  1 [-]: GETUPVAL  R2 U0        ; R2 := U0
  2 [-]: TEST      R2 0         ; if not R2 then PC := 8
  3 [-]: JMP       8            ; PC := 8
  4 [-]: MOVE      R2 R0        ; R2 := R0
  5 [-]: VARARG    R3 0         ; R3 := ...
  6 [-]: CALL      R2 0 1       ; R2(R3,...)
  7 [-]: JMP       16           ; PC := 16
  8 [-]: GETGLOBAL R2 K0        ; R2 := 0x33bdd652
  9 [-]: GETTABLE  R2 R2 K1     ; R2 := R2[0x23d5322f]
 10 [-]: GETUPVAL  R3 U1        ; R3 := U1
 11 [-]: NEWTABLE  R4 1 0       ; R4 := {}
 12 [-]: MOVE      R5 R0        ; R5 := R0
 13 [-]: VARARG    R6 0         ; R6 := ...
 14 [-]: SETLIST   R4 0 1       ; R4[(1-1)*FPF+i] := R(4+i), 1 <= i <= 0
 15 [-]: CALL      R2 3 1       ; R2(R3,R4)
 16 [-]: RETURN    R0 1         ; return 


; Function #4:
;
; Name:            
; Defined at line: 80
; #Upvalues:       9
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  15

  1 [-]: SETUPVAL  R0 U0        ; U82 := R0
  2 [-]: LOADK     R2 K0        ; R2 := 0.400000
  3 [-]: GETGLOBAL R3 K1        ; R3 := 0xc163f229
  4 [-]: MOVE      R4 R1        ; R4 := R1
  5 [-]: SUB       R5 K2 R2     ; R5 := 1.000000 - R2
  6 [-]: SUB       R5 R5 R1     ; R5 := R5 - R1
  7 [-]: CALL      R3 3 2       ; R3 := R3(R4,R5)
  8 [-]: GETUPVAL  R4 U1        ; R4 := U1
  9 [-]: LT        0 K3 R4      ; if 0.000000 >= R4 then PC := 12
 10 [-]: JMP       12           ; PC := 12
 11 [-]: ADD       R3 R3 R2     ; R3 := R3 + R2
 12 [-]: SETUPVAL  R1 U2        ; U82 := R2
 13 [-]: GETUPVAL  R4 U2        ; R4 := U2
 14 [-]: SUB       R4 R3 R4     ; R4 := R3 - R4
 15 [-]: GETUPVAL  R5 U2        ; R5 := U2
 16 [-]: ADD       R5 R3 R5     ; R5 := R3 + R5
 17 [-]: GETUPVAL  R6 U3        ; R6 := U3
 18 [-]: GETTABLE  R6 R6 K4     ; R6 := R6[0x74a11ec6]
 19 [-]: GETUPVAL  R7 U4        ; R7 := U4
 20 [-]: MUL       R7 R7 R4     ; R7 := R7 * R4
 21 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 22 [-]: GETUPVAL  R7 U3        ; R7 := U3
 23 [-]: GETTABLE  R7 R7 K4     ; R7 := R7[0x74a11ec6]
 24 [-]: GETUPVAL  R8 U4        ; R8 := U4
 25 [-]: MUL       R8 R8 R5     ; R8 := R8 * R5
 26 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 27 [-]: SUB       R7 R7 K2     ; R7 := R7 - 1.000000
 28 [-]: ADD       R8 R6 R7     ; R8 := R6 + R7
 29 [-]: GETUPVAL  R9 U4        ; R9 := U4
 30 [-]: MUL       R9 K5 R9     ; R9 := 2.000000 * R9
 31 [-]: DIV       R8 R8 R9     ; R8 := R8 / R9
 32 [-]: GETUPVAL  R9 U0        ; R9 := U0
 33 [-]: MUL       R8 R8 R9     ; R8 := R8 * R9
 34 [-]: SETUPVAL  R8 U5        ; U82 := R5
 35 [-]: GETUPVAL  R8 U6        ; R8 := U6
 36 [-]: GETUPVAL  R9 U7        ; R9 := U7
 37 [-]: MUL       R9 R9 R6     ; R9 := R9 * R6
 38 [-]: ADD       R8 R8 R9     ; R8 := R8 + R9
 39 [-]: GETUPVAL  R9 U6        ; R9 := U6
 40 [-]: GETUPVAL  R10 U7       ; R10 := U7
 41 [-]: MUL       R10 R10 R7   ; R10 := R10 * R7
 42 [-]: ADD       R9 R9 R10    ; R9 := R9 + R10
 43 [-]: GETUPVAL  R10 U8       ; R10 := U8
 44 [-]: ADD       R9 R9 R10    ; R9 := R9 + R10
 45 [-]: GETGLOBAL R10 K6       ; R10 := 0xae91e43b
 46 [-]: SELF      R10 R10 K7   ; R11 := R10; R10 := R10[0x67bc869f]
 47 [-]: LOADK     R12 K8       ; R12 := "Gauge.LeftBrace"
 48 [-]: LOADK     R13 0        ; R13 := 0.000000
 49 [-]: MOVE      R14 R8       ; R14 := R8
 50 [-]: CALL      R10 5 1      ; R10(R11,R12,R13,R14)
 51 [-]: GETGLOBAL R10 K6       ; R10 := 0xae91e43b
 52 [-]: SELF      R10 R10 K7   ; R11 := R10; R10 := R10[0x67bc869f]
 53 [-]: LOADK     R12 K9       ; R12 := "Gauge.LeftMask"
 54 [-]: LOADK     R13 0        ; R13 := 0.000000
 55 [-]: MOVE      R14 R8       ; R14 := R8
 56 [-]: CALL      R10 5 1      ; R10(R11,R12,R13,R14)
 57 [-]: GETGLOBAL R10 K6       ; R10 := 0xae91e43b
 58 [-]: SELF      R10 R10 K7   ; R11 := R10; R10 := R10[0x67bc869f]
 59 [-]: LOADK     R12 K10      ; R12 := "Gauge.RightBrace"
 60 [-]: LOADK     R13 0        ; R13 := 0.000000
 61 [-]: MOVE      R14 R9       ; R14 := R9
 62 [-]: CALL      R10 5 1      ; R10(R11,R12,R13,R14)
 63 [-]: GETGLOBAL R10 K6       ; R10 := 0xae91e43b
 64 [-]: SELF      R10 R10 K7   ; R11 := R10; R10 := R10[0x67bc869f]
 65 [-]: LOADK     R12 K11      ; R12 := "Gauge.RightMask"
 66 [-]: LOADK     R13 0        ; R13 := 0.000000
 67 [-]: MOVE      R14 R9       ; R14 := R9
 68 [-]: CALL      R10 5 1      ; R10(R11,R12,R13,R14)
 69 [-]: GETGLOBAL R10 K6       ; R10 := 0xae91e43b
 70 [-]: SELF      R10 R10 K7   ; R11 := R10; R10 := R10[0x67bc869f]
 71 [-]: LOADK     R12 K12      ; R12 := "Gauge.TargetArea"
 72 [-]: LOADK     R13 0        ; R13 := 0.000000
 73 [-]: ADD       R14 R8 R9    ; R14 := R8 + R9
 74 [-]: DIV       R14 R14 K5   ; R14 := R14 / 2.000000
 75 [-]: CALL      R10 5 1      ; R10(R11,R12,R13,R14)
 76 [-]: GETGLOBAL R10 K6       ; R10 := 0xae91e43b
 77 [-]: SELF      R10 R10 K7   ; R11 := R10; R10 := R10[0x67bc869f]
 78 [-]: LOADK     R12 K13      ; R12 := "Gauge.Flair"
 79 [-]: LOADK     R13 0        ; R13 := 0.000000
 80 [-]: ADD       R14 R8 R9    ; R14 := R8 + R9
 81 [-]: DIV       R14 R14 K5   ; R14 := R14 / 2.000000
 82 [-]: CALL      R10 5 1      ; R10(R11,R12,R13,R14)
 83 [-]: GETGLOBAL R10 K6       ; R10 := 0xae91e43b
 84 [-]: SELF      R10 R10 K7   ; R11 := R10; R10 := R10[0x67bc869f]
 85 [-]: LOADK     R12 K14      ; R12 := "Gauge.LightMask"
 86 [-]: LOADK     R13 0        ; R13 := 0.000000
 87 [-]: ADD       R14 R8 R9    ; R14 := R8 + R9
 88 [-]: DIV       R14 R14 K5   ; R14 := R14 / 2.000000
 89 [-]: CALL      R10 5 1      ; R10(R11,R12,R13,R14)
 90 [-]: GETGLOBAL R10 K6       ; R10 := 0xae91e43b
 91 [-]: SELF      R10 R10 K7   ; R11 := R10; R10 := R10[0x67bc869f]
 92 [-]: LOADK     R12 K14      ; R12 := "Gauge.LightMask"
 93 [-]: LOADK     R13 12       ; R13 := 12.000000
 94 [-]: SUB       R14 R9 R8    ; R14 := R9 - R8
 95 [-]: CALL      R10 5 1      ; R10(R11,R12,R13,R14)
 96 [-]: RETURN    R0 1         ; return 


; Function #5:
;
; Name:            
; Defined at line: 108
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  8

  1 [-]: GETGLOBAL R0 K0        ; R0 := _T
  2 [-]: GETTABLE  R0 R0 K1     ; R0 := R0[0xf6b77431]
  3 [-]: CALL      R0 1 2       ; R0 := R0()
  4 [-]: SELF      R1 R0 K2     ; R2 := R0; R1 := R0[0x20ff29f7]
  5 [-]: GETGLOBAL R3 K3        ; R3 := 0xae91e43b
  6 [-]: LOADK     R4 K4        ; R4 := "Gauge"
  7 [-]: NEWTABLE  R5 2 0       ; R5 := {}
  8 [-]: GETTABLE  R6 R0 K5     ; R6 := R0["ANCHOR_V_CENTRE"]
  9 [-]: GETTABLE  R7 R0 K6     ; R7 := R0["ANCHOR_H_CENTRE"]
 10 [-]: SETLIST   R5 2 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 2
 11 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 12 [-]: SELF      R1 R0 K7     ; R2 := R0; R1 := R0[0xfaa69527]
 13 [-]: GETGLOBAL R3 K3        ; R3 := 0xae91e43b
 14 [-]: SELF      R3 R3 K8     ; R4 := R3; R3 := R3[0x6b837788]
 15 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 16 [-]: GETGLOBAL R4 K3        ; R4 := 0xae91e43b
 17 [-]: SELF      R4 R4 K9     ; R5 := R4; R4 := R4[0xaf9fda9f]
 18 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 19 [-]: LOADBOOL  R5 1 0       ; R5 := true
 20 [-]: GETTABLE  R6 R0 K10    ; R6 := R0["mHudScalePadding"]
 21 [-]: CALL      R1 6 1       ; R1(R2,R3,R4,R5,R6)
 22 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 23 [-]: SELF      R1 R1 K11    ; R2 := R1; R1 := R1[0x67bc869f]
 24 [-]: LOADK     R3 K4        ; R3 := "Gauge"
 25 [-]: LOADK     R4 10        ; R4 := 10.000000
 26 [-]: LOADK     R5 0         ; R5 := 0.000000
 27 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 28 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 29 [-]: SELF      R1 R1 K12    ; R2 := R1; R1 := R1[0x91a24e4b]
 30 [-]: LOADK     R3 K13       ; R3 := "Gauge.Flair"
 31 [-]: LOADK     R4 1         ; R4 := 1.000000
 32 [-]: CALL      R1 4 2       ; R1 := R1(R2,R3,R4)
 33 [-]: SETUPVAL  R1 U0        ; U82 := R0
 34 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 35 [-]: SELF      R1 R1 K11    ; R2 := R1; R1 := R1[0x67bc869f]
 36 [-]: LOADK     R3 K13       ; R3 := "Gauge.Flair"
 37 [-]: LOADK     R4 10        ; R4 := 10.000000
 38 [-]: LOADK     R5 0         ; R5 := 0.000000
 39 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 40 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 41 [-]: SELF      R1 R1 K14    ; R2 := R1; R1 := R1[0x20b98db3]
 42 [-]: LOADK     R3 K15       ; R3 := "Gauge.Flair.Label.text"
 43 [-]: GETGLOBAL R4 K16       ; R4 := 0x4775d9d0
 44 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 45 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 46 [-]: SELF      R1 R1 K11    ; R2 := R1; R1 := R1[0x67bc869f]
 47 [-]: LOADK     R3 K17       ; R3 := "Gauge.Flair.Label"
 48 [-]: LOADK     R4 36        ; R4 := 36.000000
 49 [-]: LOADK     R5 K18       ; R5 := 47871.000000
 50 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 51 [-]: GETGLOBAL R1 K19       ; R1 := 0x89326c93
 52 [-]: SELF      R1 R1 K20    ; R2 := R1; R1 := R1[0x78298275]
 53 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 54 [-]: GETGLOBAL R2 K21       ; R2 := 0x7b998233
 55 [-]: MOVE      R3 R1        ; R3 := R1
 56 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 57 [-]: TEST      R2 0         ; if not R2 then PC := 61
 58 [-]: JMP       61           ; PC := 61
 59 [-]: RETURN    R0 1         ; return 
 60 [-]: JMP       71           ; PC := 71
 61 [-]: GETGLOBAL R2 K21       ; R2 := 0x7b998233
 62 [-]: GETUPVAL  R3 U1        ; R3 := U1
 63 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 64 [-]: TEST      R2 0         ; if not R2 then PC := 71
 65 [-]: JMP       71           ; PC := 71
 66 [-]: GETGLOBAL R2 K3        ; R2 := 0xae91e43b
 67 [-]: SELF      R2 R2 K22    ; R3 := R2; R2 := R2[0x1fd6abd0]
 68 [-]: GETGLOBAL R4 K23       ; R4 := 0xc75c17fc
 69 [-]: CALL      R2 3 2       ; R2 := R2(R3,R4)
 70 [-]: SETUPVAL  R2 U1        ; U82 := R1
 71 [-]: RETURN    R0 1         ; return 


; Function #6:
;
; Name:            
; Defined at line: 129
; #Upvalues:       9
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  3

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: LT        0 K0 R0      ; if 0.000000 >= R0 then PC := 9
  3 [-]: JMP       9            ; PC := 9
  4 [-]: GETUPVAL  R0 U2        ; R0 := U2
  5 [-]: SETUPVAL  R0 U1        ; U82 := R1
  6 [-]: LOADK     R0 -1        ; R0 := -1.000000
  7 [-]: SETUPVAL  R0 U0        ; U82 := R0
  8 [-]: JMP       13           ; PC := 13
  9 [-]: LOADK     R0 0         ; R0 := 0.000000
 10 [-]: SETUPVAL  R0 U1        ; U82 := R1
 11 [-]: LOADK     R0 1         ; R0 := 1.000000
 12 [-]: SETUPVAL  R0 U0        ; U82 := R0
 13 [-]: GETUPVAL  R0 U3        ; R0 := U3
 14 [-]: ADD       R0 R0 K1     ; R0 := R0 + 1.000000
 15 [-]: SETUPVAL  R0 U3        ; U82 := R3
 16 [-]: GETUPVAL  R0 U3        ; R0 := U3
 17 [-]: GETUPVAL  R1 U4        ; R1 := U4
 18 [-]: LT        0 R0 R1      ; if R0 >= R1 then PC := 30
 19 [-]: JMP       30           ; PC := 30
 20 [-]: GETUPVAL  R0 U5        ; R0 := U5
 21 [-]: TEST      R0 1         ; if R0 then PC := 26
 22 [-]: JMP       26           ; PC := 26
 23 [-]: GETUPVAL  R0 U6        ; R0 := U6
 24 [-]: EQ        1 R0 K2      ; if R0 == nil then PC := 30
 25 [-]: JMP       30           ; PC := 30
 26 [-]: GETUPVAL  R0 U7        ; R0 := U7
 27 [-]: GETUPVAL  R1 U2        ; R1 := U2
 28 [-]: GETUPVAL  R2 U8        ; R2 := U8
 29 [-]: CALL      R0 3 1       ; R0(R1,R2)
 30 [-]: RETURN    R0 1         ; return 


; Function #7:
;
; Name:            
; Defined at line: 145
; #Upvalues:       23
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  17

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
  2 [-]: GETGLOBAL R1 K1        ; R1 := 0xae91e43b
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: TEST      R0 0         ; if not R0 then PC := 7
  5 [-]: JMP       7            ; PC := 7
  6 [-]: RETURN    R0 1         ; return 
  7 [-]: GETGLOBAL R0 K1        ; R0 := 0xae91e43b
  8 [-]: SELF      R0 R0 K2     ; R1 := R0; R0 := R0[0x8a8c8d5a]
  9 [-]: GETGLOBAL R2 K3        ; R2 := 0x67652851
 10 [-]: CALL      R2 1 0       ; R2,... := R2()
 11 [-]: CALL      R0 0 1       ; R0(R1,...)
 12 [-]: LOADBOOL  R0 1 0       ; R0 := true
 13 [-]: SETUPVAL  R0 U0        ; U82 := R0
 14 [-]: GETUPVAL  R0 U1        ; R0 := U1
 15 [-]: LEN       R0 R0        ; R0 := # R0
 16 [-]: LT        0 K4 R0      ; if 0.000000 >= R0 then PC := 33
 17 [-]: JMP       33           ; PC := 33
 18 [-]: LOADK     R0 1         ; R0 := 1.000000
 19 [-]: GETUPVAL  R1 U1        ; R1 := U1
 20 [-]: LEN       R1 R1        ; R1 := # R1
 21 [-]: LOADK     R2 1         ; R2 := 1.000000
 22 [-]: FORPREP   R0 30        ; R0 -= R2; PC := 30
 23 [-]: GETUPVAL  R4 U1        ; R4 := U1
 24 [-]: GETTABLE  R4 R4 R3     ; R4 := R4[R3]
 25 [-]: GETTABLE  R4 R4 K5     ; R4 := R4[1.000000]
 26 [-]: GETUPVAL  R5 U1        ; R5 := U1
 27 [-]: GETTABLE  R5 R5 R3     ; R5 := R5[R3]
 28 [-]: GETTABLE  R5 R5 K6     ; R5 := R5[2.000000]
 29 [-]: CALL      R4 2 1       ; R4(R5)
 30 [-]: FORLOOP   R0 23        ; R0 += R2; if R0 <= R1 then begin PC := 23; R3 := R0 end
 31 [-]: NEWTABLE  R4 0 0       ; R4 := {}
 32 [-]: SETUPVAL  R4 U1        ; U82 := R1
 33 [-]: LOADBOOL  R4 0 0       ; R4 := false
 34 [-]: SETUPVAL  R4 U0        ; U82 := R0
 35 [-]: GETGLOBAL R4 K0        ; R4 := 0x7b998233
 36 [-]: GETUPVAL  R5 U2        ; R5 := U2
 37 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 38 [-]: TEST      R4 0         ; if not R4 then PC := 44
 39 [-]: JMP       44           ; PC := 44
 40 [-]: GETGLOBAL R4 K7        ; R4 := 0xbe190284
 41 [-]: SELF      R4 R4 K8     ; R5 := R4; R4 := R4[0x33307f92]
 42 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 43 [-]: SETUPVAL  R4 U2        ; U82 := R2
 44 [-]: GETGLOBAL R4 K0        ; R4 := 0x7b998233
 45 [-]: GETUPVAL  R5 U2        ; R5 := U2
 46 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 47 [-]: TEST      R4 1         ; if R4 then PC := 72
 48 [-]: JMP       72           ; PC := 72
 49 [-]: GETUPVAL  R4 U2        ; R4 := U2
 50 [-]: SELF      R4 R4 K9     ; R5 := R4; R4 := R4[0x91a24e4b]
 51 [-]: LOADK     R6 K10       ; R6 := "_root"
 52 [-]: LOADK     R7 10        ; R7 := 10.000000
 53 [-]: CALL      R4 4 2       ; R4 := R4(R5,R6,R7)
 54 [-]: GETUPVAL  R5 U2        ; R5 := U2
 55 [-]: SELF      R5 R5 K11    ; R6 := R5; R5 := R5[0x5b0290d2]
 56 [-]: LOADK     R7 K10       ; R7 := "_root"
 57 [-]: LOADK     R8 11        ; R8 := 11.000000
 58 [-]: CALL      R5 4 2       ; R5 := R5(R6,R7,R8)
 59 [-]: TEST      R5 1         ; if R5 then PC := 62
 60 [-]: JMP       62           ; PC := 62
 61 [-]: LOADK     R4 0         ; R4 := 0.000000
 62 [-]: GETUPVAL  R6 U3        ; R6 := U3
 63 [-]: EQ        1 R6 R4      ; if R6 == R4 then PC := 72
 64 [-]: JMP       72           ; PC := 72
 65 [-]: SETUPVAL  R4 U3        ; U82 := R3
 66 [-]: GETGLOBAL R6 K1        ; R6 := 0xae91e43b
 67 [-]: SELF      R6 R6 K12    ; R7 := R6; R6 := R6[0x67bc869f]
 68 [-]: LOADK     R8 K10       ; R8 := "_root"
 69 [-]: LOADK     R9 10        ; R9 := 10.000000
 70 [-]: MOVE      R10 R4       ; R10 := R4
 71 [-]: CALL      R6 5 1       ; R6(R7,R8,R9,R10)
 72 [-]: GETGLOBAL R6 K13       ; R6 := _T
 73 [-]: GETTABLE  R6 R6 K14    ; R6 := R6["FishingMinigame"]
 74 [-]: EQ        1 R6 K15     ; if R6 == nil then PC := 112
 75 [-]: JMP       112          ; PC := 112
 76 [-]: LOADK     R6 1         ; R6 := 1.000000
 77 [-]: SETUPVAL  R6 U4        ; U82 := R4
 78 [-]: GETGLOBAL R6 K13       ; R6 := _T
 79 [-]: GETTABLE  R6 R6 K14    ; R6 := R6["FishingMinigame"]
 80 [-]: GETTABLE  R6 R6 K16    ; R6 := R6["MaxSweeps"]
 81 [-]: SETUPVAL  R6 U5        ; U82 := R5
 82 [-]: GETUPVAL  R6 U6        ; R6 := U6
 83 [-]: GETGLOBAL R7 K13       ; R7 := _T
 84 [-]: GETTABLE  R7 R7 K14    ; R7 := R7["FishingMinigame"]
 85 [-]: GETTABLE  R7 R7 K17    ; R7 := R7["MaxTime"]
 86 [-]: GETGLOBAL R8 K13       ; R8 := _T
 87 [-]: GETTABLE  R8 R8 K14    ; R8 := R8["FishingMinigame"]
 88 [-]: GETTABLE  R8 R8 K18    ; R8 := R8["Threshold"]
 89 [-]: CALL      R6 3 1       ; R6(R7,R8)
 90 [-]: GETGLOBAL R6 K13       ; R6 := _T
 91 [-]: SETTABLE  R6 K14 K15   ; R6["FishingMinigame"] := nil
 92 [-]: LOADK     R6 0         ; R6 := 0.000000
 93 [-]: SETUPVAL  R6 U7        ; U82 := R7
 94 [-]: LOADK     R6 0         ; R6 := 0.000000
 95 [-]: SETUPVAL  R6 U8        ; U82 := R8
 96 [-]: LOADNIL   R6 R6        ; R6 := nil
 97 [-]: SETUPVAL  R6 U9        ; U82 := R9
 98 [-]: LOADBOOL  R6 1 0       ; R6 := true
 99 [-]: SETUPVAL  R6 U10       ; U82 := R10
100 [-]: GETGLOBAL R6 K19       ; R6 := 0x25312c9b
101 [-]: GETGLOBAL R7 K1        ; R7 := 0xae91e43b
102 [-]: LOADK     R8 K20       ; R8 := "Gauge"
103 [-]: LOADK     R9 2         ; R9 := 2.000000
104 [-]: NEWTABLE  R10 1 0      ; R10 := {}
105 [-]: LOADK     R11 10       ; R11 := 10.000000
106 [-]: SETLIST   R10 1 1      ; R10[(1-1)*FPF+i] := R(10+i), 1 <= i <= 1
107 [-]: NEWTABLE  R11 1 0      ; R11 := {}
108 [-]: LOADK     R12 100      ; R12 := 100.000000
109 [-]: SETLIST   R11 1 1      ; R11[(1-1)*FPF+i] := R(11+i), 1 <= i <= 1
110 [-]: LOADK     R12 K22      ; R12 := 0.100000
111 [-]: CALL      R6 7 1       ; R6(R7,R8,R9,R10,R11,R12)
112 [-]: GETUPVAL  R6 U10       ; R6 := U10
113 [-]: TEST      R6 0         ; if not R6 then PC := 319
114 [-]: JMP       319          ; PC := 319
115 [-]: GETGLOBAL R6 K23       ; R6 := 0x5bced4c4
116 [-]: GETTABLE  R6 R6 K24    ; R6 := R6[0xb62ecfe0]
117 [-]: LOADK     R7 0         ; R7 := 0.000000
118 [-]: GETGLOBAL R8 K23       ; R8 := 0x5bced4c4
119 [-]: GETTABLE  R8 R8 K25    ; R8 := R8[0xe4a5b3ca]
120 [-]: GETUPVAL  R9 U11       ; R9 := U11
121 [-]: GETUPVAL  R10 U7       ; R10 := U7
122 [-]: SUB       R9 R9 R10    ; R9 := R9 - R10
123 [-]: CALL      R8 2 2       ; R8 := R8(R9)
124 [-]: GETUPVAL  R9 U12       ; R9 := U12
125 [-]: GETUPVAL  R10 U13      ; R10 := U13
126 [-]: MUL       R9 R9 R10    ; R9 := R9 * R10
127 [-]: DIV       R8 R8 R9     ; R8 := R8 / R9
128 [-]: SUB       R8 K5 R8     ; R8 := 1.000000 - R8
129 [-]: CALL      R6 3 2       ; R6 := R6(R7,R8)
130 [-]: GETUPVAL  R7 U8        ; R7 := U8
131 [-]: GETUPVAL  R8 U9        ; R8 := U9
132 [-]: EQ        1 R8 K15     ; if R8 == nil then PC := 135
133 [-]: JMP       135          ; PC := 135
134 [-]: ADD       R7 R7 K5     ; R7 := R7 + 1.000000
135 [-]: GETUPVAL  R8 U9        ; R8 := U9
136 [-]: TEST      R8 0         ; if not R8 then PC := 211
137 [-]: JMP       211          ; PC := 211
138 [-]: GETGLOBAL R8 K13       ; R8 := _T
139 [-]: GETTABLE  R8 R8 K26    ; R8 := R8["FishingSpearRecalled"]
140 [-]: TEST      R8 1         ; if R8 then PC := 211
141 [-]: JMP       211          ; PC := 211
142 [-]: GETGLOBAL R8 K23       ; R8 := 0x5bced4c4
143 [-]: GETTABLE  R8 R8 K27    ; R8 := R8[0xac1b386a]
144 [-]: LOADK     R9 1         ; R9 := 1.000000
145 [-]: GETUPVAL  R10 U14      ; R10 := U14
146 [-]: DIV       R10 R6 R10   ; R10 := R6 / R10
147 [-]: CALL      R8 3 2       ; R8 := R8(R9,R10)
148 [-]: MOVE      R6 R8        ; R6 := R8
149 [-]: LT        0 K4 R6      ; if 0.000000 >= R6 then PC := 198
150 [-]: JMP       198          ; PC := 198
151 [-]: EQ        0 R6 K5      ; if R6 ~= 1.000000 then PC := 188
152 [-]: JMP       188          ; PC := 188
153 [-]: LOADBOOL  R8 0 0       ; R8 := false
154 [-]: TEST      R8 0         ; if not R8 then PC := 188
155 [-]: JMP       188          ; PC := 188
156 [-]: GETGLOBAL R8 K1        ; R8 := 0xae91e43b
157 [-]: SELF      R8 R8 K12    ; R9 := R8; R8 := R8[0x67bc869f]
158 [-]: LOADK     R10 K28      ; R10 := "Gauge.Flair"
159 [-]: LOADK     R11 1        ; R11 := 1.000000
160 [-]: GETUPVAL  R12 U15      ; R12 := U15
161 [-]: CALL      R8 5 1       ; R8(R9,R10,R11,R12)
162 [-]: GETGLOBAL R8 K1        ; R8 := 0xae91e43b
163 [-]: SELF      R8 R8 K12    ; R9 := R8; R8 := R8[0x67bc869f]
164 [-]: LOADK     R10 K28      ; R10 := "Gauge.Flair"
165 [-]: LOADK     R11 10       ; R11 := 10.000000
166 [-]: LOADK     R12 100      ; R12 := 100.000000
167 [-]: CALL      R8 5 1       ; R8(R9,R10,R11,R12)
168 [-]: GETGLOBAL R8 K19       ; R8 := 0x25312c9b
169 [-]: GETGLOBAL R9 K1        ; R9 := 0xae91e43b
170 [-]: LOADK     R10 K28      ; R10 := "Gauge.Flair"
171 [-]: LOADK     R11 8        ; R11 := 8.000000
172 [-]: NEWTABLE  R12 2 0      ; R12 := {}
173 [-]: LOADK     R13 1        ; R13 := 1.000000
174 [-]: LOADK     R14 10       ; R14 := 10.000000
175 [-]: SETLIST   R12 2 1      ; R12[(1-1)*FPF+i] := R(12+i), 1 <= i <= 2
176 [-]: NEWTABLE  R13 2 0      ; R13 := {}
177 [-]: GETUPVAL  R14 U15      ; R14 := U15
178 [-]: SUB       R14 R14 K29  ; R14 := R14 - 30.000000
179 [-]: LOADK     R15 0        ; R15 := 0.000000
180 [-]: SETLIST   R13 2 1      ; R13[(1-1)*FPF+i] := R(13+i), 1 <= i <= 2
181 [-]: LOADK     R14 1        ; R14 := 1.000000
182 [-]: LOADK     R15 0        ; R15 := 0.000000
183 [-]: CALL      R8 8 1       ; R8(R9,R10,R11,R12,R13,R14,R15)
184 [-]: GETUPVAL  R8 U16       ; R8 := U16
185 [-]: GETTABLE  R8 R8 K30    ; R8 := R8[0x659d451f]
186 [-]: GETGLOBAL R9 K31       ; R9 := 0xbdf267f3
187 [-]: CALL      R8 2 1       ; R8(R9)
188 [-]: LOADK     R6 1         ; R6 := 1.000000
189 [-]: GETUPVAL  R8 U16       ; R8 := U16
190 [-]: GETTABLE  R8 R8 K32    ; R8 := R8[0x310355a7]
191 [-]: GETGLOBAL R9 K1        ; R9 := 0xae91e43b
192 [-]: LOADK     R10 K33      ; R10 := "Gauge.Target"
193 [-]: GETGLOBAL R11 K34      ; R11 := 0x2beb0e3e
194 [-]: LOADK     R12 0        ; R12 := 0.000000
195 [-]: LOADK     R13 0        ; R13 := 0.000000
196 [-]: CALL      R8 6 2       ; R8 := R8(R9,R10,R11,R12,R13)
197 [-]: SETUPVAL  R8 U17       ; U82 := R17
198 [-]: GETGLOBAL R8 K13       ; R8 := _T
199 [-]: SETTABLE  R8 K35 R6    ; R8["FishingMinigameResult"] := R6
200 [-]: GETGLOBAL R8 K13       ; R8 := _T
201 [-]: SETTABLE  R8 K36 K37   ; R8["FishingMinigameResultAcknowledged"] := false
202 [-]: GETGLOBAL R8 K13       ; R8 := _T
203 [-]: GETUPVAL  R9 U5        ; R9 := U5
204 [-]: LE        1 R9 R7      ; if R9 <= R7 then PC := 207
205 [-]: JMP       207          ; PC := 207
206 [-]: LOADBOOL  R9 0 1       ; R9 := false; PC := 207
207 [-]: LOADBOOL  R9 1 0       ; R9 := true
208 [-]: SETTABLE  R8 K38 R9    ; R8["FishingMinigameFinalResult"] := R9
209 [-]: LOADBOOL  R8 0 0       ; R8 := false
210 [-]: SETUPVAL  R8 U9        ; U82 := R9
211 [-]: GETGLOBAL R8 K13       ; R8 := _T
212 [-]: GETTABLE  R8 R8 K36    ; R8 := R8["FishingMinigameResultAcknowledged"]
213 [-]: EQ        0 R8 K37     ; if R8 ~= false then PC := 216
214 [-]: JMP       216          ; PC := 216
215 [-]: JMP       319          ; PC := 319
216 [-]: GETGLOBAL R8 K13       ; R8 := _T
217 [-]: GETTABLE  R8 R8 K36    ; R8 := R8["FishingMinigameResultAcknowledged"]
218 [-]: EQ        0 R8 K39     ; if R8 ~= true then PC := 227
219 [-]: JMP       227          ; PC := 227
220 [-]: GETUPVAL  R8 U18       ; R8 := U18
221 [-]: CALL      R8 1 1       ; R8()
222 [-]: GETGLOBAL R8 K13       ; R8 := _T
223 [-]: SETTABLE  R8 K36 K15   ; R8["FishingMinigameResultAcknowledged"] := nil
224 [-]: LOADNIL   R8 R8        ; R8 := nil
225 [-]: SETUPVAL  R8 U9        ; U82 := R9
226 [-]: JMP       319          ; PC := 319
227 [-]: GETGLOBAL R8 K13       ; R8 := _T
228 [-]: GETTABLE  R8 R8 K26    ; R8 := R8["FishingSpearRecalled"]
229 [-]: TEST      R8 1         ; if R8 then PC := 235
230 [-]: JMP       235          ; PC := 235
231 [-]: GETUPVAL  R8 U8        ; R8 := U8
232 [-]: GETUPVAL  R9 U5        ; R9 := U5
233 [-]: LE        0 R9 R8      ; if R9 > R8 then PC := 257
234 [-]: JMP       257          ; PC := 257
235 [-]: LOADBOOL  R8 0 0       ; R8 := false
236 [-]: SETUPVAL  R8 U10       ; U82 := R10
237 [-]: GETGLOBAL R8 K19       ; R8 := 0x25312c9b
238 [-]: GETGLOBAL R9 K1        ; R9 := 0xae91e43b
239 [-]: LOADK     R10 K20      ; R10 := "Gauge"
240 [-]: LOADK     R11 2        ; R11 := 2.000000
241 [-]: NEWTABLE  R12 1 0      ; R12 := {}
242 [-]: LOADK     R13 10       ; R13 := 10.000000
243 [-]: SETLIST   R12 1 1      ; R12[(1-1)*FPF+i] := R(12+i), 1 <= i <= 1
244 [-]: NEWTABLE  R13 1 0      ; R13 := {}
245 [-]: LOADK     R14 0        ; R14 := 0.000000
246 [-]: SETLIST   R13 1 1      ; R13[(1-1)*FPF+i] := R(13+i), 1 <= i <= 1
247 [-]: LOADK     R14 K40      ; R14 := 0.150000
248 [-]: LOADK     R15 1        ; R15 := 1.000000
249 [-]: CLOSURE   R16 0        ; R16 := closure(Function #7.1)
250 [-]: GETUPVAL  R0 U17       ; R0 := U17
251 [-]: CALL      R8 9 1       ; R8(R9,R10,R11,R12,R13,R14,R15,R16)
252 [-]: GETGLOBAL R8 K13       ; R8 := _T
253 [-]: SETTABLE  R8 K41 K39   ; R8["FishingMinigameComplete"] := true
254 [-]: GETGLOBAL R8 K13       ; R8 := _T
255 [-]: SETTABLE  R8 K36 K15   ; R8["FishingMinigameResultAcknowledged"] := nil
256 [-]: JMP       319          ; PC := 319
257 [-]: GETUPVAL  R8 U7        ; R8 := U7
258 [-]: GETUPVAL  R9 U19       ; R9 := U19
259 [-]: MUL       R9 R9 R7     ; R9 := R9 * R7
260 [-]: ADD       R9 R9 K5     ; R9 := R9 + 1.000000
261 [-]: GETUPVAL  R10 U4       ; R10 := U4
262 [-]: MUL       R9 R9 R10    ; R9 := R9 * R10
263 [-]: GETGLOBAL R10 K3       ; R10 := 0x67652851
264 [-]: CALL      R10 1 2      ; R10 := R10()
265 [-]: MUL       R9 R9 R10    ; R9 := R9 * R10
266 [-]: ADD       R8 R8 R9     ; R8 := R8 + R9
267 [-]: SETUPVAL  R8 U7        ; U82 := R7
268 [-]: GETUPVAL  R8 U7        ; R8 := U7
269 [-]: GETUPVAL  R9 U13       ; R9 := U13
270 [-]: LT        0 R9 R8      ; if R9 >= R8 then PC := 284
271 [-]: JMP       284          ; PC := 284
272 [-]: GETUPVAL  R8 U20       ; R8 := U20
273 [-]: TEST      R8 0         ; if not R8 then PC := 281
274 [-]: JMP       281          ; PC := 281
275 [-]: GETUPVAL  R8 U9        ; R8 := U9
276 [-]: EQ        0 R8 K15     ; if R8 ~= nil then PC := 281
277 [-]: JMP       281          ; PC := 281
278 [-]: LOADBOOL  R8 1 0       ; R8 := true
279 [-]: SETUPVAL  R8 U9        ; U82 := R9
280 [-]: JMP       298          ; PC := 298
281 [-]: GETUPVAL  R8 U18       ; R8 := U18
282 [-]: CALL      R8 1 1       ; R8()
283 [-]: JMP       298          ; PC := 298
284 [-]: GETUPVAL  R8 U7        ; R8 := U7
285 [-]: LT        0 R8 K4      ; if R8 >= 0.000000 then PC := 298
286 [-]: JMP       298          ; PC := 298
287 [-]: GETUPVAL  R8 U20       ; R8 := U20
288 [-]: TEST      R8 0         ; if not R8 then PC := 296
289 [-]: JMP       296          ; PC := 296
290 [-]: GETUPVAL  R8 U9        ; R8 := U9
291 [-]: EQ        0 R8 K15     ; if R8 ~= nil then PC := 296
292 [-]: JMP       296          ; PC := 296
293 [-]: LOADBOOL  R8 1 0       ; R8 := true
294 [-]: SETUPVAL  R8 U9        ; U82 := R9
295 [-]: JMP       298          ; PC := 298
296 [-]: GETUPVAL  R8 U18       ; R8 := U18
297 [-]: CALL      R8 1 1       ; R8()
298 [-]: GETUPVAL  R8 U9        ; R8 := U9
299 [-]: EQ        1 R8 K15     ; if R8 == nil then PC := 304
300 [-]: JMP       304          ; PC := 304
301 [-]: GETUPVAL  R8 U9        ; R8 := U9
302 [-]: EQ        0 R8 K39     ; if R8 ~= true then PC := 319
303 [-]: JMP       319          ; PC := 319
304 [-]: GETUPVAL  R8 U21       ; R8 := U21
305 [-]: GETUPVAL  R9 U22       ; R9 := U22
306 [-]: GETUPVAL  R10 U21      ; R10 := U21
307 [-]: SUB       R9 R9 R10    ; R9 := R9 - R10
308 [-]: GETUPVAL  R10 U7       ; R10 := U7
309 [-]: GETUPVAL  R11 U13      ; R11 := U13
310 [-]: DIV       R10 R10 R11  ; R10 := R10 / R11
311 [-]: MUL       R9 R9 R10    ; R9 := R9 * R10
312 [-]: ADD       R8 R8 R9     ; R8 := R8 + R9
313 [-]: GETGLOBAL R9 K1        ; R9 := 0xae91e43b
314 [-]: SELF      R9 R9 K12    ; R10 := R9; R9 := R9[0x67bc869f]
315 [-]: LOADK     R11 K33      ; R11 := "Gauge.Target"
316 [-]: LOADK     R12 0        ; R12 := 0.000000
317 [-]: MOVE      R13 R8       ; R13 := R8
318 [-]: CALL      R9 5 1       ; R9(R10,R11,R12,R13)
319 [-]: GETGLOBAL R9 K0        ; R9 := 0x7b998233
320 [-]: GETUPVAL  R10 U17      ; R10 := U17
321 [-]: CALL      R9 2 2       ; R9 := R9(R10)
322 [-]: TEST      R9 1         ; if R9 then PC := 337
323 [-]: JMP       337          ; PC := 337
324 [-]: GETGLOBAL R9 K0        ; R9 := 0x7b998233
325 [-]: GETUPVAL  R10 U17      ; R10 := U17
326 [-]: GETTABLE  R10 R10 K42  ; R10 := R10["mInstance"]
327 [-]: CALL      R9 2 2       ; R9 := R9(R10)
328 [-]: TEST      R9 0         ; if not R9 then PC := 333
329 [-]: JMP       333          ; PC := 333
330 [-]: LOADNIL   R9 R9        ; R9 := nil
331 [-]: SETUPVAL  R9 U17       ; U82 := R17
332 [-]: JMP       337          ; PC := 337
333 [-]: GETUPVAL  R9 U17       ; R9 := U17
334 [-]: SELF      R9 R9 K43    ; R10 := R9; R9 := R9[0xfaa69527]
335 [-]: LOADK     R11 0        ; R11 := 0.000000
336 [-]: CALL      R9 3 1       ; R9(R10,R11)
337 [-]: RETURN    R0 1         ; return 


; Function #7.1:
;
; Name:            
; Defined at line: 227
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: SETUPVAL  R0 U0        ; U82 := R0
  2 [-]: RETURN    R0 1         ; return 


; Function #8:
;
; Name:            
; Defined at line: 262
; #Upvalues:       2
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  6

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETTABLE  R1 R1 K0     ; R1 := R1[0xfa221145]
  3 [-]: GETGLOBAL R2 K1        ; R2 := 0xae91e43b
  4 [-]: GETGLOBAL R3 K2        ; R3 := 0x03f57322
  5 [-]: MOVE      R4 R0        ; R4 := R0
  6 [-]: CALL      R3 2 0       ; R3,... := R3(R4)
  7 [-]: CALL      R1 0 1       ; R1(R2,...)
  8 [-]: GETGLOBAL R1 K3        ; R1 := 0x7b998233
  9 [-]: GETUPVAL  R2 U1        ; R2 := U1
 10 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 11 [-]: TEST      R1 1         ; if R1 then PC := 20
 12 [-]: JMP       20           ; PC := 20
 13 [-]: GETUPVAL  R1 U1        ; R1 := U1
 14 [-]: SELF      R1 R1 K4     ; R2 := R1; R1 := R1[0xe4162eed]
 15 [-]: LOADK     R3 K5        ; R3 := "HandleHudScale"
 16 [-]: GETGLOBAL R4 K6        ; R4 := 0x64fb1586
 17 [-]: MOVE      R5 R0        ; R5 := R0
 18 [-]: CALL      R4 2 0       ; R4,... := R4(R5)
 19 [-]: CALL      R1 0 1       ; R1(R2,...)
 20 [-]: RETURN    R0 1         ; return 


