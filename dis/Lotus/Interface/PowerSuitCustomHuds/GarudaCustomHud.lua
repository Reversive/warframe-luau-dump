; This file has been disassembled using luadec 2.0.2 by sztupy (modified by Reversive)

; Name:            
; Defined at line: 0
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       2
; Max Stack Size:  28

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x2d0fad09
  2 [-]: LOADK     R1 K1        ; R1 := "Lotus.Interface.UIUtilities"
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K0        ; R1 := 0x2d0fad09
  5 [-]: LOADK     R2 K2        ; R2 := "EE.Interface.Utilities"
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: LOADNIL   R2 R2        ; R2 := nil
  8 [-]: LOADKB    R3 0 0       ; R3 := false
  9 [-]: LOADNIL   R4 R5        ; R4 := R5 := nil
 10 [-]: NEWTABLE  R6 0 0       ; R6 := {}
 11 [-]: LOADKB    R7 0 0       ; R7 := false
 12 [-]: LOADNIL   R8 R12       ; R8 := R9 := R10 := R11 := R12 := nil
 13 [-]: GETGLOBAL R13 K3       ; R13 := 0x78ca68a2
 14 [-]: CONST     R14 0        ; R14 := 0.000000
 15 [-]: LOADK     R15 K4       ; R15 := 0.100000
 16 [-]: CALL      R13 3 2      ; R13 := R13(R14,R15)
 17 [-]: CONST     R14 1        ; R14 := 1.000000
 18 [-]: GETGLOBAL R15 K3       ; R15 := 0x78ca68a2
 19 [-]: CONST     R16 0        ; R16 := 0.000000
 20 [-]: LOADK     R17 K4       ; R17 := 0.100000
 21 [-]: CALL      R15 3 2      ; R15 := R15(R16,R17)
 22 [-]: LOADKB    R16 0 0      ; R16 := false
 23 [-]: CONST     R17 0        ; R17 := 0.000000
 24 [-]: GETGLOBAL R18 K5       ; R18 := 0xb7cbd06b
 25 [-]: LOADK     R19 K6       ; R19 := 0.130000
 26 [-]: LOADK     R20 K7       ; R20 := 0.850000
 27 [-]: CALL      R18 3 2      ; R18 := R18(R19,R20)
 28 [-]: CLOSURE   R19 0        ; R19 := closure(Function #1)
 29 [-]: MOVE      R0 R15       ; R0 := R15
 30 [-]: CLOSURE   R20 1        ; R20 := closure(Function #2)
 31 [-]: CLOSURE   R21 2        ; R21 := closure(Function #3)
 32 [-]: MOVE      R0 R3        ; R0 := R3
 33 [-]: MOVE      R0 R2        ; R0 := R2
 34 [-]: MOVE      R0 R4        ; R0 := R4
 35 [-]: MOVE      R0 R11       ; R0 := R11
 36 [-]: MOVE      R0 R12       ; R0 := R12
 37 [-]: MOVE      R0 R15       ; R0 := R15
 38 [-]: MOVE      R0 R7        ; R0 := R7
 39 [-]: MOVE      R0 R6        ; R0 := R6
 40 [-]: MOVE      R0 R13       ; R0 := R13
 41 [-]: MOVE      R0 R18       ; R0 := R18
 42 [-]: MOVE      R0 R16       ; R0 := R16
 43 [-]: MOVE      R0 R0        ; R0 := R0
 44 [-]: MOVE      R0 R17       ; R0 := R17
 45 [-]: MOVE      R0 R5        ; R0 := R5
 46 [-]: MOVE      R0 R8        ; R0 := R8
 47 [-]: SETGLOBAL R21 K8       ; Update := R21
 48 [-]: CLOSURE   R21 3        ; R21 := closure(Function #4)
 49 [-]: MOVE      R0 R9        ; R0 := R9
 50 [-]: CLOSURE   R22 4        ; R22 := closure(Function #5)
 51 [-]: MOVE      R0 R10       ; R0 := R10
 52 [-]: MOVE      R0 R21       ; R0 := R21
 53 [-]: MOVE      R0 R9        ; R0 := R9
 54 [-]: CLOSURE   R23 5        ; R23 := closure(Function #6)
 55 [-]: MOVE      R0 R20       ; R0 := R20
 56 [-]: SETGLOBAL R23 K9       ; OnProfileSaved := R23
 57 [-]: CLOSURE   R23 6        ; R23 := closure(Function #7)
 58 [-]: MOVE      R0 R7        ; R0 := R7
 59 [-]: MOVE      R0 R6        ; R0 := R6
 60 [-]: CLOSURE   R24 7        ; R24 := closure(Function #8)
 61 [-]: SETGLOBAL R24 K10      ; Shutdown := R24
 62 [-]: CLOSURE   R24 8        ; R24 := closure(Function #9)
 63 [-]: MOVE      R0 R12       ; R0 := R12
 64 [-]: MOVE      R0 R1        ; R0 := R1
 65 [-]: CLOSURE   R25 9        ; R25 := closure(Function #10)
 66 [-]: MOVE      R0 R1        ; R0 := R1
 67 [-]: CLOSURE   R26 10       ; R26 := closure(Function #11)
 68 [-]: MOVE      R0 R13       ; R0 := R13
 69 [-]: CLOSURE   R27 11       ; R27 := closure(Function #12)
 70 [-]: MOVE      R0 R2        ; R0 := R2
 71 [-]: MOVE      R0 R8        ; R0 := R8
 72 [-]: MOVE      R0 R5        ; R0 := R5
 73 [-]: MOVE      R0 R23       ; R0 := R23
 74 [-]: MOVE      R0 R24       ; R0 := R24
 75 [-]: MOVE      R0 R25       ; R0 := R25
 76 [-]: MOVE      R0 R19       ; R0 := R19
 77 [-]: MOVE      R0 R26       ; R0 := R26
 78 [-]: MOVE      R0 R20       ; R0 := R20
 79 [-]: MOVE      R0 R22       ; R0 := R22
 80 [-]: MOVE      R0 R17       ; R0 := R17
 81 [-]: MOVE      R0 R3        ; R0 := R3
 82 [-]: SETGLOBAL R27 K11      ; Initialize := R27
 83 [-]: CLOSURE   R27 12       ; R27 := closure(Function #13)
 84 [-]: MOVE      R0 R14       ; R0 := R14
 85 [-]: MOVE      R0 R0        ; R0 := R0
 86 [-]: SETGLOBAL R27 K12      ; HandleHudScale := R27
 87 [-]: RETURN    R0 1         ; return 


; Function #1:
;
; Name:            
; Defined at line: 27
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: SELF      R1 R1 K0     ; R2 := R1; R1 := R1[0x188e2bee]
  3 [-]: MOVE      R3 R0        ; R3 := R0
  4 [-]: CALL      R1 3 1       ; R1(R2,R3)
  5 [-]: RETURN    R0 1         ; return 


; Function #2:
;
; Name:            
; Defined at line: 31
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  8

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x76ea806b
  2 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x3f3ae64c]
  3 [-]: CONST     R2 0         ; R2 := 0.000000
  4 [-]: CALL      R0 3 2       ; R0 := R0(R1,R2)
  5 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
  6 [-]: MOVE      R2 R0        ; R2 := R0
  7 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  8 [-]: TEST      R1 0         ; if not R1 then PC := 11
  9 [-]: JMP       11           ; PC := 11
 10 [-]: RETURN    R0 1         ; return 
 11 [-]: SELF      R1 R0 K3     ; R2 := R0; R1 := R0[0x40e9c32b]
 12 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 13 [-]: GETGLOBAL R2 K2        ; R2 := 0x7b998233
 14 [-]: MOVE      R3 R1        ; R3 := R1
 15 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 16 [-]: TEST      R2 0         ; if not R2 then PC := 19
 17 [-]: JMP       19           ; PC := 19
 18 [-]: RETURN    R0 1         ; return 
 19 [-]: SELF      R2 R1 K4     ; R3 := R1; R2 := R1[0xef9a3ee6]
 20 [-]: CONST     R4 17        ; R4 := 17.000000
 21 [-]: CALL      R2 3 2       ; R2 := R2(R3,R4)
 22 [-]: GETGLOBAL R3 K6        ; R3 := 0xae91e43b
 23 [-]: SELF      R3 R3 K7     ; R4 := R3; R3 := R3[0x67bc869f]
 24 [-]: LOADK     R5 K8        ; R5 := "PassiveContainer.Label"
 25 [-]: CONST     R6 36        ; R6 := 36.000000
 26 [-]: MOVE      R7 R2        ; R7 := R2
 27 [-]: CALL      R3 5 1       ; R3(R4,R5,R6,R7)
 28 [-]: GETGLOBAL R3 K6        ; R3 := 0xae91e43b
 29 [-]: SELF      R3 R3 K7     ; R4 := R3; R3 := R3[0x67bc869f]
 30 [-]: LOADK     R5 K9        ; R5 := "PassiveContainer.Bg"
 31 [-]: CONST     R6 9         ; R6 := 9.000000
 32 [-]: GETGLOBAL R7 K10       ; R7 := 0x0032441c
 33 [-]: GETTABLE  R7 R7 K11    ; R7 := R7["UIColor_Black"]
 34 [-]: CALL      R3 5 1       ; R3(R4,R5,R6,R7)
 35 [-]: GETGLOBAL R3 K6        ; R3 := 0xae91e43b
 36 [-]: SELF      R3 R3 K7     ; R4 := R3; R3 := R3[0x67bc869f]
 37 [-]: LOADK     R5 K12       ; R5 := "PassiveContainer.FillBg"
 38 [-]: CONST     R6 9         ; R6 := 9.000000
 39 [-]: LOADK     R7 K13       ; R7 := 9013641.000000
 40 [-]: CALL      R3 5 1       ; R3(R4,R5,R6,R7)
 41 [-]: GETGLOBAL R3 K6        ; R3 := 0xae91e43b
 42 [-]: SELF      R3 R3 K7     ; R4 := R3; R3 := R3[0x67bc869f]
 43 [-]: LOADK     R5 K12       ; R5 := "PassiveContainer.FillBg"
 44 [-]: CONST     R6 10        ; R6 := 10.000000
 45 [-]: CONST     R7 25        ; R7 := 25.000000
 46 [-]: CALL      R3 5 1       ; R3(R4,R5,R6,R7)
 47 [-]: GETGLOBAL R3 K6        ; R3 := 0xae91e43b
 48 [-]: SELF      R3 R3 K7     ; R4 := R3; R3 := R3[0x67bc869f]
 49 [-]: LOADK     R5 K14       ; R5 := "PassiveContainer.Glow"
 50 [-]: CONST     R6 10        ; R6 := 10.000000
 51 [-]: CONST     R7 70        ; R7 := 70.000000
 52 [-]: CALL      R3 5 1       ; R3(R4,R5,R6,R7)
 53 [-]: RETURN    R0 1         ; return 


; Function #3:
;
; Name:            
; Defined at line: 50
; #Upvalues:       15
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  17

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
  2 [-]: GETGLOBAL R1 K1        ; R1 := 0xae91e43b
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: TEST      R0 1         ; if R0 then PC := 9
  5 [-]: JMP       9            ; PC := 9
  6 [-]: GETUPVAL  R0 U0        ; R0 := U0
  7 [-]: TEST      R0 1         ; if R0 then PC := 10
  8 [-]: JMP       10           ; PC := 10
  9 [-]: RETURN    R0 1         ; return 
 10 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
 11 [-]: GETUPVAL  R1 U1        ; R1 := U1
 12 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 13 [-]: TEST      R0 1         ; if R0 then PC := 20
 14 [-]: JMP       20           ; PC := 20
 15 [-]: GETUPVAL  R0 U1        ; R0 := U1
 16 [-]: SELF      R0 R0 K2     ; R1 := R0; R0 := R0[0xfaa69527]
 17 [-]: GETGLOBAL R2 K3        ; R2 := 0x67652851
 18 [-]: CALL      R2 1 0       ; R2,... := R2()
 19 [-]: CALL      R0 0 1       ; R0(R1,...)
 20 [-]: GETGLOBAL R0 K3        ; R0 := 0x67652851
 21 [-]: CALL      R0 1 2       ; R0 := R0()
 22 [-]: GETGLOBAL R1 K1        ; R1 := 0xae91e43b
 23 [-]: SELF      R1 R1 K4     ; R2 := R1; R1 := R1[0x8a8c8d5a]
 24 [-]: MOVE      R3 R0        ; R3 := R0
 25 [-]: CALL      R1 3 1       ; R1(R2,R3)
 26 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 27 [-]: GETUPVAL  R2 U2        ; R2 := U2
 28 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 29 [-]: TEST      R1 0         ; if not R1 then PC := 35
 30 [-]: JMP       35           ; PC := 35
 31 [-]: GETGLOBAL R1 K5        ; R1 := 0xbe190284
 32 [-]: SELF      R1 R1 K6     ; R2 := R1; R1 := R1[0x33307f92]
 33 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 34 [-]: SETUPVAL  R1 U2        ; U82 := R2
 35 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 36 [-]: GETUPVAL  R2 U2        ; R2 := U2
 37 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 38 [-]: TEST      R1 1         ; if R1 then PC := 55
 39 [-]: JMP       55           ; PC := 55
 40 [-]: GETUPVAL  R1 U2        ; R1 := U2
 41 [-]: SELF      R1 R1 K7     ; R2 := R1; R1 := R1[0x91a24e4b]
 42 [-]: LOADK     R3 K8        ; R3 := "_root"
 43 [-]: CONST     R4 10        ; R4 := 10.000000
 44 [-]: CALL      R1 4 2       ; R1 := R1(R2,R3,R4)
 45 [-]: GETUPVAL  R2 U3        ; R2 := U3
 46 [-]: EQ        1 R2 R1      ; if R2 == R1 then PC := 55
 47 [-]: JMP       55           ; PC := 55
 48 [-]: SETUPVAL  R1 U3        ; U82 := R3
 49 [-]: GETGLOBAL R2 K1        ; R2 := 0xae91e43b
 50 [-]: SELF      R2 R2 K9     ; R3 := R2; R2 := R2[0x67bc869f]
 51 [-]: LOADK     R4 K8        ; R4 := "_root"
 52 [-]: CONST     R5 10        ; R5 := 10.000000
 53 [-]: MOVE      R6 R1        ; R6 := R1
 54 [-]: CALL      R2 5 1       ; R2(R3,R4,R5,R6)
 55 [-]: GETGLOBAL R2 K0        ; R2 := 0x7b998233
 56 [-]: GETUPVAL  R3 U4        ; R3 := U4
 57 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 58 [-]: TEST      R2 1         ; if R2 then PC := 90
 59 [-]: JMP       90           ; PC := 90
 60 [-]: GETUPVAL  R2 U5        ; R2 := U5
 61 [-]: SELF      R2 R2 K2     ; R3 := R2; R2 := R2[0xfaa69527]
 62 [-]: GETGLOBAL R4 K3        ; R4 := 0x67652851
 63 [-]: CALL      R4 1 0       ; R4,... := R4()
 64 [-]: CALL      R2 0 1       ; R2(R3,...)
 65 [-]: GETUPVAL  R2 U4        ; R2 := U4
 66 [-]: SELF      R2 R2 K10    ; R3 := R2; R2 := R2[0xd1586535]
 67 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 68 [-]: GETTABLE  R3 R2 K11    ; R3 := R2["y"]
 69 [-]: GETUPVAL  R4 U5        ; R4 := U5
 70 [-]: SELF      R4 R4 K12    ; R5 := R4; R4 := R4[0x54ab95f9]
 71 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 72 [-]: ADD       R3 R3 R4     ; R3 := R3 + R4
 73 [-]: SETTABLE  R2 K11 R3    ; R2["y"] := R3
 74 [-]: GETGLOBAL R3 K1        ; R3 := 0xae91e43b
 75 [-]: SELF      R3 R3 K13    ; R4 := R3; R3 := R3[0x28209ddc]
 76 [-]: MOVE      R5 R2        ; R5 := R2
 77 [-]: CALL      R3 3 2       ; R3 := R3(R4,R5)
 78 [-]: GETGLOBAL R4 K1        ; R4 := 0xae91e43b
 79 [-]: SELF      R4 R4 K9     ; R5 := R4; R4 := R4[0x67bc869f]
 80 [-]: LOADK     R6 K14       ; R6 := "ShieldCharge"
 81 [-]: CONST     R7 0         ; R7 := 0.000000
 82 [-]: GETTABLE  R8 R3 K15    ; R8 := R3["x"]
 83 [-]: CALL      R4 5 1       ; R4(R5,R6,R7,R8)
 84 [-]: GETGLOBAL R4 K1        ; R4 := 0xae91e43b
 85 [-]: SELF      R4 R4 K9     ; R5 := R4; R4 := R4[0x67bc869f]
 86 [-]: LOADK     R6 K14       ; R6 := "ShieldCharge"
 87 [-]: CONST     R7 1         ; R7 := 1.000000
 88 [-]: GETTABLE  R8 R3 K11    ; R8 := R3["y"]
 89 [-]: CALL      R4 5 1       ; R4(R5,R6,R7,R8)
 90 [-]: LOADKB    R4 1 0       ; R4 := true
 91 [-]: SETUPVAL  R4 U6        ; U82 := R6
 92 [-]: GETUPVAL  R4 U7        ; R4 := U7
 93 [-]: LEN       R4 R4        ; R4 := # R4
 94 [-]: LT        0 K16 R4     ; if 0.000000 >= R4 then PC := 114
 95 [-]: JMP       114          ; PC := 114
 96 [-]: CONST     R4 1         ; R4 := 1.000000
 97 [-]: GETUPVAL  R5 U7        ; R5 := U7
 98 [-]: LEN       R5 R5        ; R5 := # R5
 99 [-]: CONST     R6 1         ; R6 := 1.000000
100 [-]: FORPREP   R4 111       ; R4 -= R6; PC := 111
101 [-]: GETUPVAL  R8 U7        ; R8 := U7
102 [-]: GETTABLE  R8 R8 R7     ; R8 := R8[R7]
103 [-]: GETTABLE  R8 R8 K17    ; R8 := R8[1.000000]
104 [-]: GETUPVAL  R9 U7        ; R9 := U7
105 [-]: GETTABLE  R9 R9 R7     ; R9 := R9[R7]
106 [-]: GETTABLE  R9 R9 K18    ; R9 := R9[2.000000]
107 [-]: GETUPVAL  R10 U7       ; R10 := U7
108 [-]: GETTABLE  R10 R10 R7   ; R10 := R10[R7]
109 [-]: GETTABLE  R10 R10 K19  ; R10 := R10[3.000000]
110 [-]: CALL      R8 3 1       ; R8(R9,R10)
111 [-]: FORLOOP   R4 101       ; R4 += R6; if R4 <= R5 then begin PC := 101; R7 := R4 end
112 [-]: NEWTABLE  R8 0 0       ; R8 := {}
113 [-]: SETUPVAL  R8 U7        ; U82 := R7
114 [-]: LOADKB    R8 0 0       ; R8 := false
115 [-]: SETUPVAL  R8 U6        ; U82 := R6
116 [-]: GETUPVAL  R8 U8        ; R8 := U8
117 [-]: SELF      R8 R8 K2     ; R9 := R8; R8 := R8[0xfaa69527]
118 [-]: MOVE      R10 R0       ; R10 := R0
119 [-]: CALL      R8 3 1       ; R8(R9,R10)
120 [-]: GETUPVAL  R8 U9        ; R8 := U9
121 [-]: SELF      R8 R8 K20    ; R9 := R8; R8 := R8[0x70596bfe]
122 [-]: GETUPVAL  R10 U8       ; R10 := U8
123 [-]: SELF      R10 R10 K12  ; R11 := R10; R10 := R10[0x54ab95f9]
124 [-]: CALL      R10 2 0      ; R10,... := R10(R11)
125 [-]: CALL      R8 0 2       ; R8 := R8(R9,...)
126 [-]: GETGLOBAL R9 K1        ; R9 := 0xae91e43b
127 [-]: SELF      R9 R9 K21    ; R10 := R9; R9 := R9[0x91e13703]
128 [-]: LOADK     R11 K22      ; R11 := "PassiveContainer.Fill"
129 [-]: LOADK     R12 K23      ; R12 := "VisibilitySize"
130 [-]: MOVE      R13 R8       ; R13 := R8
131 [-]: CONST     R14 0        ; R14 := 0.000000
132 [-]: CONST     R15 0        ; R15 := 0.000000
133 [-]: CONST     R16 0        ; R16 := 0.000000
134 [-]: CALL      R9 8 1       ; R9(R10,R11,R12,R13,R14,R15,R16)
135 [-]: GETUPVAL  R9 U11       ; R9 := U11
136 [-]: GETTABLE  R9 R9 K24    ; R9 := R9[0x0cad99b9]
137 [-]: GETGLOBAL R10 K1       ; R10 := 0xae91e43b
138 [-]: LOADK     R11 K25      ; R11 := "PassiveContainer"
139 [-]: GETUPVAL  R12 U12      ; R12 := U12
140 [-]: GETUPVAL  R13 U10      ; R13 := U10
141 [-]: GETUPVAL  R14 U13      ; R14 := U13
142 [-]: GETUPVAL  R15 U14      ; R15 := U14
143 [-]: CALL      R9 7 2       ; R9 := R9(R10,R11,R12,R13,R14,R15)
144 [-]: SETUPVAL  R9 U10       ; U82 := R10
145 [-]: RETURN    R0 1         ; return 


; Function #4:
;
; Name:            
; Defined at line: 104
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  4

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
  2 [-]: GETUPVAL  R1 U0        ; R1 := U0
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: TEST      R0 0         ; if not R0 then PC := 24
  5 [-]: JMP       24           ; PC := 24
  6 [-]: GETGLOBAL R0 K1        ; R0 := 0x89326c93
  7 [-]: SELF      R0 R0 K2     ; R1 := R0; R0 := R0[0x78298275]
  8 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  9 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 10 [-]: MOVE      R2 R0        ; R2 := R0
 11 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 12 [-]: TEST      R1 1         ; if R1 then PC := 24
 13 [-]: JMP       24           ; PC := 24
 14 [-]: SELF      R1 R0 K3     ; R2 := R0; R1 := R0[0xde321e6f]
 15 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 16 [-]: GETGLOBAL R2 K0        ; R2 := 0x7b998233
 17 [-]: MOVE      R3 R1        ; R3 := R1
 18 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 19 [-]: TEST      R2 1         ; if R2 then PC := 24
 20 [-]: JMP       24           ; PC := 24
 21 [-]: SELF      R2 R1 K4     ; R3 := R1; R2 := R1[0xf7d48ee0]
 22 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 23 [-]: SETUPVAL  R2 U0        ; U82 := R0
 24 [-]: RETURN    R0 1         ; return 


; Function #5:
;
; Name:            
; Defined at line: 116
; #Upvalues:       3
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  3

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x60130201
  2 [-]: CALL      R0 1 2       ; R0 := R0()
  3 [-]: SETUPVAL  R0 U0        ; U82 := R0
  4 [-]: GETUPVAL  R0 U1        ; R0 := U1
  5 [-]: CALL      R0 1 1       ; R0()
  6 [-]: GETGLOBAL R0 K1        ; R0 := 0x7b998233
  7 [-]: GETUPVAL  R1 U2        ; R1 := U2
  8 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  9 [-]: TEST      R0 1         ; if R0 then PC := 15
 10 [-]: JMP       15           ; PC := 15
 11 [-]: GETUPVAL  R0 U2        ; R0 := U2
 12 [-]: SELF      R0 R0 K2     ; R1 := R0; R0 := R0[0xa3ef5d65]
 13 [-]: GETUPVAL  R2 U0        ; R2 := U0
 14 [-]: CALL      R0 3 1       ; R0(R1,R2)
 15 [-]: RETURN    R0 1         ; return 


; Function #6:
;
; Name:            
; Defined at line: 126
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: CALL      R0 1 1       ; R0()
  3 [-]: RETURN    R0 1         ; return 


; Function #7:
;
; Name:            
; Defined at line: 130
; #Upvalues:       2
; #Parameters:     1
; Is_vararg:       3
; Max Stack Size:  7

  1 [-]: GETUPVAL  R2 U0        ; R2 := U0
  2 [-]: TEST      R2 0         ; if not R2 then PC := 8
  3 [-]: JMP       8            ; PC := 8
  4 [-]: MOVE      R2 R0        ; R2 := R0
  5 [-]: VARARG    R3 0         ; R3 := ...
  6 [-]: CALL      R2 0 1       ; R2(R3,...)
  7 [-]: JMP       16           ; PC := 16
  8 [-]: GETGLOBAL R2 K0        ; R2 := 0x33bdd652
  9 [-]: GETTABLE  R2 R2 K1     ; R2 := R2[0x23d5322f]
 10 [-]: GETUPVAL  R3 U1        ; R3 := U1
 11 [-]: NEWTABLE  R4 1 0       ; R4 := {}
 12 [-]: MOVE      R5 R0        ; R5 := R0
 13 [-]: VARARG    R6 0         ; R6 := ...
 14 [-]: SETLIST   R4 0 1       ; R4[(1-1)*FPF+i] := R(4+i), 1 <= i <= 0
 15 [-]: CALL      R2 3 1       ; R2(R3,R4)
 16 [-]: RETURN    R0 1         ; return 


; Function #8:
;
; Name:            
; Defined at line: 138
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: GETGLOBAL R0 K0        ; R0 := _T
  2 [-]: SETTABLE  R0 K1 K2     ; R0["GARUDA_ShowTalons"] := nil
  3 [-]: GETGLOBAL R0 K0        ; R0 := _T
  4 [-]: SETTABLE  R0 K3 K2     ; R0["GARUDA_ShowShieldCharge"] := nil
  5 [-]: GETGLOBAL R0 K0        ; R0 := _T
  6 [-]: SETTABLE  R0 K4 K2     ; R0["GARUDA_SetShieldCharge"] := nil
  7 [-]: GETGLOBAL R0 K0        ; R0 := _T
  8 [-]: SETTABLE  R0 K5 K2     ; R0["GARUDA_SetProjectileLabelOffset"] := nil
  9 [-]: GETGLOBAL R0 K0        ; R0 := _T
 10 [-]: SETTABLE  R0 K6 K2     ; R0["GARUDA_SetDamageBonus"] := nil
 11 [-]: GETGLOBAL R0 K7        ; R0 := 0x7b998233
 12 [-]: GETGLOBAL R1 K0        ; R1 := _T
 13 [-]: GETTABLE  R1 R1 K8     ; R1 := R1["HUD_RemoveMotionClip"]
 14 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 15 [-]: TEST      R0 1         ; if R0 then PC := 22
 16 [-]: JMP       22           ; PC := 22
 17 [-]: GETGLOBAL R0 K0        ; R0 := _T
 18 [-]: GETTABLE  R0 R0 K9     ; R0 := R0[0x564984b8]
 19 [-]: GETGLOBAL R1 K10       ; R1 := 0xae91e43b
 20 [-]: LOADK     R2 K11       ; R2 := "PassiveContainer"
 21 [-]: CALL      R0 3 1       ; R0(R1,R2)
 22 [-]: GETGLOBAL R0 K7        ; R0 := 0x7b998233
 23 [-]: GETGLOBAL R1 K0        ; R1 := _T
 24 [-]: GETTABLE  R1 R1 K12    ; R1 := R1["HUD_GetAnchorMgr"]
 25 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 26 [-]: TEST      R0 1         ; if R0 then PC := 37
 27 [-]: JMP       37           ; PC := 37
 28 [-]: GETGLOBAL R0 K0        ; R0 := _T
 29 [-]: GETTABLE  R0 R0 K13    ; R0 := R0[0xf6b77431]
 30 [-]: CALL      R0 1 2       ; R0 := R0()
 31 [-]: EQ        1 R0 K2      ; if R0 == nil then PC := 37
 32 [-]: JMP       37           ; PC := 37
 33 [-]: SELF      R1 R0 K14    ; R2 := R0; R1 := R0[0x7f19c438]
 34 [-]: GETGLOBAL R3 K10       ; R3 := 0xae91e43b
 35 [-]: LOADK     R4 K11       ; R4 := "PassiveContainer"
 36 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 37 [-]: RETURN    R0 1         ; return 


; Function #9:
;
; Name:            
; Defined at line: 157
; #Upvalues:       2
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  12

  1 [-]: GETGLOBAL R2 K0        ; R2 := 0x7b998233
  2 [-]: MOVE      R3 R1        ; R3 := R1
  3 [-]: CALL      R2 2 2       ; R2 := R2(R3)
  4 [-]: TEST      R2 1         ; if R2 then PC := 7
  5 [-]: JMP       7            ; PC := 7
  6 [-]: SETUPVAL  R1 U0        ; U82 := R0
  7 [-]: GETGLOBAL R2 K1        ; R2 := 0x25312c9b
  8 [-]: GETGLOBAL R3 K2        ; R3 := 0xae91e43b
  9 [-]: LOADK     R4 K3        ; R4 := "ShieldCharge"
 10 [-]: CONST     R5 0         ; R5 := 0.000000
 11 [-]: NEWTABLE  R6 1 0       ; R6 := {}
 12 [-]: CONST     R7 10        ; R7 := 10.000000
 13 [-]: SETLIST   R6 1 1       ; R6[(1-1)*FPF+i] := R(6+i), 1 <= i <= 1
 14 [-]: NEWTABLE  R7 0 0       ; R7 := {}
 15 [-]: GETUPVAL  R8 U1        ; R8 := U1
 16 [-]: GETTABLE  R8 R8 K5     ; R8 := R8[0x06d055f9]
 17 [-]: MOVE      R9 R0        ; R9 := R0
 18 [-]: CONST     R10 100      ; R10 := 100.000000
 19 [-]: CONST     R11 0        ; R11 := 0.000000
 20 [-]: CALL      R8 4 0       ; R8,... := R8(R9,R10,R11)
 21 [-]: SETLIST   R7 0 1       ; R7[(1-1)*FPF+i] := R(7+i), 1 <= i <= 0
 22 [-]: LOADK     R8 K6        ; R8 := 0.200000
 23 [-]: CALL      R2 7 1       ; R2(R3,R4,R5,R6,R7,R8)
 24 [-]: RETURN    R0 1         ; return 


; Function #10:
;
; Name:            
; Defined at line: 164
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  7

  1 [-]: LT        0 R0 K0      ; if R0 >= 10000.000000 then PC := 10
  2 [-]: JMP       10           ; PC := 10
  3 [-]: GETUPVAL  R2 U0        ; R2 := U0
  4 [-]: GETTABLE  R2 R2 K1     ; R2 := R2[0x1142c7a8]
  5 [-]: MOVE      R3 R0        ; R3 := R0
  6 [-]: CONST     R4 0         ; R4 := 0.000000
  7 [-]: CALL      R2 3 2       ; R2 := R2(R3,R4)
  8 [-]: MOVE      R1 R2        ; R1 := R2
  9 [-]: JMP       15           ; PC := 15
 10 [-]: GETUPVAL  R2 U0        ; R2 := U0
 11 [-]: GETTABLE  R2 R2 K2     ; R2 := R2[0xba3f419d]
 12 [-]: MOVE      R3 R0        ; R3 := R0
 13 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 14 [-]: MOVE      R1 R2        ; R1 := R2
 15 [-]: GETGLOBAL R2 K3        ; R2 := 0xae91e43b
 16 [-]: SELF      R2 R2 K4     ; R3 := R2; R2 := R2[0x5f56eeab]
 17 [-]: LOADK     R4 K5        ; R4 := "ShieldCharge.Damage"
 18 [-]: CONST     R5 29        ; R5 := 29.000000
 19 [-]: MOVE      R6 R1        ; R6 := R1
 20 [-]: CALL      R2 5 1       ; R2(R3,R4,R5,R6)
 21 [-]: RETURN    R0 1         ; return 


; Function #11:
;
; Name:            
; Defined at line: 175
; #Upvalues:       1
; #Parameters:     2
; Is_vararg:       0
; Max Stack Size:  9

  1 [-]: EQ        1 R0 K0      ; if R0 == nil then PC := 7
  2 [-]: JMP       7            ; PC := 7
  3 [-]: EQ        1 R1 K0      ; if R1 == nil then PC := 7
  4 [-]: JMP       7            ; PC := 7
  5 [-]: EQ        0 R0 K1      ; if R0 ~= 0.000000 then PC := 20
  6 [-]: JMP       20           ; PC := 20
  7 [-]: GETGLOBAL R2 K2        ; R2 := 0x25312c9b
  8 [-]: GETGLOBAL R3 K3        ; R3 := 0xae91e43b
  9 [-]: LOADK     R4 K4        ; R4 := "PassiveContainer.Label"
 10 [-]: CONST     R5 2         ; R5 := 2.000000
 11 [-]: NEWTABLE  R6 1 0       ; R6 := {}
 12 [-]: CONST     R7 10        ; R7 := 10.000000
 13 [-]: SETLIST   R6 1 1       ; R6[(1-1)*FPF+i] := R(6+i), 1 <= i <= 1
 14 [-]: NEWTABLE  R7 1 0       ; R7 := {}
 15 [-]: CONST     R8 0         ; R8 := 0.000000
 16 [-]: SETLIST   R7 1 1       ; R7[(1-1)*FPF+i] := R(7+i), 1 <= i <= 1
 17 [-]: LOADK     R8 K6        ; R8 := 0.150000
 18 [-]: CALL      R2 7 1       ; R2(R3,R4,R5,R6,R7,R8)
 19 [-]: RETURN    R0 1         ; return 
 20 [-]: GETUPVAL  R2 U0        ; R2 := U0
 21 [-]: SELF      R2 R2 K7     ; R3 := R2; R2 := R2[0x188e2bee]
 22 [-]: DIV       R4 R0 R1     ; R4 := R0 / R1
 23 [-]: CALL      R2 3 1       ; R2(R3,R4)
 24 [-]: GETGLOBAL R2 K2        ; R2 := 0x25312c9b
 25 [-]: GETGLOBAL R3 K3        ; R3 := 0xae91e43b
 26 [-]: LOADK     R4 K4        ; R4 := "PassiveContainer.Label"
 27 [-]: CONST     R5 2         ; R5 := 2.000000
 28 [-]: NEWTABLE  R6 1 0       ; R6 := {}
 29 [-]: CONST     R7 10        ; R7 := 10.000000
 30 [-]: SETLIST   R6 1 1       ; R6[(1-1)*FPF+i] := R(6+i), 1 <= i <= 1
 31 [-]: NEWTABLE  R7 1 0       ; R7 := {}
 32 [-]: CONST     R8 100       ; R8 := 100.000000
 33 [-]: SETLIST   R7 1 1       ; R7[(1-1)*FPF+i] := R(7+i), 1 <= i <= 1
 34 [-]: LOADK     R8 K6        ; R8 := 0.150000
 35 [-]: CALL      R2 7 1       ; R2(R3,R4,R5,R6,R7,R8)
 36 [-]: GETGLOBAL R2 K3        ; R2 := 0xae91e43b
 37 [-]: SELF      R2 R2 K8     ; R3 := R2; R2 := R2[0x42b04007]
 38 [-]: LOADK     R4 K9        ; R4 := "/Lotus/Language/Labels/DamageBonus"
 39 [-]: LOADKB    R5 0 0       ; R5 := false
 40 [-]: NEWTABLE  R6 0 1       ; R6 := {}
 41 [-]: GETGLOBAL R7 K11       ; R7 := 0x5bced4c4
 42 [-]: GETTABLE  R7 R7 K12    ; R7 := R7[0x55f27c30]
 43 [-]: MUL       R8 R0 K13    ; R8 := R0 * 100.000000
 44 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 45 [-]: SETTABLE  R6 K10 R7    ; R6["VAL"] := R7
 46 [-]: CALL      R2 5 2       ; R2 := R2(R3,R4,R5,R6)
 47 [-]: GETGLOBAL R3 K3        ; R3 := 0xae91e43b
 48 [-]: SELF      R3 R3 K14    ; R4 := R3; R3 := R3[0x5f56eeab]
 49 [-]: LOADK     R5 K4        ; R5 := "PassiveContainer.Label"
 50 [-]: CONST     R6 29        ; R6 := 29.000000
 51 [-]: MOVE      R7 R2        ; R7 := R2
 52 [-]: CALL      R3 5 1       ; R3(R4,R5,R6,R7)
 53 [-]: RETURN    R0 1         ; return 


; Function #12:
;
; Name:            
; Defined at line: 188
; #Upvalues:       12
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  10

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x2d0fad09
  2 [-]: LOADK     R1 K1        ; R1 := "Lotus.Interface.Libs.TimerMgr"
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETTABLE  R1 R0 K2     ; R1 := R0[0xde474187]
  5 [-]: CALL      R1 1 2       ; R1 := R1()
  6 [-]: SETUPVAL  R1 U0        ; U82 := R0
  7 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
  8 [-]: SELF      R1 R1 K4     ; R2 := R1; R1 := R1[0x67bc869f]
  9 [-]: LOADK     R3 K5        ; R3 := "_root"
 10 [-]: CONST     R4 10        ; R4 := 10.000000
 11 [-]: CONST     R5 0         ; R5 := 0.000000
 12 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 13 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 14 [-]: SELF      R1 R1 K4     ; R2 := R1; R1 := R1[0x67bc869f]
 15 [-]: LOADK     R3 K6        ; R3 := "PassiveContainer.Label"
 16 [-]: CONST     R4 10        ; R4 := 10.000000
 17 [-]: CONST     R5 0         ; R5 := 0.000000
 18 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 19 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 20 [-]: SELF      R1 R1 K7     ; R2 := R1; R1 := R1[0xd5181643]
 21 [-]: LOADK     R3 K8        ; R3 := "PassiveContainer.Fill"
 22 [-]: GETGLOBAL R4 K9        ; R4 := 0xf613e49a
 23 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 24 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 25 [-]: SELF      R1 R1 K4     ; R2 := R1; R1 := R1[0x67bc869f]
 26 [-]: LOADK     R3 K8        ; R3 := "PassiveContainer.Fill"
 27 [-]: CONST     R4 9         ; R4 := 9.000000
 28 [-]: LOADK     R5 K10       ; R5 := 7474702.000000
 29 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 30 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 31 [-]: SELF      R1 R1 K4     ; R2 := R1; R1 := R1[0x67bc869f]
 32 [-]: LOADK     R3 K11       ; R3 := "PassiveContainer.Glow"
 33 [-]: CONST     R4 9         ; R4 := 9.000000
 34 [-]: LOADK     R5 K12       ; R5 := 5181717.000000
 35 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 36 [-]: GETGLOBAL R1 K13       ; R1 := 0x89326c93
 37 [-]: SELF      R1 R1 K14    ; R2 := R1; R1 := R1[0x78298275]
 38 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 39 [-]: SETUPVAL  R1 U1        ; U82 := R1
 40 [-]: GETGLOBAL R1 K15       ; R1 := 0x7b998233
 41 [-]: GETUPVAL  R2 U1        ; R2 := U1
 42 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 43 [-]: TEST      R1 1         ; if R1 then PC := 56
 44 [-]: JMP       56           ; PC := 56
 45 [-]: GETUPVAL  R1 U1        ; R1 := U1
 46 [-]: SELF      R1 R1 K16    ; R2 := R1; R1 := R1[0x5e651723]
 47 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 48 [-]: GETGLOBAL R2 K15       ; R2 := 0x7b998233
 49 [-]: MOVE      R3 R1        ; R3 := R1
 50 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 51 [-]: TEST      R2 1         ; if R2 then PC := 56
 52 [-]: JMP       56           ; PC := 56
 53 [-]: SELF      R2 R1 K17    ; R3 := R1; R2 := R1[0x0803eee1]
 54 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 55 [-]: SETUPVAL  R2 U2        ; U82 := R2
 56 [-]: GETGLOBAL R2 K18       ; R2 := _T
 57 [-]: CLOSURE   R3 0         ; R3 := closure(Function #12.1)
 58 [-]: GETUPVAL  R0 U3        ; R0 := U3
 59 [-]: GETUPVAL  R0 U4        ; R0 := U4
 60 [-]: SETTABLE  R2 K19 R3    ; R2["GARUDA_ShowShieldCharge"] := R3
 61 [-]: GETGLOBAL R2 K18       ; R2 := _T
 62 [-]: CLOSURE   R3 1         ; R3 := closure(Function #12.2)
 63 [-]: GETUPVAL  R0 U3        ; R0 := U3
 64 [-]: GETUPVAL  R0 U5        ; R0 := U5
 65 [-]: SETTABLE  R2 K20 R3    ; R2["GARUDA_SetShieldCharge"] := R3
 66 [-]: GETGLOBAL R2 K18       ; R2 := _T
 67 [-]: CLOSURE   R3 2         ; R3 := closure(Function #12.3)
 68 [-]: GETUPVAL  R0 U3        ; R0 := U3
 69 [-]: GETUPVAL  R0 U6        ; R0 := U6
 70 [-]: SETTABLE  R2 K21 R3    ; R2["GARUDA_SetProjectileLabelOffset"] := R3
 71 [-]: GETGLOBAL R2 K18       ; R2 := _T
 72 [-]: CLOSURE   R3 3         ; R3 := closure(Function #12.4)
 73 [-]: GETUPVAL  R0 U3        ; R0 := U3
 74 [-]: GETUPVAL  R0 U7        ; R0 := U7
 75 [-]: SETTABLE  R2 K22 R3    ; R2["GARUDA_SetDamageBonus"] := R3
 76 [-]: GETUPVAL  R2 U8        ; R2 := U8
 77 [-]: CALL      R2 1 1       ; R2()
 78 [-]: GETUPVAL  R2 U9        ; R2 := U9
 79 [-]: CALL      R2 1 1       ; R2()
 80 [-]: GETGLOBAL R2 K23       ; R2 := 0x76ea806b
 81 [-]: SELF      R2 R2 K24    ; R3 := R2; R2 := R2[0x8792aaab]
 82 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 83 [-]: TEST      R2 0         ; if not R2 then PC := 94
 84 [-]: JMP       94           ; PC := 94
 85 [-]: GETGLOBAL R2 K25       ; R2 := 0x11a19c5e
 86 [-]: GETGLOBAL R3 K23       ; R3 := 0x76ea806b
 87 [-]: SELF      R3 R3 K26    ; R4 := R3; R3 := R3[0x3f3ae64c]
 88 [-]: CONST     R5 0         ; R5 := 0.000000
 89 [-]: CALL      R3 3 2       ; R3 := R3(R4,R5)
 90 [-]: SELF      R3 R3 K27    ; R4 := R3; R3 := R3[0x80563238]
 91 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 92 [-]: LOADK     R4 K28       ; R4 := "OnProfileSaved"
 93 [-]: CALL      R2 3 1       ; R2(R3,R4)
 94 [-]: GETGLOBAL R2 K18       ; R2 := _T
 95 [-]: GETTABLE  R2 R2 K29    ; R2 := R2[0xf6b77431]
 96 [-]: CALL      R2 1 2       ; R2 := R2()
 97 [-]: SELF      R3 R2 K30    ; R4 := R2; R3 := R2[0x20ff29f7]
 98 [-]: GETGLOBAL R5 K3        ; R5 := 0xae91e43b
 99 [-]: LOADK     R6 K31       ; R6 := "PassiveContainer"
100 [-]: NEWTABLE  R7 2 0       ; R7 := {}
101 [-]: GETTABLE  R8 R2 K32    ; R8 := R2["ANCHOR_V_BOTTOM"]
102 [-]: GETTABLE  R9 R2 K33    ; R9 := R2["ANCHOR_H_RIGHT"]
103 [-]: SETLIST   R7 2 1       ; R7[(1-1)*FPF+i] := R(7+i), 1 <= i <= 2
104 [-]: CALL      R3 5 1       ; R3(R4,R5,R6,R7)
105 [-]: SELF      R3 R2 K34    ; R4 := R2; R3 := R2[0xfaa69527]
106 [-]: GETGLOBAL R5 K3        ; R5 := 0xae91e43b
107 [-]: SELF      R5 R5 K35    ; R6 := R5; R5 := R5[0x6b837788]
108 [-]: CALL      R5 2 2       ; R5 := R5(R6)
109 [-]: GETGLOBAL R6 K3        ; R6 := 0xae91e43b
110 [-]: SELF      R6 R6 K36    ; R7 := R6; R6 := R6[0xaf9fda9f]
111 [-]: CALL      R6 2 2       ; R6 := R6(R7)
112 [-]: LOADKB    R7 1 0       ; R7 := true
113 [-]: GETTABLE  R8 R2 K37    ; R8 := R2["mHudScalePadding"]
114 [-]: CALL      R3 6 1       ; R3(R4,R5,R6,R7,R8)
115 [-]: GETGLOBAL R3 K18       ; R3 := _T
116 [-]: GETTABLE  R3 R3 K38    ; R3 := R3[0x6b23d28b]
117 [-]: GETGLOBAL R4 K3        ; R4 := 0xae91e43b
118 [-]: LOADK     R5 K31       ; R5 := "PassiveContainer"
119 [-]: CALL      R3 3 1       ; R3(R4,R5)
120 [-]: GETGLOBAL R3 K3        ; R3 := 0xae91e43b
121 [-]: SELF      R3 R3 K4     ; R4 := R3; R3 := R3[0x67bc869f]
122 [-]: LOADK     R5 K39       ; R5 := "ShieldCharge"
123 [-]: CONST     R6 10        ; R6 := 10.000000
124 [-]: CONST     R7 0         ; R7 := 0.000000
125 [-]: CALL      R3 5 1       ; R3(R4,R5,R6,R7)
126 [-]: GETGLOBAL R3 K13       ; R3 := 0x89326c93
127 [-]: SELF      R3 R3 K40    ; R4 := R3; R3 := R3[0xfb64e76c]
128 [-]: CALL      R3 2 2       ; R3 := R3(R4)
129 [-]: GETGLOBAL R4 K15       ; R4 := 0x7b998233
130 [-]: MOVE      R5 R3        ; R5 := R3
131 [-]: CALL      R4 2 2       ; R4 := R4(R5)
132 [-]: TEST      R4 1         ; if R4 then PC := 138
133 [-]: JMP       138          ; PC := 138
134 [-]: GETGLOBAL R4 K3        ; R4 := 0xae91e43b
135 [-]: SELF      R4 R4 K41    ; R5 := R4; R4 := R4[0x263a3cc2]
136 [-]: MOVE      R6 R3        ; R6 := R3
137 [-]: CALL      R4 3 1       ; R4(R5,R6)
138 [-]: SELF      R4 R2 K42    ; R5 := R2; R4 := R2[0x9d1db3eb]
139 [-]: LOADK     R6 K31       ; R6 := "PassiveContainer"
140 [-]: CALL      R4 3 2       ; R4 := R4(R5,R6)
141 [-]: GETTABLE  R4 R4 K43    ; R4 := R4["y"]
142 [-]: SETUPVAL  R4 U10       ; U82 := R10
143 [-]: LOADKB    R4 1 0       ; R4 := true
144 [-]: SETUPVAL  R4 U11       ; U82 := R11
145 [-]: RETURN    R0 1         ; return 


; Function #12.1:
;
; Name:            
; Defined at line: 209
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       3
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETUPVAL  R2 U1        ; R2 := U1
  3 [-]: VARARG    R3 0         ; R3 := ...
  4 [-]: CALL      R1 0 1       ; R1(R2,...)
  5 [-]: RETURN    R0 1         ; return 


; Function #12.2:
;
; Name:            
; Defined at line: 210
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       3
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETUPVAL  R2 U1        ; R2 := U1
  3 [-]: VARARG    R3 0         ; R3 := ...
  4 [-]: CALL      R1 0 1       ; R1(R2,...)
  5 [-]: RETURN    R0 1         ; return 


; Function #12.3:
;
; Name:            
; Defined at line: 211
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       3
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETUPVAL  R2 U1        ; R2 := U1
  3 [-]: VARARG    R3 0         ; R3 := ...
  4 [-]: CALL      R1 0 1       ; R1(R2,...)
  5 [-]: RETURN    R0 1         ; return 


; Function #12.4:
;
; Name:            
; Defined at line: 212
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       3
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETUPVAL  R2 U1        ; R2 := U1
  3 [-]: VARARG    R3 0         ; R3 := ...
  4 [-]: CALL      R1 0 1       ; R1(R2,...)
  5 [-]: RETURN    R0 1         ; return 


; Function #13:
;
; Name:            
; Defined at line: 238
; #Upvalues:       2
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: SETUPVAL  R0 U0        ; U82 := R0
  2 [-]: GETUPVAL  R1 U1        ; R1 := U1
  3 [-]: GETTABLE  R1 R1 K0     ; R1 := R1[0xfa221145]
  4 [-]: GETGLOBAL R2 K1        ; R2 := 0xae91e43b
  5 [-]: GETGLOBAL R3 K2        ; R3 := 0x03f57322
  6 [-]: MOVE      R4 R0        ; R4 := R0
  7 [-]: CALL      R3 2 0       ; R3,... := R3(R4)
  8 [-]: CALL      R1 0 1       ; R1(R2,...)
  9 [-]: RETURN    R0 1         ; return 


