; This file has been disassembled using luadec 2.0.2 by sztupy (modified by Reversive)

; Name:            
; Defined at line: 0
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       2
; Max Stack Size:  34

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x2d0fad09
  2 [-]: LOADK     R1 K1        ; R1 := "Lotus.Interface.UIUtilities"
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K0        ; R1 := 0x2d0fad09
  5 [-]: LOADK     R2 K2        ; R2 := "Lotus.Interface.LotusUtilities"
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: GETGLOBAL R2 K0        ; R2 := 0x2d0fad09
  8 [-]: LOADK     R3 K3        ; R3 := "EE.Interface.Utilities"
  9 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 10 [-]: GETGLOBAL R3 K0        ; R3 := 0x2d0fad09
 11 [-]: LOADK     R4 K4        ; R4 := "Lotus.Powersuits.PowersuitAbilities.PVPLib"
 12 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 13 [-]: LOADKB    R4 0 0       ; R4 := false
 14 [-]: LOADNIL   R5 R5        ; R5 := nil
 15 [-]: NEWTABLE  R6 0 0       ; R6 := {}
 16 [-]: LOADKB    R7 0 0       ; R7 := false
 17 [-]: LOADNIL   R8 R10       ; R8 := R9 := R10 := nil
 18 [-]: CONST     R11 0        ; R11 := 0.000000
 19 [-]: CONST     R12 1        ; R12 := 1.000000
 20 [-]: CONST     R13 0        ; R13 := 0.000000
 21 [-]: CONST     R14 0        ; R14 := 0.000000
 22 [-]: LOADNIL   R15 R18      ; R15 := R16 := R17 := R18 := nil
 23 [-]: LOADKB    R19 0 0      ; R19 := false
 24 [-]: LOADKB    R20 0 0      ; R20 := false
 25 [-]: CONST     R21 0        ; R21 := 0.000000
 26 [-]: CONST     R22 0        ; R22 := 0.000000
 27 [-]: LOADKB    R23 0 0      ; R23 := false
 28 [-]: CLOSURE   R24 0        ; R24 := closure(Function #1)
 29 [-]: CLOSURE   R25 1        ; R25 := closure(Function #2)
 30 [-]: MOVE      R0 R1        ; R0 := R1
 31 [-]: CLOSURE   R26 2        ; R26 := closure(Function #3)
 32 [-]: MOVE      R0 R13       ; R0 := R13
 33 [-]: MOVE      R0 R14       ; R0 := R14
 34 [-]: CLOSURE   R27 3        ; R27 := closure(Function #4)
 35 [-]: MOVE      R0 R26       ; R0 := R26
 36 [-]: SETGLOBAL R27 K5       ; onViewportSizeChanged := R27
 37 [-]: CLOSURE   R27 4        ; R27 := closure(Function #5)
 38 [-]: MOVE      R0 R12       ; R0 := R12
 39 [-]: MOVE      R0 R11       ; R0 := R11
 40 [-]: CLOSURE   R28 5        ; R28 := closure(Function #6)
 41 [-]: MOVE      R0 R10       ; R0 := R10
 42 [-]: MOVE      R0 R12       ; R0 := R12
 43 [-]: MOVE      R0 R2        ; R0 := R2
 44 [-]: MOVE      R0 R13       ; R0 := R13
 45 [-]: MOVE      R0 R14       ; R0 := R14
 46 [-]: CLOSURE   R29 6        ; R29 := closure(Function #7)
 47 [-]: MOVE      R0 R10       ; R0 := R10
 48 [-]: MOVE      R0 R11       ; R0 := R11
 49 [-]: MOVE      R0 R27       ; R0 := R27
 50 [-]: CLOSURE   R30 7        ; R30 := closure(Function #8)
 51 [-]: MOVE      R0 R17       ; R0 := R17
 52 [-]: MOVE      R0 R18       ; R0 := R18
 53 [-]: MOVE      R0 R19       ; R0 := R19
 54 [-]: MOVE      R0 R21       ; R0 := R21
 55 [-]: MOVE      R0 R20       ; R0 := R20
 56 [-]: MOVE      R0 R2        ; R0 := R2
 57 [-]: CLOSURE   R31 8        ; R31 := closure(Function #9)
 58 [-]: MOVE      R0 R15       ; R0 := R15
 59 [-]: CLOSURE   R32 9        ; R32 := closure(Function #10)
 60 [-]: MOVE      R0 R4        ; R0 := R4
 61 [-]: MOVE      R0 R15       ; R0 := R15
 62 [-]: MOVE      R0 R17       ; R0 := R17
 63 [-]: MOVE      R0 R5        ; R0 := R5
 64 [-]: MOVE      R0 R7        ; R0 := R7
 65 [-]: MOVE      R0 R6        ; R0 := R6
 66 [-]: MOVE      R0 R8        ; R0 := R8
 67 [-]: MOVE      R0 R9        ; R0 := R9
 68 [-]: MOVE      R0 R29       ; R0 := R29
 69 [-]: MOVE      R0 R30       ; R0 := R30
 70 [-]: MOVE      R0 R25       ; R0 := R25
 71 [-]: MOVE      R0 R24       ; R0 := R24
 72 [-]: MOVE      R0 R23       ; R0 := R23
 73 [-]: MOVE      R0 R0        ; R0 := R0
 74 [-]: MOVE      R0 R22       ; R0 := R22
 75 [-]: MOVE      R0 R16       ; R0 := R16
 76 [-]: SETGLOBAL R32 K6       ; Update := R32
 77 [-]: CLOSURE   R32 10       ; R32 := closure(Function #11)
 78 [-]: MOVE      R0 R7        ; R0 := R7
 79 [-]: MOVE      R0 R6        ; R0 := R6
 80 [-]: CLOSURE   R33 11       ; R33 := closure(Function #12)
 81 [-]: MOVE      R0 R10       ; R0 := R10
 82 [-]: MOVE      R0 R8        ; R0 := R8
 83 [-]: SETGLOBAL R33 K7       ; Shutdown := R33
 84 [-]: CLOSURE   R33 12       ; R33 := closure(Function #13)
 85 [-]: MOVE      R0 R26       ; R0 := R26
 86 [-]: MOVE      R0 R8        ; R0 := R8
 87 [-]: MOVE      R0 R2        ; R0 := R2
 88 [-]: MOVE      R0 R3        ; R0 := R3
 89 [-]: MOVE      R0 R32       ; R0 := R32
 90 [-]: MOVE      R0 R28       ; R0 := R28
 91 [-]: MOVE      R0 R31       ; R0 := R31
 92 [-]: MOVE      R0 R22       ; R0 := R22
 93 [-]: MOVE      R0 R15       ; R0 := R15
 94 [-]: MOVE      R0 R16       ; R0 := R16
 95 [-]: MOVE      R0 R17       ; R0 := R17
 96 [-]: MOVE      R0 R4        ; R0 := R4
 97 [-]: SETGLOBAL R33 K8       ; Initialize := R33
 98 [-]: CLOSURE   R33 13       ; R33 := closure(Function #14)
 99 [-]: MOVE      R0 R0        ; R0 := R0
100 [-]: SETGLOBAL R33 K9       ; HandleHudScale := R33
101 [-]: RETURN    R0 1         ; return 


; Function #1:
;
; Name:            
; Defined at line: 40
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  3

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0xbe190284
  2 [-]: GETGLOBAL R1 K1        ; R1 := 0x7b998233
  3 [-]: MOVE      R2 R0        ; R2 := R0
  4 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  5 [-]: TEST      R1 1         ; if R1 then PC := 13
  6 [-]: JMP       13           ; PC := 13
  7 [-]: SELF      R1 R0 K2     ; R2 := R0; R1 := R0[0x486e5f11]
  8 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  9 [-]: TEST      R1 0         ; if not R1 then PC := 13
 10 [-]: JMP       13           ; PC := 13
 11 [-]: LOADKB    R1 1 0       ; R1 := true
 12 [-]: RETURN    R1 2         ; return R1
 13 [-]: LOADKB    R1 0 0       ; R1 := false
 14 [-]: RETURN    R1 2         ; return R1
 15 [-]: RETURN    R0 1         ; return 


; Function #2:
;
; Name:            
; Defined at line: 48
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: GETTABLE  R0 R0 K0     ; R0 := R0[0xb73d420f]
  3 [-]: CALL      R0 1 2       ; R0 := R0()
  4 [-]: GETUPVAL  R1 U0        ; R1 := U0
  5 [-]: GETTABLE  R1 R1 K1     ; R1 := R1["UI_MODE_IN_DOJO"]
  6 [-]: EQ        0 R0 R1      ; if R0 ~= R1 then PC := 10
  7 [-]: JMP       10           ; PC := 10
  8 [-]: LOADKB    R0 1 0       ; R0 := true
  9 [-]: RETURN    R0 2         ; return R0
 10 [-]: LOADKB    R0 0 0       ; R0 := false
 11 [-]: RETURN    R0 2         ; return R0
 12 [-]: RETURN    R0 1         ; return 


; Function #3:
;
; Name:            
; Defined at line: 55
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  4

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0xae91e43b
  2 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x6b837788]
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K0        ; R1 := 0xae91e43b
  5 [-]: SELF      R1 R1 K2     ; R2 := R1; R1 := R1[0xaf9fda9f]
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: GETGLOBAL R2 K0        ; R2 := 0xae91e43b
  8 [-]: SELF      R2 R2 K3     ; R3 := R2; R2 := R2[0x091c120e]
  9 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 10 [-]: LT        1 R0 R2      ; if R0 < R2 then PC := 17
 11 [-]: JMP       17           ; PC := 17
 12 [-]: GETGLOBAL R2 K0        ; R2 := 0xae91e43b
 13 [-]: SELF      R2 R2 K4     ; R3 := R2; R2 := R2[0x2cc9d281]
 14 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 15 [-]: LT        0 R1 R2      ; if R1 >= R2 then PC := 30
 16 [-]: JMP       30           ; PC := 30
 17 [-]: GETGLOBAL R2 K5        ; R2 := 0x34291f5c
 18 [-]: GETTABLE  R2 R2 K6     ; R2 := R2[0x056bfe8b]
 19 [-]: CALL      R2 1 2       ; R2 := R2()
 20 [-]: TEST      R2 1         ; if R2 then PC := 30
 21 [-]: JMP       30           ; PC := 30
 22 [-]: GETGLOBAL R2 K0        ; R2 := 0xae91e43b
 23 [-]: SELF      R2 R2 K3     ; R3 := R2; R2 := R2[0x091c120e]
 24 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 25 [-]: MOVE      R0 R2        ; R0 := R2
 26 [-]: GETGLOBAL R2 K0        ; R2 := 0xae91e43b
 27 [-]: SELF      R2 R2 K4     ; R3 := R2; R2 := R2[0x2cc9d281]
 28 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 29 [-]: MOVE      R1 R2        ; R1 := R2
 30 [-]: MUL       R2 R0 K7     ; R2 := R0 * 0.500000
 31 [-]: SETUPVAL  R2 U0        ; U82 := R0
 32 [-]: MUL       R2 R1 K7     ; R2 := R1 * 0.500000
 33 [-]: SETUPVAL  R2 U1        ; U82 := R1
 34 [-]: RETURN    R0 1         ; return 


; Function #4:
;
; Name:            
; Defined at line: 67
; #Upvalues:       1
; #Parameters:     4
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: GETUPVAL  R4 U0        ; R4 := U0
  2 [-]: CALL      R4 1 1       ; R4()
  3 [-]: RETURN    R0 1         ; return 


; Function #5:
;
; Name:            
; Defined at line: 71
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  8

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: MUL       R0 K0 R0     ; R0 := 0.020000 * R0
  3 [-]: GETUPVAL  R1 U1        ; R1 := U1
  4 [-]: MUL       R1 R1 K2     ; R1 := R1 * 3.141593
  5 [-]: MUL       R1 R1 K3     ; R1 := R1 * 2.000000
  6 [-]: GETGLOBAL R2 K1        ; R2 := 0x5bced4c4
  7 [-]: GETTABLE  R2 R2 K4     ; R2 := R2[0x3eda26fc]
  8 [-]: MOVE      R3 R1        ; R3 := R1
  9 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 10 [-]: MUL       R2 R2 R0     ; R2 := R2 * R0
 11 [-]: GETGLOBAL R3 K1        ; R3 := 0x5bced4c4
 12 [-]: GETTABLE  R3 R3 K5     ; R3 := R3[0x00fa6bf1]
 13 [-]: MOVE      R4 R1        ; R4 := R1
 14 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 15 [-]: MUL       R3 R3 R0     ; R3 := R3 * R0
 16 [-]: GETGLOBAL R4 K6        ; R4 := 0xa421af95
 17 [-]: MOVE      R5 R2        ; R5 := R2
 18 [-]: MOVE      R6 R3        ; R6 := R3
 19 [-]: CONST     R7 0         ; R7 := 0.000000
 20 [-]: CALL      R4 4 2       ; R4 := R4(R5,R6,R7)
 21 [-]: RETURN    R4 2         ; return R4
 22 [-]: RETURN    R0 1         ; return 


; Function #6:
;
; Name:            
; Defined at line: 79
; #Upvalues:       5
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  10

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x3d106989
  2 [-]: LOADK     R2 K1        ; R2 := "ShowControlParticles("
  3 [-]: GETGLOBAL R3 K2        ; R3 := 0x64fb1586
  4 [-]: MOVE      R4 R0        ; R4 := R0
  5 [-]: CALL      R3 2 2       ; R3 := R3(R4)
  6 [-]: LOADK     R4 K3        ; R4 := ")"
  7 [-]: CONCAT    R2 R2 R4     ; R2 := R2 .. R3 .. R4
  8 [-]: CALL      R1 2 1       ; R1(R2)
  9 [-]: TEST      R0 0         ; if not R0 then PC := 41
 10 [-]: JMP       41           ; PC := 41
 11 [-]: GETGLOBAL R1 K4        ; R1 := 0x7b998233
 12 [-]: GETUPVAL  R2 U0        ; R2 := U0
 13 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 14 [-]: TEST      R1 0         ; if not R1 then PC := 41
 15 [-]: JMP       41           ; PC := 41
 16 [-]: CONST     R1 0         ; R1 := 0.000000
 17 [-]: SETUPVAL  R1 U1        ; U82 := R1
 18 [-]: GETUPVAL  R1 U2        ; R1 := U2
 19 [-]: GETTABLE  R1 R1 K5     ; R1 := R1[0x5a22d251]
 20 [-]: GETGLOBAL R2 K6        ; R2 := 0xae91e43b
 21 [-]: GETGLOBAL R3 K7        ; R3 := 0x3253e435
 22 [-]: GETUPVAL  R4 U3        ; R4 := U3
 23 [-]: GETUPVAL  R5 U4        ; R5 := U4
 24 [-]: CALL      R1 5 2       ; R1 := R1(R2,R3,R4,R5)
 25 [-]: SETUPVAL  R1 U0        ; U82 := R0
 26 [-]: GETGLOBAL R1 K8        ; R1 := 0x25312c9b
 27 [-]: GETGLOBAL R2 K6        ; R2 := 0xae91e43b
 28 [-]: LOADK     R3 K9        ; R3 := "_root"
 29 [-]: CONST     R4 0         ; R4 := 0.000000
 30 [-]: NEWTABLE  R5 1 0       ; R5 := {}
 31 [-]: CLOSURE   R6 0         ; R6 := closure(Function #6.1)
 32 [-]: GETUPVAL  R0 U1        ; R0 := U1
 33 [-]: SETLIST   R5 1 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 1
 34 [-]: NEWTABLE  R6 1 0       ; R6 := {}
 35 [-]: CONST     R7 1         ; R7 := 1.000000
 36 [-]: SETLIST   R6 1 1       ; R6[(1-1)*FPF+i] := R(6+i), 1 <= i <= 1
 37 [-]: CONST     R7 0         ; R7 := 0.500000
 38 [-]: CONST     R8 0         ; R8 := 0.250000
 39 [-]: CALL      R1 8 1       ; R1(R2,R3,R4,R5,R6,R7,R8)
 40 [-]: JMP       71           ; PC := 71
 41 [-]: TEST      R0 1         ; if R0 then PC := 71
 42 [-]: JMP       71           ; PC := 71
 43 [-]: GETGLOBAL R1 K4        ; R1 := 0x7b998233
 44 [-]: GETUPVAL  R2 U0        ; R2 := U0
 45 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 46 [-]: TEST      R1 1         ; if R1 then PC := 71
 47 [-]: JMP       71           ; PC := 71
 48 [-]: GETUPVAL  R1 U0        ; R1 := U0
 49 [-]: SELF      R1 R1 K11    ; R2 := R1; R1 := R1[0x1bff969c]
 50 [-]: CONST     R3 0         ; R3 := 0.500000
 51 [-]: CALL      R1 3 1       ; R1(R2,R3)
 52 [-]: GETGLOBAL R1 K8        ; R1 := 0x25312c9b
 53 [-]: GETGLOBAL R2 K6        ; R2 := 0xae91e43b
 54 [-]: LOADK     R3 K9        ; R3 := "_root"
 55 [-]: CONST     R4 0         ; R4 := 0.000000
 56 [-]: NEWTABLE  R5 1 0       ; R5 := {}
 57 [-]: CLOSURE   R6 1         ; R6 := closure(Function #6.2)
 58 [-]: GETUPVAL  R0 U1        ; R0 := U1
 59 [-]: SETLIST   R5 1 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 1
 60 [-]: NEWTABLE  R6 1 0       ; R6 := {}
 61 [-]: CONST     R7 1         ; R7 := 1.000000
 62 [-]: SETLIST   R6 1 1       ; R6[(1-1)*FPF+i] := R(6+i), 1 <= i <= 1
 63 [-]: CONST     R7 1         ; R7 := 1.000000
 64 [-]: CONST     R8 0         ; R8 := 0.000000
 65 [-]: CLOSURE   R9 2         ; R9 := closure(Function #6.3)
 66 [-]: GETUPVAL  R0 U0        ; R0 := U0
 67 [-]: GETUPVAL  R0 U2        ; R0 := U2
 68 [-]: GETUPVAL  R0 U3        ; R0 := U3
 69 [-]: GETUPVAL  R0 U4        ; R0 := U4
 70 [-]: CALL      R1 9 1       ; R1(R2,R3,R4,R5,R6,R7,R8,R9)
 71 [-]: RETURN    R0 1         ; return 


; Function #6.1:
;
; Name:            
; Defined at line: 84
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: SETUPVAL  R0 U0        ; U82 := R0
  2 [-]: RETURN    R0 1         ; return 


; Function #6.2:
;
; Name:            
; Defined at line: 87
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  4

  1 [-]: GETGLOBAL R1 K0        ; R1 := 0x5bced4c4
  2 [-]: GETTABLE  R1 R1 K1     ; R1 := R1[0xb62ecfe0]
  3 [-]: LOADK     R2 K2        ; R2 := 0.001000
  4 [-]: MUL       R3 R0 K3     ; R3 := R0 * 1.500000
  5 [-]: SUB       R3 K4 R3     ; R3 := 1.000000 - R3
  6 [-]: CALL      R1 3 2       ; R1 := R1(R2,R3)
  7 [-]: SETUPVAL  R1 U0        ; U82 := R0
  8 [-]: RETURN    R0 1         ; return 


; Function #6.3:
;
; Name:            
; Defined at line: 88
; #Upvalues:       4
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: SELF      R0 R0 K0     ; R1 := R0; R0 := R0[0xa2880940]
  3 [-]: CALL      R0 2 1       ; R0(R1)
  4 [-]: GETUPVAL  R0 U1        ; R0 := U1
  5 [-]: GETTABLE  R0 R0 K1     ; R0 := R0[0x5a22d251]
  6 [-]: GETGLOBAL R1 K2        ; R1 := 0xae91e43b
  7 [-]: GETGLOBAL R2 K3        ; R2 := 0x0fa8a246
  8 [-]: GETUPVAL  R3 U2        ; R3 := U2
  9 [-]: GETUPVAL  R4 U3        ; R4 := U3
 10 [-]: CALL      R0 5 1       ; R0(R1,R2,R3,R4)
 11 [-]: RETURN    R0 1         ; return 


; Function #7:
;
; Name:            
; Defined at line: 95
; #Upvalues:       3
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  4

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
  2 [-]: GETUPVAL  R1 U0        ; R1 := U0
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: TEST      R0 1         ; if R0 then PC := 25
  5 [-]: JMP       25           ; PC := 25
  6 [-]: GETGLOBAL R0 K1        ; R0 := 0x89326c93
  7 [-]: SELF      R0 R0 K2     ; R1 := R0; R0 := R0[0xb4364067]
  8 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  9 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 10 [-]: MOVE      R2 R0        ; R2 := R0
 11 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 12 [-]: TEST      R1 1         ; if R1 then PC := 25
 13 [-]: JMP       25           ; PC := 25
 14 [-]: GETUPVAL  R1 U1        ; R1 := U1
 15 [-]: GETGLOBAL R2 K3        ; R2 := 0x67652851
 16 [-]: CALL      R2 1 2       ; R2 := R2()
 17 [-]: MUL       R2 K4 R2     ; R2 := 1.750000 * R2
 18 [-]: ADD       R1 R1 R2     ; R1 := R1 + R2
 19 [-]: SETUPVAL  R1 U1        ; U82 := R1
 20 [-]: GETUPVAL  R1 U0        ; R1 := U0
 21 [-]: SELF      R1 R1 K5     ; R2 := R1; R1 := R1[0x119b677c]
 22 [-]: GETUPVAL  R3 U2        ; R3 := U2
 23 [-]: CALL      R3 1 0       ; R3,... := R3()
 24 [-]: CALL      R1 0 1       ; R1(R2,...)
 25 [-]: RETURN    R0 1         ; return 


; Function #8:
;
; Name:            
; Defined at line: 106
; #Upvalues:       6
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  15

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: SELF      R0 R0 K0     ; R1 := R0; R0 := R0[0x881b6b90]
  3 [-]: CONST     R2 0         ; R2 := 0.000000
  4 [-]: CALL      R0 3 2       ; R0 := R0(R1,R2)
  5 [-]: GETUPVAL  R1 U1        ; R1 := U1
  6 [-]: EQ        1 R1 R0      ; if R1 == R0 then PC := 22
  7 [-]: JMP       22           ; PC := 22
  8 [-]: SETUPVAL  R0 U1        ; U82 := R1
  9 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
 10 [-]: GETUPVAL  R2 U1        ; R2 := U1
 11 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 12 [-]: TEST      R1 1         ; if R1 then PC := 19
 13 [-]: JMP       19           ; PC := 19
 14 [-]: GETUPVAL  R1 U1        ; R1 := U1
 15 [-]: SELF      R1 R1 K3     ; R2 := R1; R1 := R1[0xf2deaf69]
 16 [-]: GETGLOBAL R3 K4        ; R3 := 0x49493cf5
 17 [-]: CALL      R1 3 2       ; R1 := R1(R2,R3)
 18 [-]: JMP       21           ; PC := 21
 19 [-]: LOADKB    R1 0 1       ; R1 := false; PC := 20
 20 [-]: LOADKB    R1 1 0       ; R1 := true
 21 [-]: SETUPVAL  R1 U2        ; U82 := R2
 22 [-]: GETUPVAL  R1 U0        ; R1 := U0
 23 [-]: SELF      R1 R1 K5     ; R2 := R1; R1 := R1[0xf7d48ee0]
 24 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 25 [-]: GETGLOBAL R2 K2        ; R2 := 0x7b998233
 26 [-]: MOVE      R3 R1        ; R3 := R1
 27 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 28 [-]: TEST      R2 1         ; if R2 then PC := 53
 29 [-]: JMP       53           ; PC := 53
 30 [-]: SELF      R2 R1 K6     ; R3 := R1; R2 := R1[0xa2356091]
 31 [-]: GETGLOBAL R4 K7        ; R4 := 0x7ed0a956
 32 [-]: LOADK     R5 K8        ; R5 := "/Lotus/Powersuits/PowersuitAbilities/RangerBowAbility"
 33 [-]: CALL      R4 2 0       ; R4,... := R4(R5)
 34 [-]: CALL      R2 0 2       ; R2 := R2(R3,...)
 35 [-]: GETUPVAL  R3 U2        ; R3 := U2
 36 [-]: TEST      R3 0         ; if not R3 then PC := 52
 37 [-]: JMP       52           ; PC := 52
 38 [-]: LT        0 R2 K9      ; if R2 >= 4.000000 then PC := 50
 39 [-]: JMP       50           ; PC := 50
 40 [-]: SELF      R3 R1 K10    ; R4 := R1; R3 := R1[0x5063edc3]
 41 [-]: MOVE      R5 R2        ; R5 := R2
 42 [-]: CALL      R3 3 2       ; R3 := R3(R4,R5)
 43 [-]: EQ        1 R3 K11     ; if R3 == 0.000000 then PC := 51
 44 [-]: JMP       51           ; PC := 51
 45 [-]: SELF      R3 R1 K12    ; R4 := R1; R3 := R1[0x75ecaf0b]
 46 [-]: MOVE      R5 R2        ; R5 := R2
 47 [-]: CALL      R3 3 2       ; R3 := R3(R4,R5)
 48 [-]: EQ        0 R3 K14     ; if R3 ~= 1.000000 then PC := 51
 49 [-]: JMP       51           ; PC := 51
 50 [-]: LOADKB    R3 0 1       ; R3 := false; PC := 51
 51 [-]: LOADKB    R3 1 0       ; R3 := true
 52 [-]: SETUPVAL  R3 U2        ; U82 := R2
 53 [-]: LOADKB    R3 0 0       ; R3 := false
 54 [-]: GETUPVAL  R4 U3        ; R4 := U3
 55 [-]: GETUPVAL  R5 U2        ; R5 := U2
 56 [-]: TEST      R5 0         ; if not R5 then PC := 73
 57 [-]: JMP       73           ; PC := 73
 58 [-]: GETUPVAL  R5 U1        ; R5 := U1
 59 [-]: SELF      R5 R5 K15    ; R6 := R5; R5 := R5[0x1403242c]
 60 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 61 [-]: EQ        0 R5 K11     ; if R5 ~= 0.000000 then PC := 73
 62 [-]: JMP       73           ; PC := 73
 63 [-]: GETUPVAL  R5 U1        ; R5 := U1
 64 [-]: SELF      R5 R5 K16    ; R6 := R5; R5 := R5[0x6bb20d05]
 65 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 66 [-]: MOVE      R3 R5        ; R3 := R5
 67 [-]: TEST      R3 0         ; if not R3 then PC := 73
 68 [-]: JMP       73           ; PC := 73
 69 [-]: GETUPVAL  R5 U1        ; R5 := U1
 70 [-]: SELF      R5 R5 K17    ; R6 := R5; R5 := R5[0x46afa846]
 71 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 72 [-]: MOVE      R4 R5        ; R4 := R5
 73 [-]: GETUPVAL  R5 U4        ; R5 := U4
 74 [-]: EQ        1 R5 R3      ; if R5 == R3 then PC := 94
 75 [-]: JMP       94           ; PC := 94
 76 [-]: SETUPVAL  R3 U4        ; U82 := R4
 77 [-]: GETGLOBAL R5 K18       ; R5 := 0x25312c9b
 78 [-]: GETGLOBAL R6 K19       ; R6 := 0xae91e43b
 79 [-]: LOADK     R7 K20       ; R7 := "ArrowDirection"
 80 [-]: CONST     R8 0         ; R8 := 0.000000
 81 [-]: NEWTABLE  R9 1 0       ; R9 := {}
 82 [-]: CONST     R10 10       ; R10 := 10.000000
 83 [-]: SETLIST   R9 1 1       ; R9[(1-1)*FPF+i] := R(9+i), 1 <= i <= 1
 84 [-]: NEWTABLE  R10 0 0      ; R10 := {}
 85 [-]: GETUPVAL  R11 U5       ; R11 := U5
 86 [-]: GETTABLE  R11 R11 K22  ; R11 := R11[0x06d055f9]
 87 [-]: GETUPVAL  R12 U4       ; R12 := U4
 88 [-]: CONST     R13 30       ; R13 := 30.000000
 89 [-]: CONST     R14 0        ; R14 := 0.000000
 90 [-]: CALL      R11 4 0      ; R11,... := R11(R12,R13,R14)
 91 [-]: SETLIST   R10 0 1      ; R10[(1-1)*FPF+i] := R(10+i), 1 <= i <= 0
 92 [-]: LOADK     R11 K23      ; R11 := 0.150000
 93 [-]: CALL      R5 7 1       ; R5(R6,R7,R8,R9,R10,R11)
 94 [-]: GETUPVAL  R5 U3        ; R5 := U3
 95 [-]: EQ        1 R5 R4      ; if R5 == R4 then PC := 107
 96 [-]: JMP       107          ; PC := 107
 97 [-]: SETUPVAL  R4 U3        ; U82 := R3
 98 [-]: GETUPVAL  R5 U3        ; R5 := U3
 99 [-]: MUL       R5 K24 R5    ; R5 := 90.000000 * R5
100 [-]: ADD       R5 K25 R5    ; R5 := -90.000000 + R5
101 [-]: GETGLOBAL R6 K19       ; R6 := 0xae91e43b
102 [-]: SELF      R6 R6 K26    ; R7 := R6; R6 := R6[0x67bc869f]
103 [-]: LOADK     R8 K20       ; R8 := "ArrowDirection"
104 [-]: CONST     R9 14        ; R9 := 14.000000
105 [-]: MOVE      R10 R5       ; R10 := R5
106 [-]: CALL      R6 5 1       ; R6(R7,R8,R9,R10)
107 [-]: RETURN    R0 1         ; return 


; Function #9:
;
; Name:            
; Defined at line: 142
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  6

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x89326c93
  2 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x78298275]
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: SETUPVAL  R0 U0        ; U82 := R0
  5 [-]: GETGLOBAL R0 K2        ; R0 := 0x7b998233
  6 [-]: GETUPVAL  R1 U0        ; R1 := U0
  7 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  8 [-]: TEST      R0 0         ; if not R0 then PC := 11
  9 [-]: JMP       11           ; PC := 11
 10 [-]: RETURN    R0 1         ; return 
 11 [-]: GETUPVAL  R0 U0        ; R0 := U0
 12 [-]: SELF      R0 R0 K3     ; R1 := R0; R0 := R0[0x5e651723]
 13 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 14 [-]: SELF      R0 R0 K4     ; R1 := R0; R0 := R0[0x8b72b36e]
 15 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 16 [-]: CONST     R1 0         ; R1 := 0.000000
 17 [-]: GETGLOBAL R2 K2        ; R2 := 0x7b998233
 18 [-]: GETGLOBAL R3 K5        ; R3 := _T
 19 [-]: GETTABLE  R3 R3 K6     ; R3 := R3["rangerQuiver"]
 20 [-]: GETTABLE  R3 R3 K7     ; R3 := R3["domeTimerExpiry"]
 21 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 22 [-]: TEST      R2 1         ; if R2 then PC := 39
 23 [-]: JMP       39           ; PC := 39
 24 [-]: GETGLOBAL R2 K2        ; R2 := 0x7b998233
 25 [-]: GETGLOBAL R3 K5        ; R3 := _T
 26 [-]: GETTABLE  R3 R3 K6     ; R3 := R3["rangerQuiver"]
 27 [-]: GETTABLE  R3 R3 K7     ; R3 := R3["domeTimerExpiry"]
 28 [-]: GETTABLE  R3 R3 R0     ; R3 := R3[R0]
 29 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 30 [-]: TEST      R2 1         ; if R2 then PC := 39
 31 [-]: JMP       39           ; PC := 39
 32 [-]: GETGLOBAL R2 K5        ; R2 := _T
 33 [-]: GETTABLE  R2 R2 K6     ; R2 := R2["rangerQuiver"]
 34 [-]: GETTABLE  R2 R2 K7     ; R2 := R2["domeTimerExpiry"]
 35 [-]: GETTABLE  R2 R2 R0     ; R2 := R2[R0]
 36 [-]: GETGLOBAL R3 K8        ; R3 := 0x55156ff7
 37 [-]: CALL      R3 1 2       ; R3 := R3()
 38 [-]: SUB       R1 R2 R3     ; R1 := R2 - R3
 39 [-]: LT        0 K9 R1      ; if 0.000000 >= R1 then PC := 46
 40 [-]: JMP       46           ; PC := 46
 41 [-]: GETGLOBAL R2 K5        ; R2 := _T
 42 [-]: GETTABLE  R2 R2 K10    ; R2 := R2[0xf5b69782]
 43 [-]: CONST     R3 1         ; R3 := 1.000000
 44 [-]: MOVE      R4 R1        ; R4 := R1
 45 [-]: CALL      R2 3 1       ; R2(R3,R4)
 46 [-]: CONST     R2 0         ; R2 := 0.000000
 47 [-]: GETGLOBAL R3 K2        ; R3 := 0x7b998233
 48 [-]: GETGLOBAL R4 K5        ; R4 := _T
 49 [-]: GETTABLE  R4 R4 K6     ; R4 := R4["rangerQuiver"]
 50 [-]: GETTABLE  R4 R4 K11    ; R4 := R4["sleepTimerExpiry"]
 51 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 52 [-]: TEST      R3 1         ; if R3 then PC := 69
 53 [-]: JMP       69           ; PC := 69
 54 [-]: GETGLOBAL R3 K2        ; R3 := 0x7b998233
 55 [-]: GETGLOBAL R4 K5        ; R4 := _T
 56 [-]: GETTABLE  R4 R4 K6     ; R4 := R4["rangerQuiver"]
 57 [-]: GETTABLE  R4 R4 K11    ; R4 := R4["sleepTimerExpiry"]
 58 [-]: GETTABLE  R4 R4 R0     ; R4 := R4[R0]
 59 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 60 [-]: TEST      R3 1         ; if R3 then PC := 69
 61 [-]: JMP       69           ; PC := 69
 62 [-]: GETGLOBAL R3 K5        ; R3 := _T
 63 [-]: GETTABLE  R3 R3 K6     ; R3 := R3["rangerQuiver"]
 64 [-]: GETTABLE  R3 R3 K11    ; R3 := R3["sleepTimerExpiry"]
 65 [-]: GETTABLE  R3 R3 R0     ; R3 := R3[R0]
 66 [-]: GETGLOBAL R4 K8        ; R4 := 0x55156ff7
 67 [-]: CALL      R4 1 2       ; R4 := R4()
 68 [-]: SUB       R2 R3 R4     ; R2 := R3 - R4
 69 [-]: LT        0 K9 R2      ; if 0.000000 >= R2 then PC := 76
 70 [-]: JMP       76           ; PC := 76
 71 [-]: GETGLOBAL R3 K5        ; R3 := _T
 72 [-]: GETTABLE  R3 R3 K10    ; R3 := R3[0xf5b69782]
 73 [-]: CONST     R4 4         ; R4 := 4.000000
 74 [-]: MOVE      R5 R2        ; R5 := R2
 75 [-]: CALL      R3 3 1       ; R3(R4,R5)
 76 [-]: RETURN    R0 1         ; return 


; Function #10:
;
; Name:            
; Defined at line: 170
; #Upvalues:       16
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  13

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
  2 [-]: GETGLOBAL R1 K1        ; R1 := 0xae91e43b
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: TEST      R0 1         ; if R0 then PC := 9
  5 [-]: JMP       9            ; PC := 9
  6 [-]: GETUPVAL  R0 U0        ; R0 := U0
  7 [-]: TEST      R0 1         ; if R0 then PC := 10
  8 [-]: JMP       10           ; PC := 10
  9 [-]: RETURN    R0 1         ; return 
 10 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
 11 [-]: GETUPVAL  R1 U1        ; R1 := U1
 12 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 13 [-]: TEST      R0 0         ; if not R0 then PC := 29
 14 [-]: JMP       29           ; PC := 29
 15 [-]: GETGLOBAL R0 K2        ; R0 := 0x89326c93
 16 [-]: SELF      R0 R0 K3     ; R1 := R0; R0 := R0[0x78298275]
 17 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 18 [-]: SETUPVAL  R0 U1        ; U82 := R1
 19 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
 20 [-]: GETUPVAL  R1 U1        ; R1 := U1
 21 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 22 [-]: TEST      R0 0         ; if not R0 then PC := 25
 23 [-]: JMP       25           ; PC := 25
 24 [-]: RETURN    R0 1         ; return 
 25 [-]: GETUPVAL  R0 U1        ; R0 := U1
 26 [-]: SELF      R0 R0 K4     ; R1 := R0; R0 := R0[0xde321e6f]
 27 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 28 [-]: SETUPVAL  R0 U2        ; U82 := R2
 29 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
 30 [-]: GETUPVAL  R1 U2        ; R1 := U2
 31 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 32 [-]: TEST      R0 0         ; if not R0 then PC := 37
 33 [-]: JMP       37           ; PC := 37
 34 [-]: LOADNIL   R0 R0        ; R0 := nil
 35 [-]: SETUPVAL  R0 U1        ; U82 := R1
 36 [-]: RETURN    R0 1         ; return 
 37 [-]: GETGLOBAL R0 K5        ; R0 := 0x67652851
 38 [-]: CALL      R0 1 2       ; R0 := R0()
 39 [-]: GETGLOBAL R1 K1        ; R1 := 0xae91e43b
 40 [-]: SELF      R1 R1 K6     ; R2 := R1; R1 := R1[0x8a8c8d5a]
 41 [-]: MOVE      R3 R0        ; R3 := R0
 42 [-]: CALL      R1 3 1       ; R1(R2,R3)
 43 [-]: GETGLOBAL R1 K0        ; R1 := 0x7b998233
 44 [-]: GETUPVAL  R2 U3        ; R2 := U3
 45 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 46 [-]: TEST      R1 0         ; if not R1 then PC := 52
 47 [-]: JMP       52           ; PC := 52
 48 [-]: GETGLOBAL R1 K7        ; R1 := 0xbe190284
 49 [-]: SELF      R1 R1 K8     ; R2 := R1; R1 := R1[0x33307f92]
 50 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 51 [-]: SETUPVAL  R1 U3        ; U82 := R3
 52 [-]: LOADKB    R1 1 0       ; R1 := true
 53 [-]: SETUPVAL  R1 U4        ; U82 := R4
 54 [-]: GETUPVAL  R1 U5        ; R1 := U5
 55 [-]: LEN       R1 R1        ; R1 := # R1
 56 [-]: LT        0 K9 R1      ; if 0.000000 >= R1 then PC := 76
 57 [-]: JMP       76           ; PC := 76
 58 [-]: CONST     R1 1         ; R1 := 1.000000
 59 [-]: GETUPVAL  R2 U5        ; R2 := U5
 60 [-]: LEN       R2 R2        ; R2 := # R2
 61 [-]: CONST     R3 1         ; R3 := 1.000000
 62 [-]: FORPREP   R1 73        ; R1 -= R3; PC := 73
 63 [-]: GETUPVAL  R5 U5        ; R5 := U5
 64 [-]: GETTABLE  R5 R5 R4     ; R5 := R5[R4]
 65 [-]: GETTABLE  R5 R5 K10    ; R5 := R5[1.000000]
 66 [-]: GETUPVAL  R6 U5        ; R6 := U5
 67 [-]: GETTABLE  R6 R6 R4     ; R6 := R6[R4]
 68 [-]: GETTABLE  R6 R6 K11    ; R6 := R6[2.000000]
 69 [-]: GETUPVAL  R7 U5        ; R7 := U5
 70 [-]: GETTABLE  R7 R7 R4     ; R7 := R7[R4]
 71 [-]: GETTABLE  R7 R7 K12    ; R7 := R7[3.000000]
 72 [-]: CALL      R5 3 1       ; R5(R6,R7)
 73 [-]: FORLOOP   R1 63        ; R1 += R3; if R1 <= R2 then begin PC := 63; R4 := R1 end
 74 [-]: NEWTABLE  R5 0 0       ; R5 := {}
 75 [-]: SETUPVAL  R5 U5        ; U82 := R5
 76 [-]: LOADKB    R5 0 0       ; R5 := false
 77 [-]: SETUPVAL  R5 U4        ; U82 := R4
 78 [-]: GETGLOBAL R5 K0        ; R5 := 0x7b998233
 79 [-]: GETUPVAL  R6 U6        ; R6 := U6
 80 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 81 [-]: TEST      R5 1         ; if R5 then PC := 87
 82 [-]: JMP       87           ; PC := 87
 83 [-]: GETUPVAL  R5 U6        ; R5 := U6
 84 [-]: SELF      R5 R5 K13    ; R6 := R5; R5 := R5[0xfaa69527]
 85 [-]: MOVE      R7 R0        ; R7 := R0
 86 [-]: CALL      R5 3 1       ; R5(R6,R7)
 87 [-]: GETGLOBAL R5 K0        ; R5 := 0x7b998233
 88 [-]: GETUPVAL  R6 U3        ; R6 := U3
 89 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 90 [-]: TEST      R5 1         ; if R5 then PC := 107
 91 [-]: JMP       107          ; PC := 107
 92 [-]: GETUPVAL  R5 U3        ; R5 := U3
 93 [-]: SELF      R5 R5 K14    ; R6 := R5; R5 := R5[0x91a24e4b]
 94 [-]: LOADK     R7 K15       ; R7 := "_root"
 95 [-]: CONST     R8 10        ; R8 := 10.000000
 96 [-]: CALL      R5 4 2       ; R5 := R5(R6,R7,R8)
 97 [-]: GETUPVAL  R6 U7        ; R6 := U7
 98 [-]: EQ        1 R6 R5      ; if R6 == R5 then PC := 107
 99 [-]: JMP       107          ; PC := 107
100 [-]: SETUPVAL  R5 U7        ; U82 := R7
101 [-]: GETGLOBAL R6 K1        ; R6 := 0xae91e43b
102 [-]: SELF      R6 R6 K16    ; R7 := R6; R6 := R6[0x67bc869f]
103 [-]: LOADK     R8 K15       ; R8 := "_root"
104 [-]: CONST     R9 10        ; R9 := 10.000000
105 [-]: MOVE      R10 R5       ; R10 := R5
106 [-]: CALL      R6 5 1       ; R6(R7,R8,R9,R10)
107 [-]: GETUPVAL  R6 U8        ; R6 := U8
108 [-]: CALL      R6 1 1       ; R6()
109 [-]: GETUPVAL  R6 U9        ; R6 := U9
110 [-]: CALL      R6 1 1       ; R6()
111 [-]: GETUPVAL  R6 U10       ; R6 := U10
112 [-]: CALL      R6 1 2       ; R6 := R6()
113 [-]: TEST      R6 0         ; if not R6 then PC := 142
114 [-]: JMP       142          ; PC := 142
115 [-]: GETUPVAL  R6 U11       ; R6 := U11
116 [-]: CALL      R6 1 2       ; R6 := R6()
117 [-]: TEST      R6 1         ; if R6 then PC := 129
118 [-]: JMP       129          ; PC := 129
119 [-]: GETGLOBAL R6 K1        ; R6 := 0xae91e43b
120 [-]: SELF      R6 R6 K17    ; R7 := R6; R6 := R6[0xd4cc05b4]
121 [-]: CALL      R6 2 2       ; R6 := R6(R7)
122 [-]: TEST      R6 0         ; if not R6 then PC := 129
123 [-]: JMP       129          ; PC := 129
124 [-]: GETGLOBAL R6 K1        ; R6 := 0xae91e43b
125 [-]: SELF      R6 R6 K18    ; R7 := R6; R6 := R6[0x368ad758]
126 [-]: LOADKB    R8 0 0       ; R8 := false
127 [-]: CALL      R6 3 1       ; R6(R7,R8)
128 [-]: JMP       142          ; PC := 142
129 [-]: GETUPVAL  R6 U11       ; R6 := U11
130 [-]: CALL      R6 1 2       ; R6 := R6()
131 [-]: TEST      R6 0         ; if not R6 then PC := 142
132 [-]: JMP       142          ; PC := 142
133 [-]: GETGLOBAL R6 K1        ; R6 := 0xae91e43b
134 [-]: SELF      R6 R6 K17    ; R7 := R6; R6 := R6[0xd4cc05b4]
135 [-]: CALL      R6 2 2       ; R6 := R6(R7)
136 [-]: TEST      R6 1         ; if R6 then PC := 142
137 [-]: JMP       142          ; PC := 142
138 [-]: GETGLOBAL R6 K1        ; R6 := 0xae91e43b
139 [-]: SELF      R6 R6 K18    ; R7 := R6; R6 := R6[0x368ad758]
140 [-]: LOADKB    R8 1 0       ; R8 := true
141 [-]: CALL      R6 3 1       ; R6(R7,R8)
142 [-]: GETUPVAL  R6 U13       ; R6 := U13
143 [-]: GETTABLE  R6 R6 K19    ; R6 := R6[0x0cad99b9]
144 [-]: GETGLOBAL R7 K1        ; R7 := 0xae91e43b
145 [-]: LOADK     R8 K20       ; R8 := "Quiver"
146 [-]: GETUPVAL  R9 U14       ; R9 := U14
147 [-]: GETUPVAL  R10 U12      ; R10 := U12
148 [-]: GETUPVAL  R11 U15      ; R11 := U15
149 [-]: GETUPVAL  R12 U1       ; R12 := U1
150 [-]: CALL      R6 7 2       ; R6 := R6(R7,R8,R9,R10,R11,R12)
151 [-]: SETUPVAL  R6 U12       ; U82 := R12
152 [-]: RETURN    R0 1         ; return 


; Function #11:
;
; Name:            
; Defined at line: 232
; #Upvalues:       2
; #Parameters:     1
; Is_vararg:       3
; Max Stack Size:  7

  1 [-]: GETUPVAL  R2 U0        ; R2 := U0
  2 [-]: TEST      R2 0         ; if not R2 then PC := 8
  3 [-]: JMP       8            ; PC := 8
  4 [-]: MOVE      R2 R0        ; R2 := R0
  5 [-]: VARARG    R3 0         ; R3 := ...
  6 [-]: CALL      R2 0 1       ; R2(R3,...)
  7 [-]: JMP       16           ; PC := 16
  8 [-]: GETGLOBAL R2 K0        ; R2 := 0x33bdd652
  9 [-]: GETTABLE  R2 R2 K1     ; R2 := R2[0x23d5322f]
 10 [-]: GETUPVAL  R3 U1        ; R3 := U1
 11 [-]: NEWTABLE  R4 1 0       ; R4 := {}
 12 [-]: MOVE      R5 R0        ; R5 := R0
 13 [-]: VARARG    R6 0         ; R6 := ...
 14 [-]: SETLIST   R4 0 1       ; R4[(1-1)*FPF+i] := R(4+i), 1 <= i <= 0
 15 [-]: CALL      R2 3 1       ; R2(R3,R4)
 16 [-]: RETURN    R0 1         ; return 


; Function #12:
;
; Name:            
; Defined at line: 240
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: GETGLOBAL R0 K0        ; R0 := _T
  2 [-]: SETTABLE  R0 K1 K2     ; R0["RANGER_SetArrowSwitchProp"] := nil
  3 [-]: GETGLOBAL R0 K0        ; R0 := _T
  4 [-]: SETTABLE  R0 K3 K2     ; R0["RANGER_SetActiveArrow"] := nil
  5 [-]: GETGLOBAL R0 K0        ; R0 := _T
  6 [-]: SETTABLE  R0 K4 K2     ; R0["RANGER_SetArrowTimer"] := nil
  7 [-]: GETGLOBAL R0 K0        ; R0 := _T
  8 [-]: SETTABLE  R0 K5 K2     ; R0["RANGER_ShowText"] := nil
  9 [-]: GETGLOBAL R0 K0        ; R0 := _T
 10 [-]: SETTABLE  R0 K6 K2     ; R0["RANGER_ShowControlParticles"] := nil
 11 [-]: GETGLOBAL R0 K7        ; R0 := 0x7b998233
 12 [-]: GETGLOBAL R1 K0        ; R1 := _T
 13 [-]: GETTABLE  R1 R1 K8     ; R1 := R1["HUD_GetAnchorMgr"]
 14 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 15 [-]: TEST      R0 1         ; if R0 then PC := 26
 16 [-]: JMP       26           ; PC := 26
 17 [-]: GETGLOBAL R0 K0        ; R0 := _T
 18 [-]: GETTABLE  R0 R0 K9     ; R0 := R0[0xf6b77431]
 19 [-]: CALL      R0 1 2       ; R0 := R0()
 20 [-]: EQ        1 R0 K2      ; if R0 == nil then PC := 26
 21 [-]: JMP       26           ; PC := 26
 22 [-]: SELF      R1 R0 K10    ; R2 := R0; R1 := R0[0x7f19c438]
 23 [-]: GETGLOBAL R3 K11       ; R3 := 0xae91e43b
 24 [-]: LOADK     R4 K12       ; R4 := "ArrowDirection"
 25 [-]: CALL      R1 4 1       ; R1(R2,R3,R4)
 26 [-]: GETGLOBAL R1 K7        ; R1 := 0x7b998233
 27 [-]: GETUPVAL  R2 U0        ; R2 := U0
 28 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 29 [-]: TEST      R1 1         ; if R1 then PC := 34
 30 [-]: JMP       34           ; PC := 34
 31 [-]: GETUPVAL  R1 U0        ; R1 := U0
 32 [-]: SELF      R1 R1 K13    ; R2 := R1; R1 := R1[0xa2880940]
 33 [-]: CALL      R1 2 1       ; R1(R2)
 34 [-]: GETGLOBAL R1 K7        ; R1 := 0x7b998233
 35 [-]: GETUPVAL  R2 U1        ; R2 := U1
 36 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 37 [-]: TEST      R1 1         ; if R1 then PC := 42
 38 [-]: JMP       42           ; PC := 42
 39 [-]: GETUPVAL  R1 U1        ; R1 := U1
 40 [-]: SELF      R1 R1 K14    ; R2 := R1; R1 := R1[0xdb371820]
 41 [-]: CALL      R1 2 1       ; R1(R2)
 42 [-]: RETURN    R0 1         ; return 


; Function #13:
;
; Name:            
; Defined at line: 263
; #Upvalues:       12
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  10

  1 [-]: GETGLOBAL R0 K0        ; R0 := _T
  2 [-]: GETTABLE  R0 R0 K1     ; R0 := R0[0xf6b77431]
  3 [-]: CALL      R0 1 2       ; R0 := R0()
  4 [-]: SELF      R1 R0 K2     ; R2 := R0; R1 := R0[0x20ff29f7]
  5 [-]: GETGLOBAL R3 K3        ; R3 := 0xae91e43b
  6 [-]: LOADK     R4 K4        ; R4 := "ArrowDirection"
  7 [-]: NEWTABLE  R5 2 0       ; R5 := {}
  8 [-]: GETTABLE  R6 R0 K5     ; R6 := R0["ANCHOR_V_CENTRE"]
  9 [-]: GETTABLE  R7 R0 K6     ; R7 := R0["ANCHOR_H_CENTRE"]
 10 [-]: SETLIST   R5 2 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 2
 11 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 12 [-]: SELF      R1 R0 K7     ; R2 := R0; R1 := R0[0xfaa69527]
 13 [-]: GETGLOBAL R3 K3        ; R3 := 0xae91e43b
 14 [-]: SELF      R3 R3 K8     ; R4 := R3; R3 := R3[0x6b837788]
 15 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 16 [-]: GETGLOBAL R4 K3        ; R4 := 0xae91e43b
 17 [-]: SELF      R4 R4 K9     ; R5 := R4; R4 := R4[0xaf9fda9f]
 18 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 19 [-]: LOADKB    R5 1 0       ; R5 := true
 20 [-]: GETTABLE  R6 R0 K10    ; R6 := R0["mHudScalePadding"]
 21 [-]: CALL      R1 6 1       ; R1(R2,R3,R4,R5,R6)
 22 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 23 [-]: SELF      R1 R1 K11    ; R2 := R1; R1 := R1[0x67bc869f]
 24 [-]: LOADK     R3 K12       ; R3 := "_root"
 25 [-]: CONST     R4 10        ; R4 := 10.000000
 26 [-]: CONST     R5 0         ; R5 := 0.000000
 27 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 28 [-]: GETUPVAL  R1 U0        ; R1 := U0
 29 [-]: CALL      R1 1 1       ; R1()
 30 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 31 [-]: SELF      R1 R1 K11    ; R2 := R1; R1 := R1[0x67bc869f]
 32 [-]: LOADK     R3 K4        ; R3 := "ArrowDirection"
 33 [-]: CONST     R4 10        ; R4 := 10.000000
 34 [-]: CONST     R5 0         ; R5 := 0.000000
 35 [-]: CALL      R1 5 1       ; R1(R2,R3,R4,R5)
 36 [-]: GETGLOBAL R1 K13       ; R1 := 0x2d0fad09
 37 [-]: LOADK     R2 K14       ; R2 := "Lotus.Interface.PowerSuitCustomHuds.AbilitySelector"
 38 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 39 [-]: GETTABLE  R2 R1 K15    ; R2 := R1[0xae6791ba]
 40 [-]: GETGLOBAL R3 K3        ; R3 := 0xae91e43b
 41 [-]: LOADK     R4 K16       ; R4 := "Quiver"
 42 [-]: CONST     R5 0         ; R5 := 0.000000
 43 [-]: LOADK     R6 K17       ; R6 := "/Lotus/Language/Items/RangerQuiverHoldToSwitch"
 44 [-]: CALL      R2 5 2       ; R2 := R2(R3,R4,R5,R6)
 45 [-]: SETUPVAL  R2 U1        ; U82 := R1
 46 [-]: GETUPVAL  R2 U1        ; R2 := U1
 47 [-]: GETGLOBAL R3 K19       ; R3 := 0x9971cc83
 48 [-]: SETTABLE  R2 K18 R3    ; R2["mAbilityIcons"] := R3
 49 [-]: GETUPVAL  R2 U1        ; R2 := U1
 50 [-]: GETUPVAL  R3 U2        ; R3 := U2
 51 [-]: GETTABLE  R3 R3 K21    ; R3 := R3[0x06d055f9]
 52 [-]: GETUPVAL  R4 U3        ; R4 := U3
 53 [-]: GETTABLE  R4 R4 K22    ; R4 := R4[0x32316a21]
 54 [-]: CALL      R4 1 2       ; R4 := R4()
 55 [-]: CONST     R5 3         ; R5 := 3.000000
 56 [-]: GETUPVAL  R6 U1        ; R6 := U1
 57 [-]: GETTABLE  R6 R6 K20    ; R6 := R6["MAX_ABILITIES"]
 58 [-]: CALL      R3 4 2       ; R3 := R3(R4,R5,R6)
 59 [-]: SETTABLE  R2 K20 R3    ; R2["MAX_ABILITIES"] := R3
 60 [-]: GETGLOBAL R2 K0        ; R2 := _T
 61 [-]: CLOSURE   R3 0         ; R3 := closure(Function #13.1)
 62 [-]: GETUPVAL  R0 U4        ; R0 := U4
 63 [-]: GETUPVAL  R0 U1        ; R0 := U1
 64 [-]: SETTABLE  R2 K23 R3    ; R2["RANGER_SetArrowSwitchProp"] := R3
 65 [-]: GETGLOBAL R2 K0        ; R2 := _T
 66 [-]: CLOSURE   R3 1         ; R3 := closure(Function #13.2)
 67 [-]: GETUPVAL  R0 U4        ; R0 := U4
 68 [-]: GETUPVAL  R0 U1        ; R0 := U1
 69 [-]: SETTABLE  R2 K24 R3    ; R2["RANGER_SetActiveArrow"] := R3
 70 [-]: GETGLOBAL R2 K0        ; R2 := _T
 71 [-]: CLOSURE   R3 2         ; R3 := closure(Function #13.3)
 72 [-]: GETUPVAL  R0 U4        ; R0 := U4
 73 [-]: GETUPVAL  R0 U1        ; R0 := U1
 74 [-]: SETTABLE  R2 K25 R3    ; R2["RANGER_SetArrowTimer"] := R3
 75 [-]: GETGLOBAL R2 K0        ; R2 := _T
 76 [-]: CLOSURE   R3 3         ; R3 := closure(Function #13.4)
 77 [-]: GETUPVAL  R0 U4        ; R0 := U4
 78 [-]: GETUPVAL  R0 U1        ; R0 := U1
 79 [-]: SETTABLE  R2 K26 R3    ; R2["RANGER_ShowText"] := R3
 80 [-]: GETGLOBAL R2 K0        ; R2 := _T
 81 [-]: CLOSURE   R3 4         ; R3 := closure(Function #13.5)
 82 [-]: GETUPVAL  R0 U4        ; R0 := U4
 83 [-]: GETUPVAL  R0 U5        ; R0 := U5
 84 [-]: SETTABLE  R2 K27 R3    ; R2["RANGER_ShowControlParticles"] := R3
 85 [-]: GETGLOBAL R2 K0        ; R2 := _T
 86 [-]: GETUPVAL  R3 U1        ; R3 := U1
 87 [-]: GETTABLE  R3 R3 K29    ; R3 := R3["GetAbilityLocTag"]
 88 [-]: SETTABLE  R2 K28 R3    ; R2["RANGER_GetArrowLocTag"] := R3
 89 [-]: GETUPVAL  R2 U1        ; R2 := U1
 90 [-]: SELF      R2 R2 K30    ; R3 := R2; R2 := R2[0x687ae094]
 91 [-]: CALL      R2 2 1       ; R2(R3)
 92 [-]: GETUPVAL  R2 U1        ; R2 := U1
 93 [-]: GETTABLE  R2 R2 K31    ; R2 := R2["mAbilityProperties"]
 94 [-]: GETTABLE  R2 R2 K32    ; R2 := R2[1.000000]
 95 [-]: GETGLOBAL R3 K34       ; R3 := 0x0469f296
 96 [-]: LOADK     R4 K35       ; R4 := "/Lotus/Language/Items/RangerQuiverArrow1Name"
 97 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 98 [-]: SETTABLE  R2 K33 R3    ; R2["Name"] := R3
 99 [-]: GETUPVAL  R2 U1        ; R2 := U1
100 [-]: GETTABLE  R2 R2 K31    ; R2 := R2["mAbilityProperties"]
101 [-]: GETTABLE  R2 R2 K36    ; R2 := R2[2.000000]
102 [-]: GETGLOBAL R3 K34       ; R3 := 0x0469f296
103 [-]: GETUPVAL  R4 U2        ; R4 := U2
104 [-]: GETTABLE  R4 R4 K21    ; R4 := R4[0x06d055f9]
105 [-]: GETUPVAL  R5 U3        ; R5 := U3
106 [-]: GETTABLE  R5 R5 K22    ; R5 := R5[0x32316a21]
107 [-]: CALL      R5 1 2       ; R5 := R5()
108 [-]: LOADK     R6 K37       ; R6 := "/Lotus/Language/Items/RangerPvpQuiverArrow2Name"
109 [-]: LOADK     R7 K38       ; R7 := "/Lotus/Language/Items/RangerQuiverArrow2Name"
110 [-]: CALL      R4 4 0       ; R4,... := R4(R5,R6,R7)
111 [-]: CALL      R3 0 2       ; R3 := R3(R4,...)
112 [-]: SETTABLE  R2 K33 R3    ; R2["Name"] := R3
113 [-]: GETUPVAL  R2 U1        ; R2 := U1
114 [-]: GETTABLE  R2 R2 K31    ; R2 := R2["mAbilityProperties"]
115 [-]: GETTABLE  R2 R2 K39    ; R2 := R2[3.000000]
116 [-]: GETGLOBAL R3 K34       ; R3 := 0x0469f296
117 [-]: GETUPVAL  R4 U2        ; R4 := U2
118 [-]: GETTABLE  R4 R4 K21    ; R4 := R4[0x06d055f9]
119 [-]: GETUPVAL  R5 U3        ; R5 := U3
120 [-]: GETTABLE  R5 R5 K22    ; R5 := R5[0x32316a21]
121 [-]: CALL      R5 1 2       ; R5 := R5()
122 [-]: LOADK     R6 K40       ; R6 := "/Lotus/Language/Items/RangerPvpQuiverArrow3Name"
123 [-]: LOADK     R7 K41       ; R7 := "/Lotus/Language/Items/RangerQuiverArrow3Name"
124 [-]: CALL      R4 4 0       ; R4,... := R4(R5,R6,R7)
125 [-]: CALL      R3 0 2       ; R3 := R3(R4,...)
126 [-]: SETTABLE  R2 K33 R3    ; R2["Name"] := R3
127 [-]: GETGLOBAL R2 K42       ; R2 := 0x7b998233
128 [-]: GETUPVAL  R3 U1        ; R3 := U1
129 [-]: GETTABLE  R3 R3 K31    ; R3 := R3["mAbilityProperties"]
130 [-]: GETTABLE  R3 R3 K43    ; R3 := R3[4.000000]
131 [-]: CALL      R2 2 2       ; R2 := R2(R3)
132 [-]: TEST      R2 1         ; if R2 then PC := 141
133 [-]: JMP       141          ; PC := 141
134 [-]: GETUPVAL  R2 U1        ; R2 := U1
135 [-]: GETTABLE  R2 R2 K31    ; R2 := R2["mAbilityProperties"]
136 [-]: GETTABLE  R2 R2 K43    ; R2 := R2[4.000000]
137 [-]: GETGLOBAL R3 K34       ; R3 := 0x0469f296
138 [-]: LOADK     R4 K44       ; R4 := "/Lotus/Language/Items/RangerQuiverArrow4Name"
139 [-]: CALL      R3 2 2       ; R3 := R3(R4)
140 [-]: SETTABLE  R2 K33 R3    ; R2["Name"] := R3
141 [-]: GETGLOBAL R2 K42       ; R2 := 0x7b998233
142 [-]: GETGLOBAL R3 K0        ; R3 := _T
143 [-]: GETTABLE  R3 R3 K45    ; R3 := R3["rangerQuiver"]
144 [-]: CALL      R2 2 2       ; R2 := R2(R3)
145 [-]: TEST      R2 1         ; if R2 then PC := 166
146 [-]: JMP       166          ; PC := 166
147 [-]: GETUPVAL  R2 U6        ; R2 := U6
148 [-]: CALL      R2 1 1       ; R2()
149 [-]: GETGLOBAL R2 K42       ; R2 := 0x7b998233
150 [-]: GETGLOBAL R3 K0        ; R3 := _T
151 [-]: GETTABLE  R3 R3 K45    ; R3 := R3["rangerQuiver"]
152 [-]: GETTABLE  R3 R3 K46    ; R3 := R3["projIndex"]
153 [-]: CALL      R2 2 2       ; R2 := R2(R3)
154 [-]: TEST      R2 1         ; if R2 then PC := 166
155 [-]: JMP       166          ; PC := 166
156 [-]: GETGLOBAL R2 K0        ; R2 := _T
157 [-]: GETTABLE  R2 R2 K47    ; R2 := R2[0x3216b1eb]
158 [-]: GETGLOBAL R3 K0        ; R3 := _T
159 [-]: GETTABLE  R3 R3 K45    ; R3 := R3["rangerQuiver"]
160 [-]: GETTABLE  R3 R3 K46    ; R3 := R3["projIndex"]
161 [-]: GETUPVAL  R4 U1        ; R4 := U1
162 [-]: GETTABLE  R4 R4 K20    ; R4 := R4["MAX_ABILITIES"]
163 [-]: MOD       R3 R3 R4     ; R3 := R3 % R4
164 [-]: ADD       R3 R3 K32    ; R3 := R3 + 1.000000
165 [-]: CALL      R2 2 1       ; R2(R3)
166 [-]: SELF      R2 R0 K48    ; R3 := R0; R2 := R0[0x9d1db3eb]
167 [-]: LOADK     R4 K16       ; R4 := "Quiver"
168 [-]: CALL      R2 3 2       ; R2 := R2(R3,R4)
169 [-]: GETTABLE  R2 R2 K49    ; R2 := R2["y"]
170 [-]: SETUPVAL  R2 U7        ; U82 := R7
171 [-]: GETGLOBAL R2 K50       ; R2 := 0x89326c93
172 [-]: SELF      R2 R2 K51    ; R3 := R2; R2 := R2[0x78298275]
173 [-]: CALL      R2 2 2       ; R2 := R2(R3)
174 [-]: SETUPVAL  R2 U8        ; U82 := R8
175 [-]: GETGLOBAL R2 K42       ; R2 := 0x7b998233
176 [-]: GETUPVAL  R3 U8        ; R3 := U8
177 [-]: CALL      R2 2 2       ; R2 := R2(R3)
178 [-]: TEST      R2 1         ; if R2 then PC := 224
179 [-]: JMP       224          ; PC := 224
180 [-]: GETUPVAL  R2 U8        ; R2 := U8
181 [-]: SELF      R2 R2 K52    ; R3 := R2; R2 := R2[0x5e651723]
182 [-]: CALL      R2 2 2       ; R2 := R2(R3)
183 [-]: GETGLOBAL R3 K42       ; R3 := 0x7b998233
184 [-]: MOVE      R4 R2        ; R4 := R2
185 [-]: CALL      R3 2 2       ; R3 := R3(R4)
186 [-]: TEST      R3 1         ; if R3 then PC := 191
187 [-]: JMP       191          ; PC := 191
188 [-]: SELF      R3 R2 K53    ; R4 := R2; R3 := R2[0x0803eee1]
189 [-]: CALL      R3 2 2       ; R3 := R3(R4)
190 [-]: SETUPVAL  R3 U9        ; U82 := R9
191 [-]: GETUPVAL  R3 U8        ; R3 := U8
192 [-]: SELF      R3 R3 K54    ; R4 := R3; R3 := R3[0xde321e6f]
193 [-]: CALL      R3 2 2       ; R3 := R3(R4)
194 [-]: SETUPVAL  R3 U10       ; U82 := R10
195 [-]: GETGLOBAL R3 K42       ; R3 := 0x7b998233
196 [-]: GETUPVAL  R4 U10       ; R4 := U10
197 [-]: CALL      R3 2 2       ; R3 := R3(R4)
198 [-]: TEST      R3 1         ; if R3 then PC := 224
199 [-]: JMP       224          ; PC := 224
200 [-]: GETUPVAL  R3 U10       ; R3 := U10
201 [-]: SELF      R3 R3 K55    ; R4 := R3; R3 := R3[0xf7d48ee0]
202 [-]: CALL      R3 2 2       ; R3 := R3(R4)
203 [-]: GETGLOBAL R4 K42       ; R4 := 0x7b998233
204 [-]: MOVE      R5 R3        ; R5 := R3
205 [-]: CALL      R4 2 2       ; R4 := R4(R5)
206 [-]: TEST      R4 1         ; if R4 then PC := 224
207 [-]: JMP       224          ; PC := 224
208 [-]: SELF      R4 R3 K56    ; R5 := R3; R4 := R3[0x689412a5]
209 [-]: GETGLOBAL R6 K57       ; R6 := 0x7ed0a956
210 [-]: LOADK     R7 K58       ; R7 := "/Lotus/Powersuits/PowersuitAbilities/RangerQuiverAbility"
211 [-]: CALL      R6 2 0       ; R6,... := R6(R7)
212 [-]: CALL      R4 0 2       ; R4 := R4(R5,...)
213 [-]: GETGLOBAL R5 K42       ; R5 := 0x7b998233
214 [-]: MOVE      R6 R4        ; R6 := R4
215 [-]: CALL      R5 2 2       ; R5 := R5(R6)
216 [-]: TEST      R5 0         ; if not R5 then PC := 224
217 [-]: JMP       224          ; PC := 224
218 [-]: GETGLOBAL R5 K3        ; R5 := 0xae91e43b
219 [-]: SELF      R5 R5 K11    ; R6 := R5; R5 := R5[0x67bc869f]
220 [-]: LOADK     R7 K16       ; R7 := "Quiver"
221 [-]: CONST     R8 10        ; R8 := 10.000000
222 [-]: CONST     R9 0         ; R9 := 0.000000
223 [-]: CALL      R5 5 1       ; R5(R6,R7,R8,R9)
224 [-]: LOADKB    R5 1 0       ; R5 := true
225 [-]: SETUPVAL  R5 U11       ; U82 := R11
226 [-]: RETURN    R0 1         ; return 


; Function #13.1:
;
; Name:            
; Defined at line: 279
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       3
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETUPVAL  R2 U1        ; R2 := U1
  3 [-]: GETTABLE  R2 R2 K0     ; R2 := R2["SetAbilitySwitchProp"]
  4 [-]: VARARG    R3 0         ; R3 := ...
  5 [-]: CALL      R1 0 1       ; R1(R2,...)
  6 [-]: RETURN    R0 1         ; return 


; Function #13.2:
;
; Name:            
; Defined at line: 280
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       3
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETUPVAL  R2 U1        ; R2 := U1
  3 [-]: GETTABLE  R2 R2 K0     ; R2 := R2["SetActiveAbility"]
  4 [-]: VARARG    R3 0         ; R3 := ...
  5 [-]: CALL      R1 0 1       ; R1(R2,...)
  6 [-]: RETURN    R0 1         ; return 


; Function #13.3:
;
; Name:            
; Defined at line: 281
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       3
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETUPVAL  R2 U1        ; R2 := U1
  3 [-]: GETTABLE  R2 R2 K0     ; R2 := R2["SetAbilityTimer"]
  4 [-]: VARARG    R3 0         ; R3 := ...
  5 [-]: CALL      R1 0 1       ; R1(R2,...)
  6 [-]: RETURN    R0 1         ; return 


; Function #13.4:
;
; Name:            
; Defined at line: 282
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       3
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETUPVAL  R2 U1        ; R2 := U1
  3 [-]: GETTABLE  R2 R2 K0     ; R2 := R2["ShowText"]
  4 [-]: VARARG    R3 0         ; R3 := ...
  5 [-]: CALL      R1 0 1       ; R1(R2,...)
  6 [-]: RETURN    R0 1         ; return 


; Function #13.5:
;
; Name:            
; Defined at line: 283
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       3
; Max Stack Size:  4

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETUPVAL  R2 U1        ; R2 := U1
  3 [-]: VARARG    R3 0         ; R3 := ...
  4 [-]: CALL      R1 0 1       ; R1(R2,...)
  5 [-]: RETURN    R0 1         ; return 


; Function #14:
;
; Name:            
; Defined at line: 329
; #Upvalues:       1
; #Parameters:     1
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: GETUPVAL  R1 U0        ; R1 := U0
  2 [-]: GETTABLE  R1 R1 K0     ; R1 := R1[0xfa221145]
  3 [-]: GETGLOBAL R2 K1        ; R2 := 0xae91e43b
  4 [-]: GETGLOBAL R3 K2        ; R3 := 0x03f57322
  5 [-]: MOVE      R4 R0        ; R4 := R0
  6 [-]: CALL      R3 2 0       ; R3,... := R3(R4)
  7 [-]: CALL      R1 0 1       ; R1(R2,...)
  8 [-]: RETURN    R0 1         ; return 


