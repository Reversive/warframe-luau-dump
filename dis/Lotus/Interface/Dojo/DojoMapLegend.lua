; This file has been disassembled using luadec 2.0.2 by sztupy (modified by Reversive)

; Name:            
; Defined at line: 0
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       2
; Max Stack Size:  19

  1 [-]: NEWTABLE  R0 13 0      ; R0 := {}
  2 [-]: GETGLOBAL R1 K0        ; R1 := 0x0469f296
  3 [-]: LOADK     R2 K1        ; R2 := "/Lotus/Language/Dojo/ResearchLabCorpusName"
  4 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  5 [-]: GETGLOBAL R2 K0        ; R2 := 0x0469f296
  6 [-]: LOADK     R3 K2        ; R3 := "/Lotus/Language/Dojo/ResearchLabGrineerName"
  7 [-]: CALL      R2 2 2       ; R2 := R2(R3)
  8 [-]: GETGLOBAL R3 K0        ; R3 := 0x0469f296
  9 [-]: LOADK     R4 K3        ; R4 := "/Lotus/Language/Dojo/ResearchLabInfestedName"
 10 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 11 [-]: GETGLOBAL R4 K0        ; R4 := 0x0469f296
 12 [-]: LOADK     R5 K4        ; R5 := "/Lotus/Language/Dojo/ResearchLabTennoName"
 13 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 14 [-]: GETGLOBAL R5 K0        ; R5 := 0x0469f296
 15 [-]: LOADK     R6 K5        ; R6 := "/Lotus/Language/Dojo/ResearchLabOrokinName"
 16 [-]: CALL      R5 2 2       ; R5 := R5(R6)
 17 [-]: GETGLOBAL R6 K0        ; R6 := 0x0469f296
 18 [-]: LOADK     R7 K6        ; R7 := "/Lotus/Language/Dojo/DojoDuelRoomAName"
 19 [-]: CALL      R6 2 2       ; R6 := R6(R7)
 20 [-]: GETGLOBAL R7 K0        ; R7 := 0x0469f296
 21 [-]: LOADK     R8 K7        ; R8 := "/Lotus/Language/Dojo/ClanObstacleCourseAName"
 22 [-]: CALL      R7 2 2       ; R7 := R7(R8)
 23 [-]: GETGLOBAL R8 K0        ; R8 := 0x0469f296
 24 [-]: LOADK     R9 K8        ; R9 := "/Lotus/Language/Dojo/TradingPostLegendTitle"
 25 [-]: CALL      R8 2 2       ; R8 := R8(R9)
 26 [-]: GETGLOBAL R9 K0        ; R9 := 0x0469f296
 27 [-]: LOADK     R10 K9       ; R10 := "/Lotus/Language/Dojo/AscensionAltarTitle"
 28 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 29 [-]: GETGLOBAL R10 K0       ; R10 := 0x0469f296
 30 [-]: LOADK     R11 K10      ; R11 := "/Lotus/Language/Dojo/VaultTitle"
 31 [-]: CALL      R10 2 2      ; R10 := R10(R11)
 32 [-]: GETGLOBAL R11 K0       ; R11 := 0x0469f296
 33 [-]: LOADK     R12 K11      ; R12 := "/Lotus/Language/Railjack/SummonConsole"
 34 [-]: CALL      R11 2 2      ; R11 := R11(R12)
 35 [-]: GETGLOBAL R12 K0       ; R12 := 0x0469f296
 36 [-]: LOADK     R13 K12      ; R13 := "/Lotus/Language/Railjack/ResearchConsole"
 37 [-]: CALL      R12 2 2      ; R12 := R12(R13)
 38 [-]: GETGLOBAL R13 K0       ; R13 := 0x0469f296
 39 [-]: LOADK     R14 K13      ; R14 := "/Lotus/Language/Railjack/CustomizationConsole"
 40 [-]: CALL      R13 2 2      ; R13 := R13(R14)
 41 [-]: GETGLOBAL R14 K0       ; R14 := 0x0469f296
 42 [-]: LOADK     R15 K14      ; R15 := "/Lotus/Language/Railjack/BoardingPad"
 43 [-]: CALL      R14 2 0      ; R14,... := R14(R15)
 44 [-]: SETLIST   R0 0 1       ; R0[(1-1)*FPF+i] := R(0+i), 1 <= i <= 0
 45 [-]: NEWTABLE  R1 14 0      ; R1 := {}
 46 [-]: LOADK     R2 18        ; R2 := 18.000000
 47 [-]: LOADK     R3 16        ; R3 := 16.000000
 48 [-]: LOADK     R4 17        ; R4 := 17.000000
 49 [-]: LOADK     R5 19        ; R5 := 19.000000
 50 [-]: LOADK     R6 20        ; R6 := 20.000000
 51 [-]: LOADK     R7 21        ; R7 := 21.000000
 52 [-]: LOADK     R8 22        ; R8 := 22.000000
 53 [-]: LOADK     R9 88        ; R9 := 88.000000
 54 [-]: LOADK     R10 89       ; R10 := 89.000000
 55 [-]: LOADK     R11 90       ; R11 := 90.000000
 56 [-]: LOADK     R12 92       ; R12 := 92.000000
 57 [-]: LOADK     R13 93       ; R13 := 93.000000
 58 [-]: LOADK     R14 94       ; R14 := 94.000000
 59 [-]: LOADK     R15 95       ; R15 := 95.000000
 60 [-]: SETLIST   R1 14 1      ; R1[(1-1)*FPF+i] := R(1+i), 1 <= i <= 14
 61 [-]: GETGLOBAL R2 K0        ; R2 := 0x0469f296
 62 [-]: LOADK     R3 K16       ; R3 := "/Lotus/Language/Dojo/ClanElevatorAName"
 63 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 64 [-]: GETGLOBAL R3 K17       ; R3 := 0x2d0fad09
 65 [-]: LOADK     R4 K18       ; R4 := "EE.Interface.Utilities"
 66 [-]: CALL      R3 2 2       ; R3 := R3(R4)
 67 [-]: GETGLOBAL R4 K17       ; R4 := 0x2d0fad09
 68 [-]: LOADK     R5 K19       ; R5 := "EE.Interface.AnchorMgr"
 69 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 70 [-]: LOADNIL   R5 R5        ; R5 := nil
 71 [-]: LOADBOOL  R6 0 0       ; R6 := false
 72 [-]: LOADBOOL  R7 1 0       ; R7 := true
 73 [-]: LOADNIL   R8 R10       ; R8 := R9 := R10 := nil
 74 [-]: LOADBOOL  R11 0 0      ; R11 := false
 75 [-]: LOADBOOL  R12 1 0      ; R12 := true
 76 [-]: LOADNIL   R13 R13      ; R13 := nil
 77 [-]: CLOSURE   R14 0        ; R14 := closure(Function #1)
 78 [-]: SETGLOBAL R14 K20      ; Shutdown := R14
 79 [-]: CLOSURE   R14 1        ; R14 := closure(Function #2)
 80 [-]: MOVE      R0 R7        ; R0 := R7
 81 [-]: CLOSURE   R15 2        ; R15 := closure(Function #3)
 82 [-]: MOVE      R0 R14       ; R0 := R14
 83 [-]: SETGLOBAL R15 K21      ; TransitionOut := R15
 84 [-]: CLOSURE   R15 3        ; R15 := closure(Function #4)
 85 [-]: MOVE      R0 R8        ; R0 := R8
 86 [-]: MOVE      R0 R3        ; R0 := R3
 87 [-]: MOVE      R0 R14       ; R0 := R14
 88 [-]: CLOSURE   R16 4        ; R16 := closure(Function #5)
 89 [-]: MOVE      R0 R13       ; R0 := R13
 90 [-]: CLOSURE   R17 5        ; R17 := closure(Function #6)
 91 [-]: MOVE      R0 R10       ; R0 := R10
 92 [-]: MOVE      R0 R9        ; R0 := R9
 93 [-]: MOVE      R0 R5        ; R0 := R5
 94 [-]: MOVE      R0 R13       ; R0 := R13
 95 [-]: MOVE      R0 R2        ; R0 := R2
 96 [-]: MOVE      R0 R1        ; R0 := R1
 97 [-]: MOVE      R0 R0        ; R0 := R0
 98 [-]: CLOSURE   R18 6        ; R18 := closure(Function #7)
 99 [-]: MOVE      R0 R6        ; R0 := R6
100 [-]: MOVE      R0 R15       ; R0 := R15
101 [-]: MOVE      R0 R9        ; R0 := R9
102 [-]: MOVE      R0 R12       ; R0 := R12
103 [-]: SETGLOBAL R18 K22      ; Update := R18
104 [-]: CLOSURE   R18 7        ; R18 := closure(Function #8)
105 [-]: MOVE      R0 R3        ; R0 := R3
106 [-]: MOVE      R0 R5        ; R0 := R5
107 [-]: MOVE      R0 R4        ; R0 := R4
108 [-]: MOVE      R0 R17       ; R0 := R17
109 [-]: MOVE      R0 R16       ; R0 := R16
110 [-]: MOVE      R0 R6        ; R0 := R6
111 [-]: SETGLOBAL R18 K23      ; Initialize := R18
112 [-]: CLOSURE   R18 8        ; R18 := closure(Function #9)
113 [-]: MOVE      R0 R14       ; R0 := R14
114 [-]: SETGLOBAL R18 K24      ; Close := R18
115 [-]: CLOSURE   R18 9        ; R18 := closure(Function #10)
116 [-]: MOVE      R0 R15       ; R0 := R15
117 [-]: SETGLOBAL R18 K25      ; ExitScreen := R18
118 [-]: CLOSURE   R18 10       ; R18 := closure(Function #11)
119 [-]: MOVE      R0 R6        ; R0 := R6
120 [-]: MOVE      R0 R9        ; R0 := R9
121 [-]: MOVE      R0 R11       ; R0 := R11
122 [-]: SETGLOBAL R18 K26      ; ToggleShow := R18
123 [-]: CLOSURE   R18 11       ; R18 := closure(Function #12)
124 [-]: MOVE      R0 R6        ; R0 := R6
125 [-]: MOVE      R0 R9        ; R0 := R9
126 [-]: MOVE      R0 R11       ; R0 := R11
127 [-]: SETGLOBAL R18 K27      ; Hide := R18
128 [-]: CLOSURE   R18 12       ; R18 := closure(Function #13)
129 [-]: MOVE      R0 R6        ; R0 := R6
130 [-]: MOVE      R0 R11       ; R0 := R11
131 [-]: SETGLOBAL R18 K28      ; Show := R18
132 [-]: CLOSURE   R18 13       ; R18 := closure(Function #14)
133 [-]: MOVE      R0 R5        ; R0 := R5
134 [-]: SETGLOBAL R18 K29      ; onViewportSizeChanged := R18
135 [-]: CLOSURE   R18 14       ; R18 := closure(Function #15)
136 [-]: MOVE      R0 R16       ; R0 := R16
137 [-]: SETGLOBAL R18 K30      ; OnStyleChangedCallback := R18
138 [-]: CLOSURE   R18 15       ; R18 := closure(Function #16)
139 [-]: SETGLOBAL R18 K31      ; SupportsThemes := R18
140 [-]: RETURN    R0 1         ; return 


; Function #1:
;
; Name:            
; Defined at line: 51
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: RETURN    R0 1         ; return 


; Function #2:
;
; Name:            
; Defined at line: 54
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  9

  1 [-]: LOADBOOL  R0 1 0       ; R0 := true
  2 [-]: SETUPVAL  R0 U0        ; U82 := R0
  3 [-]: GETGLOBAL R0 K0        ; R0 := 0x25312c9b
  4 [-]: GETGLOBAL R1 K1        ; R1 := 0xae91e43b
  5 [-]: LOADK     R2 K2        ; R2 := "_root"
  6 [-]: LOADK     R3 0         ; R3 := 0.000000
  7 [-]: NEWTABLE  R4 1 0       ; R4 := {}
  8 [-]: LOADK     R5 10        ; R5 := 10.000000
  9 [-]: SETLIST   R4 1 1       ; R4[(1-1)*FPF+i] := R(4+i), 1 <= i <= 1
 10 [-]: NEWTABLE  R5 1 0       ; R5 := {}
 11 [-]: LOADK     R6 0         ; R6 := 0.000000
 12 [-]: SETLIST   R5 1 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 1
 13 [-]: LOADK     R6 K4        ; R6 := 0.100000
 14 [-]: LOADK     R7 0         ; R7 := 0.000000
 15 [-]: CLOSURE   R8 0         ; R8 := closure(Function #2.1)
 16 [-]: CALL      R0 9 1       ; R0(R1,R2,R3,R4,R5,R6,R7,R8)
 17 [-]: RETURN    R0 1         ; return 


; Function #2.1:
;
; Name:            
; Defined at line: 57
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0xae91e43b
  2 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x32302b4a]
  3 [-]: CALL      R0 2 1       ; R0(R1)
  4 [-]: RETURN    R0 1         ; return 


; Function #3:
;
; Name:            
; Defined at line: 62
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: CALL      R0 1 1       ; R0()
  3 [-]: RETURN    R0 1         ; return 


; Function #4:
;
; Name:            
; Defined at line: 66
; #Upvalues:       3
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: TEST      R0 1         ; if R0 then PC := 13
  3 [-]: JMP       13           ; PC := 13
  4 [-]: LOADBOOL  R0 1 0       ; R0 := true
  5 [-]: SETUPVAL  R0 U0        ; U82 := R0
  6 [-]: GETUPVAL  R0 U1        ; R0 := U1
  7 [-]: GETTABLE  R0 R0 K0     ; R0 := R0[0x659d451f]
  8 [-]: GETGLOBAL R1 K1        ; R1 := 0x0032441c
  9 [-]: GETTABLE  R1 R1 K2     ; R1 := R1["UISound_Close"]
 10 [-]: CALL      R0 2 1       ; R0(R1)
 11 [-]: GETUPVAL  R0 U2        ; R0 := U2
 12 [-]: CALL      R0 1 1       ; R0()
 13 [-]: RETURN    R0 1         ; return 


; Function #5:
;
; Name:            
; Defined at line: 74
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
  2 [-]: GETUPVAL  R1 U0        ; R1 := U0
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: TEST      R0 1         ; if R0 then PC := 9
  5 [-]: JMP       9            ; PC := 9
  6 [-]: GETUPVAL  R0 U0        ; R0 := U0
  7 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x71e9ac81]
  8 [-]: CALL      R0 2 1       ; R0(R1)
  9 [-]: RETURN    R0 1         ; return 


; Function #6:
;
; Name:            
; Defined at line: 80
; #Upvalues:       7
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  13

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0x89326c93
  2 [-]: SELF      R0 R0 K1     ; R1 := R0; R0 := R0[0x7d108ddb]
  3 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  4 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
  5 [-]: MOVE      R2 R0        ; R2 := R0
  6 [-]: CALL      R1 2 2       ; R1 := R1(R2)
  7 [-]: TEST      R1 1         ; if R1 then PC := 18
  8 [-]: JMP       18           ; PC := 18
  9 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
 10 [-]: GETTABLE  R2 R0 K3     ; R2 := R0[1.000000]
 11 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 12 [-]: TEST      R1 1         ; if R1 then PC := 18
 13 [-]: JMP       18           ; PC := 18
 14 [-]: GETTABLE  R1 R0 K3     ; R1 := R0[1.000000]
 15 [-]: SELF      R1 R1 K4     ; R2 := R1; R1 := R1[0xbb610e5b]
 16 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 17 [-]: SETUPVAL  R1 U0        ; U82 := R0
 18 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
 19 [-]: GETUPVAL  R2 U0        ; R2 := U0
 20 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 21 [-]: TEST      R1 0         ; if not R1 then PC := 27
 22 [-]: JMP       27           ; PC := 27
 23 [-]: GETGLOBAL R1 K5        ; R1 := 0x3d106989
 24 [-]: LOADK     R2 K6        ; R2 := "ERROR: No avatar found for map legend!"
 25 [-]: CALL      R1 2 1       ; R1(R2)
 26 [-]: RETURN    R0 1         ; return 
 27 [-]: GETUPVAL  R1 U0        ; R1 := U0
 28 [-]: SELF      R1 R1 K7     ; R2 := R1; R1 := R1[0x5e651723]
 29 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 30 [-]: SELF      R1 R1 K8     ; R2 := R1; R1 := R1[0x474501e1]
 31 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 32 [-]: SETUPVAL  R1 U1        ; U82 := R1
 33 [-]: GETGLOBAL R1 K2        ; R1 := 0x7b998233
 34 [-]: GETUPVAL  R2 U1        ; R2 := U1
 35 [-]: CALL      R1 2 2       ; R1 := R1(R2)
 36 [-]: NOT       R1 R1        ; R1 := not R1
 37 [-]: GETGLOBAL R2 K9        ; R2 := 0xae91e43b
 38 [-]: SELF      R2 R2 K10    ; R3 := R2; R2 := R2[0xaade900e]
 39 [-]: LOADK     R4 K11       ; R4 := "StatContainer"
 40 [-]: LOADK     R5 11        ; R5 := 11.000000
 41 [-]: MOVE      R6 R1        ; R6 := R1
 42 [-]: CALL      R2 5 1       ; R2(R3,R4,R5,R6)
 43 [-]: TEST      R1 0         ; if not R1 then PC := 165
 44 [-]: JMP       165          ; PC := 165
 45 [-]: GETUPVAL  R2 U2        ; R2 := U2
 46 [-]: SELF      R2 R2 K12    ; R3 := R2; R2 := R2[0x20ff29f7]
 47 [-]: GETGLOBAL R4 K9        ; R4 := 0xae91e43b
 48 [-]: LOADK     R5 K11       ; R5 := "StatContainer"
 49 [-]: NEWTABLE  R6 2 0       ; R6 := {}
 50 [-]: GETUPVAL  R7 U2        ; R7 := U2
 51 [-]: GETTABLE  R7 R7 K13    ; R7 := R7["ANCHOR_V_BOTTOM"]
 52 [-]: GETUPVAL  R8 U2        ; R8 := U2
 53 [-]: GETTABLE  R8 R8 K14    ; R8 := R8["ANCHOR_H_LEFT"]
 54 [-]: SETLIST   R6 2 1       ; R6[(1-1)*FPF+i] := R(6+i), 1 <= i <= 2
 55 [-]: CALL      R2 5 1       ; R2(R3,R4,R5,R6)
 56 [-]: GETUPVAL  R2 U2        ; R2 := U2
 57 [-]: SELF      R2 R2 K15    ; R3 := R2; R2 := R2[0xfaa69527]
 58 [-]: GETGLOBAL R4 K9        ; R4 := 0xae91e43b
 59 [-]: SELF      R4 R4 K16    ; R5 := R4; R4 := R4[0x6b837788]
 60 [-]: CALL      R4 2 2       ; R4 := R4(R5)
 61 [-]: GETGLOBAL R5 K9        ; R5 := 0xae91e43b
 62 [-]: SELF      R5 R5 K17    ; R6 := R5; R5 := R5[0xaf9fda9f]
 63 [-]: CALL      R5 2 0       ; R5,... := R5(R6)
 64 [-]: CALL      R2 0 1       ; R2(R3,...)
 65 [-]: GETGLOBAL R2 K18       ; R2 := 0x2d0fad09
 66 [-]: LOADK     R3 K19       ; R3 := "Lotus.Interface.Components.ThemedStats"
 67 [-]: CALL      R2 2 2       ; R2 := R2(R3)
 68 [-]: GETTABLE  R3 R2 K20    ; R3 := R2[0xae6791ba]
 69 [-]: GETGLOBAL R4 K9        ; R4 := 0xae91e43b
 70 [-]: LOADK     R5 K21       ; R5 := "StatContainer.Stats"
 71 [-]: CALL      R3 3 2       ; R3 := R3(R4,R5)
 72 [-]: SETUPVAL  R3 U3        ; U82 := R3
 73 [-]: GETUPVAL  R3 U3        ; R3 := U3
 74 [-]: NEWTABLE  R4 0 4       ; R4 := {}
 75 [-]: NEWTABLE  R5 1 0       ; R5 := {}
 76 [-]: LOADK     R6 K24       ; R6 := "<b>"
 77 [-]: GETGLOBAL R7 K9        ; R7 := 0xae91e43b
 78 [-]: SELF      R7 R7 K25    ; R8 := R7; R7 := R7[0x42b04007]
 79 [-]: LOADK     R9 K26       ; R9 := "/Lotus/Language/Dojo/DojoLegend"
 80 [-]: LOADBOOL  R10 0 0      ; R10 := false
 81 [-]: CALL      R7 4 2       ; R7 := R7(R8,R9,R10)
 82 [-]: LOADK     R8 K27       ; R8 := "<\b>"
 83 [-]: CONCAT    R6 R6 R8     ; R6 := R6 .. R7 .. R8
 84 [-]: GETGLOBAL R7 K9        ; R7 := 0xae91e43b
 85 [-]: SELF      R7 R7 K25    ; R8 := R7; R7 := R7[0x42b04007]
 86 [-]: GETUPVAL  R9 U4        ; R9 := U4
 87 [-]: SELF      R9 R9 K28    ; R10 := R9; R9 := R9[0x6d604ba7]
 88 [-]: CALL      R9 2 2       ; R9 := R9(R10)
 89 [-]: LOADBOOL  R10 0 0      ; R10 := false
 90 [-]: CALL      R7 4 0       ; R7,... := R7(R8,R9,R10)
 91 [-]: SETLIST   R5 0 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 0
 92 [-]: SETTABLE  R4 K23 R5    ; R4["Labels"] := R5
 93 [-]: NEWTABLE  R5 1 0       ; R5 := {}
 94 [-]: LOADK     R6 K30       ; R6 := ""
 95 [-]: GETUPVAL  R7 U1        ; R7 := U1
 96 [-]: SELF      R7 R7 K31    ; R8 := R7; R7 := R7[0x9ad96941]
 97 [-]: LOADK     R9 14        ; R9 := 14.000000
 98 [-]: CALL      R7 3 0       ; R7,... := R7(R8,R9)
 99 [-]: SETLIST   R5 0 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 0
100 [-]: SETTABLE  R4 K29 R5    ; R4["Values"] := R5
101 [-]: NEWTABLE  R5 1 0       ; R5 := {}
102 [-]: LOADK     R6 K30       ; R6 := ""
103 [-]: GETUPVAL  R7 U1        ; R7 := U1
104 [-]: SELF      R7 R7 K34    ; R8 := R7; R7 := R7[0x67f7ce17]
105 [-]: LOADK     R9 14        ; R9 := 14.000000
106 [-]: CALL      R7 3 0       ; R7,... := R7(R8,R9)
107 [-]: SETLIST   R5 0 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 0
108 [-]: SETTABLE  R4 K33 R5    ; R4["ValueColors"] := R5
109 [-]: NEWTABLE  R5 0 0       ; R5 := {}
110 [-]: SETTABLE  R4 K35 R5    ; R4["CompareValues"] := R5
111 [-]: SETTABLE  R3 K22 R4    ; R3["mStats"] := R4
112 [-]: LOADK     R3 1         ; R3 := 1.000000
113 [-]: GETUPVAL  R4 U5        ; R4 := U5
114 [-]: LEN       R4 R4        ; R4 := # R4
115 [-]: LOADK     R5 1         ; R5 := 1.000000
116 [-]: FORPREP   R3 160       ; R3 -= R5; PC := 160
117 [-]: GETUPVAL  R7 U1        ; R7 := U1
118 [-]: SELF      R7 R7 K36    ; R8 := R7; R7 := R7[0xd45227bf]
119 [-]: GETUPVAL  R9 U5        ; R9 := U5
120 [-]: GETTABLE  R9 R9 R6     ; R9 := R9[R6]
121 [-]: CALL      R7 3 2       ; R7 := R7(R8,R9)
122 [-]: TEST      R7 0         ; if not R7 then PC := 160
123 [-]: JMP       160          ; PC := 160
124 [-]: GETGLOBAL R7 K37       ; R7 := 0x33bdd652
125 [-]: GETTABLE  R7 R7 K38    ; R7 := R7[0x23d5322f]
126 [-]: GETUPVAL  R8 U3        ; R8 := U3
127 [-]: GETTABLE  R8 R8 K22    ; R8 := R8["mStats"]
128 [-]: GETTABLE  R8 R8 K23    ; R8 := R8["Labels"]
129 [-]: GETGLOBAL R9 K9        ; R9 := 0xae91e43b
130 [-]: SELF      R9 R9 K25    ; R10 := R9; R9 := R9[0x42b04007]
131 [-]: GETUPVAL  R11 U6       ; R11 := U6
132 [-]: GETTABLE  R11 R11 R6   ; R11 := R11[R6]
133 [-]: SELF      R11 R11 K28  ; R12 := R11; R11 := R11[0x6d604ba7]
134 [-]: CALL      R11 2 2      ; R11 := R11(R12)
135 [-]: LOADBOOL  R12 0 0      ; R12 := false
136 [-]: CALL      R9 4 0       ; R9,... := R9(R10,R11,R12)
137 [-]: CALL      R7 0 1       ; R7(R8,...)
138 [-]: GETGLOBAL R7 K37       ; R7 := 0x33bdd652
139 [-]: GETTABLE  R7 R7 K38    ; R7 := R7[0x23d5322f]
140 [-]: GETUPVAL  R8 U3        ; R8 := U3
141 [-]: GETTABLE  R8 R8 K22    ; R8 := R8["mStats"]
142 [-]: GETTABLE  R8 R8 K29    ; R8 := R8["Values"]
143 [-]: GETUPVAL  R9 U1        ; R9 := U1
144 [-]: SELF      R9 R9 K31    ; R10 := R9; R9 := R9[0x9ad96941]
145 [-]: GETUPVAL  R11 U5       ; R11 := U5
146 [-]: GETTABLE  R11 R11 R6   ; R11 := R11[R6]
147 [-]: CALL      R9 3 0       ; R9,... := R9(R10,R11)
148 [-]: CALL      R7 0 1       ; R7(R8,...)
149 [-]: GETGLOBAL R7 K37       ; R7 := 0x33bdd652
150 [-]: GETTABLE  R7 R7 K38    ; R7 := R7[0x23d5322f]
151 [-]: GETUPVAL  R8 U3        ; R8 := U3
152 [-]: GETTABLE  R8 R8 K22    ; R8 := R8["mStats"]
153 [-]: GETTABLE  R8 R8 K33    ; R8 := R8["ValueColors"]
154 [-]: GETUPVAL  R9 U1        ; R9 := U1
155 [-]: SELF      R9 R9 K34    ; R10 := R9; R9 := R9[0x67f7ce17]
156 [-]: GETUPVAL  R11 U5       ; R11 := U5
157 [-]: GETTABLE  R11 R11 R6   ; R11 := R11[R6]
158 [-]: CALL      R9 3 0       ; R9,... := R9(R10,R11)
159 [-]: CALL      R7 0 1       ; R7(R8,...)
160 [-]: FORLOOP   R3 117       ; R3 += R5; if R3 <= R4 then begin PC := 117; R6 := R3 end
161 [-]: GETUPVAL  R7 U3        ; R7 := U3
162 [-]: SELF      R7 R7 K39    ; R8 := R7; R7 := R7[0x8d77b2b2]
163 [-]: LOADK     R9 337       ; R9 := 337.000000
164 [-]: CALL      R7 3 1       ; R7(R8,R9)
165 [-]: RETURN    R0 1         ; return 


; Function #7:
;
; Name:            
; Defined at line: 121
; #Upvalues:       4
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  4

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: TEST      R0 0         ; if not R0 then PC := 9
  3 [-]: JMP       9            ; PC := 9
  4 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
  5 [-]: GETGLOBAL R1 K1        ; R1 := 0xae91e43b
  6 [-]: CALL      R0 2 2       ; R0 := R0(R1)
  7 [-]: TEST      R0 0         ; if not R0 then PC := 10
  8 [-]: JMP       10           ; PC := 10
  9 [-]: RETURN    R0 1         ; return 
 10 [-]: GETGLOBAL R0 K1        ; R0 := 0xae91e43b
 11 [-]: SELF      R0 R0 K2     ; R1 := R0; R0 := R0[0x8a8c8d5a]
 12 [-]: GETGLOBAL R2 K3        ; R2 := 0x67652851
 13 [-]: CALL      R2 1 0       ; R2,... := R2()
 14 [-]: CALL      R0 0 1       ; R0(R1,...)
 15 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
 16 [-]: GETGLOBAL R1 K4        ; R1 := 0xbe190284
 17 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 18 [-]: TEST      R0 1         ; if R0 then PC := 27
 19 [-]: JMP       27           ; PC := 27
 20 [-]: GETGLOBAL R0 K4        ; R0 := 0xbe190284
 21 [-]: SELF      R0 R0 K5     ; R1 := R0; R0 := R0[0x9ac735d2]
 22 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 23 [-]: TEST      R0 0         ; if not R0 then PC := 27
 24 [-]: JMP       27           ; PC := 27
 25 [-]: GETUPVAL  R0 U1        ; R0 := U1
 26 [-]: CALL      R0 1 1       ; R0()
 27 [-]: GETGLOBAL R0 K0        ; R0 := 0x7b998233
 28 [-]: GETUPVAL  R1 U2        ; R1 := U2
 29 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 30 [-]: TEST      R0 1         ; if R0 then PC := 42
 31 [-]: JMP       42           ; PC := 42
 32 [-]: GETUPVAL  R0 U2        ; R0 := U2
 33 [-]: SELF      R0 R0 K6     ; R1 := R0; R0 := R0[0x4e5ff4c0]
 34 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 35 [-]: TEST      R0 0         ; if not R0 then PC := 42
 36 [-]: JMP       42           ; PC := 42
 37 [-]: GETGLOBAL R0 K1        ; R0 := 0xae91e43b
 38 [-]: SELF      R0 R0 K7     ; R1 := R0; R0 := R0[0xe4162eed]
 39 [-]: LOADK     R2 K8        ; R2 := "Hide"
 40 [-]: LOADK     R3 K9        ; R3 := ""
 41 [-]: CALL      R0 4 1       ; R0(R1,R2,R3)
 42 [-]: LOADBOOL  R0 1 0       ; R0 := true
 43 [-]: GETGLOBAL R1 K10       ; R1 := _T
 44 [-]: GETTABLE  R1 R1 K11    ; R1 := R1["HideHud"]
 45 [-]: EQ        1 R1 K12     ; if R1 == nil then PC := 52
 46 [-]: JMP       52           ; PC := 52
 47 [-]: GETGLOBAL R1 K10       ; R1 := _T
 48 [-]: GETTABLE  R1 R1 K11    ; R1 := R1["HideHud"]
 49 [-]: LT        0 K13 R1     ; if 0.000000 >= R1 then PC := 52
 50 [-]: JMP       52           ; PC := 52
 51 [-]: LOADBOOL  R0 0 0       ; R0 := false
 52 [-]: GETUPVAL  R1 U3        ; R1 := U3
 53 [-]: EQ        1 R1 R0      ; if R1 == R0 then PC := 60
 54 [-]: JMP       60           ; PC := 60
 55 [-]: SETUPVAL  R0 U3        ; U82 := R3
 56 [-]: GETGLOBAL R1 K1        ; R1 := 0xae91e43b
 57 [-]: SELF      R1 R1 K14    ; R2 := R1; R1 := R1[0x368ad758]
 58 [-]: GETUPVAL  R3 U3        ; R3 := U3
 59 [-]: CALL      R1 3 1       ; R1(R2,R3)
 60 [-]: RETURN    R0 1         ; return 


; Function #8:
;
; Name:            
; Defined at line: 147
; #Upvalues:       6
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  5

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: GETTABLE  R0 R0 K0     ; R0 := R0[0x659d451f]
  3 [-]: GETGLOBAL R1 K1        ; R1 := 0x0032441c
  4 [-]: GETTABLE  R1 R1 K2     ; R1 := R1["UISound_Open"]
  5 [-]: CALL      R0 2 1       ; R0(R1)
  6 [-]: GETGLOBAL R0 K3        ; R0 := 0xae91e43b
  7 [-]: SELF      R0 R0 K4     ; R1 := R0; R0 := R0[0x67bc869f]
  8 [-]: LOADK     R2 K5        ; R2 := "StatContainer"
  9 [-]: LOADK     R3 10        ; R3 := 10.000000
 10 [-]: LOADK     R4 0         ; R4 := 0.000000
 11 [-]: CALL      R0 5 1       ; R0(R1,R2,R3,R4)
 12 [-]: GETUPVAL  R0 U2        ; R0 := U2
 13 [-]: GETTABLE  R0 R0 K6     ; R0 := R0[0xae6791ba]
 14 [-]: GETGLOBAL R1 K3        ; R1 := 0xae91e43b
 15 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 16 [-]: SETUPVAL  R0 U1        ; U82 := R1
 17 [-]: GETUPVAL  R0 U3        ; R0 := U3
 18 [-]: CALL      R0 1 1       ; R0()
 19 [-]: GETUPVAL  R0 U4        ; R0 := U4
 20 [-]: CALL      R0 1 1       ; R0()
 21 [-]: LOADBOOL  R0 1 0       ; R0 := true
 22 [-]: SETUPVAL  R0 U5        ; U82 := R5
 23 [-]: RETURN    R0 1         ; return 


; Function #9:
;
; Name:            
; Defined at line: 159
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: CALL      R0 1 1       ; R0()
  3 [-]: RETURN    R0 1         ; return 


; Function #10:
;
; Name:            
; Defined at line: 163
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: CALL      R0 1 1       ; R0()
  3 [-]: RETURN    R0 1         ; return 


; Function #11:
;
; Name:            
; Defined at line: 167
; #Upvalues:       3
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  7

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0xae91e43b
  2 [-]: TEST      R0 0         ; if not R0 then PC := 55
  3 [-]: JMP       55           ; PC := 55
  4 [-]: GETUPVAL  R0 U0        ; R0 := U0
  5 [-]: TEST      R0 0         ; if not R0 then PC := 55
  6 [-]: JMP       55           ; PC := 55
  7 [-]: GETGLOBAL R0 K1        ; R0 := 0x7b998233
  8 [-]: GETUPVAL  R1 U1        ; R1 := U1
  9 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 10 [-]: TEST      R0 1         ; if R0 then PC := 24
 11 [-]: JMP       24           ; PC := 24
 12 [-]: GETUPVAL  R0 U1        ; R0 := U1
 13 [-]: SELF      R0 R0 K2     ; R1 := R0; R0 := R0[0x4e5ff4c0]
 14 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 15 [-]: TEST      R0 0         ; if not R0 then PC := 24
 16 [-]: JMP       24           ; PC := 24
 17 [-]: GETGLOBAL R0 K0        ; R0 := 0xae91e43b
 18 [-]: SELF      R0 R0 K3     ; R1 := R0; R0 := R0[0x67bc869f]
 19 [-]: LOADK     R2 K4        ; R2 := "StatContainer"
 20 [-]: LOADK     R3 10        ; R3 := 10.000000
 21 [-]: LOADK     R4 0         ; R4 := 0.000000
 22 [-]: CALL      R0 5 1       ; R0(R1,R2,R3,R4)
 23 [-]: JMP       52           ; PC := 52
 24 [-]: GETUPVAL  R0 U2        ; R0 := U2
 25 [-]: TEST      R0 1         ; if R0 then PC := 40
 26 [-]: JMP       40           ; PC := 40
 27 [-]: GETGLOBAL R0 K5        ; R0 := 0x25312c9b
 28 [-]: GETGLOBAL R1 K0        ; R1 := 0xae91e43b
 29 [-]: LOADK     R2 K4        ; R2 := "StatContainer"
 30 [-]: LOADK     R3 0         ; R3 := 0.000000
 31 [-]: NEWTABLE  R4 1 0       ; R4 := {}
 32 [-]: LOADK     R5 10        ; R5 := 10.000000
 33 [-]: SETLIST   R4 1 1       ; R4[(1-1)*FPF+i] := R(4+i), 1 <= i <= 1
 34 [-]: NEWTABLE  R5 1 0       ; R5 := {}
 35 [-]: LOADK     R6 100       ; R6 := 100.000000
 36 [-]: SETLIST   R5 1 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 1
 37 [-]: LOADK     R6 K7        ; R6 := 0.200000
 38 [-]: CALL      R0 7 1       ; R0(R1,R2,R3,R4,R5,R6)
 39 [-]: JMP       52           ; PC := 52
 40 [-]: GETGLOBAL R0 K5        ; R0 := 0x25312c9b
 41 [-]: GETGLOBAL R1 K0        ; R1 := 0xae91e43b
 42 [-]: LOADK     R2 K4        ; R2 := "StatContainer"
 43 [-]: LOADK     R3 0         ; R3 := 0.000000
 44 [-]: NEWTABLE  R4 1 0       ; R4 := {}
 45 [-]: LOADK     R5 10        ; R5 := 10.000000
 46 [-]: SETLIST   R4 1 1       ; R4[(1-1)*FPF+i] := R(4+i), 1 <= i <= 1
 47 [-]: NEWTABLE  R5 1 0       ; R5 := {}
 48 [-]: LOADK     R6 0         ; R6 := 0.000000
 49 [-]: SETLIST   R5 1 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 1
 50 [-]: LOADK     R6 K7        ; R6 := 0.200000
 51 [-]: CALL      R0 7 1       ; R0(R1,R2,R3,R4,R5,R6)
 52 [-]: GETUPVAL  R0 U2        ; R0 := U2
 53 [-]: NOT       R0 R0        ; R0 := not R0
 54 [-]: SETUPVAL  R0 U2        ; U82 := R2
 55 [-]: RETURN    R0 1         ; return 


; Function #12:
;
; Name:            
; Defined at line: 182
; #Upvalues:       3
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  7

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0xae91e43b
  2 [-]: TEST      R0 0         ; if not R0 then PC := 38
  3 [-]: JMP       38           ; PC := 38
  4 [-]: GETUPVAL  R0 U0        ; R0 := U0
  5 [-]: TEST      R0 0         ; if not R0 then PC := 38
  6 [-]: JMP       38           ; PC := 38
  7 [-]: GETGLOBAL R0 K1        ; R0 := 0x7b998233
  8 [-]: GETUPVAL  R1 U1        ; R1 := U1
  9 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 10 [-]: TEST      R0 1         ; if R0 then PC := 24
 11 [-]: JMP       24           ; PC := 24
 12 [-]: GETUPVAL  R0 U1        ; R0 := U1
 13 [-]: SELF      R0 R0 K2     ; R1 := R0; R0 := R0[0x4e5ff4c0]
 14 [-]: CALL      R0 2 2       ; R0 := R0(R1)
 15 [-]: TEST      R0 0         ; if not R0 then PC := 24
 16 [-]: JMP       24           ; PC := 24
 17 [-]: GETGLOBAL R0 K0        ; R0 := 0xae91e43b
 18 [-]: SELF      R0 R0 K3     ; R1 := R0; R0 := R0[0x67bc869f]
 19 [-]: LOADK     R2 K4        ; R2 := "StatContainer"
 20 [-]: LOADK     R3 10        ; R3 := 10.000000
 21 [-]: LOADK     R4 0         ; R4 := 0.000000
 22 [-]: CALL      R0 5 1       ; R0(R1,R2,R3,R4)
 23 [-]: JMP       36           ; PC := 36
 24 [-]: GETGLOBAL R0 K5        ; R0 := 0x25312c9b
 25 [-]: GETGLOBAL R1 K0        ; R1 := 0xae91e43b
 26 [-]: LOADK     R2 K4        ; R2 := "StatContainer"
 27 [-]: LOADK     R3 0         ; R3 := 0.000000
 28 [-]: NEWTABLE  R4 1 0       ; R4 := {}
 29 [-]: LOADK     R5 10        ; R5 := 10.000000
 30 [-]: SETLIST   R4 1 1       ; R4[(1-1)*FPF+i] := R(4+i), 1 <= i <= 1
 31 [-]: NEWTABLE  R5 1 0       ; R5 := {}
 32 [-]: LOADK     R6 0         ; R6 := 0.000000
 33 [-]: SETLIST   R5 1 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 1
 34 [-]: LOADK     R6 K7        ; R6 := 0.200000
 35 [-]: CALL      R0 7 1       ; R0(R1,R2,R3,R4,R5,R6)
 36 [-]: LOADBOOL  R0 0 0       ; R0 := false
 37 [-]: SETUPVAL  R0 U2        ; U82 := R2
 38 [-]: RETURN    R0 1         ; return 


; Function #13:
;
; Name:            
; Defined at line: 193
; #Upvalues:       2
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  7

  1 [-]: GETGLOBAL R0 K0        ; R0 := 0xae91e43b
  2 [-]: TEST      R0 0         ; if not R0 then PC := 21
  3 [-]: JMP       21           ; PC := 21
  4 [-]: GETUPVAL  R0 U0        ; R0 := U0
  5 [-]: TEST      R0 0         ; if not R0 then PC := 21
  6 [-]: JMP       21           ; PC := 21
  7 [-]: GETGLOBAL R0 K1        ; R0 := 0x25312c9b
  8 [-]: GETGLOBAL R1 K0        ; R1 := 0xae91e43b
  9 [-]: LOADK     R2 K2        ; R2 := "StatContainer"
 10 [-]: LOADK     R3 0         ; R3 := 0.000000
 11 [-]: NEWTABLE  R4 1 0       ; R4 := {}
 12 [-]: LOADK     R5 10        ; R5 := 10.000000
 13 [-]: SETLIST   R4 1 1       ; R4[(1-1)*FPF+i] := R(4+i), 1 <= i <= 1
 14 [-]: NEWTABLE  R5 1 0       ; R5 := {}
 15 [-]: LOADK     R6 100       ; R6 := 100.000000
 16 [-]: SETLIST   R5 1 1       ; R5[(1-1)*FPF+i] := R(5+i), 1 <= i <= 1
 17 [-]: LOADK     R6 K4        ; R6 := 0.200000
 18 [-]: CALL      R0 7 1       ; R0(R1,R2,R3,R4,R5,R6)
 19 [-]: LOADBOOL  R0 1 0       ; R0 := true
 20 [-]: SETUPVAL  R0 U1        ; U82 := R1
 21 [-]: RETURN    R0 1         ; return 


; Function #14:
;
; Name:            
; Defined at line: 200
; #Upvalues:       1
; #Parameters:     4
; Is_vararg:       0
; Max Stack Size:  8

  1 [-]: GETGLOBAL R4 K0        ; R4 := 0x7b998233
  2 [-]: GETUPVAL  R5 U0        ; R5 := U0
  3 [-]: CALL      R4 2 2       ; R4 := R4(R5)
  4 [-]: TEST      R4 1         ; if R4 then PC := 11
  5 [-]: JMP       11           ; PC := 11
  6 [-]: GETUPVAL  R4 U0        ; R4 := U0
  7 [-]: SELF      R4 R4 K1     ; R5 := R4; R4 := R4[0xfaa69527]
  8 [-]: MOVE      R6 R0        ; R6 := R0
  9 [-]: MOVE      R7 R1        ; R7 := R1
 10 [-]: CALL      R4 4 1       ; R4(R5,R6,R7)
 11 [-]: RETURN    R0 1         ; return 


; Function #15:
;
; Name:            
; Defined at line: 206
; #Upvalues:       1
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: GETUPVAL  R0 U0        ; R0 := U0
  2 [-]: CALL      R0 1 1       ; R0()
  3 [-]: RETURN    R0 1         ; return 


; Function #16:
;
; Name:            
; Defined at line: 210
; #Upvalues:       0
; #Parameters:     0
; Is_vararg:       0
; Max Stack Size:  2

  1 [-]: LOADBOOL  R0 1 0       ; R0 := true
  2 [-]: RETURN    R0 2         ; return R0
  3 [-]: RETURN    R0 1         ; return 


